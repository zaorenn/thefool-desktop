"""``fool gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.hermes/auth.json``) — the same path ``fool dashboard register``
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `fool auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from fool_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `fool gateway enroll` is not available in a managed/hosted install.\n"
