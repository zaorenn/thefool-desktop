...
1. `fool secrets bitwarden setup --help` works (dispatch path)
2. `fool update --check` works (update path)
3. `fool secrets bitwarden disable` works (handler execution)
4. `fool secrets onepassword status` works (lazy backend loads on demand)
...
def _run_hermes(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    ...
    connectivity — but ``fool update --check`` really does ``git fetch``
    ...
    return subprocess.run(
        [sys.executable, "-m", "fool_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_setup_help(self) -> None:
        """`fool secrets bitwarden setup --help` must exit 0 and print usage.
    ...
    def test_bitwarden_status(self) -> None:
        """`fool secrets bitwarden status` must exit 0 (runs lazy backend)."""
    ...
    def test_bitwarden_disable(self) -> None:
        """`fool secrets bitwarden disable` must exit 0."""
    ...
    def test_onepassword_status(self) -> None:
        """`fool secrets onepassword status` must exit 0 (1Password lazy backend)."""
    ...
    def test_onepassword_setup_help(self) -> None:
        """`fool secrets onepassword setup --help` must exit 0."""
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `fool update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`fool update --check` must not load cryptography._rust."""
...
# resolves the fool_cli.main module's lazily re-exported attribute —
# so the patch must land on fool_cli.main._cmd_update_check.
with patch('fool_cli.main._cmd_update_check', capture_update_check):
    try:
        m.main()
