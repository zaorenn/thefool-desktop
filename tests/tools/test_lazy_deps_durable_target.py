...
class TestGatingWithTarget:
    """``THEFOOL_DISABLE_LAZY_INSTALLS=1`` must STOP blocking once a durable
...
class TestInstallArgConstruction:
    ...
    def test_uv_resolution_failure_does_not_fall_through_to_pip(self, monkeypatch):
        monkeypatch.delenv(ld._LAZY_TARGET_ENV, raising=False)
        monkeypatch.setattr("thefool_cli.managed_uv.resolve_uv", lambda: "uv")
        calls = []
...
@pytest.mark.skipif(
    os.environ.get("THEFOOL_RUN_NETWORK_TESTS") != "1",
    reason="opt-in real-install test (set THEFOOL_RUN_NETWORK_TESTS=1); CI runs "
