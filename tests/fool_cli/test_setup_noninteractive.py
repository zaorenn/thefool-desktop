...
import pytest
from fool_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        ...
        from fool_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("fool_cli.main._has_any_provider_configured", return_value=False),
            patch("fool_cli.main.cmd_setup") as mock_setup,
        ...
        out = capsys.readouterr().out
        assert "fool config set model.provider custom" in out
