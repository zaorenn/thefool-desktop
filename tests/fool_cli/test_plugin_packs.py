...
from fool_cli.plugin_packs import (
    PackError,
    PackPluginEntry,
    PluginPack,
    ResolvedPackPlugin,
...
def test_resolve_pack_plugins_uses_community_index_for_bare_names():
    ...
    with mock.patch(
        "fool_cli.plugin_index.load_index", return_value=([fake_entry], "seed")
    ), mock.patch(
        "fool_cli.plugin_index.resolve_name",
...
def test_resolve_pack_plugins_carries_index_miss_as_error():
    ...
    with mock.patch(
        "fool_cli.plugin_index.load_index", return_value=([], "seed")
    ), mock.patch(
        "fool_cli.plugin_index.resolve_name", return_value=(None, [])
    ):
        resolved = resolve_pack_plugins(pack)
...
def test_install_fan_out_passes_pinned_refs_to_installer(tmp_path):
    ...
    with mock.patch.multiple("fool_cli.plugins_cmd", **patches):
...
def test_install_fan_out_continues_past_failures_and_reports():
    from fool_cli.plugins_cmd import PluginOperationError
...
def test_pack_install_exits_nonzero_on_partial_failure(tmp_path, monkeypatch):
    ...
    with mock.patch.multiple("fool_cli.plugins_cmd", **patches), mock.patch(
...
def test_pack_install_refuses_noninteractive_sessions(tmp_path, monkeypatch):
    ...
    with mock.patch(
        ...
        "fool_cli.plugin_packs.resolve_pack_plugins",
        side_effect=lambda pack: _resolved(pack),
    ), mock.patch("fool_cli.plugins_cmd._install_plugin_core", installer):
        with pytest.raises(SystemExit) as exc:
            cmd_pack_install(str(pack_file))
    assert exc.value.code == 1
...
def _seed_install_state(home, monkeypatch, *, metadata, config=None, plugins=()):
    ...
    monkeypatch.setattr(
        "fool_cli.plugins_cmd._read_install_metadata", lambda: metadata
    )
    monkeypatch.setattr("fool_cli.plugins_cmd._plugins_dir", lambda: plugins_dir)
    cfg = config or {}
    monkeypatch.setattr(
        "fool_cli.plugins_cmd._get_enabled_set",
        lambda: set((cfg.get("plugins") or {}).get("enabled") or []),
    )
    monkeypatch.setattr("fool_cli.plugin_packs._sanitized_entry_config",
...
def test_export_strips_secret_and_capability_config_keys(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "fool_cli.plugin_packs._sanitized_entry_config",
    ...
    with mock.patch("fool_cli.config.load_config", return_value=fake_cfg):
