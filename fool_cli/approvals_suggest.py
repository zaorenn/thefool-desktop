...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, fool self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/fool home prefixes and collapse whitespace.
...
def approvals_command(args) -> int:
    """Dispatch ``fool approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from fool_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: fool approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `fool approvals <subcommand> -h` for details."
    )
    return 1
