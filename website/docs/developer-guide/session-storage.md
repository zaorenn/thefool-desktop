...
Abridged — see `SCHEMA_SQL` in `fool_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `fool_state.py` for the exact SQL.
...
This is derived from `fool_constants.get_hermes_home()` which resolves to
`~/.hermes/` by default, or the value of `FOOL_HOME` environment variable.
