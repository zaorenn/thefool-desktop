...
def get_env_value(name: str, default=None):
    ...
    Wraps :func:`fool_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from fool_cli.config import get_env_value as _hermes_get_env_value
    except ImportError:
...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``FOOL_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    pair the OAuth branch reads) and pins the origin with
    :func:`fool_cli.auth._xai_validate_inference_base_url` so a tampered
    env override can't exfiltrate the bearer; on rejection it falls back to
    the default rather than raising.
    """
    override = str(
        get_env_value("FOOL_XAI_BASE_URL")
...
def resolve_xai_http_credentials(
    *,
    ...
    to ``XAI_API_KEY`` resolved via ``fool_cli.config.get_env_value`` so keys
    ...
    honors ``FOOL_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
