...
from thefool_cli import kanban_swarm as ks
...
def _check_dispatcher_presence(
    ...
    - ``running=True``: a gateway is alive for this THEFOOL_HOME and its
    ...
    be running under a different THEFOOL_HOME than the profile the request
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    # THEFOOL_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "THEFOOL_KANBAN_BOARD env var). Use `hermes kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $THEFOOL_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $THEFOOL_PROFILE or 'user')")
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $THEFOOL_PROFILE or 'user')")
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $THEFOOL_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $THEFOOL_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # thefool_cli.kanban_db, because children can import DB mutators directly.
    ...
    # env-var pin for the duration of this call. Using THEFOOL_KANBAN_BOARD
    ...
    # THEFOOL_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("THEFOOL_PROFILE_NAME", "THEFOOL_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("THEFOOL_DELEGATED_CHILD_CONTEXT"))
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    ...
    DB, so a fresh THEFOOL_HOME that has never called ``kanban init``
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from thefool_cli import kanban_diagnostics as kd
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from thefool_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from thefool_cli import kanban_decompose as decomp
