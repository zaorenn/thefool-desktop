...
# cwd. Without an explicit PYTHONPATH the child resolves ``fool_cli`` /
...
def _make_running_kanban_task(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("FOOL_PROFILE", "parent-worker")
    monkeypatch.setenv("FOOL_KANBAN_WORKSPACE", str(workspace))
    monkeypatch.setenv("FOOL_KANBAN_ATTACHMENTS_ROOT", str(attachments_root))
    ...
    monkeypatch.setenv("FOOL_KANBAN_RUN_ID", str(run_id))
...
def test_delegated_child_context_suppresses_env_gated_kanban_tools(monkeypatch, tmp_path):
    ...
    The parent process may be a dispatcher worker with FOOL_KANBAN_TASK set;
    the child is only a subagent, not the run owner.
    """
    monkeypatch.setenv("FOOL_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("FOOL_KANBAN_RUN_ID", "123")
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("FOOL_HOME", str(home))
...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    ``os.environ`` and therefore never wrote FOOL_DELEGATED_CHILD_CONTEXT into
    the sandbox env.
    """
    home = tmp_path / ".hermes"
    home.mkdir()
    ...
    monkeypatch.setenv("FOOL_KANBAN_TASK", "t_parent")
    monkeypatch.setenv("FOOL_KANBAN_RUN_ID", "123")
    monkeypatch.setenv("FOOL_KANBAN_DB", str(home / "kanban.db"))
    monkeypatch.setenv("FOOL_KANBAN_WORKSPACE", str(tmp_path / "parent-workspace"))
    monkeypatch.setenv("FOOL_KANBAN_CLAIM_LOCK", "lock")
    monkeypatch.delenv("FOOL_DELEGATED_CHILD_CONTEXT", raising=False)
    ...
    with delegated_child_context():
        env = _scrub_child_env(
            dict(os.environ),
            is_passthrough=lambda k: k.startswith("FOOL_KANBAN_"),
            is_windows=False,
        )

    assert os.environ.get("FOOL_DELEGATED_CHILD_CONTEXT") is None
    assert env["FOOL_HOME"] == str(home)
    assert env["FOOL_DELEGATED_CHILD_CONTEXT"] == "1"
    assert "FOOL_KANBAN_TASK" not in env
    assert "FOOL_KANBAN_RUN_ID" not in env
    assert "FOOL_KANBAN_DB" not in env
    assert "FOOL_KANBAN_WORKSPACE" not in env
    assert "FOOL_KANBAN_CLAIM_LOCK" not in env
