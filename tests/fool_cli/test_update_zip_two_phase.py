...
from fool_constants import venv_bin_dir, venv_python_path
...
def test_no_open_coded_venv_layout_remains_in_hermes_cli():
    ...
    exempt = {"stdio.py"}
    pkg = Path(fool_cli.__file__).parent
    offenders = []
    ...
    assert not offenders, (
        "open-coded venv layout found (use fool_constants.venv_bin_dir):\n"
        + "\n".join(offenders)
    )
...
def test_top_level_files_are_swapped_atomically(tmp_path):
    ...
    fool_constants.py, ...). Covering only directories would leave exactly
...
def test_venv_helpers_honour_an_explicit_platform_verdict():
    ...
    `fool_cli.main._is_windows`. A helper that reads `sys.platform`
