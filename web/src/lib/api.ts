...
export const FOOL_BASE_PATH = readBasePath();
const BASE = FOOL_BASE_PATH;
...
const SESSION_HEADER = "X-Fool-Session-Token";
 ...
 *
 * The dashboard's gated-mode WS auth (``fool_cli.web_server._ws_auth_ok``)
 ...
 *  - loopback / ``--insecure``: attach the ``X-Fool-Session-Token`` header.
...
export interface StatusResponse {
  ...
  /** NS-656: disk-usage rollup for the FOOL_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the FOOL_HOME volume — no staleness dimension, so no
 * sampled_at. */
