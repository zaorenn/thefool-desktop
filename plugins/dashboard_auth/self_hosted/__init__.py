...
A standards-compliant OpenID Connect Relying Party for the ``fool dashboard``
...
:class:`~fool_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``fool_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~fool_cli.dashboard_auth.Session`.
...
same precedence convention as the ``nous`` plugin)::
    ...
    FOOL_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    FOOL_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``FOOL_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "FOOL_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "FOOL_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("FOOL_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    client_secret = _resolve_setting(
        "FOOL_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(FOOL_DASHBOARD_OIDC_ISSUER + FOOL_DASHBOARD_OIDC_CLIENT_ID) "
