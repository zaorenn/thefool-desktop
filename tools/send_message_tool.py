    ...
    if not chat_id:
        ...
        if home:
            ...
            return tool_error(
                ...
                f"or set a home channel via: fool config set {home_env} <channel_id>"
            )
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("FOOL_SESSION_PLATFORM", "cli")
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("FOOL_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("FOOL_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("FOOL_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False, args=None):
    ...
    if platform == Platform.FEISHU and media_files:
        ...
        from fool_cli.plugins import discover_plugins as _dp_feishu
        _dp_feishu()
    ...
    if platform == Platform.SLACK and media_files:
        ...
        from fool_cli.plugins import discover_plugins as _dp_slack
        _dp_slack()
    ...
    if platform == Platform.WHATSAPP and media_files:
        ...
        from fool_cli.plugins import discover_plugins as _dp_wa
        _dp_wa()
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``FOOL_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``FOOL_HOME`` which has no ``gateway.pid``, so the gateway-running check
