...
- save provider credentials into `${FOOL_HOME:-~/.hermes}/.env`
...
3. Buy it and save it into `${FOOL_HOME:-~/.hermes}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${FOOL_HOME:-~/.hermes}/.env`
