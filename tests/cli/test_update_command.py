...
Also verifies that ``fool_cli.main._launch_tui`` correctly handles exit
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        ...
        patch(
            "fool_cli.config.format_managed_message",
...
def test_affirmative_answer_sets_pending_relaunch_and_returns_true(answer, capsys):
    ...
    with patch("fool_cli.config.is_managed", return_value=False):
        result = _call(self_)
