...
export default function SystemPage() {
  ...
  // spawned `fool import` runs non-interactively (stdin is /dev/null), so
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_hermes === false) {
      showToast(
        "The Fool updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      ...
      setActiveAction(resp.name ?? "fool-update");
      ...
      <ConfirmDialog
        ...
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'fool update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'fool update' (${updateInfo?.update_command ?? "fool update"}) and restart the gateway when it finishes.`
        }
          ...
          <CardContent className="flex flex-col gap-3 py-4">
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">fool portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$FOOL_HOME/backups/fool-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full The Fool backup?"
              description={`This will overwrite your current Fool configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Fool team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
