...
Usage:
    python -m fool_cli.main web          # Start on http://127.0.0.1:9119
    python -m fool_cli.main web --port 8080
"""
...
from fool_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
WEB_DIST = Path(os.environ["FOOL_WEB_DIST"]) if "FOOL_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same FOOL_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    ...
    ``fool_cli.gateway``. But the first WS connection and its initial
    ...
    for mod in (
        ...
        "fool_cli.auth",
        "fool_cli.copilot_auth",
        "fool_cli.runtime_provider",
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from fool_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    """
    try:
        from fool_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import fool_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # Desktop-spawned backends (FOOL_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hermes
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("FOOL_DESKTOP") == "1":
        ...
        try:
            from fool_cli.gateway import _reap_unsupervised_gateway_orphans
...
from fool_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# FOOL_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("FOOL_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
# FOOL-SEAM: session-header
_SESSION_HEADER_NAME = "X-Fool-Session-Token"
...
#
# This list is defined in ``fool_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from fool_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Fool-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from fool_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from fool_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from fool_cli.dashboard_auth.token_auth import token_auth_middleware
...
from fool_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from fool_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from fool_cli.model_normalize import normalize_model_for_provider
    from fool_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from fool_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    """
    from fool_state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "FOOL_DASHBOARD_FILES_ROOT"
...
# FOOL_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to FOOL_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no FOOL_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("FOOL_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # (the container/hosted layout) or when FOOL_DASHBOARD_FILES_ROOT is set.
    if _default_hermes_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    ``FOOL_HOME/images/`` — the same directory ``clipboard.paste`` /
...
# gate + path hardening as /api/fs). Logic lives in ``fool_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from fool_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from fool_cli.profiles import _check_gateway_running, profiles_to_serve
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors fool_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # FOOL_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of fool_cli.gateway blocks the
        ...
        try:
            from fool_cli.auth import get_nous_session_validity
        ...
        # disk-exhaustion incidents). One statvfs call on FOOL_HOME's
        ...
        try:
            from fool_state import SessionDB as _SDB
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from fool_cli.auth import get_nous_auth_status_local
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from fool_cli.debug import build_debug_share
...
def _spawn_hermes_action(
    ...
    *,
    ...
    Uses the running interpreter's ``fool_cli.main`` module so the action
    ...
    action_env = {**os.environ, "FOOL_NONINTERACTIVE": "1"}
    ...
    action_id = (env_overrides or {}).get("FOOL_ACTION_ID")
    if action_id:
...
async def gateway_drain(request: Request):
    ...
    active (no ``FOOL_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_hermes():
    ...
    try:
        ...
        _log.exception("Failed to spawn fool update")
...
async def check_hermes_update(force: bool = False):
    ...
    try:
        from fool_cli.banner import check_for_updates
...
from fool_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from fool_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from fool_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from fool_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
# in fool_cli/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    """
    try:
        from fool_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        ...
        from fool_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from fool_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from fool_cli.model_selection_guards import combined_selection_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (fool_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from fool_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from fool_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    try:
        from fool_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from fool_cli.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level FOOL_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in fool gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in fool gateway setup",
...
def _messaging_platform_payload(
    ...
    # FOOL_HOME contextvar override (#56986 / #69143), so the profile's
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the FOOL_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized FOOL_HOME)
...
# the canonical ``fool auth add <provider>`` command so the dashboard
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "fool auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "fool auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "fool auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "fool auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "fool auth add xai-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "fool auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from fool_cli import auth as hauth
        if provider_id == "nous":
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"fool auth add {d.slug}",
    ...
    def _run():
        with _profile_scope(profile):
            ...
            if provider_id == "anthropic":
                ...
                try:
                    from fool_cli.auth import clear_provider_auth
            ...
            try:
                from fool_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("FOOL_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from fool_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from fool_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from fool_cli.auth import _save_codex_tokens
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from fool_state import SessionDB, is_malformed_db_error
...
async def get_logs(
    ...
    from fool_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from fool_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    """
    from fool_cli import profiles as profiles_mod
    try:
        return [
            {
                "name": name,
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (FOOL_HOME already
    ...
    FOOL_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, FOOL_HOME)."""
    from fool_cli import profiles as profiles_mod
...
from fool_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from fool_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/fool_cli/
        # test_config_read_guard.py). The FOOL_HOME override scopes
...
def _normalize_mcp_server_create(
    ...
    """
    from fool_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from fool_cli.mcp_security import validate_mcp_server_entry
...
from fool_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from fool_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from fool_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
#
# Wraps the same JSON store the CLI uses (fool_cli.webhook); the webhook
...
# spawn the real `fool gateway <verb>` so behaviour matches the CLI exactly.
...
async def list_credential_pool():
    ...
    from fool_cli.auth import read_credential_pool
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    from fool_cli.auth import suppress_credential_source
...
from fool_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from fool_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _profile_setup_command(name: str) -> str:
    ...
    return "fool setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local FOOL_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local FOOL_HOME override (same mechanism as
    ...
    """
    from fool_constants import set_hermes_home_override, reset_hermes_home_override
    from fool_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the FOOL_HOME override. Returns the
    number of skills newly disabled.
    """
    from fool_constants import set_hermes_home_override, reset_hermes_home_override
    from fool_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    imported the modules before a FOOL_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
from fool_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from fool_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
if sys.platform.startswith("win"):
    try:
        from fool_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from fool_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from fool_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from fool_cli.dashboard_auth.audit import AuditEvent, audit_log
        from fool_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``FOOL_TUI_RESUME`` env var —
    matching what ``fool_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``FOOL_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``FOOL_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``FOOL_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``FOOL_HOME`` at the profile dir in the child env. Every spawned
    ...
    ``FOOL_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from fool_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # An explicit profile scope below still overrides FOOL_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    env.setdefault("FOOL_TUI_DISABLE_MOUSE", "1")
    env.setdefault("FOOL_TUI_INLINE", "1")
    ...
    env["FOOL_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["FOOL_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["FOOL_TUI_RESUME"] = resume

    if sidecar_url:
        env["FOOL_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile FOOL_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["FOOL_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``FOOL_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("FOOL_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from fool_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("FOOL_TUI_RESUME") or raw_resume
...
# FOOL_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`fool_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from fool_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (fool serve): web UI disabled — use "
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``FOOL_HOME/dashboard-themes/<name>.yaml``), inject a minimal
...
def _discover_user_themes() -> list:
    ...
    live under the server's own ``FOOL_HOME``.
    """
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as fool_cli.plugins):
    1. User plugins:    ~/.hermes/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.hermes/plugins/  (only if FOOL_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from fool_cli.plugins import get_bundled_plugins_dir
    ...
    # FOOL_HOME override (e.g. embedded /chat under --open-profile).
    #
    ...
    # ``fool_cli.plugins`` resolves plugin install locations. The
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from fool_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from fool_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from fool_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from fool_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from fool_cli.plugins_cmd import dashboard_remove_user_plugin
...
from fool_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``FOOL_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("FOOL_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``FOOL_PARENT_PID`` is the Electron Desktop PID, not necessarily this
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``FOOL_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("FOOL_PARENT_PID")
    start_marker = os.environ.get("FOOL_PARENT_START_MARKER")
    nonce = os.environ.get("FOOL_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("FOOL_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``FOOL_SERVE_HEADLESS``), so
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing FOOL_DASHBOARD_OAUTH_CLIENT_ID).
            ...
            try:
                ...
                from fool_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
    ...
    # FOOL_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("FOOL_DESKTOP") == "1":
                try:
                    from fool_cli.dashboard_procs import (
            ...
            ready_token = "FOOL_BACKEND_READY" if headless else "FOOL_DASHBOARD_READY"
