...
def _client():
    ...
    # Keep the state DB under the isolated FOOL_HOME for any handler that
    # touches it.
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        The endpoint must mirror `fool auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `fool auth add`, which clears every suppression for the
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "fool_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    ...
    ``POST /api/hermes/update`` runs ``fool update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import fool_cli.banner as banner
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr("fool_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `fool tools` configurator."""
...
def test_spawn_hermes_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    ...
    assert captured["env"]["FOOL_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    ...
    startup under FOOL_DESKTOP=1 so the stale orphan is cleared first.
    """
    import fool_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from fool_cli.gateway, so patch it
    # on that module.
    import fool_cli.gateway as g
