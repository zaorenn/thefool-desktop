...
:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `fool plugins`.
:::
...
| User | `$FOOL_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.hermes/plugins/<name>/` | Opt-in via `FOOL_ENABLE_PROJECT_PLUGINS=1`. |
...
Hermes, in `$FOOL_HOME/plugins/<name>/` when installed by a user, or in
...
package layout. No copy under `$FOOL_HOME/plugins/` is required.
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hermes_home (str): Active FOOL_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `fool memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `fool memory setup`:
...
Every field in `get_config_schema()` is prompted during `fool memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$FOOL_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::
