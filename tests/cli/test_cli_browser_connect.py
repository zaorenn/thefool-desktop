...
class TestChromeDebugLaunch:
    ...
    def test_manual_command_uses_detected_linux_browser(self):
        with patch("thefool_cli.browser_connect.shutil.which", side_effect=lambda name: "/usr/bin/chromium" if name == "chromium" else None), \
             patch("thefool_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == "/usr/bin/chromium"):
    ...
    def test_linux_candidates_include_official_brave_and_edge_stable_paths(self):
        ...
        with patch("thefool_cli.browser_connect.shutil.which", return_value=None), \
             patch("thefool_cli.browser_connect.os.path.isfile", side_effect=lambda path: path in {brave, edge}):
    ...
    def test_wsl_install_candidates_keep_posix_separators_on_nt_host(self):
        ...
        with patch("thefool_cli.browser_connect.shutil.which", return_value=None), \
             patch("thefool_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == expected):
            candidates = get_chrome_debug_candidates("Linux")
    ...
    def test_wait_for_browser_debug_ready_or_exit_detects_early_exit(self, monkeypatch):
        ...
        monkeypatch.setattr("thefool_cli.browser_connect.time.sleep", lambda _seconds: None)
        with patch("thefool_cli.browser_connect.is_browser_debug_ready", return_value=False):
    ...
    def test_launch_result_surfaces_stderr_tail_on_crash(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "thefool_cli.browser_connect.chrome_debug_data_dir", lambda: str(tmp_path)
        )
        stderr_path = tmp_path / "launch-stderr.log"
        with patch("thefool_cli.browser_connect.get_chrome_debug_candidates", return_value=[chrome]), \
             patch("thefool_cli.browser_connect.is_browser_debug_ready", return_value=False), \
    ...
    def test_launch_result_no_hint_when_no_candidates(self):
        with patch("thefool_cli.browser_connect.get_chrome_debug_candidates", return_value=[]):
            result = launch_chrome_debug(9222, "Linux")
    ...
    def test_manual_command_uses_wsl_windows_chrome_when_available(self):
        ...
        with patch("thefool_cli.browser_connect.shutil.which", return_value=None), \
             patch("thefool_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == chrome):
            command = manual_chrome_debug_command(9222, "Linux")
    ...
    def test_manual_command_uses_windows_quoting_on_windows(self):
        ...
        with patch("thefool_cli.browser_connect.shutil.which", side_effect=lambda name: chrome if name == "chrome.exe" else None), \
             patch("thefool_cli.browser_connect.os.path.isfile", side_effect=lambda path: path == chrome):
