...
def temp_pyproject(tmp_path, monkeypatch):
    """Point fool_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("fool_cli.main._resolve_install_target_python", return_value=py), \
             patch("fool_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("fool_cli.main._run_install_with_heartbeat"):

            from fool_cli.main import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("fool_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("fool_cli.main._run_install_with_heartbeat") as mock_install:
            from fool_cli.main import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        with patch("fool_cli.main._is_windows", return_value=True):
            from fool_cli.main import _resolve_install_target_python
