...
from thefool_cli.main import _read_ssh_session_token_file, cmd_dashboard
...
def test_token_anchor_is_os_home_not_active_profile(tmp_path, monkeypatch):
    ...
    ``<root>/profiles/<name>``, and a Docker-style ``THEFOOL_HOME`` can point
...
def test_windows_runtime_root_stays_at_machine_root_for_named_profile(tmp_path, monkeypatch):
    from thefool_cli import windows_ssh_runtime

    machine_root = tmp_path / "custom-hermes-root"
    monkeypatch.setenv("THEFOOL_HOME", str(machine_root / "profiles" / "writer_2"))
