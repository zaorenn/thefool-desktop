...
from fool_cli.slack_cli import _build_full_manifest, slack_manifest_command
from fool_cli.subcommands.slack import build_slack_parser


def _parse_slack_args(argv):
    """Build the real `fool slack` parser and parse argv against it."""
...
def _run_console_entrypoint(*argv: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-c",
            "from fool_cli.main import main; raise SystemExit(main())",
            *argv,
        ],
...
def test_slack_dispatcher_propagates_manifest_failure(monkeypatch):
    from fool_cli import main as main_module
    from fool_cli import slack_cli
...
class TestSlackFullManifest:
    """Generated full Slack app manifest used by `fool slack manifest`."""
