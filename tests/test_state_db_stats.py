...
Covers:
- ``thefool_state.collect_state_db_stats``: read-only, best-effort stats
  (page_count, freelist, WAL size, journal mode, row counts, FTS presence,
  pending v23 FTS-rebuild bookkeeping).
- ``thefool_state.count_db_holders``: /proc-based best-effort probe for how
  many processes hold the DB file open (Linux only; None elsewhere/on error).
- ``thefool_cli.doctor._render_state_db_stats``: formatting/threshold helper
...
from thefool_state import SessionDB, collect_state_db_stats, count_db_holders
...
def test_render_healthy_stats_no_warnings():
    from thefool_cli.doctor import _render_state_db_stats
...
def test_render_warns_on_large_db():
    from thefool_cli.doctor import (
...
def test_render_large_db_with_pending_rebuild_suggests_optimize():
    from thefool_cli.doctor import STATE_DB_SIZE_WARN_BYTES, _render_state_db_stats
