...
def qr_scan_for_bot_info(
    *,
    ...
    try:
        req = urllib.request.Request(generate_url, headers={"User-Agent": "FoolAgent/1.0"})
    ...
    while time.monotonic() < deadline:
        try:
            req = urllib.request.Request(query_url, headers={"User-Agent": "FoolAgent/1.0"})
...
def interactive_setup() -> None:
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("Approve with: fool pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("Direct messages disabled.")
        else:
            print_info("Skipped — configure later with 'fool gateway setup'")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `fool setup` to install WeCom support.",
