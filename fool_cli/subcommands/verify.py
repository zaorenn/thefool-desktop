"""``fool verify`` subcommand parser.

Follows the pattern of ``fool_cli/subcommands/doctor.py``: parser built
...
def build_verify_parser(subparsers, *, cmd_verify: Callable) -> None:
    ...
    verify_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "(or load the saved manifest at .fool/environment.json), then "
    ...
    verify_parser.add_argument(
        "--save",
        action="store_true",
        help="Save the recipe as .fool/environment.json in the project",
    )
    verify_parser.add_argument(
        "--skip-start",
        action="store_true",
