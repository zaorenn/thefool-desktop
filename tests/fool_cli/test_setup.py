...
from fool_cli import setup as setup_mod
from fool_cli.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("fool_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.main.select_provider_and_model", fake_select)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("fool_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("fool_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "fool_cli.main._model_flow_named_custom",
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    monkeypatch.setattr("fool_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("FOOL_HOME", str(tmp_path))
    ...
    monkeypatch.setattr("fool_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("fool_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("fool_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "fool_cli.setup.get_nous_subscription_features",
    ...
    from fool_cli.setup import setup_terminal_backend
...
def test_vercel_setup_prefills_project_and_team_from_link_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("fool_cli.setup.prompt", fake_prompt)
