...
`mcp_tool.py`, and `fool_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# FoolMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class FoolMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`FoolTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, FoolTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import FoolTokenStorage
                        if isinstance(storage, FoolTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import FoolTokenStorage
            if not isinstance(storage, FoolTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``fool mcp reauth``.
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`FoolMCPOAuthProvider` directly using the helpers
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `fool mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``fool mcp remove <name>`` and (indirectly) by
        ``fool mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
