"""Dependency-light venv recovery that runs BEFORE thefool_cli.main's imports.

The ``hermes`` console entry point is ``thefool_cli.main:main``.  Importing
``thefool_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``thefool_cli.env_loader``, ``yaml`` via ``thefool_cli.config``, ...).  In
...
a corrupted venv.  ``thefool_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``thefool_cli.main`` to become
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``thefool_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``thefool_cli.main`` can import at all.
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``thefool_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    """
    try:
        from thefool_cli import _install_repair as ir
