...
normal tests assume away:
  ...
  - Filesystem: THEFOOL_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own THEFOOL_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            for m in list(sys.modules.keys()):
                if m.startswith(("thefool_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from thefool_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "THEFOOL_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "thefool_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from thefool_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["THEFOOL_HOME"]).resolve())
...
def _(home, kb):
    """THEFOOL_HOME at a path with spaces — should work but catches
    ...
    try:
        ...
        print(f"  THEFOOL_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    """THEFOOL_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  THEFOOL_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    """THEFOOL_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same THEFOOL_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from thefool_cli import web_server as ws  # noqa
    except Exception:
        pass
