"""Regression tests for #60328: --yolo must set FOOL_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing FOOL_YOLO_MODE at the
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("FOOL_YOLO_MODE")

    monkeypatch.setattr(
        "fool_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("fool_cli.main.cmd_chat", lambda args: None)
    monkeypatch.delenv("FOOL_YOLO_MODE", raising=False)
    monkeypatch.setattr(sys, "argv", argv)

    from fool_cli.main import main as cli_main
