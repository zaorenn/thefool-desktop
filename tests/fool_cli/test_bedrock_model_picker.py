...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (FOOL_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via FOOL_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in FOOL_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from fool_cli.providers import FOOL_OVERLAYS
        assert "bedrock" in FOOL_OVERLAYS
