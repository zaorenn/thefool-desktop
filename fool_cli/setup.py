...
from fool_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from fool_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def is_noninteractive() -> bool:
    ...
    ``FOOL_NONINTERACTIVE=1`` (see ``fool_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("FOOL_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``FOOL_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from fool_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'fool setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        from fool_cli.auth import resolve_provider
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'fool setup' to configure"))
    ...
    # Video generation — opt-in via `fool tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from fool_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'fool setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'fool setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'fool tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'fool setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'fool setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('fool setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('fool setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('fool setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('fool setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('fool setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('fool config', Colors.GREEN)}         View current settings")
    print(
...
# between `fool tools` and `fool setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``fool model``)
    ...
    provider added to ``fool model`` is automatically available here.
    ...
    # on demand via `fool auth add`, `fool setup` vision, and
    # `fool setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``fool model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'fool setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `fool setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `fool setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `fool egress setup` then `fool egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `fool egress setup`."
            )
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into FOOL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("FOOL_MAX_ITERATIONS")
    ...
    print_info("  Run `fool setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # FOOL_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from fool_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _profile_name_from_hermes_home(hermes_home) -> str | None:
    """Return the active profile name when FOOL_HOME is a profile dir."""
...
def _setup_qqbot():
    ...
    from fool_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'fool setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    # via `fool import` or `fool setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `fool setup tools` and `fool tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``fool_cli.auth`` — lists every supported
    ...
    """
    try:
        from fool_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from fool_cli.config
    ...
    if section_key == "model":
        ...
        from fool_cli.gateway import _all_platforms, _platform_status
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: fool claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: fool claw migrate"
        )
        print_info(
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``fool setup --portal`` and ``fool portal``. This is the
    ...
    ``fool setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``fool model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``fool portal`` always offers a model picker,
    ...
    # setup calls, so `fool portal` == quick setup's Nous step.
    try:
        from fool_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `fool portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `fool portal`: %s", exc)
        print()
...
def run_setup_wizard(args):
    ...
    print()
    print_info("You can edit these files directly or use 'fool config edit'")
    ...
    # Tune later with `fool setup agent`.
    if not is_existing:
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``fool tools`` (which
    ...
    try:
        ...
        from fool_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``fool setup agent`` /
    ``fool config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `fool update` runs don't re-inject them.
        try:
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`fool update` runs from re-injecting them. Opt back in any")
            print_info("time with `fool skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `fool tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `fool plugins list` / `fool plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `fool plugins`.")
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'fool setup' and choose 'Full Setup' to reconfigure,")
