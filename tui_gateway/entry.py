...
# from shadowing Hermes's own top-level modules.  ``fool_bootstrap`` lives at
...
# shared owner in fool_cli.mcp_startup — the wait/in-flight/join helpers
...
# fool_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `FOOL_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("FOOL_TUI_SIDECAR_URL")
...
# tested; override via ``FOOL_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("FOOL_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``FOOL_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``fool_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from fool_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → fool_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's FOOL_HOME first), so a launch profile with no
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``fool_cli.mcp_startup`` thread used by the
    ...
    try:
        from fool_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def _has_configured_mcp_servers() -> bool:
    ...
    from fool_cli.mcp_startup import _has_configured_mcp_servers as configured
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's FOOL_HOME override — the shared owner in
    ``fool_cli.mcp_startup`` captures the caller's context-local override
