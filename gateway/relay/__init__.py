...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``fool gateway enroll`` writes them to
...
def relay_endpoint() -> Optional[str]:
    ...
    ``FOOL_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``fool gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from fool_cli.skin_engine import get_active_skin  # late import: boot-safe
...
def _resolve_relay_identity_token() -> str:
    ...
    ``fool gateway enroll`` CLI. Three modes, in precedence order:
    ...
    if not token_url:
        ...
        from fool_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    sets neither ``FOOL_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``fool gateway enroll``: has a PINNED
