#!/usr/bin/env node
// set-exe-identity.mjs — stamp The Fool icon + version metadata onto the
// built TheFool.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked TheFool.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `fool desktop`, the installer's
...
// path (which rebuilds via `fool desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-TheFool.exe>
//
...
// Stamp The Fool icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      FileDescription: 'The Fool',
      CompanyName: 'zaorenn',
      LegalCopyright: 'Copyright (c) 2026 zaorenn'
    }
  })

  console.log('[set-exe-identity] done — The Fool icon + identity stamped')
}
