"""Tests for thefool_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `thefool_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import thefool_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("thefool_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "thefool_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from thefool_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    the per-THEFOOL_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root THEFOOL_HOME, no ``-p``) and named-profile
    render paths.
    """
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    # Mark the drop so fake rm can refuse unlink outside THEFOOL_HOME the way
    ...
    (bin_dir / "rm").write_text(
        ...
        'if [ -n "$THEFOOL_TEST_DROPPED" ]; then\n'
        "  exit 0\n"
        "fi\n"
        f'exec {real_rm} "$@"\n',
        encoding="utf-8",
