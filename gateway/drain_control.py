...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{FOOL_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``FOOL_HOME`` is a **durable** store — on Hermes
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting FOOL_HOME."""
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable FOOL_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (FOOL_HOME is a
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    FOOL_HOME volume, or an expired same-epoch orphan (#85433), is
