...
def build_models_payload(
    ...
    *,
    ...
    try:
        from thefool_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``thefool_cli/moa_cmd.py``.
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`thefool_cli.providers.custom_provider_aliases` — lets the frontend
    do a membership check instead.
    """
    from thefool_cli.providers import custom_provider_aliases
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from thefool_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`thefool_cli.model_switch.list_picker_providers`) so the
