...
Hermes can control Spotify directly — playback, queue, search, playlists, saved tracks/albums, and listening history — using Spotify's official Web API with PKCE OAuth. Tokens are stored in `~/.hermes/auth.json` and refreshed automatically on 401; you only log in once per machine (refresh tokens expire after ~6 months; re-run `fool auth spotify` when they do).

Unlike Hermes' built-in OAuth integrations (Google, GitHub Copilot, Codex), Spotify requires every user to register their own lightweight developer app. Spotify does not let third parties ship a public OAuth app that anyone can use. It takes about two minutes and `fool auth spotify` walks you through it.
...
### One-shot: `fool tools` or first-run setup
...
Scroll to `🎵 Spotify`, press space to toggle it on, then `s` to save. The same toggle is also available during the first-run `fool setup` / `fool setup tools` flow. Spotify stays opt-in, so enabling it there runs the same provider-aware configuration as `fool tools`.
...
If no `FOOL_SPOTIFY_CLIENT_ID` is set, Hermes walks you through the app registration inline:
...
```bash
fool auth status spotify
```

Shows whether tokens are present and when the access token expires. Refresh is automatic: when any Spotify API call returns 401, the client exchanges the refresh token and retries once. Refresh tokens persist across Hermes restarts, so you only re-auth if you revoke the app in your Spotify account settings or run `fool auth logout spotify`.
...
- **The cron agent inherits your active toolsets.** Spotify must be enabled in `fool tools` for the cron session to see the Spotify tools.
...
```bash
fool auth logout spotify
```

Removes tokens from `~/.hermes/auth.json`. To also clear the app config, delete `FOOL_SPOTIFY_CLIENT_ID` (and `FOOL_SPOTIFY_REDIRECT_URI` if you set it) from `~/.hermes/.env`, or run the wizard again.
...
**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what Hermes is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `FOOL_SPOTIFY_REDIRECT_URI` in `~/.hermes/.env` to whatever you registered.
...
**`401 Unauthorized` keeps coming back** — Your refresh token was revoked (usually because you removed the app from your account, or the app was deleted). Run `fool auth spotify` again.
...
```bash
fool auth spotify --scope "user-read-playback-state user-modify-playback-state playlist-read-private"
```
...
```bash
fool auth spotify --client-id <id> --redirect-uri http://localhost:3000/callback
```

Or set them permanently in `~/.hermes/.env`:

```
FOOL_SPOTIFY_CLIENT_ID=<your_id>
FOOL_SPOTIFY_REDIRECT_URI=http://localhost:3000/callback
```
...
| `~/.hermes/.env` | `FOOL_SPOTIFY_CLIENT_ID`, optional `FOOL_SPOTIFY_REDIRECT_URI` |
