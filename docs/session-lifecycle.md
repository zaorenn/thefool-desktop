...
The canonical transcript store is SQLite via `SessionDB` (from `fool_state`). The
...
Counts consecutive restarts via a JSON file (`{FOOL_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `fool update`, `fool gateway restart`,
or `/restart`.
