...
from fool_cli.timeouts import get_provider_request_timeout
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from fool_cli.providers import FOOL_OVERLAYS, get_provider

        overlay = FOOL_OVERLAYS.get(provider)
    ...
    try:
        ...
        from fool_cli.models import normalize_provider as normalize_model_provider
        from fool_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: fool config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$FOOL_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$FOOL_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .hermes.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / FOOL_HOME
    ...
    agent._cache_ttl = "5m"
    try:
        from fool_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind FOOL_DEV_CREDITS.
    ...
    from fool_logging import setup_logging, setup_verbose_logging
    ...
    if agent.verbose_logging:
        ...
        # on the fact that fool_logging.setup_logging() does not
        ...
        # handler level inside fool_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from fool_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from fool_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `fool model` to "
                        "select a provider, or run `fool setup` for first-time "
                        "configuration."
                    )
    ...
    # A multiplexed gateway may enter a different FOOL_HOME after
    ...
    # worker (kanban_show tool is present iff FOOL_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["FOOL_SESSION_ID"] = agent.session_id
    ...
    try:
        from fool_cli.config import load_config_readonly as _load_sess_cfg
    ...
    try:
        from fool_cli.config import load_config_readonly as _load_agent_config
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from fool_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from fool_cli.plugins import get_plugin_context_engine
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from fool_cli.model_switch import _check_hermes_model_warning
