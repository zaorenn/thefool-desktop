...
entirely on the process environment: ``FOOL_HOME`` says *where* to write and
...
root = fool_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        fool_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``FOOL_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("FOOL_HOME", "FOOL_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no FOOL_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``FOOL_HOME`` is gone, and the env-only guard
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{fool_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert fool_state._process_looks_like_pytest(self._FakeProc(cmdline))

    @pytest.mark.parametrize(
        "cmdline",
        [
            ["hermes", "gateway", "start"],
            ["/usr/bin/python", "-m", "fool_cli.main", "sessions", "list"],
    ...
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not fool_state._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not fool_state._process_looks_like_pytest(_Denied())
