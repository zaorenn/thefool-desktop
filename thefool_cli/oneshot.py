...
Approvals = auto-bypassed (THEFOOL_YOLO_MODE=1 is set for the call).
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to THEFOOL_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("THEFOOL_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "hermes -z: --provider requires --model (or THEFOOL_INFERENCE_MODEL). "
...
def _run_agent(
    ...
    from thefool_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("THEFOOL_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from thefool_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("THEFOOL_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    from thefool_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                THEFOOL_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via THEFOOL_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via THEFOOL_YOLO_MODE=1
