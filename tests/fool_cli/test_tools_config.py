"""Tests for fool_cli.tools_config platform tool persistence."""
...
from fool_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    import fool_cli.tools_config as _tc
    ...
    with caplog.at_level(logging.WARNING, logger="fool_cli.tools_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    with patch("fool_cli.tools_config.save_config"):
...
def test_first_install_nous_auto_configures_video_gen(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
    ...
    monkeypatch.setattr(
        "fool_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"video_gen"},
    )
    monkeypatch.setattr("fool_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "fool_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
    ...
    configured = []
    monkeypatch.setattr(
        "fool_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from fool_cli.tools_config import PLATFORMS
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from fool_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from fool_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
class TestAgentBrowserPostSetup:
    ...
    def _stub_browser_use_install(self):
        ...
        logic under test."""
        with patch("fool_cli.tools_config._ensure_browser_use_cli") as stub:
            yield stub

    def test_warns_when_neither_npx_nor_agent_browser_on_path(self):
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch("fool_cli.tools_config._print_warning") as warn:
    ...
    def test_chromium_already_installed_skips_subprocess(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "fool_cli.tools_config._print_success"
        ) as success:
            _run_post_setup("agent_browser")
    ...
    def test_docker_with_missing_chromium_warns_instead_of_installing(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "fool_cli.tools_config._print_warning"
        ) as warn:
            _run_post_setup("agent_browser")
    ...
    def test_install_failure_prints_stderr_tail_and_does_not_invalidate_cache(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "fool_cli.tools_config._print_info"
        ) as info:
...
class TestBrowserUseCliInstalledForAllNonCamofoxBackends:
    ...
    def test_browser_post_setup_attempts_cli_install(self, key):
        with patch("fool_cli.tools_config._ensure_browser_use_cli") as ensure, patch(
    ...
    def test_camofox_post_setup_never_touches_browser_use(self):
        ...
        the CLI in."""
        with patch("fool_cli.tools_config._ensure_browser_use_cli") as ensure, patch(
            "fool_constants.find_node_executable", return_value=None
        ), patch("subprocess.run"):
    ...
    def test_ensure_helper_always_delegates_to_install_cli(self):
        ...
        $FOOL_HOME/bin when only side installs exist."""
        with patch(
            "fool_cli.tools_config.shutil.which", return_value="/usr/bin/browser-use"
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(True, "browser-use CLI already installed (/managed/bin/browser-use)"),
        ) as install:
            from fool_cli.tools_config import _ensure_browser_use_cli
    ...
    def test_ensure_helper_install_failure_is_non_fatal(self):
        ...
        with patch(
            "fool_cli.tools_config.shutil.which", return_value=None
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(False, "`uv tool install browser-use` failed:\nboom"),
        ), patch("fool_cli.tools_config._print_warning") as warn:
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from fool_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from fool_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from fool_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("fool_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("fool_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("fool_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `fool tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from fool_cli.tools_config import _get_effective_configurable_toolsets
...
#    reported as added/removed by `fool tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `fool tools` printed ``- kanban``
...
def test_vision_picker_custom_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FOOL_HOME", str(tmp_path))
    import fool_cli.tools_config as tc
...
# ── Toolsets that shipped after a platform's last `fool tools` save ────────
#
...
def test_explicit_plugin_toolset_admitted_in_platform_toolsets(monkeypatch):
    ...
    import fool_cli.tools_config as _tc_mod
