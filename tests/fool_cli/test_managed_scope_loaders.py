...
fool_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # fool_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("FOOL_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import fool_time

    assert fool_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (FOOL_TIMEZONE, FOOL_REDACT_SECRETS, FOOL_MAX_ITERATIONS,
    ...
    # writing FOOL_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
