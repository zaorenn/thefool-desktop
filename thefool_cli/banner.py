...
from thefool_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
def check_for_updates() -> Optional[int]:
    ...
    Two paths: if ``THEFOOL_REVISION`` is set (nix builds embed it), compare
    ...
    embedded_rev = os.environ.get("THEFOOL_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # THEFOOL_REVISION (that's nix-only). Returning None makes both the Rich
    ...
    try:
        from thefool_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $THEFOOL_HOME/hermes-agent/ may be a stale copy from --clone-all;
...
def _resolve_repo_dir() -> Optional[Path]:
    ...
    because ``$THEFOOL_HOME/hermes-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``thefool_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from thefool_cli.build_info import get_build_sha
...
def _format_update_notice(behind: int) -> str:
    ...
    from thefool_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else THEFOOL_CADUCEUS
    except Exception:
        _bskin = None
        _hero = THEFOOL_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    if os.getenv("THEFOOL_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from thefool_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from thefool_cli.codex_runtime_switch import get_current_runtime
        from thefool_cli.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else THEFOOL_AGENT_LOGO
        console.print(_logo)
        console.print()
