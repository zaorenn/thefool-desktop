...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'fool cron'.",
    },
    "hooks-config": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`fool config edit` (or move it aside), then re-run the migration."
        )
    return data
...
class Migrator:
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'fool cron' to recreate jobs manually.")
            else:
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'fool gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("FOOL_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "",
            "**Strongly recommended:** Run `fool claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `fool claw cleanup` to fix it.",
            "",
            "## Hermes-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `fool claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `fool setup` to configure any remaining settings",
            "- Run `fool mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `fool cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `fool cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `fool gateway install` if you need the gateway service",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("FOOL_HOME") or str(Path.home() / ".hermes"), help="Hermes home directory")
    parser.add_argument(
        "--workspace-target",
