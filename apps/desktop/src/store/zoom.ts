...
export function setZoomPercent(percent: number): void {
  window.foolDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.foolDesktop?.zoom) {
  void window.foolDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.foolDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
