...
from thefool_constants import agent_browser_runnable
...
from thefool_cli.models import _HERMES_USER_AGENT
from thefool_cli.vercel_auth import describe_vercel_auth
...
from thefool_constants import is_termux as _is_termux
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    ...
    from thefool_cli.backup import _QUICK_STATE_FILES
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``thefool_cli.sqlite_safe_read``).
...
def _format_db_size(db_path: Path) -> str:
    ...
    from thefool_cli.backup import _format_size
...
def _report_database_journal_modes(
    ...
    from thefool_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = hermes_home if hermes_home is not None else THEFOOL_HOME
    try:
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("THEFOOL_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from thefool_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from thefool_cli.auth import get_xai_oauth_auth_status
...
from thefool_cli.sizefmt import format_bytes as _human_bytes
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    # THEFOOL_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor removed its only consumer (the v3→4 migration) — it is silently
    # ignored. THEFOOL_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("THEFOOL_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("THEFOOL_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches thefool_cli.__version__.
    ...
    """
    try:
        from thefool_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != thefool_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'hermes update', or set "
            "thefool_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from thefool_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the THEFOOL_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("THEFOOL_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via THEFOOL_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    os.environ.setdefault("THEFOOL_INTERACTIVE", "1")
    ...
    try:
        import sqlite3
        from thefool_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
    ...
    # Detect drift between pyproject.toml and thefool_cli/__init__.py versions
    ...
    if env_path.exists():
        ...
        # matches thefool_cli.env_loader._load_dotenv_with_fallback.
        try:
    ...
    config_path = THEFOOL_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from thefool_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from thefool_cli.config import is_provider_enabled
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from thefool_cli.auth import PROVIDER_REGISTRY, get_auth_status
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from thefool_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from thefool_cli.config import check_config_version, migrate_config
        ...
        # Detect stale THEFOOL_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives THEFOOL_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from thefool_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("THEFOOL_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"THEFOOL_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `hermes setup` run)",
                )
                if should_fix:
                    if remove_env_value("THEFOOL_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale THEFOOL_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove THEFOOL_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the THEFOOL_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale THEFOOL_MAX_ITERATIONS in .env shadows config.yaml — "
        ...
        try:
            from thefool_cli.config import load_env as _load_env_depr
            from thefool_cli.config import read_user_config_raw as _read_raw_depr
        ...
        try:
            from thefool_cli.config import validate_config_structure
    ...
    if state_db_path.exists():
        try:
            ...
            from thefool_state import _db_opens_cleanly, repair_state_db_schema
            ...
            from thefool_state import is_malformed_db_error, repair_state_db_schema
        ...
        try:
            from thefool_state import collect_state_db_stats, count_db_holders
    ...
    try:
        from thefool_constants import is_container as _is_container
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable THEFOOL_HOME mirror
    ...
    _section("Skills Hub")
    hub_dir = THEFOOL_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    try:
        from thefool_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = THEFOOL_HOME / "config.yaml"
    ...
    try:
        from thefool_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
    ...
    try:
        from thefool_cli.doctor_live import maybe_run_live_checks
