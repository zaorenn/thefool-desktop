...
| `-FoolHome` | `%LOCALAPPDATA%\hermes`              | 覆盖数据目录                                    |
...
在首次启动时（以及检测到缺少工具时按需触发），Hermes 会运行一个小型 Python 引导程序——`fool_cli/dep_ensure.py`——检查并懒加载安装所需的非 Python 依赖。在 Windows 上，相关依赖如下：
...
1. 如果设置了 `FOOL_GIT_BASH_PATH` 环境变量，优先使用。
...
安装程序会显式设置 `FOOL_GIT_BASH_PATH`，使新 PowerShell 会话无需重新发现。如果你想让 Hermes 使用特定的 bash——例如系统 Git Bash 或通过符号链接的 WSL bash——可以覆盖此变量。
...
修复逻辑位于 `fool_cli/stdio.py::configure_windows_stdio()`，在每个入口点（`cli.py::main`、`fool_cli/main.py::main`、`gateway/run.py::main`）的早期调用。它会：
...
**禁用方式：** 在环境中设置 `FOOL_DISABLE_WINDOWS_UTF8=1` 可回退到旧版 cp1252 stdio 路径。用于排查编码 bug；正常使用中不建议设置。
...
1. `schtasks /Create /SC ONLOGON /RL LIMITED /TN FoolGateway` — 注册一个在你登录时以标准（非提升）权限运行的任务。无 UAC 提示。
...
**覆盖 `FOOL_HOME`：** 设置该环境变量以指向不同的数据目录。与 Linux 上的用法相同。
...
| 变量                          | 效果                                                                                                                                |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `FOOL_GIT_BASH_PATH`        | 覆盖 bash.exe 的发现逻辑。可指向任意 bash——完整 Git-for-Windows、通过符号链接的 WSL bash、MSYS2、Cygwin。安装程序会自动设置此变量。 |
| `FOOL_DISABLE_WINDOWS_UTF8` | 设为 `1` 可禁用 UTF-8 stdio 垫片，回退到区域设置代码页。用于排查编码 bug。                                                          |
...
**重启后 gateway 无法持续运行。**
运行 `fool gateway status`——它会合并 schtasks 条目、Startup 文件夹快捷方式（如有）和运行中的 PID。如果 schtasks 已注册但未运行，组策略可能阻止了 `ONLOGON` 触发器。运行 `schtasks /Query /TN FoolGateway /V /FO LIST` 查看任务失败原因，或通过卸载后使用 `FOOL_GATEWAY_FORCE_STARTUP=1` 重新安装来回退到 Startup 文件夹路径。
...
UTF-8 stdio 垫片未激活。检查 `FOOL_DISABLE_WINDOWS_UTF8` 是否**未**设置（`Get-ChildItem env:FOOL_DISABLE_WINDOWS_UTF8`）。如果该变量为空但仍然看到 `?`，控制台宿主（非常旧的 `cmd.exe`）可能完全不支持 UTF-8——请切换到 Windows Terminal。
