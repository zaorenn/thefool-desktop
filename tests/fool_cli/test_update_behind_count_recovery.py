...
def test_check_via_rev_recovers_exact_count():
    with patch(
        "fool_cli.banner.subprocess.run", return_value=_ls_remote_result(SHA_B)
...
def test_check_via_rev_up_to_date_short_circuits_compare():
    with patch(
        "fool_cli.banner.subprocess.run", return_value=_ls_remote_result(SHA_A)
...
def test_shallow_checkout_recovers_exact_count(tmp_path):
    ...
    with patch(
        "fool_cli.banner.subprocess.run", side_effect=_shallow_git(SHA_A, SHA_B)
...
def test_shallow_checkout_equal_tips_up_to_date_without_compare(tmp_path):
    ...
    with patch(
        "fool_cli.banner.subprocess.run", side_effect=_shallow_git(SHA_A, SHA_A)
    ), patch.object(banner, "_github_compare_behind") as compare:
