...
def test_dashboard_service_resets_home(
    ...
    container with FOOL_DASHBOARD=1 and verify the dashboard process
    ...
    the process env.
    """
    start_container(built_image, container_name, "FOOL_DASHBOARD=1", "FOOL_DASHBOARD_HOST=127.0.0.1")

    # Check if the dashboard process is running and inspect its HOME.
    r = docker_exec_sh(
        container_name,
        # Find the dashboard process (fool dashboard) and read its HOME
        # from /proc/<pid>/environ. If not running, verify the run script
        # itself exports HOME=/opt/data by grepping the script source.
        'pid=$(pgrep -f "fool dashboard" | head -1); '
        'if [ -n "$pid" ]; then '
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    FOOL_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "FOOL_DASHBOARD=1", "FOOL_DASHBOARD_HOST=0.0.0.0")
