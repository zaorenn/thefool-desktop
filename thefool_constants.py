...
def _get_platform_default_hermes_home() -> Path:
    ...
    Sabit dize bilinçli: ``thefool_constants`` çok erken yükleniyor, buraya
...
def _hermes_home_from_env() -> Path:
    """Resolve THEFOOL_HOME from the process environment only.

    Reads the ``THEFOOL_HOME`` env var, falling back to the platform-native
...
def _warn_profile_fallback_once() -> None:
    ...
    Guard: if a non-default profile is sticky-active but ``THEFOOL_HOME`` is
    ...
    if active and active != "default":
        ...
        msg = (
            f"[THEFOOL_HOME fallback] THEFOOL_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass THEFOOL_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_hermes_home() -> Path:
    ...
    :func:`set_hermes_home_override`) → ``THEFOOL_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``THEFOOL_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``THEFOOL_HOME`` explicitly (see the systemd
    template in ``thefool_cli/gateway.py`` and the kanban dispatcher in
    ``thefool_cli/kanban_db.py``).  See https://github.com/NousResearch/hermes-agent/issues/18594.
    """
    override = get_hermes_home_override()
    if override:
        return Path(override)

    if not os.environ.get("THEFOOL_HOME", "").strip():
...
def get_process_hermes_home() -> Path:
    ...
    process ``THEFOOL_HOME`` env var (falling back to the platform default),
...
# THEFOOL_HOME against the native home on every call (~80us of path
...
# Its result depends only on (THEFOOL_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates THEFOOL_HOME mid-process.
...
def get_default_hermes_root() -> Path:
    ...
    In Docker or custom deployments where ``THEFOOL_HOME`` points outside
    ``~/.hermes`` (e.g. ``/opt/data``), returns ``THEFOOL_HOME`` directly
    — that IS the root.

    In profile mode where ``THEFOOL_HOME`` is ``<root>/profiles/<name>``,
    ...
    env_home = os.environ.get("THEFOOL_HOME", "")
    ...
    if not env_home:
        ...
        try:
            ...
            # THEFOOL_HOME is under ~/.hermes (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — THEFOOL_HOME itself is the root
                result = env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``THEFOOL_OPTIONAL_SKILLS``.
    """
    override = os.getenv("THEFOOL_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``THEFOOL_OPTIONAL_MCPS``.
    """
    override = os.getenv("THEFOOL_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``THEFOOL_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<THEFOOL_HOME>/skills`` last-resort
    """
    override = os.getenv("THEFOOL_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_hermes_dir(
    ...
    *,
    ...
    Args:
        new_subpath: Preferred path relative to THEFOOL_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to THEFOOL_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Hermes home. Profile-aware callers that manage
            ...
            temporarily mutating the process or context-local THEFOOL_HOME.
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    ...
    ``$THEFOOL_HOME/node/bin``. Include both shapes on every platform so mixed
...
_HERMES_NODE_TARGET_MAJOR = int(os.environ.get("THEFOOL_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Hermes-managed Node trees live under ``$THEFOOL_HOME/node`` (or a profile's
    ``THEFOOL_HOME``). A partial upgrade or interrupted install can leave
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%THEFOOL_HOME%\\node`` on Windows.
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$THEFOOL_HOME/node`` on POSIX.
    ...
    ``THEFOOL_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
    ...
    try:
        result = subprocess.run(
            ...
            env={
                **os.environ,
                "THEFOOL_HOME": str(get_hermes_home()),
...
def bootstrap_hermes_managed_node() -> str | None:
    ...
    its own tree under ``$THEFOOL_HOME/node`` (the same tree a fresh install
...
def heal_hermes_managed_node() -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "THEFOOL_HOME": str(get_hermes_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current THEFOOL_HOME.
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``THEFOOL_HOME`` or other path
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{THEFOOL_HOME}/home`` when the profile-home directory exists."""
    hermes_home = get_hermes_home_override() or (env or {}).get("THEFOOL_HOME") or os.getenv("THEFOOL_HOME")
    if not hermes_home:
        return None
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("THEFOOL_REAL_HOME") or os.getenv("THEFOOL_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    ...
    ``THEFOOL_HOME`` scopes Hermes state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={THEFOOL_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{THEFOOL_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{THEFOOL_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    ...
    real_home = get_real_home(env)
    if real_home:
        env["THEFOOL_REAL_HOME"] = real_home
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under THEFOOL_HOME.

    Replaces the ``get_hermes_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, thefool_logging.py, thefool_time.py, etc.).
    """
    return get_hermes_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under THEFOOL_HOME."""
    return get_hermes_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under THEFOOL_HOME."""
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``thefool_cli`` modules using three different Windows
    ...
    A few sites outside ``thefool_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``thefool_cli.main._is_windows`` and friends);
...
# `thefool_cli.update_cmd`'s post-update probe consumes this same set so the
