...
describe('wake-word ear visibility', () => {
  ...
  it('stays mounted during a busy agent turn', () => {
    applyWakeStatus({ available: true, enabled: true, listening: true, phrase: 'hey fool' })
    renderControls({ busy: true, busyAction: 'stop' })

    expect(screen.getByLabelText('Wake word: "hey fool" — listening')).toBeTruthy()
  })

  it('stays mounted (enabled in config) even when a start was refused', () => {
    applyWakeStatus({ available: true, enabled: true, listening: false, phrase: 'hey fool' })
    ...
    expect(screen.getByLabelText('Wake word: "hey fool" — off')).toBeTruthy()
  })

  it('stays visible (never hides) even when unavailable and not enabled', () => {
    applyWakeStatus({ available: false, enabled: false, listening: false, phrase: 'hey fool' })
    renderControls()
  ...
  it('surfaces the backend refusal reason in the tooltip, still visible', () => {
    applyWakeStatus({ available: false, enabled: false, listening: false, phrase: 'hey fool' })
    applyWakeStartResult({ hint: 'run `fool tools` (Voice section)', reason: 'unavailable', started: false })
    renderControls()

    const ear = screen.getByLabelText('Wake word: "hey fool" — off')
  ...
  it('shows a disabled paused ear inside the voice-conversation pill', () => {
    ...
    const ear = screen.getByLabelText('Wake word: "hey fool" — paused during voice chat')
