...
3. Files written under $FOOL_HOME from a ``docker exec`` session land
   as hermes:hermes — the actual user-visible invariant.
4. The FOOL_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``$FOOL_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (fool_cli.container_boot), which s6 runs
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    Use `fool config set` which writes config.yaml under FOOL_HOME.
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    for it by writing the same file shape via `fool config set`-style
    ...
    Specifically: pretend the operator ran `fool login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the hermes UID. We use `fool auth list` since that touches the
    ...
    # `fool auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `fool config set` first to
    # provoke a write into FOOL_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    # FOOL_HOME that docker exec just wrote.
    r = subprocess.run(
