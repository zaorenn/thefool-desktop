...
def gw_session(monkeypatch):
    """A clean gateway session: FOOL_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "FOOL_HOME": "/h", "FOOL_PROFILE": "p",
        "FOOL_CONFIG": "/c.yaml", "FOOL_ENV": "/e",
        "FOOL_DELEGATED_CHILD_CONTEXT": "1",
        # other HERMES_* → dropped (broad prefix removed)
        "FOOL_BASE_URL": "https://x", "FOOL_INTERACTIVE": "1",
        "FOOL_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "FOOL_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "FOOL_BASE_URL", "FOOL_INTERACTIVE", "FOOL_KANBAN_DB",
