...
The canonical transcript store is SQLite via `SessionDB` (from `thefool_state`). The
...
Counts consecutive restarts via a JSON file (`{THEFOOL_HOME}/restart_counts.json`). If a
