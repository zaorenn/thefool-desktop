...
async def test_dashboard_cron_noop_inference_fields_keep_existing_snapshots(
    ...
    from thefool_cli import runtime_provider, web_server
...
async def test_create_cron_job_without_profile_uses_backend_own_profile(
    ...
    monkeypatch.setenv(
        "THEFOOL_HOME", str(isolated_profiles["worker_alpha"])
    )
...
async def test_create_cron_job_without_profile_defaults_when_unscoped(
    ...
    """THEFOOL_HOME at the default home (or unrecognized) keeps the legacy
    ``default`` fallback."""
    from thefool_cli import web_server

    monkeypatch.setenv("THEFOOL_HOME", str(isolated_profiles["default"]))
