...
from thefool_constants import get_default_hermes_root, get_hermes_home, display_hermes_home
from utils import (
    _preserve_file_mode,
...
from thefool_cli.sizefmt import format_bytes as _format_size
...
# THEFOOL_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under THEFOOL_HOME) —
...
# Reserved archive subtree for provider state that lives OUTSIDE THEFOOL_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside THEFOOL_HOME, resolved from config only (no network, no init).
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside THEFOOL_HOME.
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # thefool_cli.sqlite_safe_read). Verification targets snapshots and
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    # outside THEFOOL_HOME, so the walk above never sees it. Ask the active
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under THEFOOL_HOME.
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # thefool_cli.profiles might not be available (fresh install)
        ...
        try:
            from thefool_cli.gateway import ensure_gateway_service, _is_service_running
...
# Critical state files to include in quick snapshots (relative to THEFOOL_HOME).
...
_QUICK_STATE_FILES = (
    ...
    # are at $THEFOOL_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside THEFOOL_HOME and the entry is silently
...
# Relative path of the cron job database inside THEFOOL_HOME. Kept in sync with
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of THEFOOL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<THEFOOL_HOME>/backups/pre-update-<timestamp>.zip`` and
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of THEFOOL_HOME under ``backups/`` before a
    ...
    ``<THEFOOL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
