...
def _global_allow_private_urls() -> bool:
    ...
    1. ``FOOL_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
...
def _resolve_allow_private_urls() -> bool:
    ...
    env_val = os.getenv("FOOL_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``FOOL_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
