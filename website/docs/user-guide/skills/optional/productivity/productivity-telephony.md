...
- save provider credentials into `${THEFOOL_HOME:-~/.hermes}/.env`
...
3. Buy it and save it into `${THEFOOL_HOME:-~/.hermes}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${THEFOOL_HOME:-~/.hermes}/.env`
