...
FOOL_DEV_CREDITS_FIXTURE) so depletion AND the 90% grant warning fire immediately
...
def test_dev_fixtures_drive_cold_start():
    """Every FOOL_DEV_CREDITS_FIXTURE state produces a valid seed CreditsState."""
    import os
    ...
    for name, want in expected.items():
        os.environ["FOOL_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
        os.environ["FOOL_DEV_CREDITS_FIXTURE"] = name
        try:
            fx = dev_fixture_credits_state()
            assert fx is not None, name
            assert _cold_start_notices(fx) == want, (name, _cold_start_notices(fx))
        finally:
            os.environ.pop("FOOL_DEV_CREDITS_FIXTURE", None)
            os.environ.pop("FOOL_DEV_CREDITS", None)
...
def _seed(agent, fixture):
    ...
    os.environ["FOOL_DEV_CREDITS"] = "1"  # fixtures gate on the dev flag
    os.environ["FOOL_DEV_CREDITS_FIXTURE"] = fixture
    try:
