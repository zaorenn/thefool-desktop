...
verbatim from fool_cli/config.py. Must not import from fool_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "session": {
        ...
        # under $FOOL_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # identity slot (SOUL.md). Empty by default. The FOOL_ENVIRONMENT_HINT
        ...
        # ``FOOL_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             FOOL_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force FOOL_HOME/home when it exists (old strict
    ...
    "checkpoints": {
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: fool sweeps the checkpoint base at startup
    ...
    "display": {
        ...
        # REPL and `--tui` (or FOOL_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `hermes --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `hermes -c` muscle memory.  Default off so existing
        # users aren't surprised.  FOOL_TUI_RESUME=<id> always wins.
        ...
        # model — see fool_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to FOOL_HOME-relative YAML files/directories
    ...
    "dashboard": {
        ...
        # ``FOOL_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``FOOL_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``FOOL_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``FOOL_DASHBOARD_PUBLIC_URL``).
    ...
    "skills": {
        ...
        # Substitute ${FOOL_SKILL_DIR} and ${FOOL_SESSION_ID} in SKILL.md
    ...
    # dangerous command. -q runs export FOOL_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; FOOL_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # toggleable per-invocation via --accept-hooks or FOOL_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # $FOOL_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via FOOL_CRON_MAX_PARALLEL env var.
        ...
        # FOOL_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # FOOL_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    "kanban": {
        ...
        # decomposition is manual via `fool kanban decompose <id>` or
    ...
    "gateway": {
        ...
        # internal FOOL_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # the FOOL_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # ``FOOL_GATEWAY_MAX_STARTS`` / ``FOOL_GATEWAY_START_WINDOW_S``
        ...
        # user. Bridged to FOOL_MEDIA_DELIVERY_STRICT.
        "strict": False,
        ...
        # to FOOL_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # FOOL_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to FOOL_MEDIA_TRUST_RECENT_SECONDS.
    ...
    "sessions": {
        ...
        # serve the cjk index. Bridged to FOOL_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # FOOL_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    ...
    "updates": {
        ...
        #     DBs) into <FOOL_HOME>/state-snapshots/ before the update.
    ...
    "lsp": {
        ...
        #              ``<FOOL_HOME>/lsp/bin/`` on first use.
    ...
    "desktop": {
        ...
        # Bridged to the FOOL_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the FOOL_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'fool model' for endpoint-specific config)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. FOOL_BOT)",
    ...
    # FOOL_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to FOOL_TOOL_PROGRESS_MODE for backward
