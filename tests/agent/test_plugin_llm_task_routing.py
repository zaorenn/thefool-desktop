...
def _set_registry(monkeypatch, entries: List[Dict[str, Any]]) -> None:
    ...
    monkeypatch.setattr(
        "fool_cli.plugins.get_plugin_auxiliary_tasks", lambda: list(entries)
    )


def _set_builtins(monkeypatch, keys: List[str]) -> None:
    monkeypatch.setattr(
        "fool_cli.main._AUX_TASKS", [(k, k.title(), "") for k in keys]
    )
...
class TestOwnershipIntegration:
    ...
    def test_owner_stored_as_canonical_id(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "fool_cli.plugins._ensure_plugins_discovered", lambda: manager
        )
        _set_builtins(monkeypatch, ["vision"])
...
""",
        ...
        monkeypatch.setattr("fool_cli.plugins._ensure_plugins_discovered", lambda: manager)
        _set_builtins(monkeypatch, [])
