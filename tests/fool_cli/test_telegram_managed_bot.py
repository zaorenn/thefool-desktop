"""Tests for fool_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestCreatePairing:
    def test_success(self):
        ...
        with patch(
            "fool_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
...
class TestPollForToken:
    ...
    def test_immediate_success(self):
        ...
        with patch(
            "fool_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("fool_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_eventual_success(self):
        ...
        with patch("fool_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("fool_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    ...
    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "fool_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from fool_cli.setup import _profile_name_from_hermes_home
