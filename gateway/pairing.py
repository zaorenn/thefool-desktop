...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``fool_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from fool_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{FOOL_HOME}/pairing`` while newer code/docs may
    write ``{FOOL_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own FOOL_HOME using the same legacy/consolidated layout rules
    ...
    is the global pairing directory for the current FOOL_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp FOOL_HOME
