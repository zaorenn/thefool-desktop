"""Tests that `fool model` always shows the model selection menu for custom
...
def config_home(tmp_path, monkeypatch):
    """Isolated FOOL_HOME with a minimal config."""
...
class TestCustomProviderModelSwitch:
    ...
    def test_custom_endpoint_switch_prunes_stale_model_config_pool_entry(
        ...
        from fool_cli.main import _model_flow_custom
        ...
        with patch(
            ...
            return_value={
             ...
             patch("fool_cli.secret_prompt.masked_secret_prompt", return_value="sk-new"), \
             patch("fool_cli.main._prompt_custom_api_mode_selection", return_value=""), \
             patch(
                 "builtins.input",
                 side_effect=[
    ...
    def test_env_template_api_key_is_preserved_in_model_config(self, config_home, monkeypatch):
        ...
        import yaml
        from fool_cli.main import _model_flow_named_custom
        ...
        with patch("fool_cli.models.fetch_api_models", return_value=["qwen3.6-35b-fast"]) as mock_fetch, \
             patch("fool_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
    ...
    def test_key_env_custom_provider_persists_reference_not_secret(self, config_home, monkeypatch):
        ...
        with patch("fool_cli.models.fetch_api_models", return_value=["qwen3.6-35b-fast"]), \
             patch("fool_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_env_ref_base_url_preserves_api_key_ref_through_picker(
        ...
        with patch("fool_cli.main._prompt_provider_choice",
                   side_effect=_pick_neuralwatt), \
             patch("fool_cli.models.fetch_api_models",
    ...
    def test_key_env_providers_dict_entry_does_not_add_api_key(
        ...
        config_path.write_text(
            ...
            "    key_env: FOOL_CRS_HENKEE_KEY\n"
            "    transport: anthropic_messages\n"
            "    model: claude-opus-4-7\n"
            "    default_model: claude-opus-4-7\n"
            "custom_providers: []\n"
        )
        monkeypatch.setenv("FOOL_CRS_HENKEE_KEY", "cr_live_secret_xyz")
        ...
        assert entry["key_env"] == "FOOL_CRS_HENKEE_KEY"
        ...
        assert "${FOOL_CRS_HENKEE_KEY}" not in saved_text
    ...
    def test_picker_recognizes_current_provider_alias_when_name_differs(
        ...
        with patch(
            "fool_cli.main._prompt_provider_choice",
    ...
    def test_key_env_providers_dict_preserves_existing_api_key(
        ...
        provider_info = {
            ...
            "api_mode": "anthropic_messages",
            "provider_key": "crs-henkee",
            "api_key_ref": "${FOOL_CRS_HENKEE_KEY}",  # raw template preserved
        }
        ...
        assert entry["api_key"] == "${FOOL_CRS_HENKEE_KEY}"
        assert "cr_live_secret_xyz" not in saved_text


class TestCustomProviderDiscoverModels:
    """#18726: honor ``discover_models: false`` in the terminal ``fool model``
    ...
    def test_discover_false_saves_choice_from_configured_list(self, config_home):
        ...
        with patch("fool_cli.models.fetch_api_models") as mock_fetch, \
             patch("fool_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
    ...
    def test_probe_empty_falls_back_to_configured_list(self, config_home):
        ...
        with patch("fool_cli.models.fetch_api_models", return_value=[]), \
             patch("fool_cli.curses_ui.curses_radiolist", side_effect=ImportError), \
             patch("builtins.input", return_value="2"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
