"""``fool doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from fool_cli.doctor import FOOL_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (FOOL_HOME / "node" / "bin",
                          FOOL_HOME / "node",
                          FOOL_HOME / "node_modules" / ".bin"):
    ...
    # fool_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``fool mcp test``.
    """
