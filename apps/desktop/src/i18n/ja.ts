...
export const ja = defineLocale({
  ...
  boot: {
    ...
    errors: {
      ...
      backgroundExitedDuringStartup: '起動中に The Fool バックグラウンドプロセスが終了しました。',
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。The Fool Desktop を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'The Fool が応答を待っています。',
      turnDoneTitle: 'The Fool が完了しました',
      turnDoneBody: '',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を The Fool のデフォルトに戻しますか？',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は The Fool がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'The Fool が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'The Fool がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
        ...
        plugin: {
          label: 'プラグイン通知',
          description: 'The Fool がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。'
        }
      },
      test: 'テスト通知を送信',
      testTitle: 'The Fool',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、The Fool をシステム設定に合わせます。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、The Fool の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。The Fool を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'The Fool がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'The Fool が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'The Fool が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを The Fool がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから The Fool 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'The Fool はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    quickEntry: {
      enabledTitle: 'クイック入力',
      enabledDesc:
        'グローバルショートカットで小さな入力欄をどこからでも呼び出し、The Fool を開かずにプロンプトを送信します。',
    ...
    gateway: {
      ...
      intro:
        'The Fool Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している The Fool バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      localDesc:
        'ローカルホストでプライベートな The Fool バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      inheritTitle: 'デフォルトゲートウェイを使用',
      inheritDesc: 'このプロファイルのオーバーライドを削除し、デフォルト接続を使用します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの The Fool バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      remoteUrlTitle: 'リモート URL',
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/fool などのパスプレフィックスもサポートしています。',
      ...
      restartingMessage: 'The Fool Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · The Fool ${version}` : ''} に接続しました`,
      ...
      sshTitle: 'SSH で接続',
      sshDesc:
        'The Fool は SSH 経由でリモート上に起動され、このアプリにトンネルされます。リモート側で何かを起動・公開する必要はありません。ホストへの鍵ベースの SSH アクセスが前提です。',
      ...
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— The Fool を検出`,
      sshIncompleteHost: '接続する前に SSH ホストを入力してください。',
      sshErrUnreachable: 'SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。',
      sshErrAuth:
        'SSH 認証に失敗しました。鍵を ssh-agent に読み込む（ssh-add）か、~/.ssh/config に IdentityFile を設定してください。The Fool は非対話的に ssh を実行します。',
      sshErrHostKey:
        '前回の接続以降、ホスト鍵が変更されています。想定どおりか確認し、ssh-keygen -R <host> を実行してから再接続してください。',
      sshErrNotInstalled:
        'リモートホストに The Fool がインストールされていません。リモートでインストールする（curl -fsSL https://raw.githubusercontent.com/zaorenn/thefool-desktop/main/scripts/install.sh | sh）か、The Fool パスを設定してください。',
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。The Fool Desktop の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
      sshErrTimeout: 'SSH 接続がタイムアウトしました。ホストが到達不能、またはスリープ中の可能性があります。',
      sshErrUpdateRequired: 'Desktop SSH で接続する前に、リモートホストの The Fool を更新してください。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'リンクがこの MCP サーバーを The Fool に追加するよう要求しました。下の設定はリンク側から来たものです。内容を必ず確認してください。',
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。The Fool がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
  ...
  skills: {
    ...
    skillArchivedMessage: 'fool curator restore で復元できます。'
  },
  ...
  starmap: {
    ...
    emptyDesc: 'The Fool がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには The Fool を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには The Fool を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら The Fool が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'The Fool デスクトップを設定' },
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@fool:example.org' },
  ...
  profiles: {
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した The Fool 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'The Fool はモデルの変更を保存しませんでした。'
    },
    search: 'Cron ジョブを検索...',
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。The Fool が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには The Fool バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'The Fool から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
      ...
      worktreeStaleBackend:
        'このリモート接続でワークツリーを作成するには The Fool バックエンドを更新してください — git ワークツリー API 以前のバージョンです。',
  ...
  composer: {
    ...
    placeholderStarting: 'The Fool を起動中...',
    placeholderReconnecting: 'The Fool に再接続中…',
    ...
    attachUrlDesc: 'The Fool がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'The Fool にコミットと PR を任せる',
      agentShipPrompt:
  ...
  updates: {
    stages: {
      ...
      pull: 'もうすぐ完了…',
      pydeps: '仕上げ中…',
      update: 'The Fool を更新中…',
      rebuild: 'デスクトップアプリを再ビルド中…',
      restart: 'The Fool を再起動中…',
      done: '更新が完了しました',
      manual: 'ターミナルから更新',
    ...
    unsupportedMessage: 'このバージョンの The Fool はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの The Fool をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の The Fool バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'The Fool をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'The Fool は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために The Fool デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
    ...
    blockerTitle: 'The Fool を更新するためにローカルプレビューを閉じますか？',
    blockerBody:
      '更新する前に、これらのローカルプレビューを停止する必要があります。ファイルが変更または削除されることはありません。',
    foreignBlockerTitle: '他のプロセスを閉じて The Fool を更新',
    foreignBlockerBody:
      'The Fool はこれらのプロセスを安全に自動終了できません。各プロセスを所有するアプリ、ターミナル、またはサービスを閉じてから、もう一度更新してください。',
    mixedBlockerBody:
      'The Fool は以下のローカルプレビューを閉じることができます。更新を続けるには、他のプロセスを手動で閉じる必要があります。',
  ...
  install: {
    ...
    oneTimeTitle: 'The Fool には一度限りのインストールが必要です',
    ...
    setupChoiceTitle: 'The Fool Desktop をセットアップ',
    setupChoiceDesc:
      'すでに実行している The Fool ゲートウェイに接続するか、このコンピューターに The Fool をローカルインストールします。',
    connectExistingTitle: '既存の The Fool に接続',
    ...
    remoteSetupDesc:
      'ゲートウェイ URL を入力してください。The Fool Desktop がトークンとブラウザーサインインのどちらが必要かを検出します。',
    remoteUrlTitle: 'ゲートウェイ URL',
    remoteUrlDesc: 'The Fool ゲートウェイのベース URL を使用します。リモートの場合は https:// を含めてください。',
    remoteUrlPlaceholder: 'https://gateway.example.com/fool',
    probing: 'ゲートウェイ認証方式を検出中...',
    probeError: 'その The Fool ゲートウェイに到達できませんでした。',
    identityProvider: 'ID プロバイダー',
    authTitle: '認証',
    ...
    settingUpTitle: 'Fool Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の The Fool CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の The Fool インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。The Fool インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'Fool Agent のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'The Fool はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'The Fool を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — The Fool を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に The Fool を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — The Fool が自動接続します',
    ...
    authorizeThere: 'そこで The Fool を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。The Fool をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。The Fool は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'The Fool を再起動中...',
      askRestart: 'The Fool にサーバーの再起動を依頼',
      lookingRestart: taskId => `The Fool は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'The Fool はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      finishedRestarting: message =>
        `The Fool がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'The Fool がサーバーを再起動できませんでした。',
      stillWorking:
        'The Fool はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'The Fool が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'The Fool ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.fool/config.yaml) に追加されます。The Fool はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'The Fool は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
    sudoPlaceholder: 'sudo パスワード',
    secretTitle: 'シークレットが必要です',
    secretDesc: 'The Fool は続行するための認証情報が必要です。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには The Fool Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには The Fool Desktop を再起動してください',
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`fool gateway` は起動していますか？'
    }
  },
