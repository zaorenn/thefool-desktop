...
# coverage category (thefool_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Hermes-managed Node dirs (Windows installer / POSIX $THEFOOL_HOME/node)
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("THEFOOL_LOCAL_STT_COMMAND"):
        return True
    try:
    ...
    *,
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("THEFOOL_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("THEFOOL_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from thefool_cli.setup import prompt_checklist
    except Exception:
        return set()
