...
- `SOUL.md` — a durable persona file that lives in `FOOL_HOME` and serves as the agent's identity (slot #1 in the system prompt)
...
More precisely, it uses the current instance's `FOOL_HOME`, so if you run Hermes with a custom home directory, it will use:
...
- Hermes loads `SOUL.md` only from `FOOL_HOME`
...
If Hermes loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `FOOL_HOME`, the personality belongs to the Hermes instance itself.
