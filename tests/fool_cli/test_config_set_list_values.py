...
Before this fix, ``fool config set platform_toolsets.discord '["file","web"]'``
...
def user_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("FOOL_MANAGED_DIR", raising=False)
...
def test_list_literal_is_parsed_to_list(user_home):
    from fool_cli.config import set_config_value, read_raw_config
...
def test_round_trip_through_load_config(user_home):
    ...
    from fool_cli.config import set_config_value, load_config
