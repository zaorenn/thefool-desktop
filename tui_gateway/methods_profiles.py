...
handlers delegate to the same `fool_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    ...
    # instead (fool_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local FOOL_HOME override — the same
    ...
    if model and provider:
        try:
            from fool_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from fool_cli.config import load_config_readonly, read_user_config_raw
            from fool_cli.web_routers.profiles import _write_profile_model
            from fool_constants import (
                reset_hermes_home_override,
                set_hermes_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the FOOL_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from fool_cli.skills_config import get_disabled_skills
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from fool_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            description = ""
            try:
                from fool_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from fool_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from fool_cli.skills_config import save_disabled_skills
