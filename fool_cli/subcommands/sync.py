...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    fool sync status                 show gate/opt-in/head state
    fool sync pull                   pull and materialize opted-in skills
    fool sync push                   push opted-in skills
    fool sync now                    reconcile: pull then push
    fool sync enable <skill>         opt a skill into sync
    fool sync disable <skill>        opt a skill out of sync
    fool sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    fool sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  fool sync status            what is synced, and from where\n"
            "  fool sync enable my-skill   include a skill in your sync\n"
            "  fool sync now               pull, then push\n"
            "  fool sync propose my-skill  share a skill with your team\n"
        ),
