...
from thefool_cli.setup import setup_agent_settings
...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    .env `THEFOOL_MAX_ITERATIONS` fallback was removed because it was
    ...
    monkeypatch.setattr("thefool_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("thefool_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("thefool_cli.setup.save_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("thefool_cli.setup.remove_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("thefool_cli.setup.save_config", lambda *args, **kwargs: None)
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    Regression guard for the bug where `.env THEFOOL_MAX_ITERATIONS=60`
    ...
    monkeypatch.setattr(
        "thefool_cli.setup.get_env_value",
        lambda key: "60" if key == "THEFOOL_MAX_ITERATIONS" else "",
    )
    monkeypatch.setattr("thefool_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("thefool_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("thefool_cli.setup.save_env_value", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr(
        "thefool_cli.setup.remove_env_value",
    ...
    assert "THEFOOL_MAX_ITERATIONS" in removed_keys
