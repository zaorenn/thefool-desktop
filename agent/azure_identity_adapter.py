...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``fool doctor`` /
    ``fool auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``fool doctor`` and the wizard preflight — never
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `fool doctor` or `az login` to recover.",
                exc,
            )
