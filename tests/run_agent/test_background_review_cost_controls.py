...
def test_routing_to_different_model_marks_routed_and_resolves_credentials():
    ...
    with patch("fool_cli.config.load_config", return_value=cfg), patch("fool_cli.config.load_config_readonly", return_value=cfg), \
         patch("fool_cli.runtime_provider.resolve_runtime_provider", return_value=fake_rp):
        rt = br._resolve_review_runtime(agent)
...
def test_enabled_defaults_true():
    with patch("fool_cli.config.load_config_readonly", return_value={}):
...
def test_enabled_false_disables_automatic_review():
    ...
    with patch("fool_cli.config.load_config_readonly", return_value=cfg):
