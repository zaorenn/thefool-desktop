...
This module is imported by ``fool_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no fool_cli.config,
...
``fool_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(hermes_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect FOOL_HOME?"""
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect FOOL_HOME.
    """
...
def print_fast_version_info() -> None:
    ...
    from fool_cli import __release_date__, __version__
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    subcommand + the FOOL_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("FOOL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
