"""Tests for fool_cli/webhook.py — webhook subscription CLI."""
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "fool_cli.webhook._is_webhook_enabled", lambda: True
    )
...
def test_webhook_base_url_maps_wildcard_hosts_to_localhost(monkeypatch, host):
    monkeypatch.setattr(
        "fool_cli.webhook._get_webhook_config",
...
class TestWebhookEnabledGate:
    ...
    def test_blocks_list_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("fool_cli.webhook._is_webhook_enabled", lambda: False)
        webhook_command(_make_args(webhook_action="list"))
        out = capsys.readouterr().out
    ...
    def test_real_check_disabled(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "fool_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import fool_cli.webhook as wh_mod
