...
- **Optional upgrades** (the skill works zero-config without these; `setup --auto` turns on every
  one it detects, reading credentials from the shell env **and from `$FOOL_HOME/.env`** so keys
...
The engine stores data under `$PDD_DATA_DIR` (default `$FOOL_HOME/unbroker`), written
...
- **Dossiers are plaintext by default** (JSON, `0600` under `FOOL_HOME`). For at-rest encryption run
  ...
  a full-`FOOL_HOME` read; set `PDD_AGE_IDENTITY` to a separate volume for real key separation.
