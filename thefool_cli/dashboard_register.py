...
as ``THEFOOL_DASHBOARD_OAUTH_CLIENT_ID``.
...
This command:
  ...
  3. Writes ``THEFOOL_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``THEFOOL_DASHBOARD_PORTAL_URL`` into ``~/.hermes/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``THEFOOL_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        from thefool_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _print_post_register_hint(
    *,
    ...
    from thefool_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    THEFOOL_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    THEFOOL_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    THEFOOL_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
...
def cmd_dashboard_register(args) -> None:
    ...
    from thefool_cli.auth import AuthError, resolve_nous_access_token
    from thefool_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets THEFOOL_DASHBOARD_OAUTH_CLIENT_ID
    ...
    # THEFOOL_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (THEFOOL_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("THEFOOL_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("THEFOOL_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write THEFOOL_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  THEFOOL_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      THEFOOL_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    try:
        existing_portal = get_env_value("THEFOOL_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("THEFOOL_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # that same callback by taking THEFOOL_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as THEFOOL_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("THEFOOL_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("THEFOOL_DASHBOARD_PUBLIC_URL", public_url)
