...
# Without this, standalone invocations (e.g. after `fool update` reloads
# the module) fail with ModuleNotFoundError for fool_time et al.
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `fool fallback add`, "
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy fool_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from fool_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``fool tools`` config for the ``cron`` platform.
    ...
    try:
        from fool_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``FOOL_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("FOOL_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid FOOL_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
# (FOOL_CRON_TIMEOUT, default 600s): a wedged lock holder stops touching
...
def _cron_inactivity_seconds() -> float:
    """Parse FOOL_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    try:
        ...
        logger.warning("Invalid FOOL_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``fool update`` / ``fool gateway stop`` / systemd restart, or an
...
def _get_hermes_home() -> Path:
    ...
    profile-scoped gateway, so resolving the active FOOL_HOME at call time
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("FOOL_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid FOOL_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within FOOL_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against FOOL_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within FOOL_HOME/scripts/.
    try:
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    model = job.get("model") or os.getenv("FOOL_MODEL") or ""
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            return (
                ...
                "`fool setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    #
    # Bounded with its own timeout (separate from FOOL_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("FOOL_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid FOOL_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    # gateway message. Do not seed FOOL_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads FOOL_SESSION_PLATFORM
    #     and FOOL_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via FOOL_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     FOOL_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read FOOL_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the FOOL_CRON_AUTO_DELIVER_* vars set
    # below, so clearing FOOL_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # FOOL_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ FOOL_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_delivery_vars = (
        "FOOL_CRON_AUTO_DELIVER_PLATFORM",
        "FOOL_CRON_AUTO_DELIVER_CHAT_ID",
        "FOOL_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    _cron_session_var = _VAR_MAP["FOOL_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `fool chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with FOOL_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $FOOL_KANBAN_TASK -- letting
        ...
        # recorded this FOOL_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        from fool_cli.env_loader import (
            load_hermes_dotenv,
        ...
        if delivery_target:
            _VAR_MAP["FOOL_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["FOOL_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["FOOL_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > FOOL_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``fool cron edit --model``
        ...
        prefill_file = (
            os.getenv("FOOL_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject FOOL_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # FOOL_HOME. When a workdir is configured, also inject project
        ...
        # override via FOOL_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from fool_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
def tick(
    ...
    *,
    ...
    try:
        # Global emergency stop (`fool pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `fool resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `fool cron run` that claimed a job
        ...
        # Set FOOL_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("FOOL_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid FOOL_CRON_MAX_PARALLEL value; defaulting to unbounded")
