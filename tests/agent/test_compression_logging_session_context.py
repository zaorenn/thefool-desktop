...
which moves ``FOOL_SESSION_ID`` env + ContextVar). The ``[session_id]`` tag on
log lines comes from a SEPARATE mechanism — ``fool_logging._session_context``
...
def test_logging_session_context_follows_compression_rotation(tmp_path: Path) -> None:
    ...
    fool_logging.set_session_context(parent_sid)
    try:
        ...
        current = getattr(fool_logging._session_context, "session_id", None)
