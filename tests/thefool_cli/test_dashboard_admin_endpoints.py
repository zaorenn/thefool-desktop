...
def _client():
    ...
    # Keep the state DB under the isolated THEFOOL_HOME for any handler that
    # touches it.
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "thefool_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestDebugShareEndpoint:
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        import thefool_cli.debug as dbg
        ...
        monkeypatch.setattr("thefool_cli.dump.run_dump", lambda a: None)
...
def test_spawn_hermes_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    ...
    assert captured["env"]["THEFOOL_NONINTERACTIVE"] == "1"
...
def test_desktop_lifespan_reaps_orphan_gateways_on_startup(
    ...
    startup under THEFOOL_DESKTOP=1 so the stale orphan is cleared first.
    """
    import thefool_cli.web_server as ws
    ...
    # web_server imports the reaper lazily from thefool_cli.gateway, so patch it
    # on that module.
    import thefool_cli.gateway as g
