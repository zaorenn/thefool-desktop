...
3. **Project** — `./.hermes/plugins/<name>/` (requires `FOOL_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `fool memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `fool plugins list` and the interactive `fool plugins` UI), but none load until you explicitly enable them:
...
The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `fool plugins enable <name>`.
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `fool memory` and `fool plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `FOOL_HOME` or `/tmp/hermes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under FOOL_HOME | always | Never |
...
**State** — everything lives at `$FOOL_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `FOOL_HOME` or `/tmp/hermes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `fool plugins enable disk-cleanup` (or check the box in `fool plugins`).

**Disabling again:** `fool plugins disable disk-cleanup`.
...
**Enabling:** `fool plugins enable security-guidance` (or check the box in `fool plugins`).

**Disabling again:** `fool plugins disable security-guidance`.
...
```bash
fool tools          # → Langfuse Observability → Cloud or Self-Hosted
```
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `FOOL_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Hermes session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `fool chat` session lives under one Langfuse session.

**Verify:**

```bash
fool plugins list                 # observability/langfuse should show "enabled"
fool chat -q "hello"              # check the Langfuse UI for a "Hermes turn" trace
```
...
| `FOOL_LANGFUSE_RELEASE` | — | Release/version tag |
| `FOOL_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `FOOL_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `FOOL_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |
...
**Disabling:** `fool plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
**Disabling:** `fool plugins disable google_meet`. Any saved transcripts stay in `~/.hermes/workspace/meetings/` until you remove them.
...
- Unlock state is persisted to `$FOOL_HOME/plugins/hermes-achievements/state.json`
...
**State files** — live under `$FOOL_HOME/plugins/hermes-achievements/`:
...
**Enabling:** Nothing to enable — `hermes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `fool dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/hermes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.hermes/plugins/hermes-achievements/` that ships no dashboard. The plugin's state files under `$FOOL_HOME/plugins/hermes-achievements/` survive — reinstalling preserves your unlock history.
...
- Manifest source is reported as `bundled` in `fool plugins list`
