...
from fool_cli.profiles import export_profile, _DEFAULT_EXPORT_EXCLUDE_ROOT
...
def _patch_named_profile(monkeypatch, profiles_root, profile_dir):
    monkeypatch.setattr("fool_cli.profiles._get_profiles_root", lambda: profiles_root)
    monkeypatch.setattr("fool_cli.profiles.get_profile_dir", lambda n: profile_dir)
    monkeypatch.setattr("fool_cli.profiles.validate_profile_name", lambda n: None)
