...
def _(rid, params: dict) -> dict:
    from fool_cli.input_sanitize import sanitize_user_prompt_text
        ...
        if (
            ...
            if not is_truthy_value(params.get("confirm_truncate")):
                ...
                return _err(
                    ...
                    "(update your Fool client if a rewind was intended)",
                )
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from fool_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _(rid, params: dict) -> dict:
    ...
    def run():
        ...
        try:
            ...
            # FOOL_HOME override (the ContextVar set on the session-create
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not The Fool Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Fool/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not The Fool Desktop. If it serves The Fool/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    def run():
        ...
        try:
            ...
            # the session's FOOL_HOME override (the ContextVar set on the
