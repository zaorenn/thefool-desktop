...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("FOOL_YOLO_MODE", ""))
...
# os.environ["FOOL_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export FOOL_INTERACTIVE.
...
def set_hermes_interactive_context(interactive: bool) -> contextvars.Token:
    ...
    Use this instead of mutating ``os.environ["FOOL_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``FOOL_INTERACTIVE`` env var for legacy callers.
    """
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``FOOL_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _hermes_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("FOOL_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("FOOL_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("FOOL_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("FOOL_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("FOOL_CRON_SESSION")
...
def _is_single_query_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("FOOL_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("FOOL_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set FOOL_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind FOOL_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds FOOL_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("FOOL_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``FOOL_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $FOOL_HOME, or by the resolved absolute
...
# go stale when FOOL_HOME is set after this module is imported, e.g. under the
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart fool gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "fool update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `fool gateway restart`
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # it tracks HOME / FOOL_HOME even when those are set after this module is
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / FOOL_HOME such as ``/``,
...
def _rewrite_resolved_hermes_home(command: str) -> str:
    ...
    Resolves the active ``FOOL_HOME`` at call time (and its symlink-resolved
    ...
    ``$FOOL_HOME``. Matches both POSIX and Windows separators. No-op when the
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``fool send`` message
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["FOOL_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "FOOL_SPINNER_PAUSE" in os.environ:
            del os.environ["FOOL_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    ...
    into one place:
      - process-scoped ``--yolo`` / ``FOOL_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            FOOL_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export FOOL_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set FOOL_INTERACTIVE or "
                "FOOL_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set FOOL_INTERACTIVE or "
            "FOOL_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("FOOL_EXEC_ASK"):
        ...
        # can see (FOOL_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``FOOL_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from fool_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from fool_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("FOOL_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # FOOL_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from fool_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (FOOL_EXEC_ASK) still takes this path even when INTERACTIVE
