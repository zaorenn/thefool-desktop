...
function createSandbox() {
  ...
  return { root, foolHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(foolHome, mockUrl) {
  fs.writeFileSync(
    path.join(foolHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(foolHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.foolHome, mock.url)
  console.log(`  FOOL_HOME: ${sandbox.foolHome}`)
  ...
  const env = {
    ...
    FOOL_DESKTOP_APP_NAME: `FoolDevMock-${Date.now()}`,
  }
