...
for _stream in (sys.stdout, sys.stderr):
    if hasattr(_stream, "reconfigure"):
        try:
            ...
            pass
from fool_constants import get_bundled_skills_dir, get_hermes_home, get_optional_skills_dir
...
def _get_bundled_dir() -> Path:
    ...
    Checks FOOL_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    if (FOOL_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``FOOL_HOME/skills/`` to prevent the catastrophic wipe of
    ...
    # path join, a missing ``FOOL_HOME`` default, a malicious
    ...
    # ``FOOL_HOME``. Require a strict-child relationship so both escape
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = FOOL_HOME / NO_BUNDLED_SKILLS_MARKER
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (FOOL_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
