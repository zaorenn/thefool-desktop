...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run The Fool as an MCP server",
        description=(
            "Manage MCP server connections and run The Fool as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'fool mcp add' to connect to a new server, or\n"
            "'fool mcp serve' to expose The Fool conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run The Fool as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `fool mcp add ...` to fall through to
    # interactive chat.
