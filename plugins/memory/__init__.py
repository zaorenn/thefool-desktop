...
def _get_project_plugins_dir() -> Optional[Path]:
    ...
    Gated on ``FOOL_ENABLE_PROJECT_PLUGINS`` exactly as the general
    ...
    """
    try:
        from fool_cli.plugins import _env_enabled

        if not _env_enabled("FOOL_ENABLE_PROJECT_PLUGINS"):
            return None
...
def _entry_point_package_dir(entry_point) -> Optional[Path]:
    ...
    if entry_point is None:
        return None
    try:
        from fool_cli.plugins import resolve_module_origin
...
def load_memory_provider(
    ...
    *,
    ...
    (``$FOOL_HOME/plugins/<name>/``), and pip entry-point providers.
...
class _ProviderCollector:
    ...
    def _plugin_context(self):
        ...
        if self._context is None:
            from fool_cli.plugins import PluginContext, PluginManifest, get_plugin_manager
