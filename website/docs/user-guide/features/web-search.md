...
Both are configured through a single backend selection. Providers are chosen via `fool tools` or set directly in `config.yaml`.
...
| **xAI (Grok)** | `XAI_API_KEY` or `fool auth add xai-oauth` | ✔ | — | Paid (SuperGrok or per-token) |
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, web search and extract are available through the **[Tool Gateway](tool-gateway.md)** via managed Firecrawl — no API key needed. New installs can run `fool setup --portal` to log in and turn on all gateway tools at once; existing installs can flip just web via `fool tools`.
:::
...
### Quick setup via `fool tools`

Run `fool tools`, navigate to **Web Search & Extract**, and pick a provider. The wizard prompts for the required URL or API key and writes it to your config.
...
Or set via `fool tools` → Web Search & Extract → SearXNG.
...
Run `fool setup` to see which web backend is detected:
