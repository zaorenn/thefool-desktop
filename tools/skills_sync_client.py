...
# sync.base_url (or the FOOL_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: FOOL_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("FOOL_SYNC_BASE_URL")
...
# precedence as base_url: the FOOL_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   FOOL_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   FOOL_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   FOOL_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``FOOL_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    ...
    """
    return _sync_config_bool("FOOL_SYNC_ENABLED", "enabled", default=False)
...
def sync_org_auto_propose() -> bool:
    ...
    return _sync_config_bool(
        "FOOL_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    """
    return _sync_config_bool("FOOL_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Hermes Cloud "on by default"):** every *eligible* skill syncs
      ...
      deployment sets (via ``FOOL_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def stable_device_id() -> str:
    ...
    env_name = (os.environ.get("FOOL_SYNC_DEVICE_NAME") or "").strip()
...
def maybe_push_skills(*, message: str = "fool skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (FOOL_SYNC_ENABLED)
