...
// The electron and @fool/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@fool/shared'))) as {
...
test('token only persists on token-auth remotes; oauth/cloud drop it', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'C', url: 'https://c.fool.cloud', authMode: 'oauth', token: { enc: 'x' } },
    registry
  )
...
test('remote input normalizes URL and auth mode; cloud keeps org', () => {
  ...
  const cloud = normalizeConnectionInput(
    { kind: 'cloud', label: 'Cloud', url: 'https://foo.fool.cloud', authMode: 'oauth', org: 'nous' },
    registry
  )
  ...
  const input = {
    ...
    connections: [
      ...
      },
      {
        id: 'cloud-1',
        kind: 'cloud',
        label: 'The Fool Cloud',
        url: 'https://a.fool.cloud',
        authMode: 'oauth',
        org: 'nous'
      },
...
test('migrate: v1 cloud keeps cloud provenance + org', () => {
  const registry = migrateV1ToRegistry({
    mode: 'cloud',
    remote: { url: 'https://a.fool.cloud', authMode: 'oauth', org: 'nous' }
  })
