...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    actions = [
        DiagnosticAction(
            ...
            payload={
                "command": (
                    f"fool config set {primary_slot}.provider auto"
                )
            },
            suggested=True,
        ),
    ...
    if not fallback_explicit and not main_visible:
        actions.append(DiagnosticAction(
            ...
            payload={
                "command": (
                    f"fool config set {fallback_slot}.provider auto"
                )
            },
        ))
    if not auto_decompose:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Specify manually: fool kanban specify {task_id}",
            payload={"command": f"fool kanban specify {task_id}"},
        ))
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: fool kanban log {task_id}",
                payload={"command": f"fool kanban log {task_id}"},
                suggested=True,
            ))
...
def _rule_review_dependency_deadlock(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(DiagnosticAction(
            ...
            payload={"command": f"fool kanban complete {task_id}"},
            suggested=True,
        ))
    if task_id and child_ids:
        actions.append(DiagnosticAction(
            ...
            payload={"command": f"fool kanban unlink {task_id} {child_ids[0]}"},
        ))
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Check block reasons: fool kanban events {task_id}",
            payload={"command": f"fool kanban events {task_id}"},
            suggested=True,
        ))
    return [Diagnostic(
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * Dispatcher is misconfigured (wrong board, wrong FOOL_HOME).
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "fool kanban diagnostics"},
        ),
    ]
