...
Profile-aware location: ``$THEFOOL_HOME/logs/dashboard-auth.log``.
...
from ``thefool_constants`` or other thefool_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$THEFOOL_HOME/logs/dashboard-auth.log``.

    Uses ``thefool_constants.get_hermes_home()`` (a leaf module — no import
