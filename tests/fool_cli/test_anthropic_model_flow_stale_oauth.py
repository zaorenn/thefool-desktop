"""Tests for Bug #12905 fix — stale OAuth token detection in fool model flow.

Bug 3: `fool model` with `provider=anthropic` skips OAuth re-authentication
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("fool_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from fool_cli.main import _model_flow_anthropic
        cfg = {}
