...
MIGRATION_MARKER = (
    "# managed by hermes-agent — `fool codex-runtime migrate` regenerates this section"
)
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (FOOL_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a FOOL_HOME pointing
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    env: dict[str, str] = {}
    # FOOL_HOME passes through IF SET so the MCP subprocess sees the same
    ...
    # codex's subprocess to inherit whatever FOOL_HOME its launcher sets
    ...
    # — that would override the launcher's FOOL_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("FOOL_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("FOOL_HOME") or ""
    ...
    env["FOOL_QUIET"] = "1"
    env["FOOL_REDACT_SECRETS"] = env.get("FOOL_REDACT_SECRETS", "true")
