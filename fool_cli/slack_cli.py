...
Typical workflow::
    ...
    $ fool slack manifest > slack-manifest.json
    # or:
    $ fool slack manifest --write
...
def _build_full_manifest(
    ...
    """
    from fool_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with The Fool in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with The Fool in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Fool agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``fool_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$FOOL_HOME/slack-manifest.json``)
    ...
    """
    name = getattr(args, "name", None) or "The Fool"
    description = getattr(args, "description", None) or "Your Fool agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "fool slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"fool slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "fool slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "fool slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Fool app\n"
