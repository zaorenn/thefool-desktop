...
|---|---|---|---|
| **What it is** | Buzz Desktop spawns Hermes locally as a managed harness | Buzz's `buzz-acp` bridges a channel to `fool acp` over stdio | Hermes' gateway joins Buzz as a first-class messaging platform |
...
| **Setup** | Automatic discovery | `buzz-acp` env vars | `fool gateway setup` → Buzz |

## ① Buzz Desktop managed runtime

Buzz Desktop ships Hermes as a preset runtime. With Hermes installed the normal way, open **Settings → Runtimes** and Hermes appears automatically — discovery resolves the `hermes-acp` launcher on your login-shell PATH, which the installer writes to `~/.local/bin` (and `fool update` self-heals on older installs).
