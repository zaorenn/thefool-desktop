...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from thefool_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (thefool_cli/web_server.py) so
...
class GatewayConfig:
    ...
    # gateway behaves exactly as before — single THEFOOL_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through thefool_cli.config.load_config, so without this a managed
