...
from fool_cli.main, these tests mock the delegation point and verify
...
from fool_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
from fool_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("fool_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("fool_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    monkeypatch.setattr("fool_cli.nous_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(
