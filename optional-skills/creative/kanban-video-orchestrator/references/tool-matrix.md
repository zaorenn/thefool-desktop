...
key is present in `${FOOL_HOME:-~/.hermes}/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `${FOOL_HOME:-~/.hermes}/.env` → environment variable.
