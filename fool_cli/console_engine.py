...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'fool doctor'")
        and last.startswith("Run 'fool setup'")
    ):
        return text.rstrip()
...
def _split_line(line: str) -> list[str]:
    try:
        ...
        from fool_cli._subprocess_compat import split_command_line
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="fool", add_help=False)  # FOOL-SEAM: prog-name
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("fool_cli.main")
    builder = getattr(module, builder_name)
...
class FoolConsoleEngine:
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        extracted = {
            ...
            "prompt-size": (
                "fool_cli.subcommands.prompt_size",
            ...
            "import": (
                "fool_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _adder_handler("send", (), "fool_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("fool_cli.portal_cli", "add_parser"),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("fool_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("fool_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
...
def _logs(_engine: FoolConsoleEngine, args: list[str]) -> str:
    ...
    from fool_cli.logs import list_logs, tail_log
...
def _sessions_repair(_engine: FoolConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from fool_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: FoolConsoleEngine, args: list[str]) -> str:
    ...
    return _dispatch_extracted_subcommand(
        ...
        fixed=(),
        args=(),
        module_name="fool_cli.subcommands.profile",
...
def _cron_list(_engine: FoolConsoleEngine, args: list[str]) -> str:
    ...
    ns = parser.parse_args(args)
    from fool_cli.cron import cron_list
...
def _cron_pause(_engine: FoolConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from fool console")
