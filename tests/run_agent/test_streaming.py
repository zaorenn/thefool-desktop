...
class TestStreamingFallback:
    ...
    def test_sse_connection_lost_retried_as_transient(self, mock_close, mock_create):
        ...
        # Should retry 3 times (default FOOL_STREAM_RETRIES=2 → 3 attempts)
...
class TestPartialToolCallWarning:
    ...
    def test_partial_tool_call_surfaces_warning(self, mock_close, mock_create):
        ...
        import os as _os
        _prev = _os.environ.get("FOOL_STREAM_RETRIES")
        _os.environ["FOOL_STREAM_RETRIES"] = "0"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
                _os.environ.pop("FOOL_STREAM_RETRIES", None)
            else:
                _os.environ["FOOL_STREAM_RETRIES"] = _prev
...
class TestSilentRetryMidToolCall:
    ...
    def test_silent_retry_recovers_tool_call(
        ...
        import os as _os
        _prev = _os.environ.get("FOOL_STREAM_RETRIES")
        _os.environ["FOOL_STREAM_RETRIES"] = "2"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
    ...
    def test_silent_retry_exhausted_falls_back_to_stub(
        ...
        import os as _os
        _prev = _os.environ.get("FOOL_STREAM_RETRIES")
        _os.environ["FOOL_STREAM_RETRIES"] = "1"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
...
class TestBedrockStreamLivenessWatchdog:
    ...
    def test_stalled_stream_bumps_streak_and_aborts(self, monkeypatch):
        ...
        monkeypatch.setenv("FOOL_STREAM_STALE_TIMEOUT", "0.5")
        monkeypatch.setenv("FOOL_STREAM_STALE_GIVEUP", "5")
    ...
    def test_successful_stream_resets_streak(self, monkeypatch):
        ...
        monkeypatch.setenv("FOOL_STREAM_STALE_TIMEOUT", "60")
