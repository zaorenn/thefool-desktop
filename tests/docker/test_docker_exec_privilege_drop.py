...
3. Files written under $THEFOOL_HOME from a ``docker exec`` session land
   as hermes:hermes — the actual user-visible invariant.
4. The THEFOOL_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``$THEFOOL_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (thefool_cli.container_boot), which s6 runs
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    Use `hermes config set` which writes config.yaml under THEFOOL_HOME.
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    # provoke a write into THEFOOL_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    # THEFOOL_HOME that docker exec just wrote.
    r = subprocess.run(
