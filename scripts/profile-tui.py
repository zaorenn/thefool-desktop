#!/usr/bin/env python3
"""Drive the Hermes TUI under FOOL_DEV_PERF and summarize the pipeline.
...
launches `node dist/entry.js` directly with FOOL_TUI_RESUME set so it
bypasses the fool_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  FOOL_PERF_LOG     (default ~/.hermes/perf.log)
  FOOL_PERF_NODE    (default node from $PATH)
  FOOL_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests FOOL_DEV_PERF wiring is broken).
"""
...
DEFAULT_LOG = Path(os.environ.get("FOOL_PERF_LOG", str(get_hermes_home() / "perf.log")))
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — FOOL_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["FOOL_DEV_PERF"] = "1"
    env["FOOL_DEV_PERF_MS"] = str(args.threshold_ms)
    env["FOOL_DEV_PERF_LOG"] = str(log)
    env["FOOL_TUI_RESUME"] = sid
    ...
    node = os.environ.get("FOOL_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="FOOL_DEV_PERF_MS (0 = capture all)")
