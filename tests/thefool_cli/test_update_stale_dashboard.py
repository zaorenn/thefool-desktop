...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``thefool_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``thefool_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from thefool_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"thefool_cli.main.X\")``
    ...
    live = sys.modules.get("thefool_cli.main")
    if live is None:
        live = importlib.import_module("thefool_cli.main")
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m thefool_cli.main dashboard"),
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("thefool_cli.main._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("thefool_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "thefool_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("thefool_cli._subprocess_compat.bounded_probe_run") as mock_probe:
            mock_probe.return_value = subprocess.CompletedProcess(
                args=["wmic"],
                returncode=0,
                stdout=(
                    "CommandLine=python -m thefool_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("thefool_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["thefool_cli.main"]
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("thefool_cli.dashboard_procs._hermes_home_for_pid", return_value=None), \
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        from thefool_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_dedupes_identical_normalized_cmdlines(self):
        ...
        a = ["/usr/bin/python3", "-m", "thefool_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "thefool_cli.main", "dashboard", "--port", "8300"]
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        assert argv == ["/usr/bin/python3", "-m", "thefool_cli.main", "serve"]
...
class TestPostUpdateStaleModuleReload:
    ...
    adds a new symbol to ``thefool_cli._subprocess_compat`` (as #87134 added
    ``bounded_probe_run``), the post-update dashboard cleanup's lazy
    ``from thefool_cli._subprocess_compat import bounded_probe_run`` hits the
    ...
    def test_node_failures_skip_reload_and_kill(self):
        ...
        with patch.object(update_cmd, "_reload_process_scan_modules") as mock_reload, \
             patch("thefool_cli.main._kill_stale_dashboard_processes") as mock_kill:
    ...
    def test_reload_restores_missing_symbol(self):
        ...
        import thefool_cli._subprocess_compat as compat
        from thefool_cli import update_cmd
        ...
        try:
            ...
            stale = sys.modules["thefool_cli._subprocess_compat"]
            assert hasattr(stale, "bounded_probe_run")
        finally:
            importlib.reload(sys.modules["thefool_cli._subprocess_compat"])
            importlib.reload(sys.modules["thefool_cli.dashboard_procs"])
    ...
    def test_config_reload_list_includes_process_scan_modules(self):
        ...
        assert "thefool_cli._subprocess_compat" in reloaded
        assert "thefool_cli.dashboard_procs" in reloaded
