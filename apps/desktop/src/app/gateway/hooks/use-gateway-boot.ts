...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting The Fool...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting The Fool…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to The Fool gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "The Fool couldn't start"
