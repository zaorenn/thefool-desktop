...
billing screens drive. Companion to ``thefool_cli/nous_account.py`` (which owns
...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``THEFOOL_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → the
...
# thefool_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``THEFOOL_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("THEFOOL_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
