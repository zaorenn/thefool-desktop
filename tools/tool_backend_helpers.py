...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `fool model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``fool auth add
    ...
    3. The credential pool / auth store for ``provider_id`` (``fool auth
    ...
    try:
        ...
        # `fool auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``fool auth add openai-api``) via
