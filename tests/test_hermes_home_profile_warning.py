...
When FOOL_HOME is unset but an active_profile file indicates a non-default
...
def fresh_constants(monkeypatch, tmp_path):
    """Import fool_constants fresh and reset the one-shot warn flag."""
    import importlib
...
class TestGetFoolHomeProfileWarning:
    def test_classic_mode_no_active_profile_no_warning(
        ...
        assert "FOOL_HOME fallback" not in capsys.readouterr().err


    def test_named_profile_unset_home_warns_once(
        self, fresh_constants, tmp_path, capsys
    ):
        """active_profile=coder + FOOL_HOME unset → warn loudly, still return fallback."""
        ...
        assert err.count("FOOL_HOME fallback") == 1
        assert "'coder'" in err
        assert "#18594" in err
        ...
        assert "FOOL_HOME fallback" not in err2

    def test_hermes_home_set_suppresses_warning(
        self, fresh_constants, tmp_path, capsys, monkeypatch
    ):
        """Even if active_profile is 'coder', setting FOOL_HOME suppresses warning."""
