...
``fool memory setup honcho``) without the interactive picker — which matters
...
from fool_cli import memory_setup
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets FOOL_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"FOOL_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("fool_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("fool_cli.tools_config.subprocess.run", fake_run):
