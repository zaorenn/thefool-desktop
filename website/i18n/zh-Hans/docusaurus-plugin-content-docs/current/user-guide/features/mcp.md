...
对于知名 MCP 服务器，`fool mcp add` 接受 `--preset` 标志，自动填写传输层细节，无需手动查找命令和参数。预设只提供默认值——你在同一命令行传入的其他内容（环境变量、头信息、过滤规则）仍然优先生效。
...
你可以使用任意本地名称（`fool mcp add my-codex --preset codex` 完全可以）；预设只提供 `command`/`args` 默认值。
...
```bash
fool mcp serve              # 普通模式
fool mcp serve --verbose    # 在 stderr 输出调试日志
```
...
- 内嵌的 `fool mcp serve` 目前只暴露 **stdio-only** MCP 服务器。如果你需要 HTTP MCP 服务器，请运行单独的适配器——或者，更常见的做法是使用 Hermes 的 MCP **客户端**侧，它已经同时支持 stdio 和 HTTP（`mcp_servers.yaml` / `config.yaml` 中的 `url` + `headers`；参见上方的 [HTTP 服务器](#http-servers)）。
