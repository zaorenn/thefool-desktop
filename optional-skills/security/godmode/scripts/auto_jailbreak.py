...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("THEFOOL_HOME", os.path.expanduser("~/.hermes")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("THEFOOL_HOME", Path.home() / ".hermes")) / "skills" / "red-teaming" / "godmode"
...
THEFOOL_HOME = Path(os.getenv("THEFOOL_HOME", Path.home() / ".hermes"))
CONFIG_PATH = THEFOOL_HOME / "config.yaml"
PREFILL_PATH = THEFOOL_HOME / "prefill.json"
