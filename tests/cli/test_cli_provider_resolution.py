...
from thefool_cli import main as hermes_main
...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setattr("thefool_cli.config.load_config", lambda: config)
    monkeypatch.setattr("thefool_cli.runtime_provider.load_config", lambda: config)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "thefool_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_hermes_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr("thefool_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("thefool_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("thefool_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home.mkdir()
    monkeypatch.setenv("THEFOOL_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "thefool_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "thefool_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("thefool_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("thefool_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "thefool_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "thefool_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "thefool_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "thefool_cli.codex_models.get_codex_model_ids",
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    saved_env = {}
    monkeypatch.setattr("thefool_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("thefool_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("thefool_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("thefool_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "thefool_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("thefool_cli.config.save_config", lambda cfg: None)
    ...
    monkeypatch.setattr("thefool_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("thefool_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("thefool_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "thefool_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("thefool_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("thefool_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "thefool_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "thefool_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("thefool_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("thefool_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("thefool_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("thefool_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("thefool_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("thefool_cli.auth.get_provider_auth_state", lambda provider_id: None)
    ...
    monkeypatch.setattr("thefool_cli.auth._login_nous", _fake_login)
...
def test_auto_provider_name_localhost():
    from thefool_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from thefool_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "thefool_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("thefool_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("thefool_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="THEFOOL_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "THEFOOL_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from thefool_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
