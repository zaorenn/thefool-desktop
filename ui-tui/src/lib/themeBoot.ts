...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into FOOL_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.FOOL_TUI_THEME || env.FOOL_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.FOOL_TUI_BACKGROUND
  ) {
    env.FOOL_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.FOOL_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.FOOL_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into FOOL_TUI_THEME.
