  ...
  if (!hasDetails || !setup) {
    return (
      <p className="border border-destructive/50 px-3 py-2 text-xs text-destructive">
        This provider is installed but unavailable. It may need local dependencies or a manual setup step before The Fool can activate it.
      </p>
    );
  }
  ...
  return (
    ...
    >
      <p className={isBlocked ? "text-destructive" : "text-muted-foreground"}>
        {needsDependencySetup
          ? "Finish these setup steps before The Fool can activate this provider."
      ...
      {setup.required_env.length && needsDependencySetup ? (
        <div className="grid gap-2">
          <p className="text-muted-foreground">
            Required environment values. Fill the matching fields below, or set them in the Fool environment.
          </p>
          <div className="flex flex-wrap gap-2">
                ...
                <div className="flex flex-col gap-4 min-w-0">
                  ...
                  {!selectedMemoryName && (
                    <p className="text-xs text-muted-foreground">
                      The Fool will use the built-in MEMORY.md and USER.md files.
                    </p>
                  )}
