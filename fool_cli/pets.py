"""CLI subcommand: ``fool pets <subcommand>``.
...
def _cmd_list(args) -> int:
    ...
    if getattr(args, "installed", False):
        ...
        if not pets:
            _print("No pets installed. Try: fool pets install boba")
            return 0
    ...
    _print("\nInstall one with: fool pets install <slug>")
    return 0
...
def _cmd_install(args) -> int:
    ...
    if getattr(args, "select", False) or not _has_active_pet():
        ...
        _print(f"  Make it active with: fool pets select {slug}")
    return 0
...
def _cmd_select(args) -> int:
    ...
    if not slug:
        pets = store.installed_pets()
        if not pets:
            _err("✗ no pets installed — run: fool pets install boba")
            return 1
    ...
    if pet is None or not pet.exists:
        _err(f"✗ '{slug}' is not installed — run: fool pets install {slug}")
        return 1
...
def _cmd_show(args) -> int:
    ...
    if pet is None:
        _err("✗ no pet to show — run: fool pets install boba")
        return 1
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        _print("  → no pets installed. Run: fool pets install boba")
        ok = False
    elif active is None:
        _print("  → active pet unresolved. Run: fool pets select <slug>")
        ok = False
    elif not enabled:
        _print("  → pet display is disabled. Run: fool pets select " + active.slug)
