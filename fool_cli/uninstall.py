    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                ...
                if '# Fool Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('fool' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hermes
                if 'fool' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the fool wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "fool",
        Path.home() / ".local" / "bin" / "fool-acp",
        Path.home() / ".local" / "bin" / "hermes-agent",
        Path("/usr/local/bin/fool"),
        Path("/usr/local/bin/fool-acp"),
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains fool_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'fool_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``fool gateway run`` processes
    ...
    try:
        from fool_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from fool_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``FOOL_HOME`` and ``FOOL_GIT_BASH_PATH``
...
#   4. On the ``fool dashboard`` + gateway paths, drops files into
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # Also match if FOOL_HOME was customised to somewhere else — find-and-nuke
    ...
    # backslash-fool as a word-ish boundary.
    return markers
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete FOOL_HOME and FOOL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("FOOL_HOME", "FOOL_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its FOOL_HOME directory.
    ...
    current FOOL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m fool_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "fool_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's FOOL_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``fool uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$FOOL_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from fool_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    print(color("│         ⚕ The Fool Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No The Fool Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Fool agent stays installed.", Colors.CYAN))
    print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Hermes agent is still installed. Run 'fool' to use the CLI,")
...
def run_uninstall(args):
    ...
    # offer to clean them up too instead of leaving zombie FOOL_HOMEs
    ...
    # full wipe (code + ~/.fool data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Fool Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their FOOL_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL The Fool data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Fool code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, hermes_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • The Fool PATH entries from shell configs / Windows User PATH")
    print("  • The Fool wrapper scripts and The Fool-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • The Fool config/data: {hermes_home}")
        ...
        print(f"  • Keep The Fool config/data: {hermes_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    wipe ``$FOOL_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No The Fool-owned PATH entries in User environment")

        log_info("Removing FOOL_HOME / FOOL_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No The Fool-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing fool command...")
    ...
    log_info("Removing The Fool-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No The Fool-managed node/npm/npx symlinks found")
    ...
    #     but the packaged app + Electron userData live OUTSIDE FOOL_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from fool_cli.gui_uninstall import uninstall_gui
    ...
    #     under FOOL_HOME but they're install tooling, not config — safe to
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile FOOL_HOME dirs live under
    ...
    print()
    print("Thank you for using Fool Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m fool_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import fool_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``fool_constants`` + ``fool_cli.colors``
    (and lazily ``fool_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m fool_cli.uninstall")
    parser.add_argument(
        "--mode",
