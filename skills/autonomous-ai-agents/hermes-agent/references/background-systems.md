...
the `cronjob` tool, the `fool cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `fool curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `fool curator run --consolidate`. Routine background curation costs
  zero tokens.
...
see a focused `kanban_*` toolset gated by `FOOL_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `FOOL_KANBAN_BOARD` pinned in env); tenant is a soft namespace
