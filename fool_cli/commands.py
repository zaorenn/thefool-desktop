...
class CommandDef:
    ...
    # Names a key in ``fool_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``fool_cli.slash_exec.run_execute`` and apply only
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`fool_cli.plugins.PluginContext.register_command`. They behave
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``fool skills list`` and the agent's ``/skill-name`` dispatch but
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``fool skills list`` and
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from fool_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via fool_cli.personality."""
        try:
