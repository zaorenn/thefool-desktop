...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``FOOL_PLATFORM`` env var and
    ``FOOL_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("FOOL_PLATFORM")
                ...
                try:
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Fool command; skipping "
...
def build_preloaded_skills_prompt(
    ...
    a deployment's ``FOOL_TUI_SKILLS`` env var could force-load a skill an
