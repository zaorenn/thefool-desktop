...
* ``thefool_cli.config._normalize_custom_provider_entry`` passed the raw
  string through, so ``agent_init``'s accepted-set check dropped it and
  fell through to hostname detection.
* ``thefool_cli.runtime_provider._parse_api_mode`` returned None, with the
  same fall-through.
...
from thefool_cli.config import _canonical_api_mode, _normalize_custom_provider_entry
from thefool_cli.runtime_provider import _parse_api_mode, _VALID_API_MODES
...
class TestCanonicalApiMode:
    ...
    def test_every_alias_lands_in_the_valid_set(self):
        ...
        from thefool_cli.config import _API_MODE_ALIASES
