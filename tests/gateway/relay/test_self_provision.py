...
def _arm(monkeypatch, *, url="wss://connector.example/relay", token="nas-token"):
    ...
    monkeypatch.setattr("fool_cli.auth.resolve_nous_access_token", lambda: token)
...
def test_no_nas_token_is_non_fatal(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.auth.resolve_nous_access_token", _boom)
...
def test_relay_display_name_suppresses_stock_brand(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.skin_engine.get_active_skin", lambda: _Skin())
