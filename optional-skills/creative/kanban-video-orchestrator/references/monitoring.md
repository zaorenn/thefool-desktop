...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `fool profile list` (profile exists?), `fool status` (gateway/dispatcher up?), and verify tenant. |
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `fool profile list` and start
