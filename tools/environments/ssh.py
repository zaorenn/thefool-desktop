...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "fool-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.fool"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.fool directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.fool"
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="fool-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        ...
        rel_base = f"{self._remote_home}/.fool".lstrip("/")
        ssh_cmd = self._build_ssh_command()
