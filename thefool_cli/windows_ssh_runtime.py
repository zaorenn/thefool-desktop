...
def _root() -> Path:
    ...
    # helper's token, including custom THEFOOL_HOME layouts.
...
def process_state(pid: int, creation_time_ns: int, hermes_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    # `-c <bootstrap that runs thefool_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "thefool_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "thefool_cli.main" in argv[2])
    )
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves thefool_cli's own location and prepends its parent, because the
    ...
    # the thefool_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the thefool_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    bootstrap = (
        ...
        "runpy.run_module('thefool_cli.main',run_name='__main__',alter_sys=True)"
    )
