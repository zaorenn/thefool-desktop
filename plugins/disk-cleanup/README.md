...
Scoped strictly to `$FOOL_HOME` and `/tmp/hermes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `FOOL_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `FOOL_HOME` or `/tmp/hermes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$FOOL_HOME/disk-cleanup/` is itself excluded
- `$FOOL_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
