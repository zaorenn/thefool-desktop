...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from fool_cli.cli_output import prompt as _shared_prompt
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return FoolTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # fool_cli/main.py for why the dest is renamed.
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'fool mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
