...
When Hermes runs in Docker with ``FOOL_UID=1000`` / ``FOOL_GID=911``,
the entrypoint chowns the top-level ``FOOL_HOME`` once at startup. But
...
class TestResolveFoolUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("FOOL_UID", "1000")
        monkeypatch.setenv("FOOL_GID", "911")
        from fool_cli.config import _resolve_hermes_uid_gid
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("FOOL_UID", raising=False)
        monkeypatch.delenv("FOOL_GID", raising=False)
        from fool_cli import config as cfg
