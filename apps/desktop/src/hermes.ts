import { JsonRpcGatewayClient } from '@thefool/shared'
...
// the call; each pooled backend already has its own THEFOOL_HOME, so no backend
...
// Cron jobs are stored per-profile (<THEFOOL_HOME>/cron/jobs.json), and the
