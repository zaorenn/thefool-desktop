...
from thefool_cli import resource_limits
...
def test_fresh_process_import_without_posix_resource_is_a_safe_noop():
    code = textwrap.dedent(
        ...
        spec = importlib.util.spec_from_file_location(
            "thefool_cli._resource_limits_without_posix_resource",
            module_path,
        )
...
def test_serve_startup_applies_limit_before_web_server(monkeypatch):
    ...
    import thefool_cli.plugins
    import thefool_cli.web_server

    # cmd_dashboard(headless_backend=True) exports THEFOOL_SERVE_HEADLESS=1 into
    ...
    # 404 path.
    monkeypatch.setenv("THEFOOL_SERVE_HEADLESS", "0")
    ...
    monkeypatch.setattr(thefool_cli.plugins, "discover_plugins", lambda: None)
    monkeypatch.setattr(
...
def test_named_profile_reroute_defers_limit_to_final_process(monkeypatch, tmp_path):
    ...
    assert calls == []
    assert exec_call["argv"][1:5] == ["-m", "thefool_cli.main", "-p", "default"]
    assert exec_call["env"]["THEFOOL_HOME"] == str(tmp_path)
