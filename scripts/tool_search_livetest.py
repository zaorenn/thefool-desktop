...
ORIGINAL_HOME = os.environ.get("THEFOOL_HOME")
...
def reset_module_state():
    """Drop cached modules so the new THEFOOL_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "thefool_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["THEFOOL_HOME"] = str(home)
...
def main():
    ...
    # Restore original THEFOOL_HOME
    if ORIGINAL_HOME is not None:
        os.environ["THEFOOL_HOME"] = ORIGINAL_HOME
    else:
