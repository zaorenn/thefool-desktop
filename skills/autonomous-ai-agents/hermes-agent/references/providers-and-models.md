...
Set via `fool model` (picker) or `fool setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `fool auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `fool auth add openai-codex` |
| qwen-oauth | OAuth | `fool auth add qwen-oauth` |
| minimax-oauth | OAuth | `fool auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`fool auth`).
...
#   fool config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
