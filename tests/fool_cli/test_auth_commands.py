...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from fool_cli.auth_commands import auth_add_command
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "fool_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `fool auth add nous` must also populate providers.nous so the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``fool auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``fool auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("fool_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from fool_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from fool_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``fool auth add xai-oauth`` runs must produce independent pool entries.
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`fool logout --provider openai-codex` must still clear model.provider.
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from fool_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from fool_cli.auth import is_source_suppressed
    from fool_cli.auth_commands import auth_remove_command

    with patch(
        "fool_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "fool_cli.copilot_auth.get_copilot_api_token",
