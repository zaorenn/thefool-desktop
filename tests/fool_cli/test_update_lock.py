"""Cross-process update mutual exclusion (``fool_cli.update_lock``).
...
dashboard-spawned ``fool update`` was mid-``npm install``, which then failed
...
def test_marker_path_follows_process_hermes_home(tmp_path, monkeypatch):
    ...
    All three resolve the *process* FOOL_HOME; a profile-scoped path would
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``fool update``.
...
class TestAncestryHandoff:
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("fool_cli.update_lock._pid_alive", lambda pid: True)
