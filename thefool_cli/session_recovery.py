...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``thefool_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``thefool_state._backup_db_file``.
    """
    from thefool_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
