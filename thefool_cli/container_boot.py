...
``$THEFOOL_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $THEFOOL_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$THEFOOL_HOME``,
    not under ``profiles/``). The dispatcher in ``thefool_cli.gateway``
    ...
    ``$THEFOOL_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's THEFOOL_HOME (typically /opt/data).
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("THEFOOL_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    the gateway and dashboard containers share a bind-mounted THEFOOL_HOME,
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $THEFOOL_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted THEFOOL_HOME, both race to flock() the same s6-log lock
