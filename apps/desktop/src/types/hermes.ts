...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when The Fool doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in fool_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `fool model` uses — instead of
...
export interface ModelOptionProvider {
   ...
   *  the `fool model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) The Fool simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (fool.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors fool_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with The Fool, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `fool tools` active-provider detection). */
  is_active: boolean
 ...
 *  not The Fool); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/fool/update/check` — the backend's own update state.
