...
Setting `addToSystemPackages = true` does two things: puts the `hermes` CLI on your system PATH **and** sets `THEFOOL_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `hermes` in your shell creates a separate `~/.hermes/` directory.
:::
...
- For developers working on the hermes codebase, set `THEFOOL_DEV=1` to bypass container routing and run the local checkout directly
...
This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$THEFOOL_HOME/config.yaml` on each activation.
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$THEFOOL_HOME/.env` at activation time (`nixos-rebuild switch`). Hermes reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
The agent identity file is separate: Hermes loads its primary `SOUL.md` from `$THEFOOL_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
:::tip
Environment variables in `env` values are resolved from `$THEFOOL_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Tokens are stored in `$THEFOOL_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.
...
# Native mode
sudo -u hermes THEFOOL_HOME=/var/lib/hermes/.hermes \
  hermes mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
1. **`THEFOOL_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `THEFOOL_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it hermes-agent hermes config set ...` is also blocked)
...
Plugins are symlinked into `$THEFOOL_HOME/plugins/` at activation time. Hermes discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
nix build .#checks.x86_64-linux.managed-guard       # THEFOOL_MANAGED blocks mutation
...
| `managed-guard` | `THEFOOL_MANAGED=true hermes config set ...` prints the NixOS error |
| `bundled-skills` | Skills directory exists, contains SKILL.md files, `THEFOOL_BUNDLED_SKILLS` is set in wrapper |
...
| `stateDir` | `str` | `"/var/lib/hermes"` | State directory (`THEFOOL_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `hermes` CLI to system PATH and set `THEFOOL_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$THEFOOL_HOME/.env` at activation time |
...
| `extraPlugins` | `listOf package` | `[]` | Directory plugin packages to symlink into `$THEFOOL_HOME/plugins/`. Each must contain `plugin.yaml` |
