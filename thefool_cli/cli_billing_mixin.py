...
Import discipline mirrors ``thefool_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded; honors THEFOOL_DEV_CREDITS_FIXTURE for offline testing.
        """
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``thefool_cli.auth``): ``webbrowser.open()`` returns ``True`` even when
        ...
        if not url:
            return False
        try:
            from thefool_cli.auth import _can_open_graphical_browser, _is_remote_session
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from thefool_cli.nous_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        try:
            from thefool_cli import nous_billing as _nb
