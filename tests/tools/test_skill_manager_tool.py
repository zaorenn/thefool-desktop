...
# come up. The user unpins via `fool curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "fool curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
...
def _curator_pass(tmp_path, *, monkeypatch):
    ...
    Points FOOL_HOME at ``tmp_path/.hermes`` so skill_usage's archive path
