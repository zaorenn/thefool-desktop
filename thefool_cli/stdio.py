...
Entry points (``cli.py`` ``main``, ``thefool_cli/main.py`` CLI dispatch,
...
def configure_windows_stdio() -> bool:
    ...
    Set ``THEFOOL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("THEFOOL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
