...
hand-rolled parsers — ``fool_cli.config.load_env``,
``fool_cli.main._has_any_provider_configured``, and
...
def test_config_load_env_strips_export_prefix(tmp_path):
    from fool_cli.config import invalidate_env_cache, load_env
...
def test_has_any_provider_configured_with_export_prefix(tmp_path, monkeypatch):
    ...
    import fool_cli.main as hmain

    importlib.reload(hmain)
    # get_env_path() derives from FOOL_HOME (set above) → tmp_path/.env, so
