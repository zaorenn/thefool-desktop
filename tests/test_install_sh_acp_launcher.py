...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    script = (
        "set -e\n"
        f"FOOL_BIN={hermes_bin}\n"
        f"FOOL_ENTRYPOINT={entrypoint}\n"
...
def _extract_hermes_agent_shim_block() -> str:
    match = FOOL_AGENT_BLOCK.search(INSTALL_SH.read_text(encoding="utf-8"))
    assert match, (
...
def test_hermes_agent_launcher_cleanup_on_uninstall(tmp_path):
    ...
    from fool_cli.uninstall import remove_wrapper_script
