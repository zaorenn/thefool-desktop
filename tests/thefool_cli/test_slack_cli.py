...
from thefool_cli.slack_cli import _build_full_manifest, slack_manifest_command
...
def _run_console_entrypoint(*argv: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-c",
            "from thefool_cli.main import main; raise SystemExit(main())",
            *argv,
        ],
...
def test_slack_dispatcher_propagates_manifest_failure(monkeypatch):
    from thefool_cli import main as main_module
    from thefool_cli import slack_cli
