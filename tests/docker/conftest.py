...
Override the image with ``FOOL_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("FOOL_TEST_IMAGE", "hermes-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``FOOL_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("FOOL_TEST_IMAGE"):
        return IMAGE_TAG
