...
- **no `FOOL_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`FOOL_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${FOOL_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
FOOL_HOME=/tmp/cdp-probe-home \
FOOL_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
FOOL_DESKTOP_CDP_PORT=9333 \
  npx electron . --user-data-dir=/tmp/cdp-probe-userdata
```

The separate `--user-data-dir` dodges Electron's single-instance lock, so it
cannot collide with a running `hgui`; the separate `FOOL_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `FOOL_HOME` baked in, if you
...
- **A throwaway `FOOL_HOME` has no backend.** The app logs `ECONNREFUSED` for
