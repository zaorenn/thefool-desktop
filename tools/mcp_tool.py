...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`fool_cli.mcp_config._unwrap_exception_group` with
...
class ElicitationHandler:
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (FOOL_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit FOOL_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Hermes' logging (agent.log via fool_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `fool setup` to install MCP support, "
                "then retry."
            )
            ...
            try:
                ...
                if not self._ready.is_set():
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``fool mcp login``. Parking
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local FOOL_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local FOOL_HOME override onto the MCP loop.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``fool tools`` interactive configuration — connects to each
