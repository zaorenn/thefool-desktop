...
the existing module-level functions in fool_cli.gateway and
...
# in ``fool_cli.gateway`` (systemd/launchd) and ``fool_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in fool_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from fool_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from fool_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from fool_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from fool_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in fool_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from fool_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from fool_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from fool_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from fool_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``fool_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's FOOL_HOME blindly;
    derive the shared profile root from the current FOOL_HOME and map the
    ...
    hermes_home = Path(os.environ.get("FOOL_HOME", "/opt/data"))
...
class GatewayNotRegisteredError(S6Error):
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`fool profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources FOOL_HOME (and any extra env) via with-contenv —
             so e.g. ``-e FOOL_HOME=/data/hermes`` is honored at run
        ...
        ``fool_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$FOOL_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``FOOL_HOME``,
        ...
        allocator in ``fool_cli.profiles._allocate_gateway_port``).
        ...
        # gateway/run.py) so s6 always wins. The FOOL_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own FOOL_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${FOOL_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the FOOL_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e FOOL_HOME=/data/hermes``
