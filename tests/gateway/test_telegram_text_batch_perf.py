...
class TestEnvFloatClamped:
    ...
    def test_rejects_nan(self, monkeypatch):
        monkeypatch.setenv("FOOL_TEST_VAR", "nan")
        result = TelegramAdapter._env_float_clamped("FOOL_TEST_VAR", 0.5)
        assert math.isfinite(result)
        assert result == 0.5


    def test_clamps_below_min(self, monkeypatch):
        monkeypatch.setenv("FOOL_TEST_VAR", "0.01")
        assert TelegramAdapter._env_float_clamped(
            "FOOL_TEST_VAR", 0.5, min_value=0.1,
        ) == 0.1
