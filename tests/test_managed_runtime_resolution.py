...
Hermes installs runtimes for itself — ``uv`` at ``$FOOL_HOME/bin/uv``, Node at
``$FOOL_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
...
# Runtimes Hermes provisions into FOOL_HOME and must therefore resolve
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        ...
        + "\n\n$FOOL_HOME/bin (uv) and $FOOL_HOME/node are not on an "
        ...
        "  node/npm -> fool_constants.find_node_executable()\n"
        "  PATH env -> fool_constants.iter_hermes_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(fool_constants, helper))


def test_managed_uv_helpers_exist():
    from fool_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
