...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if not account_info.logged_in:
        ...
        return (
            f"Log in to Nous Portal to use {capability}: run `fool model`. "
    ...
    if account_info.paid_service_access is None:
        ...
        if include_refresh_hint:
            detail += " Run `fool model` to refresh your session."
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `fool model` to refresh The Fool."
        return message
...
def _no_paid_access_message(
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            ...
            f" member spend cap at {billing_url}, then run `fool model`"
            f" to refresh."
        )
...
def _fresh_account_info(
    *,
    ...
    try:
        from fool_cli.auth import get_provider_auth_state, resolve_nous_access_token
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from fool_cli.auth import _decode_jwt_claims
    except Exception:
        return None
