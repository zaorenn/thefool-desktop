...
(``THEFOOL_SESSION_PLATFORM``, ``THEFOOL_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["THEFOOL_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("THEFOOL_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("THEFOOL_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("THEFOOL_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("THEFOOL_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("THEFOOL_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_TYPE: ContextVar = ContextVar("THEFOOL_SESSION_CHAT_TYPE", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("THEFOOL_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("THEFOOL_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("THEFOOL_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_ID_ALT: ContextVar = ContextVar("THEFOOL_SESSION_USER_ID_ALT", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("THEFOOL_SESSION_USER_NAME", default=_UNSET)
...
_SESSION_SCOPE_ID: ContextVar = ContextVar("THEFOOL_SESSION_SCOPE_ID", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("THEFOOL_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("THEFOOL_SESSION_ID", default=_UNSET)
# In-process UI session/window id for multi-session desktop/TUI hosts. This is
# intentionally separate from THEFOOL_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("THEFOOL_UI_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("THEFOOL_SESSION_MESSAGE_ID", default=_UNSET)

_SESSION_PROFILE: ContextVar = ContextVar("THEFOOL_SESSION_PROFILE", default=_UNSET)
...
_CRON_SESSION: ContextVar = ContextVar("THEFOOL_CRON_SESSION", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("THEFOOL_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("THEFOOL_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("THEFOOL_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("THEFOOL_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "THEFOOL_SESSION_PLATFORM": _SESSION_PLATFORM,
    "THEFOOL_SESSION_SOURCE": _SESSION_SOURCE,
    "THEFOOL_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "THEFOOL_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    "THEFOOL_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "THEFOOL_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "THEFOOL_SESSION_USER_ID": _SESSION_USER_ID,
    "THEFOOL_SESSION_USER_ID_ALT": _SESSION_USER_ID_ALT,
    "THEFOOL_SESSION_USER_NAME": _SESSION_USER_NAME,
    "THEFOOL_SESSION_SCOPE_ID": _SESSION_SCOPE_ID,
    "THEFOOL_SESSION_KEY": _SESSION_KEY,
    "THEFOOL_SESSION_ID": _SESSION_ID,
    "THEFOOL_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "THEFOOL_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "THEFOOL_SESSION_PROFILE": _SESSION_PROFILE,
    "THEFOOL_CRON_SESSION": _CRON_SESSION,
    "THEFOOL_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "THEFOOL_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "THEFOOL_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``THEFOOL_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("THEFOOL_SESSION_ID")`` with an ``os.environ`` fallback,
    ...
    ``THEFOOL_SESSION_ID`` for the rest of the process — leaking the child id
...
def set_session_vars(
    ...
    ``os.environ["THEFOOL_CRON_SESSION"]`` fallback, ``"1"`` marks a cron job,
...
def reset_session_vars() -> None:
    ...
    *A's* ``THEFOOL_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``THEFOOL_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``THEFOOL_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("THEFOOL_SESSION_*", default)``.
...
# value (``telegram``) to THEFOOL_SESSION_PLATFORM, while the CLI, TUI, and
# desktop bind THEFOOL_SESSION_SOURCE (``cli``, ``tui``, ``desktop``) and leave
...
# THEFOOL_SESSION_PLATFORM but have no attachment channel behind them.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``THEFOOL_PLATFORM``, then the session platform, then the session
    source, and reports messaging when any of them names a surface outside
    :data:`NON_MESSAGING_SESSION_SURFACES`.
    """
    import os

    platform = os.getenv("THEFOOL_PLATFORM") or get_session_env("THEFOOL_SESSION_PLATFORM", "")
...
def async_delivery_supported() -> bool:
    ...
    Kanban workers (identified by ``THEFOOL_KANBAN_TASK``), which are one-shot
