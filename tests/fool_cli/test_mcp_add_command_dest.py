...
(``fool_cli/_parser.py``).  The dispatcher in ``fool_cli/main.py``
...
result, ``fool mcp add foo --url ...`` silently launches chat instead
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`fool mcp add foo --url ...` must keep args.command == "mcp".
