...
Complements tests/thefool_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/thefool_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/thefool_cli/test_kanban_db.py.
    monkeypatch.setenv("THEFOOL_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_default_spawn_does_not_auto_load_any_skill(kanban_home, monkeypatch):
    ...
    env = captured["env"]
    assert env.get("THEFOOL_KANBAN_TASK") == tid
    assert env.get("THEFOOL_PROFILE") == "some-profile"
...
def test_cli_daemon_help_marks_deprecated():
    ...
    from thefool_cli import kanban as kb_cli
...
def _drive_worker_exit(conn, tid, fake_pid, raw_status):
    ...
    Deliberately resolves ``thefool_cli.kanban_db`` fresh and uses that single
