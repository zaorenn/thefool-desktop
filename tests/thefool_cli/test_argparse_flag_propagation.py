...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("thefool_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "thefool_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets THEFOOL_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    # One driver subprocess parses ALL argvs: thefool_cli.main is a very heavy
    # import (previously 11 separate `python -m thefool_cli.main` spawns with a
...
class TestChatSubparserInheritedValueFlags:
    ...
    Uses the real `thefool_cli._parser.build_top_level_parser()` rather than
