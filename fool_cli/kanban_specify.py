...
Used by ``fool kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``fool_cli/goals.py`` — same aux
...
FOOL_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("FOOL_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)
...
def _profile_author() -> str:
    """Mirror of ``fool_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("FOOL_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=FOOL_KANBAN_SPECIFY_MAX_TOKENS,
