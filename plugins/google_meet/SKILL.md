...
#   Then set OPENAI_API_KEY or FOOL_MEET_REALTIME_KEY in ~/.hermes/.env
```
...
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `FOOL_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
Local:
```
$FOOL_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$FOOL_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `hermes meet node approve`.
