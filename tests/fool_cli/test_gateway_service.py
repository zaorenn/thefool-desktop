...
import fool_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`fool gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `fool gateway restart` in the background; when a
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``FOOL_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` FOOL_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir FOOL_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="FOOL_HOME=/tmp/pytest-of-alice/pytest-42/'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="FOOL_HOME=/tmp/hermes-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="FOOL_HOME={tmp_path}/hermes-home"\n'
...
class TestGeneratedSystemdUnits:
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `fool gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import fool_cli.resource_limits as resource_limits
...
class TestSystemUnitFoolHome:
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        monkeypatch.setenv("FOOL_HOME", str(root_hermes))
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "fool_constants.iter_hermes_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "fool_constants.hermes_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'FOOL_HOME=/home/alice/.hermes' in unit
        assert '/root/.hermes' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's FOOL_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)
...
class TestSystemUnitRefreshSyncsFoolHome:
    ...
    def test_refresh_adopts_unit_hermes_home_before_rewriting(self, tmp_path, monkeypatch):
        ...
        # Correct installed unit (operator's FOOL_HOME + drain timeout).
        monkeypatch.setenv("FOOL_HOME", str(alice_hermes))
        ...
        # Simulate sudo without inherited FOOL_HOME (falls back to root).
        monkeypatch.setenv("FOOL_HOME", str(root_hermes))
        ...
        assert os.environ["FOOL_HOME"] == str(alice_hermes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned FOOL_HOME *before* it reads/compares the unit.
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FOOL_HOME", str(root_profile))
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("FOOL_HOME", str(root_home / ".hermes"))
        ...
        # WorkingDirectory is anchored at the target user's FOOL_HOME (stable,
...
class TestLegacyFoolUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m fool_cli.main gateway run --replace\n"
    )
...
class TestMigrateLegacyCommand:
    """Tests for the `fool gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `fool gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import fool_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "fool_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "fool_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemScopeWizardPreCheck:
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``fool gateway start --system``).
...
class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``fool gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (FOOL_HOME), never the source checkout / worktree, so a relocated or
