...
* The requested key is passed to the child ONLY via the ``FOOL_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``FOOL_SECRET_KEY``) — it is never called per-key in a loop, so a
...
# fool_cli.env_loader can consume both providers identically.
...
def _run_helper(
    ...
    The key is passed as DATA via ``FOOL_SECRET_KEY`` — never interpolated
    ...
    if source_env is os.environ:
        ...
        env = dict(source_env)
    env["FOOL_SECRET_KEY"] = secret_key
...
def get_command_secret(
    *,
    ...
    ``FOOL_SECRET_KEY``.  Returns None on any failure — never raises."""
...
# ---------------------------------------------------------------------------
# Public entry point — called from fool_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_command_secrets(
    *,
    ...
    # FOOL_SECRET_KEY and parse its stdout as a dotenv blob.
    stdout = _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
