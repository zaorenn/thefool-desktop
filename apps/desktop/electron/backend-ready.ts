...
// compile and import the whole `fool_cli.main` → `web_server` → FastAPI/
 ...
 * FOOL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS env override (for users on slow
...
function resolvePortAnnounceTimeoutMs(env = process.env) {
  const parsed = Number(env.FOOL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS)
...
function waitForDashboardPort(child, timeoutMs = resolvePortAnnounceTimeoutMs()) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`The Fool backend: exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for The Fool backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
