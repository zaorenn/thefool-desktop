...
This module is imported by ``thefool_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no thefool_cli.config,
...
``thefool_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(hermes_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect THEFOOL_HOME?"""
...
def _resolved_home() -> str:
    hermes_home = os.environ.get("THEFOOL_HOME", "").strip()
    if hermes_home:
        return hermes_home
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect THEFOOL_HOME.
    """
...
def print_fast_version_info() -> None:
    ...
    from thefool_cli import __release_date__, __version__
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    subcommand + the THEFOOL_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("THEFOOL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
