...
class TestCmdSetupNonTtyGuard:
    ...
    def test_missing_access_token_only(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            "thefool_cli.secrets_cli.bw.find_bws", lambda install_if_missing=False: "/usr/bin/bws"
        )
        monkeypatch.setattr(
            "thefool_cli.secrets_cli._bws_version", lambda _: "2.0.0"
        )

        from thefool_cli.secrets_cli import cmd_setup
    ...
    def test_missing_server_url_with_env_var_passes(self, monkeypatch):
        ...
        monkeypatch.setattr("thefool_cli.secrets_cli.load_config", lambda: {})
        monkeypatch.setattr("thefool_cli.secrets_cli.save_env_value", lambda *a: None)
        monkeypatch.setattr("thefool_cli.secrets_cli.get_env_path", lambda: "/tmp/.env")
        monkeypatch.setattr(
            "thefool_cli.secrets_cli.bw.fetch_bitwarden_secrets",
            lambda **kw: ({"KEY": "val"}, []),
        )
