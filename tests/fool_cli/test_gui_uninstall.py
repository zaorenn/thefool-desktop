"""Tests for fool_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import fool_cli.gui_uninstall as gu


def _make_agent(hermes_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = hermes_home / "hermes-agent"
    (agent_root / "fool_cli").mkdir(parents=True)
    (agent_root / "fool_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(hermes_home: Path) -> None:
    """Create the source-built GUI artifacts a `fool desktop` run produces."""
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `fool desktop` installs is removable."""
    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.setenv("XDG_DATA_HOME", str(tmp_path / "xdg"))

    from fool_cli import linux_desktop_entry as lde
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    assert (hermes_home / "hermes-agent" / "fool_cli").is_dir()
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import fool_cli.uninstall as uninstall
