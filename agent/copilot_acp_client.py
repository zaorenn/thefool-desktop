...
def _resolve_args() -> list[str]:
    raw = os.getenv("FOOL_COPILOT_ACP_ARGS", "").strip()
    if not raw:
        return ["--acp", "--stdio"]
...
def _acp_supported(command: str, args: list[str]) -> bool | None:
    ...
    FOOL_COPILOT_ACP_ARGS transport is the operator's business.
    """
    if "--acp" not in args:
        return True
...
def _resolve_home_dir() -> str:
    ...
    # subprocess with no HOME; callers can set FOOL_HOME explicitly if they
    ...
    def _run_prompt(self, prompt_text: str, *, timeout_seconds: float) -> tuple[str, str]:
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                f"FOOL_COPILOT_ACP_COMMAND / FOOL_COPILOT_ACP_ARGS "
                f"to a working pair."
            )
