...
``fool_cli.config.migrate_config``. Each step is a function
...
``fool_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``fool_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``fool_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``fool_cli.config`` (e.g.
``patch("fool_cli.config.read_raw_config", ...)``) keep working, because
...
#: FOOL_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_hermes_home()}/config.yaml and run `fool setup` to "
...
def _cfg():
    """Return the live ``fool_cli.config`` module (lazy, cycle-free)."""
    from fool_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$FOOL_HOME/plugins/`` for currently installed user plugins.
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `fool curator status` has no
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `fool config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `fool config set`"
                )
...
# behavior set it to true explicitly via `fool config set`.
