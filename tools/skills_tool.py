...
# Cache validation (mirrors fool_cli/profiles.py::_count_skills, d5eee133e):
...
def _skills_dir() -> Path:
    ...
    set FOOL_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped FOOL_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from FOOL_HOME/.env."""
    ...
    # the canonical readers in fool_cli/config.py).
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set FOOL_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (FOOL_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("FOOL_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("FOOL_SESSION_PLATFORM"))
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``FOOL_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``FOOL_PLATFORM`` environment variable
    3. ``FOOL_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from fool_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("FOOL_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``fool skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile FOOL_HOME; the module-level
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from fool_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: fool plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `fool skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `fool sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
