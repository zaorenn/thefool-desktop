...
selected with `FOOL_CUA_DRIVER_CMD` stays
...
```
FOOL_CUA_DRIVER_CMD=/path/to/your/cua-driver
```
...
`FOOL_CUA_DRIVER_VERSION`**, so a local build (reported as
...
`FOOL_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
FOOL_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
FOOL_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```
