"""Tests for thefool_cli.security_advisories.
...
import thefool_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect THEFOOL_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full thefool_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "thefool_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "thefool_cli.config.save_config",
