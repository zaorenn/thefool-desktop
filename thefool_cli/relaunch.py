...
from thefool_cli._parser import (
...
def resolve_hermes_bin() -> Optional[str]:
    ...
    Priority:
      ...
      3. ``None`` → caller should fall back to ``python -m thefool_cli.main``.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m thefool_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "thefool_cli.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m thefool_cli.main``) or a ``.cmd`` batch file, and both
