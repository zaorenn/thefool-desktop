...
Interpreter resolution order is: `FOOL_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.hermes/.hermes_history` or under `FOOL_HOME`.
...
ui-tui/
  ...
  src/
    ...
    types/
      hermes-ink.d.ts            type declarations for @fool/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent FoolCLI subprocess for slash commands
```
