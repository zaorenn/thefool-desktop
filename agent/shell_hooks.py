...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`thefool_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ...
  ``THEFOOL_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``thefool_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
from thefool_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, windows_hide_flags
...
# thefool_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``thefool_cli.config.load_config``
    ...
    setting.  ``THEFOOL_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / THEFOOL_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("THEFOOL_SAFE_MODE"):
        logger.info("THEFOOL_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / THEFOOL_ACCEPT_HOOKS=1 / "
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from thefool_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`thefool_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / THEFOOL_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``THEFOOL_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("THEFOOL_ACCEPT_HOOKS", "").strip().lower()
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`thefool_cli.plugins.invoke_hook`
