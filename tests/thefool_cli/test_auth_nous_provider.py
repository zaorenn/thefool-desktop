...
from thefool_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        import ssl
        from thefool_cli.auth import _resolve_verify
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("THEFOOL_HOME", str(hermes_home))
    monkeypatch.setenv("THEFOOL_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.DEBUG, logger="thefool_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from thefool_cli.auth import get_nous_auth_status
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import thefool_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from thefool_cli.auth import PROVIDER_REGISTRY, _login_nous
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    """
    from thefool_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    """
    from thefool_cli.auth import _refresh_access_token
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect THEFOOL_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("THEFOOL_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without THEFOOL_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    """
    from thefool_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("THEFOOL_SHARED_AUTH_DIR", raising=False)
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from thefool_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
...
class TestNousDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        from thefool_cli.auth import _nous_device_auth_timeout_message
