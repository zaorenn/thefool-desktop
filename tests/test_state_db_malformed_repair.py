...
def test_auto_heal_attempted_once_per_process(tmp_path, monkeypatch):
    ...
    _build_healthy_db(db_path)
    _corrupt_duplicate_fts(db_path)
    monkeypatch.setattr(fool_state, "_repair_attempted_paths", set())

    calls = {"n": 0}
    real_repair = fool_state.repair_state_db_schema
    ...
    monkeypatch.setattr(fool_state, "repair_state_db_schema", fake_repair)
    ...
    monkeypatch.setattr(fool_state, "repair_state_db_schema", real_repair)
...
def test_fts_read_corruption_repaired_in_place(tmp_path):
    ...
    from fool_state import _db_opens_cleanly
...
def test_repair_rebuilds_stale_btree_indexes(tmp_path):
    ...
    reason = fool_state._db_opens_cleanly(db_path)
    assert reason is not None
    ...
    assert fool_state._db_opens_cleanly(db_path) is None
    raw = sqlite3.connect(str(db_path))
...
# gateway service, the Desktop app's own `fool serve` backend, interactive
...
def _lock_held_by_other_process(db_path: Path, hold_seconds: float = 30.0):
    ...
    script = _HOLD_LOCK_SCRIPT.format(
        root=str(Path(fool_state.__file__).parent),
...
def test_repair_skips_surgery_while_another_process_holds_the_lock(
    ...
    _build_healthy_db(db_path)
    _corrupt_duplicate_fts(db_path)
    monkeypatch.setattr(fool_state, "_REPAIR_LOCK_TIMEOUT_SECONDS", 0.5)
    ...
    assert fool_state._db_opens_cleanly(db_path) is not None
...
import sys, json
sys.path.insert(0, {root!r})
from fool_state import repair_state_db_schema
...
def test_two_processes_repairing_at_once_perform_surgery_once(tmp_path):
    ...
    script = _REPAIR_SCRIPT.format(
        root=str(Path(fool_state.__file__).parent), db=str(db_path)
    )
    procs = [
        subprocess.Popen(
            [sys.executable, "-c", script],
