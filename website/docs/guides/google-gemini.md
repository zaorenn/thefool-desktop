...
After running `fool model`, your `~/.hermes/config.yaml` will contain:
...
The `fool model` picker shows Gemini models maintained in Hermes' provider registry. Common choices include:
...
Model availability changes over time. If a model disappears or is not enabled for your key, run `fool model` again and pick one from the current list.
...
If you have not configured Gemini yet, exit the session and run `fool model` first. `/model` switches among already-configured providers and models; it does not collect new API keys.
...
The selected model is not available for your account, region, or key. Run `fool model` again and pick another Gemini model from the current list.

### Gemma model is not shown in `fool model`
...
Upgrade Hermes and rerun `fool model`. The native Gemini adapter sanitizes tool schemas for Gemini's stricter function-declaration format; older builds or custom endpoints may not.
