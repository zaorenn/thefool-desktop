...
Why this exists
---------------
``fool_cli/web_server.py`` owns all dashboard runtime state: the ephemeral
...
``APIRouter`` modules under ``fool_cli/web_routers/`` need those helpers, but
...
thin proxy that resolves ``fool_cli.web_server.<name>`` *at call time*.  This
...
def _server():
    """Return the live ``fool_cli.web_server`` module (imported on demand)."""
    mod = sys.modules.get("fool_cli.web_server")
    if mod is None:  # pragma: no cover - routers are only mounted by web_server
        import fool_cli.web_server as mod  # type: ignore[no-redef]
    return mod
