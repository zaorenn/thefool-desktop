"""Hermes update pipeline — extracted from ``thefool_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``thefool_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``thefool_cli.main`` reference — so existing call sites and test monkeypatches
that target ``thefool_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``thefool_cli.main``.
...
Imports are one-way: ``thefool_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``thefool_cli.main`` reference.

    Lets callers keep patching ``thefool_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``thefool_cli.main`` -> ``thefool_cli.update_cmd`` stays one-way
    at import time.
    """
    from thefool_cli import main
...
def _reload_config_modules() -> None:
    ...
    This function force-reloads ``thefool_cli.config_defaults``,
    ``thefool_cli.config``, and ``thefool_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``thefool_cli._subprocess_compat`` and
    ``thefool_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "thefool_cli.config_defaults",
        "thefool_cli.config",
        "thefool_cli.config_migrations",
        "thefool_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from thefool_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from thefool_cli.config import migrate_config
...
_UPDATE_CRITICAL_FILES = (
    "thefool_cli/main.py",
    "thefool_cli/config.py",
    "thefool_cli/__init__.py",
    "thefool_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    """
    from thefool_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from thefool_constants so this can't drift
...
def _reload_process_scan_modules() -> None:
    ...
    ``from thefool_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``thefool_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``thefool_constants.py``
...
def _print_update_completion(message: str) -> None:
    ...
    print(message)
    action_id = os.environ.get("THEFOOL_ACTION_ID", "")
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False):
    ...
    from thefool_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from thefool_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from thefool_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _capture_active_tool_dependencies() -> list[str]:
    ...
    try:
        from thefool_cli import tools_config
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from thefool_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from thefool_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$THEFOOL_HOME/bin/uv``, but on Termux the official
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """
    from thefool_cli.config import detect_install_method, recommended_update_command_for_method
    ...
    if method == "docker":
        ...
        from thefool_cli.config import format_docker_update_message
    ...
    from thefool_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from thefool_cli.banner import _github_compare_behind
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of THEFOOL_HOME under
    ...
    snapshot_id = None
    try:
        from thefool_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from thefool_constants import
        # get_hermes_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from thefool_cli.config import get_hermes_home as _get_home
    ...
    try:
        from thefool_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    Windows: the Desktop app's backend (``python.exe -m thefool_cli.main
    ...
    for proc in proc_iter:
        ...
        # this venv's path, or a `-m thefool_cli.main ...` invocation tied to
        # this install (install root in the cmdline or as the working dir).
        if not is_holder and venv_prefix in cmdline_low:
            is_holder = True
        if not is_holder and "thefool_cli.main" in cmdline_low:
        ...
        # the `-m thefool_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``thefool_cli.main`` used to import
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from thefool_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    - its cmdline is a Hermes backend (``thefool_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    def _is_backend(argv_low: str) -> bool:
        return "thefool_cli.main" in argv_low and (
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    try:
        ...
        from thefool_cli.gateway import (
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m thefool_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    try:
        from thefool_cli import gateway_windows
        from thefool_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``thefool_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``thefool_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    ``$THEFOOL_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
...
def _rebuild_desktop_after_update(
    ...
    desktop_build_cmd = [sys.executable, "-m", "thefool_cli.main", "desktop", "--build-only"]
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # orphan merge-conflict markers in thefool_cli/config.py bricked
        ...
        # the old bytecode (e.g. get_hermes_home added to thefool_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # symbols in thefool_constants or lazy_deps. The dependency install
        ...
        # seed_profile_skills() uses subprocess with an explicit THEFOOL_HOME so
        # it is not affected by sync_skills()'s module-level THEFOOL_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's THEFOOL_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from thefool_cli.profiles import backfill_profile_envs
        ...
        # ``sys.modules`` cache holds the OLD ``thefool_cli.config`` and
        # ``thefool_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from thefool_cli.config import
        ...
        # it and warn loudly.
        try:
            from thefool_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            ...
            try:
                from thefool_cli.gateway import _get_restart_exit_wait_budget
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``thefool_cli.gateway`` no longer imports,
