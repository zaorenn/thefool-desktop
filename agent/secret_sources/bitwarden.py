...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `fool chat -q ...`
...
_ENCRYPTED_CACHE_INFO = b"fool-bws-encrypted-cache-v1"
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    callers; falls back to `$FOOL_HOME` / `~/.hermes` when home is None.
    """
...
def install_bws(*, force: bool = False) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="fool-bws-") as tmpdir:
        tmp = Path(tmpdir)
