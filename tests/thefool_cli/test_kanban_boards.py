...
* ``_default_spawn`` injects ``THEFOOL_KANBAN_BOARD`` into worker env.
"""
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated THEFOOL_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test THEFOOL_HOME plus a path-init cache reset
    ...
    # Also reset thefool_constants cache so get_default_hermes_root() re-reads.
    try:
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``THEFOOL_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("THEFOOL_KANBAN_DB", str(forced))
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("THEFOOL_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``THEFOOL_KANBAN_BOARD`` / DB / workspaces on spawn.
    ...
    def test_default_spawn_sets_env_vars(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["THEFOOL_KANBAN_BOARD"] == "spawntest"
        assert env["THEFOOL_KANBAN_TASK"] == "t_abc"
        # DB path should match the per-board DB, not the legacy default.
        expected_db = fresh_home / "kanban" / "boards" / "spawntest" / "kanban.db"
        assert env["THEFOOL_KANBAN_DB"] == str(expected_db)
        expected_ws = fresh_home / "kanban" / "boards" / "spawntest" / "workspaces"
        assert env["THEFOOL_KANBAN_WORKSPACES_ROOT"] == str(expected_ws)
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "thefool_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
