...
# $THEFOOL_HOME — auth.json, .env, config.yaml — ends up root-owned and
...
# $THEFOOL_HOME is uid-aligned with the supervised processes. When invoked
...
#
# Opt-out: set THEFOOL_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
case "${THEFOOL_DOCKER_EXEC_AS_ROOT:-}" in
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "hermes-shim: re-run with --user hermes or set THEFOOL_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi
