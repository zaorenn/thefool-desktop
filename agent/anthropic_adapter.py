...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in fool_cli/runtime_provider.py.
    """
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``fool models``) never mutates ``~/.hermes/auth.json``
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        from fool_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("zaorenn", "Anthropic")
                block["text"] = text
