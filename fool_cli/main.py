...
Usage:
    ...
    fool gateway status      # Show gateway status
...
#
# Guarded against ModuleNotFoundError because ``fool_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``fool update``
...
from fool_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python fool_cli/main.py`` (script
# mode — sys.path[0] is fool_cli/, not the repo root) can import the
# canonical helpers; everything else lives in fool_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from fool_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``fool update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from fool_cli.env_loader import ...`` / ``from fool_cli.config import
...
# package directory, so if IT can't import, nothing else in fool_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from fool_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from fool_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    ...
    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``fool tools`` or
...
# that run *before* fool_cli.config is importable. Mirrors the explicit
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``FOOL_TUI=1``, then a real-TTY gate (a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("FOOL_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``FOOL_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("FOOL_TUI_NO_EARLY_DISABLE") == "1":
        return
...
from fool_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from fool_cli.subcommands.cron import build_cron_parser
from fool_cli.subcommands.sync import build_sync_parser
from fool_cli.subcommands.gateway import build_gateway_parser
from fool_cli.subcommands.profile import build_profile_parser
from fool_cli.subcommands.model import build_model_parser
from fool_cli.subcommands.setup import build_setup_parser

from fool_cli.subcommands.whatsapp import build_whatsapp_parser
from fool_cli.subcommands.slack import build_slack_parser
from fool_cli.subcommands.login import build_login_parser
from fool_cli.subcommands.logout import build_logout_parser
from fool_cli.subcommands.auth import build_auth_parser
from fool_cli.subcommands.status import build_status_parser
from fool_cli.subcommands.pause import build_pause_parser
from fool_cli.subcommands.webhook import build_webhook_parser
from fool_cli.subcommands.hooks import build_hooks_parser
from fool_cli.subcommands.doctor import build_doctor_parser
from fool_cli.subcommands.verify import build_verify_parser
from fool_cli.subcommands.security import build_security_parser
from fool_cli.subcommands.approvals import build_approvals_parser
from fool_cli.subcommands.dump import build_dump_parser
from fool_cli.subcommands.debug import build_debug_parser
from fool_cli.subcommands.backup import build_backup_parser
from fool_cli.subcommands.import_cmd import build_import_cmd_parser
...
from fool_cli.subcommands.uninstall import build_uninstall_parser
from fool_cli.subcommands.dashboard import build_dashboard_parser
from fool_cli.subcommands.gui import build_gui_parser
from fool_cli.subcommands.logs import build_logs_parser
from fool_cli.subcommands.prompt_size import build_prompt_size_parser
from fool_cli.subcommands.memory import build_memory_parser
from fool_cli.subcommands.acp import build_acp_parser
from fool_cli.subcommands.tools import build_tools_parser
from fool_cli.subcommands.insights import build_insights_parser
from fool_cli.subcommands.monitoring import build_monitoring_parser
from fool_cli.subcommands.skills import build_skills_parser
from fool_cli.subcommands.pairing import build_pairing_parser
from fool_cli.subcommands.plugins import build_plugins_parser
from fool_cli.subcommands.mcp import build_mcp_parser
from fool_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (fool tools, fool setup, fool model) use
...
# every subsequent ``os.getenv("FOOL_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.hermes/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set FOOL_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    # after the subcommand (`fool chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors fool_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If FOOL_HOME is already set and no explicit flag was given, trust it
    ...
    # See issue #22502.
    hermes_home_env = os.environ.get("FOOL_HOME", "")
    ...
    # reserved ``gateway-default`` slot runs bare ``fool gateway run`` to mean
    # "the root FOOL_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("FOOL_S6_SUPERVISED_CHILD"):
        try:
    ...
    # 3. If we found a profile, resolve and set FOOL_HOME
...
# Bridge security.redact_secrets from config.yaml → FOOL_REDACT_SECRETS env
# var BEFORE fool_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "FOOL_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["FOOL_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from fool_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from fool_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if fool_constants not importable yet
...
from fool_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to fool_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (fool_cli.main._model_flow_*) keep resolving unchanged.
from fool_cli.model_setup_flows import (
...
def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("FOOL_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("FOOL_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`fool_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from fool_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from fool_cli.config import get_env_path, get_hermes_home, load_config
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from fool_cli.config import split_model_config_default
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from fool_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'fool sessions list' to see available sessions, or "
            ...
            try:
                from fool_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @fool/ink is missing or node_modules is behind package-lock.json.
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``FOOL_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("FOOL_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``FOOL_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("FOOL_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "FOOL_HOME": hermes_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``fool update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("FOOL_QUIET"):
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (FOOL_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("FOOL_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $FOOL_HOME/node tree,
        ...
        if not path and bin == "node":
            try:
                from fool_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("FOOL_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with FOOL_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset FOOL_TUI_DIR (e.g. `unset FOOL_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # fool_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        if result.returncode != 0:
            ...
            from fool_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @fool/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @fool/ink resolves through
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from fool_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("FOOL_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["FOOL_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("FOOL_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["FOOL_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("FOOL_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["FOOL_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["FOOL_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    try:
        from fool_cli.config import apply_terminal_config_to_env
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["FOOL_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["FOOL_MODEL"] = model
        env["FOOL_INFERENCE_MODEL"] = model
    if provider:
        env["FOOL_TUI_PROVIDER"] = provider
        env["FOOL_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["FOOL_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["FOOL_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["FOOL_TUI_SKILLS"] = value
    if query:
        env["FOOL_TUI_QUERY"] = query
    if image:
        env["FOOL_TUI_IMAGE"] = image
    if checkpoints:
        env["FOOL_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["FOOL_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["FOOL_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["FOOL_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["FOOL_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["FOOL_ACCEPT_HOOKS"] = "1"
    ...
    # FOOL_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set FOOL_TUI_RESUME themselves.
    env.pop("FOOL_TUI_RESUME", None)
    if resume_session_id:
        env["FOOL_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `fool update` so
    ...
    if code == 42:
        from fool_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    """
    if os.environ.get("FOOL_KANBAN_BOARD"):
        return
    try:
        from fool_cli.kanban_db import get_current_board

        os.environ["FOOL_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'fool sessions list' to see available sessions.")
            sys.exit(1)
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        from fool_cli.foreign_sessions import (
    ...
    try:
        from fool_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from fool_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'fool doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'fool setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from fool_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    # rare launch right after `fool update` changed a bundled skill.
    ...
    if getattr(args, "ignore_user_config", False):
        os.environ["FOOL_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["FOOL_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["FOOL_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from fool_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from fool_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from fool_constants import find_node_executable, with_hermes_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with The Fool?")
        print()
    ...
    # has no creds.json.  Every subsequent `fool gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: fool gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `fool gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  fool gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Fool Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  fool gateway")
        ...
        print()
        print("  Or install as a service: fool gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'fool whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``fool whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``fool_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from fool_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from fool_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from fool_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``fool model``) and the setup wizard
    ...
    from fool_cli.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("FOOL_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'fool model' for "
                "available providers, or run 'fool doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in fool_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``fool model``
...
def _clear_stale_openai_base_url():
    ...
    """
    from fool_cli.config import get_env_value, save_env_value, load_config
...
# `fool model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `fool model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`fool_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  The Fool only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``fool model`` flow, then route aux tasks
    to them here.
    """
    from fool_cli.config import load_config
    from fool_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from fool_cli.auth import _prompt_model_selection
    from fool_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from fool_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use The Fool URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
# downstream call sites read `fool_cli.main._PROVIDER_MODELS` directly,
...
# surface so argparse wiring and test monkeypatches on fool_cli.main.<name>
...
_LAZY_COMMAND_EXPORTS = {
    "fool_cli.sessions_cmd": (
        "cmd_sessions",
    ),
    "fool_cli.dashboard_procs": (
...
def _self():
    ...
    keeps monkeypatches on fool_cli.main.<name> working, exactly like a
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from fool_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``fool setup`` / ``fool model``.
    ...
    """
    from fool_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from fool_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `fool setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from fool_constants import display_hermes_home as _dhh_fn
...
def cmd_login(args):
    ...
    from fool_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from fool_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from fool_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from fool_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from fool_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub == "status":
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set FOOL_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "FOOL_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
...
def cmd_webhook(args):
    ...
    from fool_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub in {None, ""}:
        ...
        print(
            ...
            "\n"
            "Run `fool slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from fool_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from fool_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from fool_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from fool_cli.hooks import hooks_command
...
def cmd_dump(args):
    ...
    from fool_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from fool_cli.debug import run_debug
...
def cmd_skin(args):
    ...
    from fool_cli.skin_cmd import skin_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from fool_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from fool_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from fool_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from fool_cli.config import detect_install_method
    from fool_cli.slash_exec import CommandContext, execute_command
    ...
    try:
        from fool_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from fool_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from fool_cli.uninstall import run_gui_uninstall
    ...
    from fool_cli.uninstall import run_uninstall
...
# Update pipeline lives in fool_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on fool_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in FOOL_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``fool update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``fool update``
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``fool update``
    ...
    ``fool_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "fool_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $FOOL_HOME."""
    from fool_constants import get_hermes_home
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``hermes`` launcher present but ``fool_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``fool update`` to stash the
    lockfile — repeatedly.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "fool_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``fool web``).
    ...
    # build path usable on a stock `py -m fool_cli.main web` invocation.
    ...
    # as `fool update`'s _update_node_dependencies() (ui-tui + web +
...
#   - ``fool update`` can unconditionally call ``fool desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``fool desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $FOOL_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $FOOL_HOME."""
    from fool_constants import get_hermes_home
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``fool desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``fool update`` that pulled new source but hasn't built yet).
    """
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Fool.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working The Fool.exe from backup.")
        print("    Your existing version was kept and still works. Run `fool desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `fool desktop --force-build` to rebuild, or re-run the Fool")
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``fool desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ The Fool Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ The Fool Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `fool desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the FOOL_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    ...
    shell. ``fool uninstall --gui`` removes it.
    """
    try:
        from fool_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from fool_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["FOOL_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["FOOL_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "hermes_root", None):
        env["FOOL_DESKTOP_HERMES_ROOT"] = str(Path(args.hermes_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["FOOL_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["FOOL_DESKTOP_CWD"] = os.getcwd()
    ...
    if sys.platform == "linux" and "FOOL_DESKTOP_PASSWORD_STORE" not in os.environ:
        ...
        if password_store:
            env["FOOL_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # fool update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `fool update` path).
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on The Fool.exe, close any")
                    print("  running The Fool desktop window and retry.")
    ...
    if source_mode:
        print("→ Launching The Fool Desktop from source build...")
    ...
    print(f"→ Launching packaged The Fool Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers live in fool_cli/dashboard_procs.py
...
# fool_cli.main.<name> keep resolving unchanged.
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``fool update``.
...
# =========================================================================
# Fork detection and upstream management for `fool update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $FOOL_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``fool update``.
    ...
    protocol stream (``fool acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        from fool_cli import _install_repair as _ir
        ...
        from fool_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``fool update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from fool_constants import venv_bin_dir
...
def _quarantine_running_hermes_exe(
    ...
    when ``fool update`` runs, ``hermes.exe`` IS the live process, and uv
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `fool update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``fool update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    ``hermes.exe`` shim is locked during ``fool update``, or when uv/distlib
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m fool_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``fool update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `fool update` output above.")
        return
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from fool_constants import venv_python_path
...
class _UpdateOutputStream:
    """Stream wrapper used during ``fool update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``fool update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``fool update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``fool update --gateway``) the update is already
    ...
    try:
        ...
        # fool_cli.config.get_hermes_home to simulate setup failure.
        from fool_cli.config import get_hermes_home as _get_hermes_home
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `fool update --branch=<x>` would actually pull.
    ...
    from fool_cli.update_lock import (
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `fool profile` — show current profile status
    ...
    if action == "list":
        ...
        from fool_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from fool_constants import get_hermes_home as _hh
        ...
        from fool_cli import profile_describer as _pd
        ...
        from fool_cli.profiles import profile_exists, validate_alias_name
        ...
        from fool_cli.profiles import rename_profile
        ...
        from fool_cli.profiles import export_profile
        ...
        from fool_cli.profiles import import_profile
        ...
        import tempfile
        from fool_cli.profile_distribution import (
            plan_install,
        ...
        from fool_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `fool profile install` can be updated."
                )
                sys.exit(1)
        ...
        from fool_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     The Fool {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from fool_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.hermes_requires:
        print(f"  Requires: The Fool {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No fool dashboard processes running.")
        return 0

    print(f"{len(live)} fool dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``fool dashboard register`` for OAuth.
    ...
    try:
        from fool_cli.web_server import should_require_auth
    ...
    try:
        from fool_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `fool dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from fool_cli.config import load_config, save_config
        from fool_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from fool_cli.windows_ssh_runtime import read_token
    ...
    # expanded against the account's $HOME, independent of FOOL_HOME and the
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``FOOL_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``fool dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No fool dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `fool update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with fool serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with FOOL_DESKTOP=1 plus
    # FOOL_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # FOOL_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `fool dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited FOOL_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # FOOL_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (FOOL_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets FOOL_SERVE_HEADLESS below.
    if os.environ.get("FOOL_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("FOOL_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("FOOL_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("FOOL_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → FOOL_HOME points into
    ...
    if (
        ...
        and os.environ.get("FOOL_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "fool_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: FOOL_HOME is explicitly pinned to the
        ...
        # FOOL_HOME.  We must resolve the root explicitly instead of just
        # dropping FOOL_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV FOOL_HOME=/opt/data`), so an unset
        # FOOL_HOME falls back to $HOME/.hermes = /opt/data/.hermes — an
        ...
        try:
            from fool_constants import get_default_hermes_root
            env["FOOL_HOME"] = str(get_default_hermes_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop FOOL_HOME) rather than block the reroute.
            env.pop("FOOL_HOME", None)
    ...
    if _headless_backend:
        ...
        os.environ["FOOL_SERVE_HEADLESS"] = "1"
    elif "FOOL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["FOOL_WEB_DIST"])
            if "FOOL_WEB_DIST" in os.environ
            else PROJECT_ROOT / "fool_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom FOOL_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "FOOL_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # FOOL_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["FOOL_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ FOOL_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset FOOL_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads FOOL_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["FOOL_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from FOOL_WEB_DIST: {_dist_root}")
    ...
    try:
        from fool_cli.mcp_startup import start_background_mcp_discovery
    ...
    from fool_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from fool_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from fool_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from fool_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from fool_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from fool_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from fool_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from fool_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``fool_cli/_parser.py``.
#
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    import it, so the CLI registration never happens and ``fool photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("FOOL_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that FOOL_YOLO_MODE is set before ANY
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from fool_cli.plugins import start_background_plugin_discovery
...
def _try_fast_chat_launch() -> bool:
    ...
    ``hermes`` / ``hermes -w -s foo --yolo`` / ``fool chat`` don't need the
    ...
    """
    if os.environ.get("FOOL_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from fool_cli.config import get_container_exec_info
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("FOOL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["FOOL_DEFER_AGENT_STARTUP"] = "1"
            os.environ["FOOL_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["FOOL_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from fool_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from fool_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from fool_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from fool_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from fool_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from fool_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from fool_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from fool_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from fool_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    ``FOOL_AGENT`` is the Hermes-specific marker. setdefault: never
...
def main():
    ...
    # ``fool update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    # If the checkout changed since the last launch (fool update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``fool update``
    ...
    # ``fool skills install update``) merely defers recovery one launch;
    ...
    # =========================================================================
    # model command  (parser built in fool_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from fool_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from fool_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `fool model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from fool_cli import secrets_cli as _secrets_cli
    from fool_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `fool proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    from fool_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``fool proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from fool_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in fool_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in fool_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in fool_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `fool whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in fool_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from fool_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in fool_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in fool_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in fool_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in fool_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in fool_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in fool_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in fool_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from fool_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from fool_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from fool_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in fool_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in fool_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in fool_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in fool_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in fool_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in fool_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in fool_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in fool_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from fool_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in fool_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: fool_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from fool_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in fool_cli/subcommands/config.py)
    # =========================================================================
    ...
    # =========================================================================
    # pairing command  (parser built in fool_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in fool_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from fool_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in fool_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``hermes --help``, ``fool version``, ``fool logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from fool_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from fool_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from fool_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from fool_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in fool_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in fool_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `fool computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `fool tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `fool computer-use doctor` to run cua-driver's\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from fool_cli.tools_config import (
        ...
        if action == "status":
            import os as _os
            import subprocess
            from fool_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("FOOL_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                    from fool_cli.tools_config import _cua_driver_env
    ...
    # =========================================================================
    # mcp command  (parser built in fool_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    # cmd_sessions lives in fool_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on fool_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in fool_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in fool_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in fool_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in fool_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in fool_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in fool_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in fool_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in fool_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # run `fool desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in fool_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in fool_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in fool_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # --yolo: set FOOL_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # ``fool egress start`` refusing when credential_source=bitwarden
