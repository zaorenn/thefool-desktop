...
def config_home(tmp_path, monkeypatch):
    """Isolated THEFOOL_HOME with a minimal string-format config."""
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from thefool_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        from thefool_cli.auth import _update_config_for_provider
        ...
        with patch("thefool_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        with patch("thefool_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("thefool_cli.auth.deactivate_provider"), \
...
class TestBaseUrlValidation:
    ...
    def test_empty_base_url_keeps_default(self, config_home, monkeypatch):
        ...
        from thefool_cli.main import _model_flow_api_key_provider
        from thefool_cli.config import load_config, get_env_value

        with patch("thefool_cli.auth._prompt_model_selection", return_value="MiniMax-M2"), \
             patch("thefool_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestZaiEndpointPicker:
    ...
    def test_custom_proxy_rejects_invalid_url(self, config_home, monkeypatch, capsys):
        ...
        with patch("thefool_cli.main._prompt_provider_choice", return_value=custom_idx), \
             patch("thefool_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("thefool_cli.auth.deactivate_provider"), \
    ...
    def test_current_endpoint_is_default_choice(self, config_home, monkeypatch):
        ...
        from thefool_cli.auth import ZAI_ENDPOINTS
        from thefool_cli.model_setup_flows import _select_zai_endpoint
        ...
        with patch("thefool_cli.main._prompt_provider_choice", side_effect=fake_choice):
