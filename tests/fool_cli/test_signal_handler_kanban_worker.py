...
The fix: when the process is a dispatcher-spawned worker (``FOOL_KANBAN_TASK``
...
def _synthetic_worker_script() -> str:
    ...
    shape without needing the full fool_cli boot path (config, providers,
...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Mirrors fool_cli/kanban_db.py:_pid_alive on Linux.
...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    """With FOOL_KANBAN_TASK set, SIGTERM should kill the process in <2s
    even when a non-daemon thread is still alive."""
    proc = _spawn_synthetic({"FOOL_KANBAN_TASK": "t_test_28181"})
    try:
        t0 = time.time()
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.02)
        pytest.fail(
            "process still alive 2s after SIGTERM with FOOL_KANBAN_TASK set "
