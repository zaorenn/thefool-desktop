...
web_server - reached via the late-binding seam in :mod:`fool_cli.web_deps`
...
from fool_cli.web_deps import late, LateState
from fool_cli.web_models import (
    MCPCatalogInstall,
    MCPEnabledToggle,
    MCPServerCreate,
    MCPServersReplace,
...
_log = logging.getLogger("fool_cli.web_server")
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from fool_cli.mcp_config import _replace_mcp_servers
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the FOOL_HOME
...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to The Fool and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to The Fool for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to The Fool.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `fool mcp catalog` / `fool mcp install` read.  ``profile`` scopes
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds FOOL_HOME-derived paths in the child.
