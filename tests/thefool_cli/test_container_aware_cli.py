...
def container_env(tmp_path, monkeypatch):
    """Set up a fake THEFOOL_HOME with .container-mode file."""
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    monkeypatch.setenv("THEFOOL_HOME", str(hermes_home))
    monkeypatch.delenv("THEFOOL_DEV", raising=False)
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("thefool_constants.is_container", return_value=False):
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from thefool_cli.main import _exec_in_container
