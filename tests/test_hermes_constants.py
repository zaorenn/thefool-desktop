"""Tests for fool_constants module."""
...
class TestGetDefaultFoolRoot:
    ...
    def test_no_hermes_home_returns_native(self, tmp_path, monkeypatch):
        """When FOOL_HOME is not set, returns ~/.hermes."""
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (FOOL_HOME=<root>/profiles/<name>),
        returns the Docker root, not the profile dir."""
        docker_root = tmp_path / "opt" / "data"
        profile = docker_root / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FOOL_HOME", str(profile))
    ...
    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; FOOL_HOME / home changes invalidate.

        get_default_hermes_root() resolves FOOL_HOME against the native
        ...
        (native home, FOOL_HOME) compared for free each call.
        """
        # FOOL_HOME set to a Docker-profile path: every call resolves the
        ...
        monkeypatch.setattr(
            fool_constants, "_default_hermes_root_memo", None, raising=False
        )
        ...
        # FOOL_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("FOOL_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_hermes_root() == docker_root
        assert resolve_calls["n"] > before, (
            "FOOL_HOME change must force a fresh resolution"
        )
...
class TestGetFoolHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When FOOL_HOME is unset on Windows, use %LOCALAPPDATA%\\hermes."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("FOOL_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(fool_constants, "_profile_fallback_warned", False)
...
class TestFoolManagedNode:
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FOOL_HOME", str(home))
        monkeypatch.setattr(fool_constants, "node_tool_runnable", lambda path: True)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fool_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(fool_constants, "heal_hermes_managed_node", lambda: False)
        monkeypatch.setattr(
            fool_constants,
            "node_tool_runnable",
            lambda path: False,
...
class TestNodeToolRunnable:
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fool_constants, "heal_hermes_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = fool_constants._HERMES_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = fool_constants.find_hermes_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert fool_constants.find_hermes_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert fool_constants.find_hermes_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(fool_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(fool_constants, "_container_detected", True)
        assert is_container() is True
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert fool_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert fool_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert fool_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert fool_constants.windows_path_to_wsl("/home/alex") is None
        assert fool_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(fool_constants, "is_wsl", lambda: True)
        assert fool_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert fool_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert fool_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def _install_fake_psutil(self, monkeypatch, procs):
        ...
        monkeypatch.setattr(fool_constants.sys, "platform", "win32")
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(fool_constants.sys, "platform", "darwin")
        monkeypatch.setenv("FOOL_HOME", str(tmp_path))
        assert fool_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert fool_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("FOOL_HOME", str(home))
        monkeypatch.setenv(
            "FOOL_NODE_TARGET_MAJOR",
            str(fool_constants._HERMES_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(fool_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            fool_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            fool_constants,
        ...
        monkeypatch.setattr(
            fool_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(fool_constants._HERMES_NODE_TARGET_MAJOR)
        ...
        result = fool_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fool_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_hermes_managed_node() is False
        assert fool_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_hermes_managed_node() is False
        assert fool_constants._managed_node_heal_attempted is True
