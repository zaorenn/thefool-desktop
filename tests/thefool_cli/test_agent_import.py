"""Tests for thefool_cli.agent_import — ``hermes import-agent``.
...
Uses the profile_env fixture pattern from tests/thefool_cli/test_profiles.py:
Path.home() and THEFOOL_HOME are redirected to tmp_path so nothing touches
the real ~/.hermes.
"""
...
from thefool_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and THEFOOL_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, THEFOOL_HOME -> tmp/.hermes."""
...
class TestExistingConfigPreserved:
    ...
    def test_failed_write_does_not_truncate_the_existing_config(
        ...
        from thefool_cli import agent_import
