...
fool auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
fool auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
fool auth add anthropic --type oauth
...
Run `fool auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `fool auth` | Interactive pool management wizard |
| `fool auth list` | Show all pools and credentials |
| `fool auth list <provider>` | Show a specific provider's pool |
| `fool auth add <provider>` | Add a credential (prompts for type and key) |
| `fool auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `fool auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `fool auth remove <provider> <index>` | Remove credential by 1-based index |
| `fool auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `fool auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `fool model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via fool model:
fool auth list
# Shows:
...
fool auth add Together.ai --api-key sk-together-second-key
```
...
| Manual entries | Added via `fool auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `fool auth add`) are never auto-pruned.
