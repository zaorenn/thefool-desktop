"""Tests for thefool_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import thefool_cli.gui_uninstall as gu


def _make_agent(hermes_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = hermes_home / "hermes-agent"
    (agent_root / "thefool_cli").mkdir(parents=True)
    (agent_root / "thefool_cli" / "__init__.py").write_text("")
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    assert (hermes_home / "hermes-agent" / "thefool_cli").is_dir()
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import thefool_cli.uninstall as uninstall
