...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`fool_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ...
  ``FOOL_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``fool_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
from fool_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, windows_hide_flags
...
# fool_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``fool_cli.config.load_config``
    ...
    setting.  ``FOOL_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / FOOL_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("FOOL_SAFE_MODE"):
        logger.info("FOOL_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / FOOL_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``fool hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from fool_cli.plugins import SHELL_UNSUPPORTED_HOOKS, VALID_HOOKS
...
def _evaluate_result(
    ...
    * non-JSON / unparseable stdout on a ``fail_closed`` blocking hook —
    ...
    test helper (:func:`run_once`) so ``fool hooks test`` reflects
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`fool_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / FOOL_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ The Fool is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``FOOL_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("FOOL_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `fool hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``fool hooks test`` and ``fool hooks doctor``.

    ``kwargs`` is the same dict that :func:`fool_cli.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``fool hooks test`` could
    ...
    blocking and ``fail_closed`` semantics, so what ``fool hooks test``
