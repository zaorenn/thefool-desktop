...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted The Fool & Nous-trained models',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before The Fool stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters The Fool can read from one file request.',
  approvals: {
    mode: 'How The Fool handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When The Fool updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
