...
Pre-fix bug (issue #33925): ``cli.FoolCLI._toggle_yolo`` mutated only
``os.environ["FOOL_YOLO_MODE"]``. That env var is captured once at
...
read (``session_id``). This avoids the heavy ``FoolCLI`` construction
path used in ``test_cli_init.py``, which is incompatible with this test
file's path layout — ``FoolCLI.__init__`` imports a lot of optional
...
def _clear_approval_state(monkeypatch):
    ...
    # Hermes-driven test runs may inherit FOOL_YOLO_MODE=1 from the parent
...
def _make_stand_in(session_id: str = SESSION_KEY) -> SimpleNamespace:
    ...
    to invoking them on a fully-constructed ``FoolCLI`` for the
    behaviour under test, and avoids the brittle prompt_toolkit / config
    stubbing required to instantiate ``FoolCLI`` from this test file.
    """
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_reflects_toggle(self):
        ...
        assert FoolCLI._is_session_yolo_active(stand_in) is False

        with patch("cli._cprint"):
            FoolCLI._toggle_yolo(stand_in)

        assert FoolCLI._is_session_yolo_active(stand_in) is True
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``hermes --yolo`` sets ``FOOL_YOLO_MODE`` before tool imports, so
...
class TestToggleYoloEndToEnd:
    ...
    def test_toggle_yolo_bypasses_dangerous_command_check(self):
        ...
        try:
            with patch("cli._cprint"):
                FoolCLI._toggle_yolo(stand_in)  # YOLO ON
...
class TestSessionRotationTransfersYolo:
    ...
    def test_transfer_moves_yolo_to_new_session(self):
        ...
        try:
            ...
            FoolCLI._transfer_session_yolo(stand_in, "old-id", "new-id")
    ...
    def test_transfer_handles_empty_inputs_safely(self):
        ...
        # to transfer from "" / to "".
        FoolCLI._transfer_session_yolo(stand_in, "", "new")
        FoolCLI._transfer_session_yolo(stand_in, "old", "")
