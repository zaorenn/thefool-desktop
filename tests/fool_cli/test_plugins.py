"""Tests for the Hermes plugin system (fool_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<hermes_home>/plugins/``; we derive
    ...
    plugin isn't necessarily the current ``FOOL_HOME`` env var — e.g.
    when writing fixtures for two profiles up front and only switching
    ``FOOL_HOME``/``set_hermes_home_override()`` per-profile afterwards
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <FOOL_HOME>/config.yaml.
        # Config is always read from FOOL_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        if home is not None:
            hermes_home = Path(home)
        else:
            import os
            hermes_home_str = os.environ.get("FOOL_HOME")
            if hermes_home_str:
...
class TestPluginDiscovery:
    ...
    def test_enabled_portable_plugin_registers_components(
        ...
        from fool_cli.agent_plugins import MCP_SCHEMA_V1, PLUGIN_SCHEMA_V1
    ...
    def test_disabled_portable_plugin_registers_nothing(self, tmp_path, monkeypatch):
        from fool_cli.agent_plugins import PLUGIN_SCHEMA_V1
        from fool_cli import plugins as plugins_mod
    ...
    def test_portable_author_object_is_normalized_to_stable_string(
        ...
        bundled.mkdir()
        monkeypatch.setenv("FOOL_HOME", str(home))
        monkeypatch.setenv("FOOL_BUNDLED_PLUGINS", str(bundled))
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FOOL_HOME", str(tmp_path / "hermes_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("fool_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_entrypoint_memory_provider_auto_coerced_to_exclusive(
        ...
        monkeypatch.setattr(
            "fool_cli.plugins.importlib.metadata.entry_points",
    ...
    def test_entrypoint_duplicate_does_not_block_directory_provider_activation(
        ...
        # Same-name directory provider under $FOOL_HOME/plugins/.
        hermes_home = tmp_path / "hermes_test"
...
class TestForceReloadSymmetry:
    ...
    def test_re_register_config_hooks_clears_idempotence_set(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "fool_cli.config.load_config", lambda: {"hooks": {}}
        )
...
class TestPreToolCallDirective:
    ...
    def test_first_party_observer_receives_pre_tool_call(self, monkeypatch):
        from fool_cli import observability
        from fool_cli.plugins import get_pre_tool_call_directive
...
class TestResolvePreToolBlock:
    ...
    def test_approve_gate_receives_tool_observability_context(self, monkeypatch):
        from fool_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("fool_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("fool_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from fool_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from fool_cli.plugins import PluginContext, PluginManifest
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `fool plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_register_command_empty_name_rejected(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="fool_cli.plugins"):
    ...
    def test_plugin_manager_scoped_by_hermes_home_override(self, tmp_path):
        ...
        subagent/embedded callers: it swaps ``FOOL_HOME`` via a
        context-local ContextVar, which — per
        ``fool_constants.set_hermes_home_override`` — deliberately does
        NOT touch ``os.environ``. A regression test that only flips the
        ``FOOL_HOME`` env var never exercises this path.
        """
        ...
        def write_engine_plugin(home: Path) -> None:
            _make_plugin_dir(
                ...
                register_body=(
                    ...
                    "        def __init__(self):\n"
                    "            self.home = os.environ.get('FOOL_HOME')\n\n"
                    "        @property\n"
                    "        def name(self):\n"
                    "            return 'home-engine'\n\n"
                    "        def update_from_response(self, usage):\n"
        ...
        # Note: HomeEngine reads os.environ['FOOL_HOME'] itself (simulating
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("fool_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("fool_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("fool_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("fool_cli.plugins.PluginContext.dispatch_tool.__module__", "fool_cli.plugins"):
...
class TestPluginDebugLogging:
    """FOOL_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("FOOL_PLUGINS_DEBUG", raising=False)
        from fool_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from FOOL_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """FOOL_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """FOOL_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".hermes" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("FOOL_HOME", str(prof))
