...
def _is_dispatcher_owned_worker() -> bool:
    """False when FOOL_KANBAN_* is present but this execution does not own it
...
#   - cli.py, fool_cli/*      -> inline on startup (no event loop)
    ...
    if quiet_mode:
        ...
        if profile_scope != CHECK_FN_CACHE_BYPASS:
            cache_key = (
                ...
                cfg_fp,
                bool(os.environ.get("FOOL_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by FOOL_KANBAN_TASK and
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from fool_cli.config import load_config as _load
        cfg = _load() or {}
