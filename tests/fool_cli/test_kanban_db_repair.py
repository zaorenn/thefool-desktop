...
and the ``fool kanban repair`` CLI verb."""
...
def test_dispatch_tick_runs_wal_checkpoint_at_interval(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FOOL_KANBAN_DB", str(db_path))
...
# ---------------------------------------------------------------------------
# repair_db() API + `fool kanban repair` CLI verb
# ---------------------------------------------------------------------------
...
def cli_home(tmp_path, monkeypatch):
    """Isolated FOOL_HOME so kanban_db_path() resolves inside tmp_path."""
