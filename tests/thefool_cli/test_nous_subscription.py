...
from thefool_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    monkeypatch.setattr(
        "thefool_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("thefool_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_path_check(monkeypatch):
    ...
    monkeypatch.setattr(
        "thefool_constants.agent_browser_runnable",
...
def test_has_agent_browser_import_failure_falls_back_to_hermes_managed_node_path(
    ...
    monkeypatch.setattr(
        "thefool_constants.with_hermes_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "thefool_constants.agent_browser_runnable",
