...
def _write_through_provider_state_to_global_root(
    ...
    ``thefool_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        """
        env_var = (
            "THEFOOL_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "THEFOOL_XAI_REFRESH_TIMEOUT_SECONDS"
        )
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from thefool_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        try:
            from thefool_cli.auth import resolve_qwen_runtime_credentials
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from thefool_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
