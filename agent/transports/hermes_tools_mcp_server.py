...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on FOOL_KANBAN_TASK env var
    ...
    # only — the kanban tool gates them on FOOL_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this The Fool process", name
            )
            continue

        description = spec.get("description") or f"The Fool {name} tool"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    os.environ.setdefault("FOOL_QUIET", "1")
    os.environ.setdefault("FOOL_REDACT_SECRETS", "true")
