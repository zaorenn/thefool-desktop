...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``THEFOOL_HOME``. PR #8398.
        """
        from aiohttp import web
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.hermes/.env`` via ``thefool_cli.config``.
    """
    print()
    ...
    try:
        from thefool_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("thefool_cli.config not available; set LINE_* vars manually in ~/.hermes/.env")
        return
