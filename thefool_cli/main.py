...
# IMPORTANT: thefool_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``thefool_bootstrap`` is a
...
# new code references ``thefool_bootstrap`` but the editable install's
...
from thefool_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python thefool_cli/main.py`` (script
# mode — sys.path[0] is thefool_cli/, not the repo root) can import the
# canonical helpers; everything else lives in thefool_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from thefool_cli import _startup_fast  # noqa: E402
...
# ``from thefool_cli.env_loader import ...`` / ``from thefool_cli.config import
...
# package directory, so if IT can't import, nothing else in thefool_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from thefool_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from thefool_cli.oneshot import run_oneshot
...
# that run *before* thefool_cli.config is importable. Mirrors the explicit
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``THEFOOL_TUI=1``, then a real-TTY gate (a
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("THEFOOL_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``THEFOOL_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("THEFOOL_TUI_NO_EARLY_DISABLE") == "1":
        return
...
from thefool_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from thefool_cli.subcommands.cron import build_cron_parser
from thefool_cli.subcommands.sync import build_sync_parser
from thefool_cli.subcommands.gateway import build_gateway_parser
from thefool_cli.subcommands.profile import build_profile_parser
from thefool_cli.subcommands.model import build_model_parser
from thefool_cli.subcommands.setup import build_setup_parser

from thefool_cli.subcommands.whatsapp import build_whatsapp_parser
from thefool_cli.subcommands.slack import build_slack_parser
from thefool_cli.subcommands.login import build_login_parser
from thefool_cli.subcommands.logout import build_logout_parser
from thefool_cli.subcommands.auth import build_auth_parser
from thefool_cli.subcommands.status import build_status_parser
from thefool_cli.subcommands.pause import build_pause_parser
from thefool_cli.subcommands.webhook import build_webhook_parser
from thefool_cli.subcommands.hooks import build_hooks_parser
from thefool_cli.subcommands.doctor import build_doctor_parser
from thefool_cli.subcommands.verify import build_verify_parser
from thefool_cli.subcommands.security import build_security_parser
from thefool_cli.subcommands.approvals import build_approvals_parser
from thefool_cli.subcommands.dump import build_dump_parser
from thefool_cli.subcommands.debug import build_debug_parser
from thefool_cli.subcommands.backup import build_backup_parser
from thefool_cli.subcommands.import_cmd import build_import_cmd_parser
from thefool_cli.subcommands.import_agent import build_import_agent_parser
from thefool_cli.subcommands.config import build_config_parser
from thefool_cli.subcommands.skin import build_skin_parser
from thefool_cli.subcommands.console import build_console_parser
from thefool_cli.subcommands.version import build_version_parser
from thefool_cli.subcommands.update import build_update_parser
from thefool_cli.subcommands.uninstall import build_uninstall_parser
from thefool_cli.subcommands.dashboard import build_dashboard_parser
from thefool_cli.subcommands.gui import build_gui_parser
from thefool_cli.subcommands.logs import build_logs_parser
from thefool_cli.subcommands.prompt_size import build_prompt_size_parser
from thefool_cli.subcommands.memory import build_memory_parser
from thefool_cli.subcommands.acp import build_acp_parser
from thefool_cli.subcommands.tools import build_tools_parser
from thefool_cli.subcommands.insights import build_insights_parser
from thefool_cli.subcommands.monitoring import build_monitoring_parser
from thefool_cli.subcommands.skills import build_skills_parser
from thefool_cli.subcommands.pairing import build_pairing_parser
from thefool_cli.subcommands.plugins import build_plugins_parser
from thefool_cli.subcommands.mcp import build_mcp_parser
from thefool_cli.subcommands.claw import build_claw_parser
...
#
# Many modules cache THEFOOL_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("THEFOOL_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.hermes/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set THEFOOL_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    # Mirrors thefool_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If THEFOOL_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.hermes/profiles/coder or
    # /opt/data/profiles/coder).  If THEFOOL_HOME points to the hermes root
    # instead (e.g. systemd hardcodes THEFOOL_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("THEFOOL_HOME", "")
    ...
    # run-script as THEFOOL_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # "the root THEFOOL_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("THEFOOL_S6_SUPERVISED_CHILD"):
        try:
    ...
    # 3. If we found a profile, resolve and set THEFOOL_HOME
...
# Bridge security.redact_secrets from config.yaml → THEFOOL_REDACT_SECRETS env
# var BEFORE thefool_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves thefool_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from thefool_cli.config import read_raw_config as _read_raw_early
    ...
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "THEFOOL_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["THEFOOL_REDACT_SECRETS"] = str(_early_redact).lower()
...
try:
    from thefool_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from thefool_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if thefool_constants not importable yet
...
# Provider model-selection wizard flows extracted to thefool_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (thefool_cli.main._model_flow_*) keep resolving unchanged.
from thefool_cli.model_setup_flows import (
...
def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("THEFOOL_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("THEFOOL_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`thefool_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from thefool_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from thefool_cli.config import get_env_path, get_hermes_home, load_config
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from thefool_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _resolve_continue_arg(args, *, use_tui: bool) -> None:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            try:
                from thefool_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @thefool/ink is missing or node_modules is behind package-lock.json.
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``THEFOOL_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("THEFOOL_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``THEFOOL_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("THEFOOL_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "THEFOOL_HOME": hermes_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("THEFOOL_QUIET"):
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (THEFOOL_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("THEFOOL_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $THEFOOL_HOME/node tree,
        ...
        if not path and bin == "node":
            try:
                from thefool_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("THEFOOL_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with THEFOOL_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset THEFOOL_TUI_DIR (e.g. `unset THEFOOL_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # thefool_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from thefool_constants import with_hermes_node_path
        ...
        if result.returncode != 0:
            ...
            from thefool_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @thefool/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @thefool/ink resolves through
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from thefool_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("THEFOOL_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["THEFOOL_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("THEFOOL_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["THEFOOL_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("THEFOOL_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["THEFOOL_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["THEFOOL_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    try:
        from thefool_cli.config import apply_terminal_config_to_env
    ...
    env["THEFOOL_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["THEFOOL_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["THEFOOL_MODEL"] = model
        env["THEFOOL_INFERENCE_MODEL"] = model
    if provider:
        env["THEFOOL_TUI_PROVIDER"] = provider
        env["THEFOOL_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["THEFOOL_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["THEFOOL_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["THEFOOL_TUI_SKILLS"] = value
    if query:
        env["THEFOOL_TUI_QUERY"] = query
    if image:
        env["THEFOOL_TUI_IMAGE"] = image
    if checkpoints:
        env["THEFOOL_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["THEFOOL_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["THEFOOL_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["THEFOOL_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["THEFOOL_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["THEFOOL_ACCEPT_HOOKS"] = "1"
    ...
    # THEFOOL_TUI_RESUME is an internal hand-off from the Python wrapper to the
    ...
    # still set THEFOOL_TUI_RESUME themselves.
    env.pop("THEFOOL_TUI_RESUME", None)
    if resume_session_id:
        env["THEFOOL_TUI_RESUME"] = resume_session_id
    ...
    if code == 42:
        from thefool_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``THEFOOL_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("THEFOOL_KANBAN_BOARD"):
        return
    try:
        from thefool_cli.kanban_db import get_current_board

        os.environ["THEFOOL_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def _resolve_use_tui(args) -> bool:
    ...
    try:
        ...
        return False
    if os.environ.get("THEFOOL_TUI") == "1":
        return True
    try:
        from thefool_cli.config import load_config
...
def cmd_chat(args):
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        from thefool_cli.foreign_sessions import (
    ...
    try:
        from thefool_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from thefool_cli.config import load_config as _load_config_for_xai_check
    ...
    if _termux_should_prefetch_update_check():
        try:
            from thefool_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    if getattr(args, "ignore_user_config", False):
        os.environ["THEFOOL_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["THEFOOL_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["THEFOOL_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from thefool_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from thefool_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from thefool_constants import find_node_executable, with_hermes_node_path
...
def cmd_whatsapp_cloud(args):
    ...
    ``thefool_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from thefool_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from thefool_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from thefool_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    from thefool_cli.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("THEFOOL_INFERENCE_PROVIDER") or "auto"
    )
    ...
    # PROVIDER_GROUPS in thefool_cli/models.py). A multi-member group shows one
...
def _clear_stale_openai_base_url():
    ...
    """
    from thefool_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`thefool_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from thefool_cli.config import load_config
    from thefool_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from thefool_cli.auth import _prompt_model_selection
    from thefool_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from thefool_cli.setup import _curses_prompt_choice
...
# downstream call sites read `thefool_cli.main._PROVIDER_MODELS` directly,
...
# surface so argparse wiring and test monkeypatches on thefool_cli.main.<name>
...
def _self():
    ...
    keeps monkeypatches on thefool_cli.main.<name> working, exactly like a
...
def _prompt_api_key(
    ...
    """
    from thefool_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from thefool_cli.env_loader import format_secret_source_suffix
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from thefool_constants import display_hermes_home as _dhh_fn
...
def cmd_login(args):
    ...
    from thefool_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from thefool_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from thefool_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from thefool_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from thefool_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub == "status":
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set THEFOOL_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "THEFOOL_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
...
def cmd_webhook(args):
    ...
    from thefool_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from thefool_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from thefool_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from thefool_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from thefool_cli.hooks import hooks_command
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from thefool_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    ...
    from thefool_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from thefool_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from thefool_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from thefool_cli.config import config_command
...
def cmd_skin(args):
    ...
    from thefool_cli.skin_cmd import skin_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from thefool_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from thefool_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from thefool_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from thefool_cli.config import detect_install_method
    from thefool_cli.slash_exec import CommandContext, execute_command
    ...
    try:
        from thefool_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from thefool_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from thefool_cli.uninstall import run_gui_uninstall
    ...
    from thefool_cli.uninstall import run_uninstall
...
# Update pipeline lives in thefool_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on thefool_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in THEFOOL_HOME) because
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    ``thefool_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "thefool_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $THEFOOL_HOME."""
    from thefool_constants import get_hermes_home
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``hermes`` launcher present but ``thefool_cli`` not importable.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "thefool_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m thefool_cli.main web` invocation.
...
#
# Stamp file: $THEFOOL_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $THEFOOL_HOME."""
    from thefool_constants import get_hermes_home
...
def _desktop_launch_options() -> tuple[list[str], str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the THEFOOL_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    ...
    """
    try:
        from thefool_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from thefool_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["THEFOOL_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["THEFOOL_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "hermes_root", None):
        env["THEFOOL_DESKTOP_HERMES_ROOT"] = str(Path(args.hermes_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["THEFOOL_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["THEFOOL_DESKTOP_CWD"] = os.getcwd()
    ...
    # `THEFOOL_DESKTOP_DISABLE_GPU=... hermes desktop` keeps working.
    config_electron_flags, config_disable_gpu, config_password_store = _desktop_launch_options()
    if config_disable_gpu != "auto" and "THEFOOL_DESKTOP_DISABLE_GPU" not in os.environ:
        env["THEFOOL_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `THEFOOL_DESKTOP_PASSWORD_STORE=... hermes desktop` keeps working.
    if sys.platform == "linux" and "THEFOOL_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["THEFOOL_DESKTOP_PASSWORD_STORE"] = password_store
...
# Dashboard process-hygiene helpers live in thefool_cli/dashboard_procs.py
...
# thefool_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "hermes dashboard",
            "thefool_cli.main dashboard",
            "thefool_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "hermes serve",
            "thefool_cli.main serve",
            "thefool_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
# Install-scoped breadcrumbs live next to the venv (not under $THEFOOL_HOME)
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        from thefool_cli import _install_repair as _ir
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from thefool_constants import venv_bin_dir
    ...
    *,
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m thefool_cli.main <command>"
        )
    else:
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from thefool_constants import venv_python_path
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # thefool_cli.config.get_hermes_home to simulate setup failure.
        from thefool_cli.config import get_hermes_home as _get_hermes_home
...
def cmd_update(args):
    ...
    from thefool_cli.update_lock import (
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from thefool_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from thefool_constants import get_hermes_home as _hh
        ...
        from thefool_cli import profile_describer as _pd
        ...
        from thefool_cli.profiles import profile_exists, validate_alias_name
        ...
        from thefool_cli.profiles import rename_profile
        ...
        from thefool_cli.profiles import export_profile
        ...
        from thefool_cli.profiles import import_profile
        ...
        import tempfile
        from thefool_cli.profile_distribution import (
            plan_install,
        ...
        from thefool_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from thefool_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from thefool_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from thefool_cli.web_server import should_require_auth
    ...
    try:
        from thefool_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    if choice == "2":
        ...
        print(
            ...
            "THEFOOL_DASHBOARD_OAUTH_CLIENT_ID into ~/.hermes/.env for you.\n"
    ...
    try:
        from thefool_cli.config import load_config, save_config
        from thefool_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from thefool_cli.windows_ssh_runtime import read_token
    ...
    # expanded against the account's $HOME, independent of THEFOOL_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_hermes_home(): a non-default sticky profile (or any THEFOOL_HOME pointing
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``THEFOOL_WEB_DIST`` to ``.../app.asar/dist`` or
...
def cmd_dashboard(args):
    ...
    # Desktop Electron spawns its backend with THEFOOL_DESKTOP=1 plus
    # THEFOOL_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # THEFOOL_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `hermes dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited THEFOOL_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # THEFOOL_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (THEFOOL_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets THEFOOL_SERVE_HEADLESS below.
    if os.environ.get("THEFOOL_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("THEFOOL_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("THEFOOL_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("THEFOOL_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → THEFOOL_HOME points into
    ...
    if (
        ...
        and os.environ.get("THEFOOL_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "thefool_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: THEFOOL_HOME is explicitly pinned to the
        ...
        # THEFOOL_HOME.  We must resolve the root explicitly instead of just
        # dropping THEFOOL_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV THEFOOL_HOME=/opt/data`), so an unset
        # THEFOOL_HOME falls back to $HOME/.hermes = /opt/data/.hermes — an
        ...
        try:
            from thefool_constants import get_default_hermes_root
            env["THEFOOL_HOME"] = str(get_default_hermes_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop THEFOOL_HOME) rather than block the reroute.
            env.pop("THEFOOL_HOME", None)
    ...
    if _headless_backend:
        ...
        os.environ["THEFOOL_SERVE_HEADLESS"] = "1"
    elif "THEFOOL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["THEFOOL_WEB_DIST"])
            if "THEFOOL_WEB_DIST" in os.environ
            else PROJECT_ROOT / "thefool_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom THEFOOL_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "THEFOOL_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # THEFOOL_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["THEFOOL_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ THEFOOL_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset THEFOOL_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads THEFOOL_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["THEFOOL_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from THEFOOL_WEB_DIST: {_dist_root}")
    ...
    try:
        from thefool_cli.mcp_startup import start_background_mcp_discovery
    ...
    from thefool_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from thefool_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from thefool_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from thefool_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from thefool_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from thefool_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from thefool_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from thefool_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``thefool_cli/_parser.py``.
#
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("THEFOOL_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that THEFOOL_YOLO_MODE is set before ANY
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from thefool_cli.plugins import start_background_plugin_discovery
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["THEFOOL_SAFE_MODE"] = "1"
    os.environ["THEFOOL_IGNORE_USER_CONFIG"] = "1"
    os.environ["THEFOOL_IGNORE_RULES"] = "1"
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("THEFOOL_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    try:
        from thefool_cli.config import get_container_exec_info
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("THEFOOL_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["THEFOOL_DEFER_AGENT_STARTUP"] = "1"
            os.environ["THEFOOL_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["THEFOOL_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from thefool_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from thefool_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from thefool_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from thefool_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from thefool_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from thefool_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from thefool_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from thefool_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from thefool_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    ``THEFOOL_AGENT`` is the Hermes-specific marker. setdefault: never
...
def main():
    ...
    # =========================================================================
    # model command  (parser built in thefool_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from thefool_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from thefool_cli.fallback_cmd import cmd_fallback
    ...
    from thefool_cli import secrets_cli as _secrets_cli
    from thefool_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    from thefool_cli import proxy_cli as _proxy_cli
    ...
    # migrate command
    # =========================================================================
    from thefool_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in thefool_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in thefool_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in thefool_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    # =========================================================================
    # slack command  (parser built in thefool_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from thefool_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in thefool_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in thefool_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in thefool_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in thefool_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in thefool_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in thefool_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in thefool_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from thefool_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from thefool_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from thefool_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in thefool_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in thefool_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in thefool_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in thefool_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in thefool_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in thefool_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in thefool_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in thefool_cli/subcommands/backup.py)
    # =========================================================================
    ...
    from thefool_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in thefool_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: thefool_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from thefool_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in thefool_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in thefool_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in thefool_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in thefool_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in thefool_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from thefool_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in thefool_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    try:
        from thefool_cli.curator import register_cli as _register_curator_cli
    ...
    try:
        from thefool_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from thefool_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in thefool_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in thefool_cli/subcommands/tools.py)
    # =========================================================================
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from thefool_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("THEFOOL_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                    from thefool_cli.tools_config import _cua_driver_env
                    ...
                    pass
                from thefool_cli.tools_config import _cua_version_summary
                ...
                origin = " [custom binary from THEFOOL_CUA_DRIVER_CMD]" if override else ""
                if version:
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by THEFOOL_CUA_DRIVER_CMD, or unset "
    ...
    # =========================================================================
    # mcp command  (parser built in thefool_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    # cmd_sessions lives in thefool_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on thefool_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in thefool_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in thefool_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in thefool_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in thefool_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in thefool_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in thefool_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in thefool_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in thefool_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # =========================================================================
    # gui command  (parser built in thefool_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in thefool_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in thefool_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # --yolo: set THEFOOL_YOLO_MODE *before* plugin discovery.  The call to
