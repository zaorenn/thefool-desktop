...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``THEFOOL_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``THEFOOL_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
deployments where ``THEFOOL_HOME`` points outside ``~/.hermes`` (e.g.
``/opt/hermes``), ``<root>`` is ``THEFOOL_HOME``. Legacy env-var
overrides still work:

* ``THEFOOL_KANBAN_DB`` — pin the database file path directly.
* ``THEFOOL_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``THEFOOL_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``THEFOOL_KANBAN_DB``,
``THEFOOL_KANBAN_WORKSPACES_ROOT``, and ``THEFOOL_KANBAN_BOARD`` into
...
from thefool_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``thefool_constants.VALID_REASONING_EFFORTS`` plus
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("THEFOOL_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active THEFOOL_HOME so dispatcher- and
...
# ``THEFOOL_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``THEFOOL_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("THEFOOL_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``THEFOOL_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("THEFOOL_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``THEFOOL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "THEFOOL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
def kanban_home() -> Path:
    ...
    1. ``THEFOOL_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``THEFOOL_HOME`` is ``<root>/profiles/<name>``, and returns
       ``THEFOOL_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``THEFOOL_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("THEFOOL_KANBAN_HOME", "").strip()
    if override:
...
def get_current_board() -> str:
    ...
    1. ``THEFOOL_KANBAN_BOARD`` env var (set by the dispatcher on worker
    ...
    env = os.environ.get("THEFOOL_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``THEFOOL_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("THEFOOL_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``THEFOOL_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("THEFOOL_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``THEFOOL_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("THEFOOL_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
class Task:
    ...
    # ``thefool_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``THEFOOL_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (thefool_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``THEFOOL_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("THEFOOL_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``thefool_cli.sqlite_safe_read``).
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from thefool_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see thefool_cli.sqlite_safe_read), so it must only run once the board has
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``THEFOOL_KANBAN_DB`` env → ``THEFOOL_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        from thefool_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See thefool_state._WAL_INCOMPAT_MARKERS for detection logic.
                from thefool_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``THEFOOL_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from thefool_cli.sqlite_safe_read import file_length_matches_header
...
def create_task(
    ...
    *,
    ...
    if project_id:
        from thefool_cli import projects_db as _pdb
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``THEFOOL_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
...
# without thrashing. Overridable via ``THEFOOL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from thefool_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``THEFOOL_KANBAN_DB``
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from thefool_cli.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from thefool_cli.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from thefool_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_hermes_argv() -> list[str]:
    ...
    # ``thefool_cli.main`` is the console-script target declared in
...
def _resolve_hermes_argv() -> list[str]:
    ...
    1. ``$THEFOOL_BIN`` — explicit operator override. Path-like values are
    ...
    3. ``sys.executable -m thefool_cli.main`` — fallback for setups where
    ...
    local (not imported from gateway) because ``thefool_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("THEFOOL_BIN", "").strip()
    if env_bin:
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``THEFOOL_KANBAN_TASK`` is set.
    """
    if not hermes_home:
        return None
    try:
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for THEFOOL_HOME=%r (%s)",
            hermes_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    ``THEFOOL_KANBAN_DB`` / ``THEFOOL_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject THEFOOL_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `hermes -p <name>` the
    # _apply_profile_override() runs *before* thefool_constants is imported.
    # If THEFOOL_HOME is absent from the child's env, get_hermes_home() falls
    # back to Path.home() / ".hermes" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from thefool_cli.profiles import resolve_profile_env
    try:
        env["THEFOOL_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via THEFOOL_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # THEFOOL_HOME never had profiles created.
        pass
    if task.tenant:
        env["THEFOOL_TENANT"] = task.tenant
    env["THEFOOL_KANBAN_TASK"] = task.id
    env["THEFOOL_KANBAN_WORKSPACE"] = workspace
    ...
    # ("work kanban task t_…").
    env["THEFOOL_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["THEFOOL_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["THEFOOL_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["THEFOOL_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["THEFOOL_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["THEFOOL_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # rewrites THEFOOL_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hermes_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["THEFOOL_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["THEFOOL_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["THEFOOL_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["THEFOOL_KANBAN_BOARD"] = resolved_board
    # THEFOOL_PROFILE is the author the kanban_comment tool defaults to.
    # `hermes -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["THEFOOL_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited THEFOOL_TUI=1
    ...
    env.pop("THEFOOL_TUI", None)

    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped THEFOOL_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("THEFOOL_HOME"))
    if worker_toolsets:
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled THEFOOL_PROFILE values like "hermes-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``thefool_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from thefool_constants import get_default_hermes_root
