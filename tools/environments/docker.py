...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``FOOL_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("FOOL_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using FOOL_DOCKER_BINARY override: %s", override)
        return override
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``FOOL_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `fool egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `fool egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `fool egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `fool egress setup` to mint provider "
    ...
    # through the proxy.  Keep the FOOL_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"FOOL_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if egress_env_overrides:
            try:
                from fool_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from fool_cli.config import load_config as _load_cfg_for_precedence
