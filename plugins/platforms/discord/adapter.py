...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("FOOL_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("FOOL_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = self._finite_positive_config_float(
            ...
            15.0,
            env_key="FOOL_DISCORD_LIVENESS_INTERVAL_SECONDS",
        )
        self._liveness_failure_threshold = self._config_int(
            "websocket_liveness_failure_threshold",
            2,
            env_key="FOOL_DISCORD_LIVENESS_FAILURE_THRESHOLD",
        )
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``FOOL_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _register_slash_commands(self) -> None:
        ...
        # fool_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from fool_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from fool_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from fool_cli.commands import discord_skill_commands_by_category
...
def _component_check_auth(
    ...
    the pairing store (``fool pairing approve``).
    ...
    # so users approved via ``fool pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``fool update`` prompts.
...
# is not in this process (e.g. ``fool cron`` running standalone) and no live
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``fool_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `fool setup` to install Discord support.",
        # Interactive setup wizard — replaces the central
        # fool_cli/setup.py::_setup_discord function.  Same shape as Teams.
