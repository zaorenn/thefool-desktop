"""Tests for thefool_cli.build_info — baked-in build SHA resolution.

The build SHA is written by the Dockerfile's ``THEFOOL_GIT_SHA`` build-arg
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    ...
    from thefool_cli import build_info
