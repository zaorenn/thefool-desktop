...
from fool_cli import main as hermes_main
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "fool_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("fool_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home.mkdir()
    monkeypatch.setenv("FOOL_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "fool_cli.auth.get_provider_auth_state",
        lambda provider: {
    ...
    monkeypatch.setattr(
        "fool_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "fool_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("fool_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("fool_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "fool_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "fool_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "fool_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "fool_cli.codex_models.get_codex_model_ids",
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    saved_env = {}
    monkeypatch.setattr("fool_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("fool_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("fool_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("fool_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "fool_cli.models.probe_api_models",
    ...
    monkeypatch.setattr("fool_cli.config.save_config", lambda cfg: None)
    ...
    monkeypatch.setattr("fool_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("fool_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "fool_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("fool_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("fool_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "fool_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "fool_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("fool_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("fool_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("fool_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("fool_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("fool_cli.auth.get_provider_auth_state", lambda provider_id: None)
    ...
    monkeypatch.setattr("fool_cli.auth._login_nous", _fake_login)
...
def test_auto_provider_name_localhost():
    from fool_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from fool_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "fool_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("fool_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("fool_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="FOOL_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "FOOL_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from fool_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
