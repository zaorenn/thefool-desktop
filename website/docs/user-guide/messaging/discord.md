...
| `FOOL_DISCORD_TEXT_BATCH_DELAY_SECONDS` | No | `0.6` | Grace window the adapter waits before flushing a queued text chunk. Useful for smoothing streamed output. |
| `FOOL_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS` | No | `2.0` | Delay between split chunks when a single message exceeds Discord's length limit. |
...
No extra configuration is needed — any skill installed via `fool skills install` is automatically registered as a Discord slash command on the next gateway restart.
...
3. Click **Save Changes**, then restart the gateway (`fool gateway restart`).
...
**Fix**: Check that `fool gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.
