...
class _RecordingDB:
    """Stand-in for ``fool_state.SessionDB`` that counts ``close()`` calls.
...
def profile_dbs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("fool_state.SessionDB", _factory)
    monkeypatch.setattr(
    ...
    # off the real agent/secret/FOOL_HOME machinery.
