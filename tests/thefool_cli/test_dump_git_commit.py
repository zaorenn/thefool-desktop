"""Tests for thefool_cli.dump._get_git_commit — git SHA resolution for ``hermes dump``.
...
written by the Dockerfile's ``THEFOOL_GIT_SHA`` build-arg.
...
def test_get_git_commit_uses_live_git_when_available(tmp_path):
    ...
    with patch("thefool_cli.dump.subprocess.run", return_value=git_result) as mock_run, \
         patch("thefool_cli.build_info.get_build_sha") as mock_build:
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    with patch("thefool_cli.dump.subprocess.run", return_value=git_result):
        live = dump._get_git_commit(repo_dir)

    # Baked-SHA path.
    failed = MagicMock(returncode=128, stdout="")
    with patch("thefool_cli.dump.subprocess.run", return_value=failed), \
         patch("thefool_cli.build_info.get_build_sha", return_value="b2f477a3"):
