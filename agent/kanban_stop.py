...
def kanban_stop_nudge_enabled() -> bool:
    ...
    On when ``THEFOOL_KANBAN_TASK`` is set (dispatcher-spawned worker), unless
    ``THEFOOL_KANBAN_STOP_NUDGE`` explicitly disables it.
    """
    env = os.environ.get("THEFOOL_KANBAN_STOP_NUDGE")
    if env is not None and env.strip().lower() in {"0", "false", "no", "off"}:
        return False
    task = (os.environ.get("THEFOOL_KANBAN_TASK") or "").strip()
    return bool(task)
...
def build_kanban_stop_nudge(
    *,
    ...
    tid = (task_id or os.environ.get("THEFOOL_KANBAN_TASK") or "").strip() or "this task"
    return (
