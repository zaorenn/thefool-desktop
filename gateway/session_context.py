...
(``FOOL_SESSION_PLATFORM``, ``FOOL_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["FOOL_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("FOOL_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("FOOL_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("FOOL_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("FOOL_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("FOOL_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_TYPE: ContextVar = ContextVar("FOOL_SESSION_CHAT_TYPE", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("FOOL_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("FOOL_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("FOOL_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_ID_ALT: ContextVar = ContextVar("FOOL_SESSION_USER_ID_ALT", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("FOOL_SESSION_USER_NAME", default=_UNSET)
...
_SESSION_SCOPE_ID: ContextVar = ContextVar("FOOL_SESSION_SCOPE_ID", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("FOOL_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("FOOL_SESSION_ID", default=_UNSET)
# In-process UI session/window id for multi-session desktop/TUI hosts. This is
# intentionally separate from FOOL_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("FOOL_UI_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("FOOL_SESSION_MESSAGE_ID", default=_UNSET)

_SESSION_PROFILE: ContextVar = ContextVar("FOOL_SESSION_PROFILE", default=_UNSET)
...
_CRON_SESSION: ContextVar = ContextVar("FOOL_CRON_SESSION", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("FOOL_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("FOOL_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("FOOL_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("FOOL_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "FOOL_SESSION_PLATFORM": _SESSION_PLATFORM,
    "FOOL_SESSION_SOURCE": _SESSION_SOURCE,
    "FOOL_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "FOOL_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    "FOOL_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "FOOL_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "FOOL_SESSION_USER_ID": _SESSION_USER_ID,
    "FOOL_SESSION_USER_ID_ALT": _SESSION_USER_ID_ALT,
    "FOOL_SESSION_USER_NAME": _SESSION_USER_NAME,
    "FOOL_SESSION_SCOPE_ID": _SESSION_SCOPE_ID,
    "FOOL_SESSION_KEY": _SESSION_KEY,
    "FOOL_SESSION_ID": _SESSION_ID,
    "FOOL_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "FOOL_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "FOOL_SESSION_PROFILE": _SESSION_PROFILE,
    "FOOL_CRON_SESSION": _CRON_SESSION,
    "FOOL_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "FOOL_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "FOOL_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``FOOL_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("FOOL_SESSION_ID")`` with an ``os.environ`` fallback,
    ...
    ``FOOL_SESSION_ID`` for the rest of the process — leaking the child id
...
def set_session_vars(
    ...
    ``os.environ["FOOL_CRON_SESSION"]`` fallback, ``"1"`` marks a cron job,
...
def reset_session_vars() -> None:
    ...
    *A's* ``FOOL_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``FOOL_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``FOOL_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("FOOL_SESSION_*", default)``.
...
# value (``telegram``) to FOOL_SESSION_PLATFORM, while the CLI, TUI, and
# desktop bind FOOL_SESSION_SOURCE (``cli``, ``tui``, ``desktop``) and leave
...
# FOOL_SESSION_PLATFORM but have no attachment channel behind them.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``FOOL_PLATFORM``, then the session platform, then the session
    source, and reports messaging when any of them names a surface outside
    :data:`NON_MESSAGING_SESSION_SURFACES`.
    """
    import os

    platform = os.getenv("FOOL_PLATFORM") or get_session_env("FOOL_SESSION_PLATFORM", "")
...
def async_delivery_supported() -> bool:
    ...
    Kanban workers (identified by ``FOOL_KANBAN_TASK``), which are one-shot
