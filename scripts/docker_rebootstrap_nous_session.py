...
``stage2-hook.sh`` seeds ``auth.json`` from ``FOOL_AUTH_JSON_BOOTSTRAP`` only
...
``FOOL_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Nous
...
Design constraints
------------------
- Pure stdlib, no fool_cli imports: runs early in the boot hook, before the
...
# from FOOL_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
...
def _nous_entry_is_terminal(nous_state: Any) -> bool:
    ...
    Mirrors the ``terminal`` predicate in ``fool_cli.auth.get_nous_session_validity``:
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a FOOL_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    FOOL_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             FOOL_AUTH_JSON_BOOTSTRAP path handle it)
