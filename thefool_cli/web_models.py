...
Extracted verbatim from ``thefool_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from thefool_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ProfileCreate(BaseModel):
    ...
    # because skills_hub.SKILLS_DIR is import-time-bound and the THEFOOL_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under THEFOOL_HOME.
    output: str = ""
