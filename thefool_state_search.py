...
``thefool_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import thefool_state (cycle) — shared
module-level constants live in thefool_state_common.
"""
...
from thefool_state_common import (
    FTS_CJK_STALE_KEY,
    FTS_SQL,
    FTS_STALE_KEY,
    FTS_STORAGE_VERSION,
...
# Moved methods logged under the "thefool_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("thefool_state")
...
class SessionSearchMixin:
    ...
    def search_messages(
        ...
        Threshold: THEFOOL_SEARCH_SLOW_MS (default 1000; 0 logs every call).
        """
        started = time.time()
        rows = None
        try:
            ...
            try:
                threshold = float(os.getenv("THEFOOL_SEARCH_SLOW_MS", "1000"))
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``THEFOOL_DISABLE_FTS_TRIGRAM`` or not yet created), so
