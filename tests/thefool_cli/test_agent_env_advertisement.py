"""Tests for the AI_AGENT / THEFOOL_AGENT harness-attribution env vars.
...
cross-session leak guard that strips ``THEFOOL_SESSION_*`` from subprocess
...
from thefool_cli.main import _advertise_agent_env
...
class TestAdvertiseAgentEnv:
    def test_sets_both_vars_when_unset(self, monkeypatch):
        ...
        monkeypatch.delenv("THEFOOL_AGENT", raising=False)
        _advertise_agent_env()
        assert os.environ["AI_AGENT"] == HARNESS_ID
        assert os.environ["THEFOOL_AGENT"] == "true"
...
class TestWrapCommandAdvertisesHarness:
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'THEFOOL_AGENT="${THEFOOL_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT HERMES=$THEFOOL_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "THEFOOL_AGENT")}
        out = subprocess.run(
        ...
        outer_env = dict(clean_env, AI_AGENT="pi", THEFOOL_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
