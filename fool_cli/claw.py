"""fool claw — OpenClaw migration commands.

Usage:
    fool claw migrate              # Preview then migrate (always shows preview first)
    fool claw migrate --dry-run    # Preview only, no changes
    fool claw migrate --yes        # Skip confirmation prompt
    fool claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    fool claw migrate --no-backup  # Skip pre-migration snapshot
    fool claw cleanup              # Archive leftover OpenClaw directories
    fool claw cleanup --dry-run    # Preview what would be archived
"""
...
from fool_cli.config import get_hermes_home, get_config_path, load_config, save_config
...
def _cmd_migrate(args):
    ...
    # migrate-fool posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: fool claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: fool claw migrate --yes")
            return
    ...
    if not no_backup:
        try:
            from fool_cli.backup import create_pre_migration_backup, _format_size
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: fool import {backup_archive.name}")
        except Exception as e:
            print()
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: fool claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: fool claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  fool claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  fool claw migrate --migrate-secrets")
            print()
