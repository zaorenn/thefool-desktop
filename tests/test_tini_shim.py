...
These run without Docker: the shim's FOOL_TINI_SHIM_TARGET /
FOOL_TINI_SHIM_WRAPPER hooks let us record the argv that would be
handed to /init.
"""
...
def _run_shim(
    ...
    env = os.environ.copy()
    env["FOOL_TINI_SHIM_TARGET"] = str(init)
    env["FOOL_TINI_SHIM_WRAPPER"] = str(wrapper)
...
def test_shim_script_is_executable_bit_friendly() -> None:
    ...
    assert "FOOL_TINI_SHIM_TARGET" in text
