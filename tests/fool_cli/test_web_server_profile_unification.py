...
profile switcher can target any profile's FOOL_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via FOOL_HOME.
"""
import json
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import fool_cli.mcp_config as mcp_config
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same FOOL_HOME the rest
        ...
        monkeypatch.setattr(
            "fool_cli.tools_config.valid_post_setup_keys",
