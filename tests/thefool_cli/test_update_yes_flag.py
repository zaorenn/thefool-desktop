...
from thefool_cli.main import cmd_update
...
class TestUpdateYesConfigMigration:
    ...
    @patch("thefool_cli.update_cmd._reload_config_modules")
    @patch("thefool_cli.update_cmd._run_migrate_config_fresh")
    @patch("thefool_cli.update_cmd._run_config_check_fresh", return_value=(1, 2))
    @patch("thefool_cli.config.get_missing_config_fields", return_value=[])
    @patch("thefool_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``thefool_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``thefool_cli.main`` first could leave
...
class TestUnicodeDecodeErrorInUpdatePrompts:
    ...
    def test_stash_restore_unicode_decode_error_falls_through_to_skip(self, tmp_path, capsys):
        from thefool_cli.update_cmd import _restore_stashed_changes
    ...
    def test_upstream_remote_prompt_unicode_decode_error_falls_through_to_skip(
        ...
        from thefool_cli.update_cmd import _sync_with_upstream_if_needed

        with patch(
            "thefool_cli.update_cmd._has_upstream_remote", return_value=False
        ), patch(
            "thefool_cli.update_cmd._should_skip_upstream_prompt", return_value=False
        ), patch(
            "thefool_cli.update_cmd._add_upstream_remote"
        ) as mock_add, patch(
            "builtins.input",
            side_effect=UnicodeDecodeError("utf-8", b"\xff", 0, 1, "invalid byte"),
        ):
