"""Tests for fool_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $FOOL_HOME/node and prepends ~/.local/bin to
...
def _make_hermes_node(hermes_home: Path) -> Path:
    """Create a fake $FOOL_HOME/node/bin/{node,npm,npx} tree."""
