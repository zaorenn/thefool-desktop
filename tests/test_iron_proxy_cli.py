"""Unit tests for ``fool_cli.proxy_cli`` command handlers.
...
from fool_cli import proxy_cli


@pytest.fixture
def hermes_home(tmp_path, monkeypatch):
    """Point FOOL_HOME at a temp dir so the wizard doesn't touch the
...
def test_cmd_disable_uses_public_status_pid_not_private_read_pid(
    ...
    # mutation only.
    from fool_cli.config import load_config as _lc
    cfg2 = _lc()
