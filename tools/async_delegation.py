...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        for evt_key, env_name in (
            ("scope_id", "FOOL_SESSION_SCOPE_ID"),
            ("user_id", "FOOL_SESSION_USER_ID"),
            ("user_name", "FOOL_SESSION_USER_NAME"),
        ):
...
def _current_origin_session_id() -> str:
    ...
    The obvious source — ``FOOL_SESSION_ID`` via ``get_session_env`` — is
    ...
    try:
        ...
        if get_session_env("FOOL_SESSION_PLATFORM", "") != "api_server":
            return ""
        return get_session_env("FOOL_SESSION_CHAT_ID", "") or ""
    except Exception:
        return ""
