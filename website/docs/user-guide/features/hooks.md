...
- Runtime event-name validity comes from `fool_cli.plugins.VALID_HOOKS`. `hermes hooks list` lists configured shell/outbound hooks, not every available event; `hermes hooks test <event>` reports the valid set only when an invalid event is supplied.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`fool_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.
...
2. `FOOL_ACCEPT_HOOKS=1` environment variable
...
hooks:
  outbound:
    - name: ci-notify                       # optional label for logs
      ...
      secret_env: FOOL_OUTBOUND_WEBHOOK_SECRET   # env var holding the HMAC secret
...
| `X-Fool-Event` | The hook event name |
| `X-Fool-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-Fool-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-Fool-Delivery` header) — remember recently seen ids and skip duplicates. Hermes retries failed deliveries once, so the same id can legitimately arrive twice.
