...
import fool_cli.doctor as doctor
import fool_cli.gateway as gateway_cli
from fool_cli import doctor as doctor_mod
from fool_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "FOOL_HOME", hermes_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("FOOL_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_hermes_home(self, tmp_path, provider=""):
        """Create a minimal FOOL_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        monkeypatch.setattr(doctor_mod, "FOOL_HOME", home)
        ...
        try:
            from fool_cli import auth as _auth_mod
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (fool_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".hermes"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp FOOL_HOME.

        ``run_doctor`` reads the module-level ``FOOL_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("FOOL_HOME")`` alone
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from fool_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("fool_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "fool_cli.doctor", raising=False)
        from fool_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale FOOL_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"FOOL_MAX_ITERATIONS={ghost}\n")
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("FOOL_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("FOOL_MAX_ITERATIONS", raising=False)
    ...
    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        ...
        assert "FOOL_MAX_ITERATIONS=90" in out
        assert "shadows" in out
        # Warn-only must NOT mutate .env.
        assert "FOOL_MAX_ITERATIONS=90" in (hermes_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale FOOL_MAX_ITERATIONS" in out
        env_after = (hermes_home / ".env").read_text(encoding="utf-8")
        assert "FOOL_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_hermes_tool_progress_warning_says_unsupported_since_floor(self):
        """FOOL_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"FOOL_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["FOOL_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"FOOL_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["FOOL_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"FOOL_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: FOOL_TOOL_PROGRESS_MODE" in out
