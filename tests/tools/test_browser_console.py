...
class TestBrowserConsole:
    ...
    def test_restrict_evaluate_reads_browser_config(self):
        ...
        with patch("thefool_cli.config.read_raw_config", return_value={"browser": {"restrict_evaluate": "true"}}):
            assert _restrict_browser_evaluate() is True
        with patch("thefool_cli.config.read_raw_config", return_value={"browser": {"restrict_evaluate": False}}):
...
class TestBrowserVisionConfig:
    ...
    def test_browser_vision_uses_configured_temperature_and_timeout(self, tmp_path):
        ...
        with (
            patch("thefool_constants.get_hermes_dir", return_value=shots_dir),
            ...
            patch("tools.browser_tool._get_vision_model", return_value="test-model"),
            patch("thefool_cli.config.load_config", return_value={"auxiliary": {"vision": {"temperature": 1, "timeout": 45}}}),
