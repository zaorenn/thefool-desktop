...
Extracted from ``thefool_cli/main.py`` as part of the god-file decomposition
...
in main.py re-imports them (``from thefool_cli.model_setup_flows import *``-style
explicit import) so existing call sites — and test monkeypatches that target
``thefool_cli.main._model_flow_*`` — keep resolving against main.py's namespace.
...
imported lazily inside the flows (``from thefool_cli.main import ...`` resolves at
call time, when main.py is fully loaded) so this module never imports
``thefool_cli.main`` at import time -> no import cycle.
"""
...
def _prune_replaced_custom_model_config_credentials(
    ...
    *,
    ...
    try:
        ...
        from thefool_cli.auth import read_credential_pool, write_credential_pool
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from thefool_cli.main import _prompt_api_key
    from thefool_constants import OPENROUTER_BASE_URL
    ...
    from thefool_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from thefool_constants import AI_GATEWAY_BASE_URL
    from thefool_cli.main import _prompt_api_key
    from thefool_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
    ...
    from thefool_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    """
    from thefool_cli.auth import _save_model_choice, deactivate_provider
    from thefool_cli.config import load_config, save_config
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from thefool_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from thefool_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from thefool_cli.main import _DEFAULT_QWEN_PORTAL_MODELS
    from thefool_cli.auth import (
    ...
    from thefool_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from thefool_cli.main import _auto_provider_name, _prompt_custom_api_mode_selection, _save_custom_provider
    from thefool_cli.auth import _save_model_choice, deactivate_provider
    from thefool_cli.config import (
    ...
    from thefool_cli.models import probe_api_models
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from thefool_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from thefool_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from thefool_cli import azure_detect
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from thefool_cli.main import _custom_provider_api_key_config_value, _custom_provider_base_url_config_value, _save_custom_provider
    from thefool_cli.auth import _save_model_choice, deactivate_provider
    from thefool_cli.config import load_config, save_config
    from thefool_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    from thefool_cli.main import _current_reasoning_effort, _prompt_reasoning_effort_selection, _set_reasoning_effort
    from thefool_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from thefool_cli.config import save_env_value, load_config, save_config
    from thefool_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from thefool_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from thefool_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set THEFOOL_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from thefool_cli.main import _infer_stepfun_region, _prompt_api_key, _prompt_provider_choice, _stepfun_base_url_for_region
    from thefool_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    from thefool_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    from thefool_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from thefool_cli.config import load_config, save_config, get_env_value
    from thefool_cli.models import _PROVIDER_MODELS
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``thefool_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from thefool_cli.main import _prompt_provider_choice
    from thefool_cli.auth import ZAI_ENDPOINTS
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        ...
        from thefool_cli.models import fetch_lmstudio_models
        ...
        from thefool_cli.models import fetch_ollama_cloud_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from thefool_cli.main import _run_anthropic_oauth_flow
    from thefool_cli.auth import (
        _prompt_model_selection,
        _save_model_choice,
        deactivate_provider,
