...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "fool_state.db", "response_store.db",
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires The Fool {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this The Fool distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``fool profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Fool profile distribution."
            )
...
def plan_install(
    ...
    from fool_cli import __version__ as hermes_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Fool distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.fool).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `fool profile update` to upgrade in place, "
