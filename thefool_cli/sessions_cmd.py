"""``hermes sessions`` command — extracted from ``thefool_cli/main.py``.
...
Helpers that stay in ``thefool_cli.main`` (``get_hermes_home``,
...
``thefool_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def _prune_never_active_keyed(db, args):
    ...
    """
    from thefool_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "recover":
        ...
        from thefool_cli.session_recovery import (
    ...
    if action == "import":
        from thefool_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from thefool_state import workspace_key as _ws_key
        ...
        from thefool_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from thefool_cli.session_export_html import (
        ...
        # Markdown / QMD export
        from thefool_cli.session_export_md import (
