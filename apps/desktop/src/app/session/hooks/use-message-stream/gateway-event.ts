 ...
 * billing-specific toast — never the generic "The Fool error" — with a smart CTA
      ...
      if (event.type === 'gateway.ready') {
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'The Fool reported an error',
        ...
        // native OS notification when The Fool is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'The Fool reported an error'
