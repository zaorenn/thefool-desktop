"""Dependency-light venv recovery that runs BEFORE fool_cli.main's imports.

The ``hermes`` console entry point is ``fool_cli.main:main``.  Importing
``fool_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``fool_cli.env_loader``, ``yaml`` via ``fool_cli.config``, ...).  In
...
a corrupted venv.  ``fool_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``fool_cli.main`` to become
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``fool_cli.main``
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``fool_cli.main``'s third-party imports, maps nothing yet, so the
