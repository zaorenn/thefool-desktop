...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``fool_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``fool_state._backup_db_file``.
    """
    from fool_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
