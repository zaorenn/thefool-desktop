...
* The user is signed in via xAI Grok OAuth — SuperGrok subscription —
  i.e. ``fool auth add xai-oauth`` has been run and the stored refresh
  token still works.
...
def _resolve_xai_bearer() -> Tuple[str, str, str]:
    ...
    if not api_key:
        raise RuntimeError(
            "No xAI credentials available. Run `fool auth add xai-oauth` "
...
def check_x_search_requirements() -> bool:
    ...
    :func:`fool_cli.auth.resolve_xai_oauth_runtime_credentials` which
