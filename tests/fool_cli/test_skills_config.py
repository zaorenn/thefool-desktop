"""Tests for fool_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("fool_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("fool_cli.config.load_config")
    ...
    @patch("fool_cli.config.load_config")
    @patch.dict("os.environ", {"FOOL_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """FOOL_SESSION_PLATFORM should be used when FOOL_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_hermes_platform_takes_precedence(self, tmp_path, monkeypatch):
        """FOOL_PLATFORM should win over FOOL_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from fool_cli.skills_config import _get_categories
        skills = [
