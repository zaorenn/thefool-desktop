...
test('cleanStaleAppOutDir removes a populated unpacked directory', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'fool-before-pack-'))
  try {
...
test('preserveRollbackBackup moves a working build to .bak', () => {
  ...
  try {
    ...
    fs.mkdirSync(appOutDir, { recursive: true })
    fs.writeFileSync(path.join(appOutDir, 'TheFool.exe'), 'MZ-old-build', 'utf8')
    fs.writeFileSync(path.join(appOutDir, 'resources.pak'), 'x', 'utf8')

    const preserved = preserveRollbackBackup(appOutDir, 'TheFool.exe')
    ...
    assert.equal(
      fs.readFileSync(path.join(`${appOutDir}.bak`, 'TheFool.exe'), 'utf8'),
      'MZ-old-build'
    )
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true })
...
test('preserveRollbackBackup replaces a stale .bak from an older update', () => {
  ...
  try {
    const appOutDir = path.join(tempRoot, 'win-unpacked')
    fs.mkdirSync(appOutDir, { recursive: true })
    fs.writeFileSync(path.join(appOutDir, 'TheFool.exe'), 'current', 'utf8')
    fs.mkdirSync(`${appOutDir}.bak`, { recursive: true })
    fs.writeFileSync(path.join(`${appOutDir}.bak`, 'TheFool.exe'), 'two-updates-ago', 'utf8')

    assert.equal(preserveRollbackBackup(appOutDir, 'TheFool.exe'), true)
    assert.equal(fs.readFileSync(path.join(`${appOutDir}.bak`, 'TheFool.exe'), 'utf8'), 'current')
  } finally {
    fs.rmSync(tempRoot, { recursive: true, force: true })
  }
})
...
test('preserveRollbackBackup refuses a partial tree missing the product exe', () => {
  ...
  try {
    ...
    assert.equal(preserveRollbackBackup(appOutDir, 'TheFool.exe'), false)
...
test('beforePack on win32 preserves the previous build instead of wiping it', async () => {
  ...
  try {
    const appOutDir = path.join(tempRoot, 'win-unpacked')
    fs.mkdirSync(appOutDir, { recursive: true })
    fs.writeFileSync(path.join(appOutDir, 'TheFool.exe'), 'MZ-working', 'utf8')

    // No packager info in the context → default 'TheFool.exe' product name.
...
test('beforePack on linux keeps the plain wipe (no .bak)', async () => {
  ...
  try {
    const appOutDir = path.join(tempRoot, 'linux-unpacked')
    fs.mkdirSync(appOutDir, { recursive: true })
    fs.writeFileSync(path.join(appOutDir, 'TheFool.exe'), 'x', 'utf8')
