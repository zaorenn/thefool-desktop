"""Hermes update pipeline — extracted from ``fool_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``fool_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``fool_cli.main`` reference — so existing call sites and test monkeypatches
that target ``fool_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``fool_cli.main``.
...
Imports are one-way: ``fool_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``fool_cli.main`` reference.

    Lets callers keep patching ``fool_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``fool_cli.main`` -> ``fool_cli.update_cmd`` stays one-way
    at import time.
    """
    from fool_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``fool update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``fool update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``fool_cli.config_defaults``,
    ``fool_cli.config``, and ``fool_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``fool_cli._subprocess_compat`` and
    ``fool_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "fool_cli.config_defaults",
        "fool_cli.config",
        "fool_cli.config_migrations",
        "fool_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from fool_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from fool_cli.config import migrate_config
...
_UPDATE_CRITICAL_FILES = (
    "fool_cli/main.py",
    "fool_cli/config.py",
    "fool_cli/__init__.py",
    "fool_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``fool update`` can be driven by a
    ...
    """
    from fool_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from fool_constants so this can't drift
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``fool update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `fool update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `fool update`.
    ...
    if interrupted:
        ...
        print(
            "  A previous `fool sessions optimize-storage` run was "
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``fool update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``fool update`` invocations skip the block until a newer
...
def _reload_process_scan_modules() -> None:
    ...
    ``from fool_cli._subprocess_compat import bounded_probe_run``. If the
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``fool_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `fool update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``fool_constants.py``
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False):
    ...
    try:
        ...
        print(
            "  Re-run `fool update` to retry; if the agent won't start, "
    ...
    from fool_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `fool update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        from fool_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from fool_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if The Fool behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes fool completely unrunnable (SyntaxError on import).
    ...
    print("  Review `git diff` / `git status` if The Fool behaves unexpectedly.")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Fool repository.")
...
def _invalidate_update_cache():
    ...
    ``fool update``, every profile is now current.
    """
    homes = []
...
def _format_concurrent_instances_message(
    ...
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `fool update --force` if you've already")
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``fool tools``."""
    try:
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``fool tools`` dependencies into a rebuilt venv.
    ...
    print()
    print(f"→ Restoring {len(missing)} The Fool Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``fool update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `fool update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `fool update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from fool_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from fool_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$FOOL_HOME/bin/uv``, but on Termux the official
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `fool update` is exactly the step
...
def _repair_node_deps_on_current_checkout(print_completion) -> None:
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `fool update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `fool update`.")
            failed = []
            if any(
    ...
    # `fool update` case. Synchronous and can block ~11s on a true cold
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `fool update`.")
        return list(labels)
    ...
    # `fool update` comes from the *desktop* build, not this step; that
...
def _log_only_write(text: str) -> None:
    ...
    During ``fool update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    """
    from fool_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``fool update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from fool_cli.config import format_docker_update_message
    ...
    from fool_cli.gitlock import clear_stale_git_locks
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from fool_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``fool update`` without requiring a reinstall.  The
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of FOOL_HOME under
      ``backups/`` (restorable via ``fool import``; the #48200 wrong-path
    ...
    print(f"  Restore:  fool import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``fool update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``fool update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run fool from any interpreter), so report healthy to
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    Windows: the Desktop app's backend (``python.exe -m fool_cli.main
    ...
    for proc in proc_iter:
        ...
        # the `-m fool_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``fool_cli.main`` used to import
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other The Fool processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← The Fool Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``fool update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from fool_cli._scan_venv_blockers import _is_pausable_gateway
...
def _orphaned_desktop_backend_pids(
    ...
    - its cmdline is a Hermes backend (``fool_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    def _is_backend(argv_low: str) -> bool:
        return "fool_cli.main" in argv_low and (
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("→ Stopping Windows gateway process(es) before updating The Fool...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m fool_cli.main
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``fool gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from fool_cli import gateway_windows
        from fool_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``fool_cli.gateway`` itself no longer imports, which is one of the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``fool_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``fool update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    ``$FOOL_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``fool update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``fool update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `fool desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "fool_cli.main", "desktop", "--build-only"]
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (non-fatal; run `fool desktop` to retry)")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # (most commonly the Desktop app's `fool serve` backend) will keep .pyd
    ...
    # process by design (a CLI `fool update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all The Fool windows/gateways and re-run: fool update")
            else:
        ...
        # orphan merge-conflict markers in fool_cli/config.py bricked
        # every user who ran ``fool update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``fool update`` again later once a fix lands.")
                    else:
        ...
        # no error, no warning, ``fool doctor`` healthy. Compare pre-pull
        ...
        # the old bytecode (e.g. get_hermes_home added to fool_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # symbols in fool_constants or lazy_deps. The dependency install
        ...
        if not import_ok:
            ...
            print("    Run `fool update` again — if it persists, reinstall:")
        ...
        # seed_profile_skills() uses subprocess with an explicit FOOL_HOME so
        # it is not affected by sync_skills()'s module-level FOOL_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's FOOL_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from fool_cli.profiles import backfill_profile_envs
        ...
        # ``fool update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``fool_cli.config`` and
        # ``fool_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from fool_cli.config import
        ...
        # it and warn loudly.
        try:
            from fool_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `fool curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``fool update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `fool update`
                # must stay fast; `fool computer-use install --upgrade`
        ...
        # When running as ``fool update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            try:
                from fool_cli.gateway import _get_restart_exit_wait_budget
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `fool update` restart it automatically, allow\n"
        ...
        # every `fool update` surfaces the issue until the user migrates.
        try:
            from fool_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy The Fool gateway unit(s) detected:")
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``fool_cli.gateway`` no longer imports,
