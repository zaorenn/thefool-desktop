...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _hermes_env="${FOOL_HOME:-$HOME/.hermes}/.env"; [ -f "$_hermes_env" ] && grep -q "^GITHUB_TOKEN=" "$_hermes_env"; then
      ...
      GITHUB_TOKEN=$(uv run python "${FOOL_HOME:-$HOME/.hermes}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
```bash
source "${FOOL_HOME:-$HOME/.hermes}/skills/github/github-auth/scripts/gh-env.sh"
