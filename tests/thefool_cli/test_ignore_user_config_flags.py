...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("THEFOOL_IGNORE_USER_CONFIG", "THEFOOL_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``THEFOOL_IGNORE_USER_CONFIG=1``.
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With THEFOOL_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("THEFOOL_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting THEFOOL_IGNORE_RULES=1 flips HermesCLI.ignore_rules True."""
        monkeypatch.setenv("THEFOOL_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("THEFOOL_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``thefool_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("THEFOOL_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("THEFOOL_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("THEFOOL_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("THEFOOL_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "THEFOOL_IGNORE_USER_CONFIG" not in os.environ
        assert "THEFOOL_IGNORE_RULES" not in os.environ
