...
(``FOOL_DISABLE_LAZY_INSTALLS=1``) and lazy installs are redirected to a
writable durable target (``FOOL_LAZY_INSTALL_TARGET``). honcho/hindsight
...
#
# This is the exact hosted-Fly condition: FOOL_DISABLE_LAZY_INSTALLS=1 seals
# the venv, but FOOL_LAZY_INSTALL_TARGET redirects installs to a writable
...
class TestSealedVenvDurableTarget:
    ...
    def test_ensure_installs_into_durable_target_on_sealed_venv(
        ...
        monkeypatch.setenv("FOOL_LAZY_INSTALL_TARGET", str(tmp_path / "lazy"))
        ...
        def fake_install(specs, **kw):
            ...
            captured["target_env"] = os.environ.get("FOOL_LAZY_INSTALL_TARGET")
    ...
    def test_sealed_venv_without_target_blocks(self, feature, monkeypatch):
        ...
        monkeypatch.setenv("FOOL_DISABLE_LAZY_INSTALLS", "1")
        monkeypatch.delenv("FOOL_LAZY_INSTALL_TARGET", raising=False)
        monkeypatch.setattr(
            "fool_cli.config.load_config",
            lambda: {"security": {"allow_lazy_installs": True}},
        )
