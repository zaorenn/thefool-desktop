...
variant that the ``_UNSET``-strip guard does NOT catch:
    ...
    *concurrent* message A had ALREADY called ``set_session_vars``, B inherits
    ...
    ``THEFOOL_SESSION_*`` identity through the subprocess-env bridge. The bridge's
...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "THEFOOL_SESSION_CHAT_ID": env.get("THEFOOL_SESSION_CHAT_ID"),
        "THEFOOL_SESSION_THREAD_ID": env.get("THEFOOL_SESSION_THREAD_ID"),
        "THEFOOL_SESSION_KEY": env.get("THEFOOL_SESSION_KEY"),
    }
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("THEFOOL_SESSION_CHAT_ID", "THEFOOL_SESSION_THREAD_ID", "THEFOOL_SESSION_KEY"):
    ...
    assert captured["bound"]["THEFOOL_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["THEFOOL_SESSION_KEY"] == FOREIGN["session_key"]
...
# flag read via ``async_delivery_supported()``, not a string ``THEFOOL_SESSION_*``
