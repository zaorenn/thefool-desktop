...
YOLO 模式在 CLI 和 gateway 会话中均可使用。在内部，它会设置 `FOOL_YOLO_MODE` 环境变量，该变量在每次命令执行前都会被检查。
...
运行 gateway 时，`FOOL_EXEC_ASK=1` 环境变量会自动设置。
...
某些技能需要在沙箱中访问**文件**（而非仅环境变量）——例如，Google Workspace 将 OAuth token 存储为活跃 profile 的 `FOOL_HOME` 下的 `google_token.json`。技能在 frontmatter 中声明这些文件：
...
加载后，Hermes 会检查这些文件是否存在于活跃 profile 的 `FOOL_HOME` 中，并将其注册为挂载：
...
Hermes 内置了一个公告扫描器，用于标记活跃 venv 中与已知受损版本目录匹配的 Python 包（例如 2026 年 5 月的 `mistralai 2.4.6` 供应链投毒事件）。实现位于 `fool_cli/security_advisories.py`。
