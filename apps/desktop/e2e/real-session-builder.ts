 ...
 * AIAgent loop, using the E2E mock provider configured in `foolHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(foolHome: string) {
  ...
  static async start(foolHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(foolHome)
