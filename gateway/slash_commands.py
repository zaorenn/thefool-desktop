...
from fool_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: fool gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`fool gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from fool_cli.context_switch_guard import (
                        ...
                        from fool_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        # Persist the selection only — fool_cli.personality never writes
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from fool_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `fool cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from fool_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        ``fool skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _hermes_home
        from fool_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `fool skills diff <name>` is a
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from fool_cli.session_listing import (
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (fool_state.py:1994-2009) that searches by the complete
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `fool bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``fool debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        Spawns ``fool update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``fool update`` may trigger. Marker
        ...
        # Spawn `fool update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `fool update --gateway`
