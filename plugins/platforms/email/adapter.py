...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `fool config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `fool gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
...
# fool_cli/gateway.py, and the _send_email dispatch in
