...
├── thefool_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── thefool_cli/               # CLI command implementations
...
- **Session persistence**: All conversations are stored in SQLite (`thefool_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.hermes/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
Add to `_BUILTIN_SKINS` dict in `thefool_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `thefool_cli/skin_engine.py` for the full schema and existing skins as examples.
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `thefool_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `thefool_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `thefool_cli/gateway_windows.py::
