"""Tests for fool tools disable/enable/list command (backend)."""
...
class TestToolsDisableBuiltin:
    ...
    def test_disable_removes_toolset_from_platform(self):
        ...
        with patch("fool_cli.tools_config.load_config", return_value=config), \
             patch("fool_cli.tools_config.save_config") as mock_save:
...
class TestToolsDisableMcp:
    ...
    def test_disable_unknown_server_prints_error(self, capsys):
        config = {"mcp_servers": {}}
        with patch("fool_cli.tools_config.load_config", return_value=config), \
             patch("fool_cli.tools_config.save_config"):
...
def test_tools_action_accepts_deferred_plugin_without_materializing(action, capsys):
    ...
    try:
        with patch(
            "fool_cli.plugins.discover_plugins",
            side_effect=discover_deferred_platform,
        ) as discover, \
             patch("fool_cli.tools_config.load_config", return_value=config), \
             patch("fool_cli.tools_config.save_config") as save:
