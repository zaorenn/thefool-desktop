...
// backend now surfaces the full `fool model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in fool_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `fool model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — The Fool runs the flow for you.`}
          </p>
        )}
