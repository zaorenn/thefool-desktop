...
NOT from FOOL_SESSION_ID: constructing a child agent calls
set_current_session_id(child.session_id), clobbering the FOOL_SESSION_ID
...
def _patch_delegate(monkeypatch):
    ...
    def clobbering_build_child(**kw):
        ...
        # FOOL_SESSION_ID ContextVar + os.environ, clobbering the spawner's
...
def test_apiserver_session_with_id_dispatches_background(monkeypatch):
    """async_delivery=False + a raw session id (FOOL_SESSION_ID) →
    ...
    monkeypatch.setenv("FOOL_SESSION_ID", "raw-sid-7")
    set_session_vars(
        platform="api_server",
        chat_id="raw-sid-7",
    ...
    # FOOL_SESSION_ID with (see clobbering_build_child).
