...
shared ``fool_bootstrap.harden_import_path`` guard.
...
def test_entry_calls_shared_harden_guard_before_heavy_imports():
    """entry.py must call fool_bootstrap.harden_import_path() before it
    ...
    assert harden_call_line is not None, (
        "entry.py must call fool_bootstrap.harden_import_path()"
    )
...
def test_guard_handles_absolute_cwd_path():
    ...
    try:
        ...
        fool_bootstrap.harden_import_path(src_root="/opt/hermes")
