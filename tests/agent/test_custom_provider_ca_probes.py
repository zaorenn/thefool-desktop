...
* ``urllib``-based ``/models`` catalog discovery probe
  (``fool_cli.models.probe_api_models`` via ``_custom_provider_ssl_context``).
...
from fool_cli.models import _custom_provider_ssl_context
...
class TestResolveRequestsVerifyProviderScoped:
    ...
    def test_provider_ca_used_for_matching_base_url(self, clean_env, bundle_file):
        with patch(
            "fool_cli.config.get_compatible_custom_providers",
    ...
    def test_no_base_url_does_not_consult_config(self, clean_env, bundle_file):
        ...
        clean_env.setenv("FOOL_CA_BUNDLE", bundle_file)
        probe = MagicMock(return_value=[])
        with patch("fool_cli.config.get_compatible_custom_providers", probe):
