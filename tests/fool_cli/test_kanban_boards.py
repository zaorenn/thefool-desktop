...
* ``_default_spawn`` injects ``FOOL_KANBAN_BOARD`` into worker env.
"""
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated FOOL_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test FOOL_HOME plus a path-init cache reset
    ...
    # Also reset fool_constants cache so get_default_hermes_root() re-reads.
    try:
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``FOOL_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("FOOL_KANBAN_DB", str(forced))
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("FOOL_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``FOOL_KANBAN_BOARD`` / DB / workspaces on spawn.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "fool_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
