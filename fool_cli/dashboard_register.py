"""``fool dashboard register`` — register a self-hosted dashboard OAuth client.
...
as ``FOOL_DASHBOARD_OAUTH_CLIENT_ID``.
...
This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     ...
     "run `fool setup`" hint when the user isn't logged in.
  ...
  3. Writes ``FOOL_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``FOOL_DASHBOARD_PORTAL_URL`` into ``~/.hermes/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``FOOL_DASHBOARD_PORTAL_URL`` env (used for testing against a
    ...
    try:
        from fool_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``fool dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `fool auth add nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    from fool_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    FOOL_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    FOOL_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    FOOL_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — Nous login only *engages* on a non-loopback bind. A plain\n"
        "  `fool dashboard` (localhost) leaves the gate off and serves locally\n"
...
def cmd_dashboard_register(args) -> None:
    ...
    from fool_cli.auth import AuthError, resolve_nous_access_token
    from fool_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets FOOL_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `fool dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `fool setup` (or `fool auth add nous`) first, then retry.")
        else:
    ...
    # FOOL_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (FOOL_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("FOOL_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("FOOL_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write FOOL_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  FOOL_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      FOOL_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    try:
        existing_portal = get_env_value("FOOL_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("FOOL_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # that same callback by taking FOOL_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as FOOL_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("FOOL_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("FOOL_DASHBOARD_PUBLIC_URL", public_url)
