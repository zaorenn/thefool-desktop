...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import fool_cli.oneshot as oneshot
        from fool_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "fool_state", mod("fool_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "fool_cli.config",
        mod("fool_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "fool_cli.models",
        mod("fool_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "fool_cli.tools_config",
        mod("fool_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["FOOL_TUI_ACTIVE_SESSION_FILE"])
    ...
    env = captured["env"]
    assert env["FOOL_MODEL"] == "nous/hermes-test"
    assert env["FOOL_INFERENCE_MODEL"] == "nous/hermes-test"
    assert env["FOOL_TUI_PROVIDER"] == "nous"
    assert env["FOOL_INFERENCE_PROVIDER"] == "nous"
    assert env["FOOL_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["FOOL_TUI_ACTIVE_SESSION_FILE"])
...
def test_make_tui_argv_dev_prebuilds_hermes_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("FOOL_TUI_DIR", raising=False)
