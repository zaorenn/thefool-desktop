...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $FOOL_HOME.
    ...
    with patch("fool_cli.config.get_managed_system", return_value=None), \
         patch("fool_cli.config.get_hermes_home", return_value=home):
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $FOOL_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
    with patch("fool_cli.config.get_hermes_home", return_value=home):
        from fool_cli.config import stamp_install_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("fool_cli.config.get_managed_system", return_value=None), \
         patch("fool_cli.config.get_hermes_home", return_value=tmp_path), \
         patch("fool_constants.is_container", return_value=True):
        from fool_cli.config import detect_install_method
