...
2. **Isolated FOOL_HOME.** FOOL_HOME points to a per-test tempdir so
   code reading ``~/.hermes/*`` via ``get_hermes_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hermes"`` instead of the canonical
   ``get_hermes_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No FOOL_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox FOOL_HOME before ANY test module is imported ──────────────────
# `fool_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
...
#
# The `_isolate_env` fixture below also sandboxes FOOL_HOME, but fixtures run
...
# the REAL Hermes root — capture it BEFORE the sandbox rewires FOOL_HOME,
...
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("FOOL_KANBAN_HOME", "").strip()
_PRE_SANDBOX_HERMES_HOME = os.environ.get("FOOL_HOME", "")


def _hermes_home_points_at_production(value: str) -> bool:
    """True when a pre-set FOOL_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    FOOL_HOME=~/.hermes``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``fool_state.DEFAULT_DB_PATH``) froze paths
...
if _hermes_home_points_at_production(os.environ.get("FOOL_HOME", "")):
    _SESSION_HERMES_HOME = tempfile.mkdtemp(prefix="hermes-test-home-")
    os.environ["FOOL_HOME"] = _SESSION_HERMES_HOME
...
# like a real CLI") — which used to disarm fool_state's live-DB guard in
# the child at the same moment the child lost the FOOL_HOME redirect.
# FOOL_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# fool_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export FOOL_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["FOOL_TEST_ISOLATION"] = os.environ.get("FOOL_HOME", "") or "1"

#: FOOL_HOME as it stood when conftest was imported - i.e. before any test
...
FOOL_HOME_AT_CONFTEST_IMPORT = os.environ.get("FOOL_HOME", "")
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``fool_cli.voice.speak_text`` — real synthesis,
    ...
    "FOOL_SESSION_CHAT_ID",
    "FOOL_SESSION_CHAT_NAME",
    "FOOL_SESSION_CHAT_TYPE",
    "FOOL_SESSION_THREAD_ID",
    "FOOL_SESSION_SOURCE",
    "FOOL_SESSION_KEY",
    "FOOL_GATEWAY_SESSION",
    "FOOL_CRON_SESSION",
    "_HERMES_GATEWAY",
    "FOOL_PLATFORM",
    "FOOL_MODEL",
    "FOOL_INFERENCE_MODEL",
    "FOOL_INFERENCE_PROVIDER",
    "FOOL_TUI_PROVIDER",
    "FOOL_MANAGED",
    "FOOL_MANAGED_DIR",
    "FOOL_DEV",
    "FOOL_CONTAINER",
    "FOOL_EPHEMERAL_SYSTEM_PROMPT",
    "FOOL_TIMEZONE",
    "FOOL_REDACT_SECRETS",
    "FOOL_BACKGROUND_NOTIFICATIONS",
    "FOOL_EXEC_ASK",
    "FOOL_HOME_MODE",
    "FOOL_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.hermes/kanban.db instead of the per-test FOOL_HOME.
    "FOOL_KANBAN_DB",
    "FOOL_KANBAN_BOARD",
    "FOOL_KANBAN_HOME",
    "FOOL_KANBAN_WORKSPACES_ROOT",
    "FOOL_KANBAN_LOGS_ROOT",
    "FOOL_KANBAN_TASK",
    "FOOL_KANBAN_WORKSPACE",
    "FOOL_KANBAN_RUN_ID",
    "FOOL_KANBAN_CLAIM_LOCK",
    "FOOL_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "FOOL_DASHBOARD_OAUTH_CLIENT_ID",
    "FOOL_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and FOOL_HOME to per-test tempdirs so code that
    ...
    monkeypatch.setenv("FOOL_HONCHO_HOST", "hermes")

    # 3. Redirect FOOL_HOME to a per-test tempdir. Code that reads
    ...
    monkeypatch.setenv("FOOL_HOME", str(fake_hermes_home))
    ...
    # fool_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("FOOL_TEST_ISOLATION", str(fake_hermes_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("FOOL_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. fool_state computes ``DEFAULT_DB_PATH = get_hermes_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from fool_state import ...``) that
    ...
    hermes_state_mod = sys.modules.get("fool_state")
    ...
    monkeypatch.setenv("FOOL_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.hermes/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import fool_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path FOOL_HOME can't leak forward. Paths
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from fool_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# allow-list because test-level fixtures legitimately move FOOL_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session FOOL_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``FOOL_KANBAN_HOME`` env var when set and non-empty
    ...
    if _PRE_SANDBOX_HERMES_HOME and not _hermes_home_points_at_production(
        ...
        # FOOL_HOME was genuinely set to a CUSTOM root before the sandbox
    ...
    # No pre-existing FOOL_HOME: the real root is the platform default,
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    move FOOL_HOME to sibling tempdirs are unaffected.

    Only patches when ``fool_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("fool_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import fool_cli.kanban_db`
...
# ``fool_state._ensure_test_isolation`` (the single choke point every
...
#     installs where FOOL_HOME is not ~/.hermes) into the guard's
...
# so subprocess children that import fool_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("fool_state")
...
def _reset_tui_gateway_server_state():
    ...
    # to its default.
    try:
        from fool_constants import get_hermes_home_override, set_hermes_home_override
...
# correlated with a ``tests/fool_cli/`` pytest run starting up.
#
...
def _wal_is_usable() -> bool:
    ...
    IMPORTANT: this must NOT import ``fool_state``. That module computes
    ...
    redirects ``FOOL_HOME`` — permanently caches the DEVELOPER'S REAL
    ...
    duplicated from ``fool_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["FOOL_VOICE_TTS"] = "1"``. The
...
#      ``fool_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``fool_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``fool_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``fool_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
def _live_system_guard(request, monkeypatch):
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m fool_cli.main".
                if (
                    "hermes" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        # Block any subprocess that would run `fool update` (or the
        # equivalent `python -m fool_cli.main update`).  These commands
        ...
        if "update" in low and (
            ...
            raise RuntimeError(
                ...
                "`fool update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    try:
        import fool_cli.voice as _voice
    except Exception:
