"""Tests for ``fool dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``fool update``
...
from fool_cli.main import cmd_dashboard
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("fool_cli.main._scan_dashboard_processes", return_value=[]), \
    ...
    def test_status_with_processes(self, capsys):
        ...
        with patch("fool_cli.main._scan_dashboard_processes", return_value=processes), \
             ...
             patch("fool_cli.main._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
...
class TestDashboardStop:
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("fool_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("fool_cli.main._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``fool dashboard --stop`` must not end up ALSO starting
    a new server."""
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("fool_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"fool_cli.web_server": fake_ws}), \
...
class TestArgparseWiring:
    ...
    ``fool dashboard --stop`` / ``--status`` actually parse."""

    def test_flags_are_registered(self):
        from fool_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("fool_cli.main")
