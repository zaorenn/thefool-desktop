...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from thefool_cli.doctor import THEFOOL_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (THEFOOL_HOME / "node" / "bin",
                          THEFOOL_HOME / "node",
                          THEFOOL_HOME / "node_modules" / ".bin"):
    ...
    # thefool_cli.doctor uses so this probe can't diverge from it, including
