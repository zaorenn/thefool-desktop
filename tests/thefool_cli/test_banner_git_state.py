...
def test_format_banner_version_label_on_upstream_main():
    from thefool_cli import banner
...
def test_get_git_banner_state_reads_origin_and_head(tmp_path):
    ...
    with patch("thefool_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_via_local_git_ssh_fastpath_ahead_not_behind(tmp_path):
    ...
    def fake_git_stdout(args, *, cwd, timeout=5):
        if args == ["remote", "get-url", "origin"]:
            return "git@github.com:zaorenn/thefool-desktop.git"  # FOOL-SEAM: banner-repo
