...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default FOOL_AGENT banner) | `""` |
...
Hermes Mod respects the `FOOL_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `fool_cli/skin_engine.py`.
