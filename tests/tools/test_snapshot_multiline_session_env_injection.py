...
def test_multiline_session_chat_name_not_executed_via_snapshot(tmp_path: Path):
    """Continuation lines of FOOL_SESSION_CHAT_NAME must not run on source."""
    marker = tmp_path / "pwned"
    chat_name = f"demo\ntouch {marker} #"
    proc = _run_dump_and_source(
        tmp_path=tmp_path,
        env_name="FOOL_SESSION_CHAT_NAME",
        env_value=chat_name,
        marker=marker,
    )
...
def test_multiline_session_user_name_not_executed_via_snapshot(tmp_path: Path):
    """Same hole via FOOL_SESSION_USER_NAME (display-name path)."""
    marker = tmp_path / "pwned_user"
    user_name = f"alice\ntouch {marker} #"
    proc = _run_dump_and_source(
        tmp_path=tmp_path,
        env_name="FOOL_SESSION_USER_NAME",
        env_value=user_name,
        marker=marker,
    )
    assert proc.returncode == 0, (
