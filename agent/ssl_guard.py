...
def _skip_ssl_guard_enabled() -> bool:
    return os.getenv("FOOL_SKIP_SSL_GUARD", "").strip().lower() in _SKIP_VALUES


def _repair_hint() -> str:
    return (
        "Repair: run `fool doctor --fix` (auto-reinstalls certifi), or "
...
def verify_ca_bundle() -> None:
    ...
    if _skip_ssl_guard_enabled():
        logger.debug("SSL CA bundle guard skipped via FOOL_SKIP_SSL_GUARD")
        return
