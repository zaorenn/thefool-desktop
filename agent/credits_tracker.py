...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    ...
    2. A peek into the in-process pricing cache in ``fool_cli.models``
    ...
    if not base_url:
        return False
    try:
        from fool_cli.models import _is_model_free, _pricing_cache
...
# ── Dev test fixtures (FOOL_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set FOOL_DEV_CREDITS_FIXTURE to either a
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for FOOL_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag FOOL_DEV_CREDITS
    is also on, so a stray FOOL_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("FOOL_DEV_CREDITS")):
        return None
    raw = os.environ.get("FOOL_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
