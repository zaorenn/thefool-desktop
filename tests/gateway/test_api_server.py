...
class TestModelsEndpoint:
    ...
    def test_resolve_model_name_default_profile(self):
        ...
        with patch("fool_cli.profiles.get_active_profile_name", return_value="default"):
    ...
    async def test_model_options_returns_shared_inventory(self, adapter, monkeypatch):
        ...
        from fool_cli import inventory
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["features"]["session_continuity_header"] == "X-Fool-Session-Id"
...
class TestToolsetsEndpoint:
    ...
    async def test_toolsets_returns_resolved_tools(self, adapter):
        ...
        with patch(
            "fool_cli.tools_config._get_effective_configurable_toolsets",
            return_value=fake_toolsets,
        ), patch(
            "fool_cli.tools_config._get_platform_tools",
            return_value={"default"},
        ), patch(
            "fool_cli.tools_config.get_nous_subscription_features",
            return_value=feature_snapshot,
        ) as resolve_features, patch(
            "fool_cli.tools_config._toolset_has_keys",
            return_value=True,
        ) as has_keys, patch(
...
class TestChatCompletionsAgentIncomplete:
    ...
    async def test_hard_failure_redacts_secret_like_error_text(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            body = json.dumps(data)
            assert raw_secret not in body
            assert raw_secret not in resp.headers.get("X-Fool-Error", "")
            assert "OPENAI_API_KEY=" in body
...
# ---------------------------------------------------------------------------
# X-Fool-Session-Id header (session continuity)
# ---------------------------------------------------------------------------
...
class TestSessionIdHeader:
    ...
    async def test_traversal_session_id_header_rejected(self, auth_adapter):
        """Security (#5958): a path-traversal X-Fool-Session-Id must be
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                for bad in ("../../../../etc/pwned", "/abs/path", "..\\win"):
                    resp = await cli.post(
                        "/v1/chat/completions",
                        headers={"X-Fool-Session-Id": bad, "Authorization": "Bearer sk-secret"},
    ...
    async def test_provided_session_id_loads_history_from_db(self, auth_adapter):
        """When X-Fool-Session-Id is provided, history comes from SessionDB not request body."""
        mock_result = {"final_response": "OK", "messages": [], "api_calls": 1}
        db_history = [
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Fool-Session-Id": "existing-session", "Authorization": "Bearer sk-secret"},
...
# ---------------------------------------------------------------------------
# X-Fool-Session-Key header (long-term memory scoping)
# ---------------------------------------------------------------------------
...
class TestSessionKeyHeader:
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_create_agent", side_effect=_fake_create_agent):
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Fool-Session-Key": "agent:main:webui:dm:user-7",
    ...
    async def test_responses_endpoint_accepts_session_key(self, auth_adapter):
        """Responses API honors the same X-Fool-Session-Key contract."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    headers={
                        "X-Fool-Session-Key": "webui:chan-1",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "hermes-agent", "input": "hello", "store": False},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Fool-Session-Key") == "webui:chan-1"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_capabilities_advertises_session_key_header(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["features"]["session_key_header"] == "X-Fool-Session-Key"
...
class TestApiKeyStartupGuardFailsClosed:
    ...
    def _blocking_auth_import():
        ...
        def _blocked(name, *args, **kwargs):
            if name == "fool_cli.auth":
                raise ImportError("simulated: fool_cli.auth unavailable")
...
class TestCreateAgentModelRecovery:
    def test_create_agent_defaults_to_provider_catalog_model_when_empty(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "fool_cli.models.get_default_model_for_provider",
