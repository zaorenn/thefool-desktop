...
``thefool_cli.config.migrate_config``. Each step is a function
...
``thefool_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``thefool_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``thefool_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``thefool_cli.config`` (e.g.
``patch("thefool_cli.config.read_raw_config", ...)``) keep working, because
...
#: THEFOOL_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def _cfg():
    """Return the live ``thefool_cli.config`` module (lazy, cycle-free)."""
    from thefool_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$THEFOOL_HOME/plugins/`` for currently installed user plugins.
