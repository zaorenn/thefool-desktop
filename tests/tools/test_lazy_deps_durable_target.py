...
class TestGatingWithTarget:
    """``FOOL_DISABLE_LAZY_INSTALLS=1`` must STOP blocking once a durable
    ...
    def test_normal_mode_unaffected(self, monkeypatch):
        ...
        monkeypatch.delenv("FOOL_DISABLE_LAZY_INSTALLS", raising=False)
        monkeypatch.delenv(ld._LAZY_TARGET_ENV, raising=False)
        monkeypatch.setattr(
            "fool_cli.config.load_config", lambda: {}, raising=False
        )
...
class TestInstallArgConstruction:
    ...
    def test_uv_resolution_failure_does_not_fall_through_to_pip(self, monkeypatch):
        monkeypatch.delenv(ld._LAZY_TARGET_ENV, raising=False)
        monkeypatch.setattr("fool_cli.managed_uv.resolve_uv", lambda: "uv")
        calls = []
...
@pytest.mark.skipif(
    os.environ.get("FOOL_RUN_NETWORK_TESTS") != "1",
    reason="opt-in real-install test (set FOOL_RUN_NETWORK_TESTS=1); CI runs "
