...
export const ru: Translations = {
  ...
  pluginsPage: {
    ...
    headline:
      "Поиск, установка, включение и обновление плагинов The Fool (аналог `fool plugins`).",
    ...
    removeConfirm: "Удалить этот плагин из ~/.fool/plugins/?",
    removeHint: "Удалять можно только плагины, установленные пользователем в ~/.fool/plugins.",
  ...
  skills: {
    ...
    filters: "Фильтры",
    noSkills: "Навыки не найдены. Навыки загружаются из ~/.fool/skills/",
  ...
  env: {
    ...
    customHint: "Произвольные переменные окружения, сохранённые в вашем .env, которые The Fool не распознаёт. Используйте их для внедрения переменных окружения для навыков, серверов MCP или собственных инструментов.",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Коллекционные значки The Fool, полученные на основе реальной истории сессий. Известные, но ещё не полученные достижения отображаются как «Обнаруженные»; «Секретные» достижения остаются скрытыми до появления первого подходящего поведения.",
      scan_subtitle:
        "Анализ истории сессий The Fool. Первое сканирование может занять 5–10 секунд при большой истории.",
    },
    actions: {
    ...
    stats: {
      ...
      latest: "Последнее",
      latest_hint_empty: "запускайте The Fool чаще",
      none_yet: "Пока нет",
    },
    state: {
      unlocked: "Разблокировано",
    ...
    guide: {
      ...
      secret_body:
        "Секретные достижения скрывают свой точный триггер. Как только The Fool обнаруживает связанный сигнал, карточка становится «Обнаруженной» и показывает требование.",
      scan_status_header: "Статус сканирования",
      scan_status_body:
        "The Fool сканирует локальную историю один раз, затем карточки появятся автоматически. Если это занимает несколько секунд — ничего не зависло.",
