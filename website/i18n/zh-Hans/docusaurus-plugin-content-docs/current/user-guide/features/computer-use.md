...
使用 `fool computer-use status` 验证安装结果。

**方式二：通过交互式界面启用工具集。**

1. 运行 `fool tools`，选择 `🖱️ Computer Use (macOS)` → `cua-driver (background)`。
...
- **`fool update`** — 更新 Hermes 本身时，如果 `cua-driver` 在 PATH 中，更新结束时会重新运行上游安装程序。对非 macOS 用户及未安装 cua-driver 的用户无操作。
- **`fool computer-use install --upgrade`** — 手动强制刷新。无论 cua-driver 是否已安装，都会重新运行上游安装程序。在不等待下次 Agent 更新的情况下获取最新修复时使用此命令。

`fool computer-use status` 会在二进制路径旁显示已安装的版本号。
...
- **私有 SPI 风险。** Apple 可能在任何 OS 更新中更改 SkyLight 的符号接口。Hermes 始终安装最新版 cua-driver，并在已安装的二进制文件低于其测试基线版本（按操作系统分别设定）时发出警告。没有版本固定开关——如需可复现的版本，请将 `FOOL_CUA_DRIVER_CMD` 指向特定的二进制文件。
...
```
FOOL_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
```

完全替换后端（用于测试）：

```
FOOL_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## 故障排查

**`computer_use backend unavailable: cua-driver is not installed`** — 运行 `fool computer-use install` 获取 cua-driver 二进制文件，或运行 `fool tools` 并启用 Computer Use 工具集。
