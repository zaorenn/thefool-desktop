...
crash and a supervised dashboard alongside (when ``FOOL_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``FOOL_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``FOOL_S6_SUPERVISED_CHILD`` sentinel breaks the loop.
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``FOOL_DASHBOARD=1`` is set, ``docker run <image> gateway
