...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (THEFOOL_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a THEFOOL_HOME pointing
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    THEFOOL_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # THEFOOL_HOME passes through IF SET so the MCP subprocess sees the same
    ...
    # codex's subprocess to inherit whatever THEFOOL_HOME its launcher sets
    ...
    # — that would override the launcher's THEFOOL_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("THEFOOL_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("THEFOOL_HOME") or ""
    if hermes_home and _looks_like_test_tempdir(hermes_home):
        hermes_home = ""
    if hermes_home:
        env["THEFOOL_HOME"] = hermes_home
    ...
    env["THEFOOL_QUIET"] = "1"
    env["THEFOOL_REDACT_SECRETS"] = env.get("THEFOOL_REDACT_SECRETS", "true")
