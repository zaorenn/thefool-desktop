...
# ---------------------------------------------------------------------------
# Shared fixture — isolate THEFOOL_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from thefool_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from thefool_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        from thefool_cli.fallback_cmd import cmd_fallback_list
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("thefool_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("thefool_cli.main._require_tty"):
            from thefool_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        with patch("thefool_cli.setup._curses_prompt_choice", return_value=1):
            from thefool_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from thefool_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "thefool_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
