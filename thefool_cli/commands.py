...
class CommandDef:
    ...
    # Names a key in ``thefool_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``thefool_cli.slash_exec.run_execute`` and apply only
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`thefool_cli.plugins.PluginContext.register_command`. They behave
...
class SlashCommandCompleter(Completer):
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via thefool_cli.personality."""
        try:
