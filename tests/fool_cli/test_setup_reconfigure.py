...
- Bare `fool setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `fool setup --quick` runs the narrower "fill in missing items" flow.
- `fool setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("fool_cli.setup.ensure_hermes_home", {}),
        ("fool_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("fool_cli.config.is_managed", {"return_value": False}),
        ("fool_cli.setup.load_config", {"return_value": {}}),
        ("fool_cli.setup.save_config", {}),
        ("fool_cli.setup.get_env_value", {"return_value": None}),
        ("fool_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("fool_cli.setup._print_setup_summary", {}),
        ("fool_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("fool_cli.setup.load_config", {"return_value": {}}),
        ("fool_cli.setup.save_config", {}),
        ("fool_cli.auth.get_active_provider", {"return_value": None}),
        ("fool_cli.setup.get_env_value", {"return_value": None}),
        ("fool_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `fool setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="fool_cli.setup.prompt_choice",
                quick="fool_cli.setup._run_quick_setup",
                model="fool_cli.setup.setup_model_provider",
                terminal="fool_cli.setup.setup_terminal_backend",
                agent="fool_cli.setup.setup_agent_settings",
                gateway="fool_cli.setup.setup_gateway",
                tools="fool_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("fool_cli.setup.prompt_choice", {"return_value": 0}),
                first="fool_cli.setup._run_first_time_quick_setup",
            )
            from fool_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "fool_cli.setup.run_setup_wizard",
