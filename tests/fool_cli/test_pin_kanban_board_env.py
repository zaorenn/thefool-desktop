...
board into `FOOL_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`hermes kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `fool kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `FOOL_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("FOOL_KANBAN_BOARD")
    ...
    try:
        ...
        if prev is None:
            ...
            os.environ["FOOL_KANBAN_BOARD"] = prev


def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    main_mod = importlib.import_module("fool_cli.main")

    import fool_cli.kanban_db as kdb
    monkeypatch.setattr(kdb, "get_current_board", lambda: "space")

    main_mod._pin_kanban_board_env()

    assert main_mod.os.environ.get("FOOL_KANBAN_BOARD") == "space"


def test_pin_does_not_overwrite_existing_env(monkeypatch):
    monkeypatch.setenv("FOOL_KANBAN_BOARD", "preset")
    main_mod = importlib.import_module("fool_cli.main")
    ...
    assert main_mod.os.environ.get("FOOL_KANBAN_BOARD") == "preset"
