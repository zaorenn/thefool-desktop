...
1. Unit contract on :func:`thefool_cli._subprocess_compat.noninteractive_git_env`.
...
class TestNoninteractiveGitEnv:
    ...
    def test_defaults_to_process_environ_copy(self, monkeypatch):
        monkeypatch.setenv("THEFOOL_TEST_SENTINEL", "xyz")
        env = noninteractive_git_env()
        assert env["THEFOOL_TEST_SENTINEL"] == "xyz"
        assert env["GIT_TERMINAL_PROMPT"] == "0"
