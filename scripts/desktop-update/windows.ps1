...
function Show-ErrorFinale([string]$Message) {
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"fool debug share`" in a terminal to send a report."
...
# the page can be QA'd in any browser; FOOL_SELFTEST_FAIL=1 exercises the
# error state, FOOL_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:FOOL_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:FOOL_SELFTEST_HOLD_SECONDS }
    Start-Sleep -Seconds $hold
    if ($env:FOOL_SELFTEST_FAIL) {
