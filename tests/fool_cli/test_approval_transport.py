...
def _request():
    from fool_cli.approval_transport import ApprovalRequest
...
def test_transport_registry_is_manager_and_profile_isolated(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("FOOL_HOME", str(tmp_path / "profiles" / "work"))
    ...
    monkeypatch.setenv("FOOL_HOME", str(tmp_path / "profiles" / "personal"))
    assert first.get_approval_transport("phone") is None


def test_host_accepts_bound_sync_and_async_decisions():
    from fool_cli.approval_transport import invoke_approval_transport
...
def test_host_rejects_decision_completed_after_deadline(monkeypatch):
    import fool_cli.approval_transport as transport_module
...
def test_host_caps_hung_transport_workers():
    from fool_cli.approval_transport import (
...
def test_live_temp_home_fixture_plugin_routes_and_hardline_stays_core_owned(
    ...
    """Real discovery + config + guard path under an isolated FOOL_HOME."""
...
def present(request):
    output = Path(os.environ["FOOL_HOME"]) / "transport-invocations.jsonl"
