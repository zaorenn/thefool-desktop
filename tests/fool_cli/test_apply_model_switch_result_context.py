...
``FoolCLI._apply_model_switch_result()`` printed ``ModelInfo.context_window``
...
def _run_display(monkeypatch, result):
    ...
    cli_mod.FoolCLI._apply_model_switch_result(_StubCLI(), result, False)
    return captured
...
def test_global_switch_clears_context_pin_owned_by_previous_route(monkeypatch):
    ...
    with (
        ...
        patch("fool_cli.config.load_config_readonly", return_value=configured),
    ):
        cli_mod.FoolCLI._apply_model_switch_result(cli, result, True)
