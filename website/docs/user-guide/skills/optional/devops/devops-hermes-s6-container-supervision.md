...
| | |
|---|---|
| Source | Optional — install with `fool skills install official/devops/hermes-s6-container-supervision` |
...
│   └── dashboard/run                  ← if FOOL_DASHBOARD=1, runs `fool dashboard`
│
...
| `fool_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `fool gateway start/stop/restart` and routes to s6 when running in a container. |
...
The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `fool profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
