...
board into `THEFOOL_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
...
def _isolate_kanban_board_env():
    """Snapshot `THEFOOL_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("THEFOOL_KANBAN_BOARD")
    ...
    try:
        ...
        if prev is None:
            ...
            os.environ["THEFOOL_KANBAN_BOARD"] = prev


def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    main_mod = importlib.import_module("thefool_cli.main")

    import thefool_cli.kanban_db as kdb
    monkeypatch.setattr(kdb, "get_current_board", lambda: "space")

    main_mod._pin_kanban_board_env()

    assert main_mod.os.environ.get("THEFOOL_KANBAN_BOARD") == "space"


def test_pin_does_not_overwrite_existing_env(monkeypatch):
    monkeypatch.setenv("THEFOOL_KANBAN_BOARD", "preset")
    main_mod = importlib.import_module("thefool_cli.main")
    ...
    assert main_mod.os.environ.get("THEFOOL_KANBAN_BOARD") == "preset"
