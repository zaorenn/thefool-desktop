...
import thefool_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "thefool_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("thefool_cli.config.is_managed", return_value=False):
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        # (idempotency key) and once for THEFOOL_DASHBOARD_PORTAL_URL. Route by
        ...
        def fake_get_env(key):
            if key == "THEFOOL_DASHBOARD_OAUTH_CLIENT_ID":
                return existing_client_id
            return None

        with patch(
            "thefool_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("thefool_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "thefool_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "thefool_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["THEFOOL_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
        assert "THEFOOL_DASHBOARD_PORTAL_URL" not in saved
...
class TestIdempotentRerun(TestHappyPath):
    ...
    ``existing_client_id`` (the THEFOOL_DASHBOARD_OAUTH_CLIENT_ID a prior run
    ...
    def test_stale_id_falls_through_to_create_prints_registered(self, capsys):
        ...
        assert saved["THEFOOL_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-new"
...
class TestCustomPortalPersistence:
    """`--portal-url` / THEFOOL_DASHBOARD_PORTAL_URL is persisted to .env.
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        THEFOOL_DASHBOARD_PORTAL_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "THEFOOL_DASHBOARD_PORTAL_URL":
                return existing_portal
            return None

        with patch(
            "thefool_cli.auth.resolve_nous_access_token", return_value="tok"
        ), patch("thefool_cli.config.is_managed", return_value=False), patch.dict(
            dr.os.environ, {}, clear=False
        ), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "thefool_cli.config.get_env_value", side_effect=fake_get_env_value
        ), patch(
            "thefool_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
        ):
            # The ambient process env may carry THEFOOL_DASHBOARD_PORTAL_URL
            # (e.g. staging dev shells); drop it so `custom_portal_supplied`
            # is driven solely by the args.portal_url under test.
            dr.os.environ.pop("THEFOOL_DASHBOARD_PORTAL_URL", None)
            dr.cmd_dashboard_register(args)
        return saved
...
class TestPublicUrlPersistence:
    """`--redirect-uri` derives & persists THEFOOL_DASHBOARD_PUBLIC_URL in .env.
    ...
    THEFOOL_DASHBOARD_PUBLIC_URL, so the value that's actually consumed is the
    ORIGIN (scheme://host). We derive the origin from the supplied redirect URI
    and persist THAT as THEFOOL_DASHBOARD_PUBLIC_URL — the var the runtime reads
    ...
    def _run(self, *, args, existing_public=None):
        ...
        THEFOOL_DASHBOARD_PUBLIC_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "THEFOOL_DASHBOARD_PUBLIC_URL":
    ...
    def test_no_redirect_flag_does_not_overwrite_existing(self, capsys):
        ...
        assert "THEFOOL_DASHBOARD_PUBLIC_URL" not in saved
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        with patch(
            ...
            "thefool_cli.config.get_env_value", return_value=None
        ), patch(
            "thefool_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
        ...
        assert saved["THEFOOL_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["THEFOOL_DASHBOARD_PUBLIC_URL"] == "https://hermes.example.com"
