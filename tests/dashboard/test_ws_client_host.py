...
The contract these tests pin down:
  ...
  * Explicit ``THEFOOL_DASHBOARD_WS_HOST`` env var → wins always, regardless
    of the bind host.
...
def clear_ws_host_env(monkeypatch):
    """Ensure no ``THEFOOL_DASHBOARD_WS_HOST`` leaks in from the test shell."""
    monkeypatch.delenv("THEFOOL_DASHBOARD_WS_HOST", raising=False)
    yield monkeypatch
...
class TestResolveClientWsHost:
    ...
    def test_blank_env_falls_back_to_bind(
        ...
        """An explicitly empty override (e.g. ``THEFOOL_DASHBOARD_WS_HOST=``)
        ...
        monkeypatch.setenv("THEFOOL_DASHBOARD_WS_HOST", "   ")
        _set_bound(saved_app_state, "0.0.0.0")
        assert web_server._resolve_client_ws_host() == "127.0.0.1"
...
def test_netloc_helper_handles_ipv6_bracket_form():
    ...
    branch when ``THEFOOL_DASHBOARD_WS_HOST`` points at an IPv6 address.
