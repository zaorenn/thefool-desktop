...
# Katalog boylece surumle birlikte geliyor ve guncellemeler `fool update`
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    network-free. The cache is kept fresh by the picker/`fool update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``fool update`` pulls the latest repo, so the freshly-pulled
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``fool model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
