 ...
 * THEFOOL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS, clamped to a 45s floor.
 */
...
test('honors a valid THEFOOL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS override', () => {
  const env = { THEFOOL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '120000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), 120_000)
})

test('clamps an override below the floor up to the 45s minimum', () => {
  const env = { THEFOOL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '1000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), MIN_PORT_ANNOUNCE_TIMEOUT_MS)
})

test('rounds a fractional override', () => {
  const env = { THEFOOL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '60000.7' }
...
test('falls back to the default for malformed / non-positive overrides', () => {
  for (const bad of ['', 'abc', '0', '-5', 'NaN', undefined]) {
    const env = bad === undefined ? {} : { THEFOOL_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: bad }
    assert.equal(
...
test('resolves with a THEFOOL_BACKEND_READY port (headless `serve`)', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'THEFOOL_BACKEND_READY port=43210\n')
...
test('parses the port even when the line arrives split across chunks', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'THEFOOL_DASHBOARD_READY po')
...
test('a late announcement after timeout does not throw (listeners torn down)', async () => {
  ...
  assert.doesNotThrow(() => {
    child.stdout.emit('data', 'THEFOOL_DASHBOARD_READY port=9999\n')
  })
})
