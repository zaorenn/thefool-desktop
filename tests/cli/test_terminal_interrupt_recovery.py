...
The recovery path lives in ``FoolCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A FoolCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(FoolCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise FoolCLI._recover_terminal_after_interrupt()."""
...
class TestFinallyBlockWiring:
    ...
    def test_recovery_is_invoked_behind_interrupt_guard(self):
        src = inspect.getsource(FoolCLI.run)
    ...
    def test_recovery_helper_exists(self):
        assert hasattr(FoolCLI, "_recover_terminal_after_interrupt")
        assert callable(FoolCLI._recover_terminal_after_interrupt)
