...
def main_mod(monkeypatch):
    import thefool_cli.main as mod
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import thefool_cli.oneshot as oneshot
        from thefool_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    from thefool_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "thefool_state", mod("thefool_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "thefool_cli.config",
        mod("thefool_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "thefool_cli.models",
        mod("thefool_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "thefool_cli.tools_config",
        mod("thefool_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["THEFOOL_TUI_ACTIVE_SESSION_FILE"])
    ...
    env = captured["env"]
    assert env["THEFOOL_MODEL"] == "nous/hermes-test"
    assert env["THEFOOL_INFERENCE_MODEL"] == "nous/hermes-test"
    assert env["THEFOOL_TUI_PROVIDER"] == "nous"
    assert env["THEFOOL_INFERENCE_PROVIDER"] == "nous"
    assert env["THEFOOL_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["THEFOOL_TUI_ACTIVE_SESSION_FILE"])
...
def test_make_tui_argv_dev_prebuilds_hermes_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("THEFOOL_TUI_DIR", raising=False)
