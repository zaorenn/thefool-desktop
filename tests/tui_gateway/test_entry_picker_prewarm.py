...
guard is covered in ``tests/fool_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``fool_cli.model_switch.prewarm_picker_cache_async``
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from fool_cli.model_switch,
