...
``$FOOL_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $FOOL_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$FOOL_HOME``,
    not under ``profiles/``). The dispatcher in ``fool_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``fool gateway start`` (no ``-p``) targets. Without it,
    bare ``fool gateway start`` inside the container would land on
    ...
    ``$FOOL_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's FOOL_HOME (typically /opt/data).
    ...
    # ``fool gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `fool profile create` (config.yaml
            # is not — that comes later via `fool setup`). Use it as the
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("FOOL_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``fool dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted FOOL_HOME,
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $FOOL_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted FOOL_HOME, both race to flock() the same s6-log lock
