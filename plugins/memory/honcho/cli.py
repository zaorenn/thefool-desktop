...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `fool update` -- no output, no exceptions.
    """
    try:
        from fool_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _local_config_path() -> Path:
    ...
    Always returns $FOOL_HOME/honcho.json so each profile/instance gets
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom fool_cli already uses for gateway refs).
    """
    try:
...
def cmd_setup(args) -> None:
    ...
    try:
        ...
        print("  Run: fool config set memory.provider honcho")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if fool_cli internals move."""
    try:
...
def register_cli(subparser) -> None:
    """Build the ``fool honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``fool honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to fool memory setup)",
    )
