...
class TestSpawnGatewayRestartReapsOrphans:
    ...
    @patch("thefool_cli.web_server._gateway_subcommand", return_value=["gateway", "restart"])
    @patch("thefool_cli.web_server._spawn_hermes_action")
    @patch("thefool_cli.web_server._ACTION_PROCS", {})
    def test_reap_called_before_spawn(self, mock_spawn, mock_subcmd):
        ...
        from thefool_cli.web_server import _spawn_gateway_restart

        with patch(
            "thefool_cli.gateway._reap_unsupervised_gateway_orphans"
        ) as mock_reap:
