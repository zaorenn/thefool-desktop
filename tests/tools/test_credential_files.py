...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        with patch.dict(os.environ, {"THEFOOL_HOME": str(hermes_home)}):
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape THEFOOL_HOME."""
    ...
    def test_nested_subdir_inside_hermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of THEFOOL_HOME must be allowed."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside THEFOOL_HOME pointing outside must be rejected."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("THEFOOL_HOME", str(hermes_home))
        ...
        # The resolved path escapes THEFOOL_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape THEFOOL_HOME."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("THEFOOL_HOME", str(hermes_home))
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside THEFOOL_HOME via config must still mount."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("THEFOOL_HOME", str(hermes_home))
...
class TestCacheDirectoryMounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``THEFOOL_HOME/images``, not
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — THEFOOL_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping THEFOOL_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"THEFOOL_HOME": str(home)}):
