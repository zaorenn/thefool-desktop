...
When Hermes runs in Docker with ``THEFOOL_UID=1000`` / ``THEFOOL_GID=911``,
the entrypoint chowns the top-level ``THEFOOL_HOME`` once at startup. But
...
class TestResolveHermesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("THEFOOL_UID", "1000")
        monkeypatch.setenv("THEFOOL_GID", "911")
        from thefool_cli.config import _resolve_hermes_uid_gid
...
class TestChownToHermesUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("THEFOOL_UID", "1000")
        monkeypatch.setenv("THEFOOL_GID", "911")
        from thefool_cli import config as cfg
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("THEFOOL_UID", raising=False)
        monkeypatch.delenv("THEFOOL_GID", raising=False)
        from thefool_cli import config as cfg
