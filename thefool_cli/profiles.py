...
Each profile is a fully independent THEFOOL_HOME directory with its own
...
_PROFILE_DIRS = [
    ...
    # See thefool_constants.get_subprocess_home().
    "home",
]
...
# Allow-list for ``export_profile("default")``: when THEFOOL_HOME equals the
...
# artifacts* at the root of THEFOOL_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``thefool_constants``.
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current THEFOOL_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where THEFOOL_HOME points outside
    ``~/.hermes``, profiles live under ``THEFOOL_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) THEFOOL_HOME path.

    In standard deployments this is ``~/.hermes``.
    In Docker/custom deployments where THEFOOL_HOME is outside ``~/.hermes``
    (e.g. ``/opt/data``), returns THEFOOL_HOME directly.
    """
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its THEFOOL_HOME directory."""
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``THEFOOL_HOME``.
    """
    try:
...
def profiles_to_serve(
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own THEFOOL_HOME. When
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches THEFOOL_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "THEFOOL_HOME": str(profile_dir)},
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``THEFOOL_HOME``
    ...
    hermes_markers = ("thefool_cli.main", "hermes-gateway", "tui_gateway")
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # ...or by THEFOOL_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("THEFOOL_HOME", "")
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``THEFOOL_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from thefool_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set THEFOOL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("THEFOOL_HOME")
    try:
        os.environ["THEFOOL_HOME"] = str(profile_dir)
        from thefool_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["THEFOOL_HOME"] = old_home
        elif "THEFOOL_HOME" in os.environ:
            del os.environ["THEFOOL_HOME"]
...
def get_active_profile_name() -> str:
    """Infer the current profile name from THEFOOL_HOME.

    Returns ``"default"`` if THEFOOL_HOME is not set or points to ``~/.hermes``.
    Returns the profile name if THEFOOL_HOME points into ``~/.hermes/profiles/<name>``.
    Returns ``"custom"`` if THEFOOL_HOME is set to an unrecognized path.
    """
    from thefool_constants import get_hermes_home
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      THEFOOL_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Hermes file the
      user may have lying alongside THEFOOL_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``security.redact_secrets`` / ``THEFOOL_REDACT_SECRETS`` — share archives
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a THEFOOL_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the THEFOOL_HOME environment variable.

    When THEFOOL_HOME is already set, the configured spelling IS the
    ...
    validate_profile_name(canon)
    env_home = os.environ.get("THEFOOL_HOME", "").strip()
    if env_home:
