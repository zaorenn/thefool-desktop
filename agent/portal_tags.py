...
The version is sourced live from ``fool_cli.__version__`` so it auto-aligns
...
``fool_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``fool_cli`` cannot be imported (should
