...
profile's FOOL_HOME and EXECUTE under it.
...
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active FOOL_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever FOOL_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    FOOL_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import fool_constants

    monkeypatch.setattr(
        fool_constants, "_get_platform_default_hermes_home", lambda: root
    )
    monkeypatch.setenv("FOOL_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile FOOL_HOME (<root>/profiles/<name>), the cron store
    ...
    assert fool_constants.get_hermes_home().resolve() == profile_home.resolve()
    assert fool_constants.get_default_hermes_root().resolve() == root.resolve()

    # cron/jobs.py computes FOOL_DIR from get_hermes_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.FOOL_DIR.resolve() == profile_home.resolve()
        assert (
