...
FOOL_CRON_AUTO_DELIVER_* contextvars that run_job publishes per run.
...
def _enter_cron_context(platform=None, chat_id=None, thread_id=None):
    ...
    if platform is not None:
        extra.append(
            (_VAR_MAP["FOOL_CRON_AUTO_DELIVER_PLATFORM"],
             _VAR_MAP["FOOL_CRON_AUTO_DELIVER_PLATFORM"].set(platform))
        )
        extra.append(
            (_VAR_MAP["FOOL_CRON_AUTO_DELIVER_CHAT_ID"],
             _VAR_MAP["FOOL_CRON_AUTO_DELIVER_CHAT_ID"].set(str(chat_id)))
        )
        if thread_id is not None:
            extra.append(
                (_VAR_MAP["FOOL_CRON_AUTO_DELIVER_THREAD_ID"],
                 _VAR_MAP["FOOL_CRON_AUTO_DELIVER_THREAD_ID"].set(str(thread_id)))
            )
    return tokens, extra
