"""Tests for fool_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $FOOL_HOME under tmp_path; no real
...
from fool_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "fool_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    if config:
        ...
        # `fool profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    FOOL_HOME root (the implicit default profile)."""
    if state is not None:
...
def test_main_skips_reconcile_in_dashboard_container_s6v3(
    ...
    """
    from fool_cli import container_boot
