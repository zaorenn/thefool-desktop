...
Ledger location: `$FOOL_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `FOOL_CITATION_LEDGER`.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `FOOL_CITATION_LEDGER`) if their
