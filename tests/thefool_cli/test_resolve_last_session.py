...
from thefool_cli.main import _resolve_last_session
...
def test_search_sessions_exposes_last_active_column(tmp_path, monkeypatch):
    ...
    db = thefool_state.SessionDB(db_path=Path(tmp_path / "state.db"))
    try:
...
def test_resolve_last_session_real_db_prefers_workspace(monkeypatch, tmp_path):
    ...
    real_db = thefool_state.SessionDB
    ...
    monkeypatch.setattr(
        "thefool_cli.main.subprocess.run",
        lambda cmd, **kw: __import__("subprocess").CompletedProcess(
            cmd, 0, stdout=str(repo_a), stderr=""
        ),
    )
    monkeypatch.setattr("thefool_state.SessionDB", lambda: real_db(db_path=state_db))
