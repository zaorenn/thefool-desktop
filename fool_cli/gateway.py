...
Handles: fool gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when fool_constants is cached from a pre-update version during `fool update`.
...
# — restart may fail", and `fool update` can deadlock behind it.  The loop
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``fool gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``fool gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    # Forward slashes on both sides of the FOOL_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. FOOL_HOME
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``fool update`` hung exactly there
            ...
            # watchdog probe.
            from fool_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``fool update``
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from fool_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "fool_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m fool_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``fool update``.
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose FOOL_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``fool update`` users
    # with no running gateway until they re-invoke ``fool gateway``
    ...
    # interpreter and captures the stable cwd + env overlay (FOOL_HOME,
    ...
    if sys.platform == "win32":
        try:
            from fool_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / FOOL_HOME) the windowless
        # base interpreter needs to import fool_cli.  Empty on POSIX, where
...
def _hermes_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``FOOL_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``FOOL_HOME``.

    Under ``sudo``, ``FOOL_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hermes_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``FOOL_HOME`` for the actual gateway
    ...
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("FOOL_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("FOOL_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["FOOL_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}fool gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}fool gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from fool_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: fool gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `fool gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``fool gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from fool_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (fool_cli/gateway.py restart
    ...
    if is_windows():
        try:
            ...
            # dormant on every standard `fool gateway install` deployment.
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (FOOL_HOME-scoped).
...
from fool_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``fool gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``FOOL_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("FOOL_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from fool_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current FOOL_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (FOOL_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this FOOL_HOME.

    Default ``~/.hermes`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.hermes/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other FOOL_HOME appends a short hash for uniqueness.
    """
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: fool gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo fool gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    fool gateway uninstall")
    print_info("    sudo fool gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: fool gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``fool setup`` (end of wizard)
    and ``fool import``: if this host supports a service manager and no
    ...
    ``fool gateway install --system`` explicitly (that path prompts and
    ...
    if is_container():
        ...
        print_info("   fool gateway run          # Run as container main process")
        print_info("")
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: fool gateway")
        return False
    ...
    try:
        ...
        if not _is_service_installed():
            ...
            if supports_systemd:
                ...
                from fool_cli import gateway_windows
        ...
        print_info("  You can retry manually: fool gateway install")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # fool_constants cached from before this symbol existed. See
            # _reload_hermes_constants() in fool_cli/managed_uv.py.
            from fool_cli.managed_uv import _reload_hermes_constants
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current FOOL_HOME to the equivalent under a target user's home.
    ...
    """
    current_hermes_raw = os.environ.get("FOOL_HOME", "").strip()
    current_hermes = (
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m fool_cli.main``, so module
    ...
    dir, or a clone that ``fool update`` later relocates/removes — the path
    ...
    ``FOOL_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if FOOL_HOME
...
def _append_node_dir_for_service(
    ...
    The Hermes-managed Node under ``$FOOL_HOME/node`` goes first when it
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's FOOL_HOME (stable, always exists)
...
Group={group_name}
ExecStart={python_path} -m fool_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="FOOL_HOME={hermes_home}"
Restart=always
RestartSec=5
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m fool_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="FOOL_HOME={hermes_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned FOOL_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo fool gateway … --system``, FOOL_HOME is often stripped
    # and falls back to ``/root/.hermes``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / FOOL_HOME comparisons
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir FOOL_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved FOOL_HOME
    ...
    exported a throwaway ``FOOL_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``fool gateway restart``
    with ``FOOL_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="FOOL_HOME=..."`` lines and launchd
    ``<key>FOOL_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'FOOL_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>FOOL_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp FOOL_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: FOOL_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported FOOL_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single FOOL_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only FOOL_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp FOOL_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="FOOL_HOME=..."``. Writing that to the developer's
    ...
    # FOOL_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo fool gateway uninstall --system")
    print_info("         fool gateway install")
    print_info("         fool gateway start")
...
def systemd_install(
    ...
    # Existing system units already pin FOOL_HOME; adopt it before any
    ...
    # ``sudo fool gateway install --system --force`` would bake /root/.hermes
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}fool gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}fool gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}fool gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # FOOL_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `fool gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `fool gateway status` or logs for final state."
            )
            return
        _wait_for_systemd_service_restart(system=system, previous_pid=pid)
        return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}fool gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}fool gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}fool gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}fool gateway start{scope_flag}"
        )
...
# process invocation (each ``fool gateway start/stop/status`` call).
...
#      background process (the `nohup fool gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m fool_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active FOOL_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "fool_cli.main"]
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``fool update`` sees the flag on the live grandchild argv and hands
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup fool gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup fool gateway run --replace "
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `fool gateway start`)
    ...
    nofile_block = ""
    try:
        from fool_cli.resource_limits import configured_nofile_soft_limit
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  fool gateway status             # Check status")
    from fool_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    # `fool gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `fool update`
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: fool gateway stop")
            else:
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``FOOL_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``fool gateway run`` is always a misconfiguration, so we
    ...
    try:
        ...
        from fool_cli.profiles import normalize_profile_name
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``fool gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``fool gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other FOOL_HOME roots.
    """
    ...
    print("  Use 'fool gateway restart' to replace it,")
    print("  or 'fool gateway stop' first.")
    print("  Or use 'fool gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("FOOL_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        ...
        "$FOOL_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set FOOL_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `fool gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set FOOL_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("FOOL_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # /restart) rather than through `fool gateway restart` which already
    ...
    # `fool gateway start/restart` — leaving the gateway vulnerable to
    ...
    # FOOL_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("FOOL_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from fool_constants import get_hermes_home as _ghh
    ...
    # the env vars ``FOOL_GATEWAY_MAX_STARTS`` /
    # ``FOOL_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("FOOL_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("FOOL_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``fool gateway run`` enters through this CLI wrapper, not through
...
def _all_platforms() -> list[dict]:
    ...
    ``fool setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run fool under
        WSL.
    """
...
def _setup_standard_platform(platform: dict):
    ...
    from fool_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from fool_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'fool pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: fool pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'fool gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `fool gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `fool pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `fool pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "FoolAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``fool_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from fool_cli import setup as _s
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: fool gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: fool gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: fool gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo fool gateway install --system"
                        )
                    print_info("  Or run in foreground:  fool gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: fool gateway run")
                print_info(
                    "  For persistence:   tmux new -s hermes 'fool gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: fool gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup fool gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'fool gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`fool_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from fool_cli.service_manager import (
    ...
    if profile is None:
        ...
        # FOOL_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``fool gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``fool gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``fool gateway run`` are unaffected.
      2. ``FOOL_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``fool gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``FOOL_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("FOOL_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("FOOL_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${FOOL_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if FOOL_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or FOOL_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-fool service's
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: fool gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: fool gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hermes 'fool gateway run'"
                )
                print()
            ...
            print()
            print(
                "  fool gateway run                              # direct foreground"
            )
            print(
            ...
            from fool_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
            ...
            print()
            print("To run the gateway: fool gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: fool gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: fool gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: fool gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `fool gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `fool gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: fool gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  fool gateway install")
                    print("  sudo fool gateway install --system")
            else:
                ...
                if is_termux():
                    ...
                    print(
                        "  fool gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  fool gateway install  # Install as user service")
                    print(
                        "  sudo fool gateway install --system  # Install as boot-time system service"
                    )
