...
``CommandDef.execute`` (fool_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/fool_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``fool_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: fool bundles create <name> --skill <s1> --skill <s2>\n"
