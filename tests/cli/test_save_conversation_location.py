...
def hermes_home(tmp_path, monkeypatch):
    ...
    if hasattr(fool_constants, "_hermes_home_cache"):
        fool_constants._hermes_home_cache = None
    return home
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    ...
    # Import fresh to pick up the FOOL_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "fool_constants"]:
    ...
    cli.FoolCLI.save_conversation(stub, "/save json")
...
def test_save_conversation_bare_shows_usage(hermes_home, capsys):
    ...
    cli.FoolCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(hermes_home, capsys):
    ...
    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.FoolCLI.save_conversation(stub, "/save pdf")
