...
Hermes owns its own uv binary at ``$FOOL_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$FOOL_HOME/bin`` so the installer writes directly there — no PATH
...
from fool_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime
...
def managed_uv_path() -> Path:
    ...
    ``$FOOL_HOME/bin/uv`` on POSIX, ``$FOOL_HOME\\bin\\uv.exe`` on
...
def _reload_hermes_constants():
    ...
    return importlib.reload(fool_constants)
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("fool_cli.main")
...
def _install_uv(target: Path) -> None:
    ...
    ``$FOOL_HOME/bin/`` instead of ``~/.local/bin/``.
    """
