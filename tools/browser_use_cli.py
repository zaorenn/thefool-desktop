...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Run `fool tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Hermes' own bin dir ($FOOL_HOME/bin) — where install.sh puts uv/uvx
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Hermes' own ``$FOOL_HOME/bin`` copy — the
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    ``fool_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$FOOL_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
...
def _resolve_backend_cdp(
    ...
    try:
        ...
        return (
            ...
            "switch backends via `fool tools` → Browser Automation."
        )
