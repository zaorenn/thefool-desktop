"""Tests for fool_cli configuration management."""
...
class TestEnsureFoolHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from fool_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``fool logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from fool_cli import config as cfg_mod
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from fool_cli.config import _deep_merge
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"FOOL_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"FOOL_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"FOOL_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"FOOL_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("fool_cli.config.sanitize_env_file", return_value=2),
            patch(
                "fool_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("fool_cli.config.read_raw_config", return_value={}),
            patch("fool_cli.config.get_missing_env_vars", return_value=[]),
            patch("fool_cli.config.get_missing_config_fields", return_value=[]),
            patch("fool_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from fool_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """FOOL_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `fool setup`) lets a
        ...
        via config.yaml; FOOL_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from fool_cli.config import OPTIONAL_ENV_VARS
        assert "FOOL_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `fool setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `fool setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_hermes_home(self):
        from fool_cli.config_migrations import support_floor_message
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from fool_cli.config_migrations import run_migrations
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from fool_cli.config import _persist_migration, read_raw_config
