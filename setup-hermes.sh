...
FOOL_BIN="$SCRIPT_DIR/venv/bin/hermes"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$FOOL_BIN" "$COMMAND_LINK_DIR/hermes"
...
FOOL_SKILLS_DIR="${FOOL_HOME:-$HOME/.hermes}/skills"
mkdir -p "$FOOL_SKILLS_DIR"
...
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    ...
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$FOOL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  fool status        # Check configuration"
if is_termux; then
    echo "  fool gateway       # Run gateway in foreground"
else
    echo "  fool gateway install # Install gateway service (messaging + cron)"
fi
echo "  fool cron list     # View scheduled jobs"
echo "  fool doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m fool_cli.main setup
fi
