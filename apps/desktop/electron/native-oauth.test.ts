...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/fool', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/fool/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/fool'), 'https://gw.example.com/fool/auth/native/refresh')
})
