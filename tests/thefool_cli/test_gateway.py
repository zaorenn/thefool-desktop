...
import thefool_cli.gateway as gateway
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # sandboxes ``THEFOOL_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``THEFOOL_HOME`` value, which
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        diag_path = pathlib.Path(os.environ["THEFOOL_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            ...
            "THEFOOL_GATEWAY_EXIT_DIAG": "1",
            "THEFOOL_GATEWAY_MAX_STARTS": "0",
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["THEFOOL_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "THEFOOL_HOME": str(tmp_path),
        "THEFOOL_GATEWAY_EXIT_DIAG": "0",
        "THEFOOL_TEST_GATEWAY_OUTCOME": outcome,
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("thefool_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("thefool_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test THEFOOL_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="THEFOOL_HOME=/home/alice/.hermes"\n'
        ),
    )
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "thefool_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        ...
        import thefool_cli.gateway_windows as gateway_windows
