...
FOOL_HOME with a config.yaml routing ``secrets.provider: command`` through
``fool_cli.env_loader._apply_external_secret_sources``.

Security invariants under test (ported from the desktop TS provider):

* the requested key travels ONLY via the ``FOOL_SECRET_KEY`` env var —
...
# ---------------------------------------------------------------------------
# Dispatch E2E through env_loader against a real temp FOOL_HOME
# ---------------------------------------------------------------------------
