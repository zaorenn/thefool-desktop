...
from thefool_cli.main import cmd_update, PROJECT_ROOT
...
def _patch_managed_uv(request):
    ...
    with patch("thefool_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("thefool_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("thefool_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield
...
def _patch_gateway_discovery():
    ...
    """
    with patch("thefool_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("thefool_cli.gateway.supports_systemd_services", return_value=False), \
         patch("thefool_cli.gateway.find_profile_gateway_processes", return_value=[]):
        yield
...
class TestCmdUpdateNpmLockfileCache:
    ...
    def test_record_npm_lockfile_hash(self, tmp_path, monkeypatch):
        from thefool_cli import main as hm
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        monkeypatch.setattr(thefool_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            thefool_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("THEFOOL_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("THEFOOL_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $THEFOOL_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("thefool_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "thefool_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "thefool_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch(
            "thefool_cli.update_cmd._reload_config_modules"
        ), patch(
            "thefool_cli.update_cmd._run_config_check_fresh", return_value=(1, 2)
        ), patch(
            "thefool_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ) as migrate_config, patch("thefool_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "thefool_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "thefool_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "thefool_cli.update_cmd._reload_config_modules"
        ), patch(
            "thefool_cli.update_cmd._run_config_check_fresh", return_value=(5, 24)
        ), patch(
            "thefool_cli.update_cmd._run_migrate_config_fresh",
    ...
    def test_version_bump_only_surfaces_migration_resets(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "thefool_cli.update_cmd._run_config_check_fresh", return_value=(33, 34)
        ), patch(
            "thefool_cli.update_cmd._run_migrate_config_fresh",
            return_value={
                "env_added": [],
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "thefool_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "thefool_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "thefool_cli.update_cmd._reload_config_modules"
        ), patch(
            "thefool_cli.update_cmd._run_config_check_fresh", return_value=(1, 24)
        ), patch(
            "thefool_cli.update_cmd._run_migrate_config_fresh",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("thefool_cli.main.sys") as mock_sys:
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    returned the OLD cached ``thefool_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD thefool_cli.config with the OLD
        DEFAULT_CONFIG["_config_version"] after git pull.
        """
        from unittest.mock import patch

        import thefool_cli.update_cmd as update_cmd
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        with (
            patch("thefool_cli.profiles.list_profiles", return_value=all_profiles),
            patch("thefool_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("thefool_cli.profiles.list_profiles", return_value=[default_p]),
            patch("thefool_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateCheckBranchFlag:
    ...
    @patch("thefool_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from thefool_cli.main import _update_via_zip
...
class TestNodeRuntimeNpmResolution:
    ...
    def test_update_rebuilds_desktop_that_disappears_mid_update(self):
        ...
        from thefool_cli import main as hm
        from thefool_cli import update_cmd
        ...
        desktop_build.assert_called_once_with(
            [hm.sys.executable, "-m", "thefool_cli.main", "desktop", "--build-only"],
            cwd=PROJECT_ROOT,
            env=ANY,
        )
    ...
    def test_git_failure_zip_fallback_rebuilds_missing_desktop(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            patch("thefool_cli.managed_uv.ensure_uv", return_value="uv"),
            patch("thefool_cli.managed_uv.update_managed_uv"),
            patch(
            ...
            patch("thefool_cli.model_catalog.seed_cache_from_checkout", return_value=False),
        ):
