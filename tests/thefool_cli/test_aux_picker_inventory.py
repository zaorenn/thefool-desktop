...
``thefool_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
def configured_home(tmp_path, monkeypatch):
    """A THEFOOL_HOME with one ``providers:`` entry and one legacy
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from thefool_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    from thefool_cli import inventory
    ...
    with patch("thefool_cli.model_switch.list_authenticated_providers", _capture):
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    with (
        patch("thefool_cli.model_switch.list_authenticated_providers", _direct),
        patch("thefool_cli.inventory.build_aux_picker_rows", _substrate),
        patch("thefool_cli.main._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through thefool_cli.inventory.build_aux_picker_rows so custom "
