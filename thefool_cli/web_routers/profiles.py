...
late-binding seam in :mod:`thefool_cli.web_deps` so tests that
...
from thefool_cli.web_deps import late
from thefool_cli.web_models import (
    ProfileCreate,
    ProfileActiveUpdate,
    ProfileExport,
    ProfileImport,
...
_log = logging.getLogger("thefool_cli.web_server")
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    the repo-scan policy, the THEFOOL_HOME junk filters — through the
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's THEFOOL_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    the running dashboard/gateway is scoped to (derived from THEFOOL_HOME).
    """
    from thefool_cli import profiles as profiles_mod
    try:
...
async def update_profile_soul(name: str, body: ProfileSoulUpdate):
    ...
    try:
        ...
        # profiles seed SOUL.md at the umask default (thefool_cli.profiles
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the THEFOOL_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    try:
        from thefool_cli import profile_describer
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps thefool_cli.profiles.export_profile /
