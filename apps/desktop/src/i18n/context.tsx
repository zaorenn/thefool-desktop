...
import { type FoolConfigRecord, getFoolConfigRecord, saveFoolConfig } from '@/hermes'
...
export interface I18nConfigClient {
  getConfig: () => Promise<FoolConfigRecord>
  saveConfig: (config: FoolConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.foolDesktop?.api) {
      return Promise.resolve({})
    }

    return getFoolConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.foolDesktop?.api) {
...
export function getConfigDisplayLanguage(config: FoolConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: FoolConfigRecord, locale: Locale): FoolConfigRecord {
