...
from thefool_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Set, Tuple, TypeVar

from thefool_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from thefool_state_portability import SessionPortabilityMixin
from thefool_state_schema import SessionSchemaMixin
from thefool_state_search import SessionSearchMixin
...
def _default_db_path() -> Path:
    ...
    redirects ``THEFOOL_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_hermes_home()`` fresh so a runtime
       ``THEFOOL_HOME`` redirect takes effect regardless of import order.
    """
...
# FTS write corruption").  The hermetic conftest redirects THEFOOL_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without THEFOOL_HOME, a stale worktree
# without the re-pin, or a developer shell that exports THEFOOL_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "THEFOOL_STATE_DB_GUARD_BYPASS"
...
#: root here so custom-``THEFOOL_HOME`` deployments are covered too.
...
def _real_platform_state_root() -> Optional[Path]:
    ...
    Deliberately avoids ``Path.home()`` / ``thefool_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``thefool_state``
...
#: redirects ``THEFOOL_HOME`` to the per-session tmp isolation root.  Its
...
#: child that received the patched ``THEFOOL_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "THEFOOL_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``THEFOOL_HOME`` redirect: that child aims at the production DB *and*
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``THEFOOL_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``THEFOOL_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                "Tests must run against a temporary THEFOOL_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "THEFOOL_HOME. If this test genuinely needs the live "
...
# thefool_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    ``thefool_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``thefool_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see thefool_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``journal_size_limit`` — max journal/WAL size in bytes
    ...
    """
    try:
        # Local import avoids a circular import with thefool_cli.config.
        from thefool_cli.config import cfg_get, load_config_readonly
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``thefool_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    to check). Shared by :class:`SessionDB` and ``thefool_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (thefool_state.py:645-723). The supported degraded-runtime
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("THEFOOL_FTS5_CJK_SO")
    if env:
...
def _cjk_fts_config_enabled() -> bool:
    ...
    return os.getenv("THEFOOL_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship thefool_state without thefool_cli). A
    ...
    try:
        ...
        logger.debug(
            "thefool_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch thefool_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``thefool_cli.backup.is_zeroed_sqlite_file`` when available; this
    local copy keeps SessionDB openable without importing the CLI package
    in constrained embed paths.
    """
    try:
        from thefool_cli.backup import is_zeroed_sqlite_file
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``thefool_state._set_last_init_error(None)`` explicitly.
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: thefool_state already depends on agent.* (see
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``THEFOOL_SESSION_SOURCE``, so their runs
