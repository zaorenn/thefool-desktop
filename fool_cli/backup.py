...
from fool_constants import get_default_hermes_root, get_hermes_home, display_hermes_home
from utils import (
    _preserve_file_mode,
...
from fool_cli.sizefmt import format_bytes as _format_size
...
# FOOL_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under FOOL_HOME) —
...
# Reserved archive subtree for provider state that lives OUTSIDE FOOL_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside FOOL_HOME, resolved from config only (no network, no init).
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside FOOL_HOME.
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # fool_cli.sqlite_safe_read). Verification targets snapshots and
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    # outside FOOL_HOME, so the walk above never sees it. Ask the active
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under FOOL_HOME.
        ...
        try:
            from fool_cli.gateway import ensure_gateway_service, _is_service_running
...
_QUICK_STATE_FILES = (
    ...
    # are at $FOOL_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside FOOL_HOME and the entry is silently
...
# Relative path of the cron job database inside FOOL_HOME. Kept in sync with
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of FOOL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<FOOL_HOME>/backups/pre-update-<timestamp>.zip`` and
