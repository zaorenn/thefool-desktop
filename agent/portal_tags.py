...
The version is sourced live from ``thefool_cli.__version__`` so it auto-aligns
...
``thefool_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``thefool_cli`` cannot be imported (should
