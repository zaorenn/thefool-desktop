"""Tests for thefool_cli.mcp_catalog and thefool_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("THEFOOL_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_hermes_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp THEFOOL_HOME."""
    hh = tmp_path / "hermes-home"
    hh.mkdir()
    monkeypatch.setenv("THEFOOL_HOME", str(hh))
    monkeypatch.setattr(
        "thefool_cli.config.get_hermes_home", lambda: hh
    )
    monkeypatch.setattr(
        "thefool_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "thefool_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_hermes_home() lazily through thefool_constants
    monkeypatch.setattr(
        "thefool_constants.get_hermes_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from thefool_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from thefool_cli.mcp_catalog import list_catalog
    ...
    def test_suggest_empty_block_rejected(self, catalog_dir):
        ...
        from thefool_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from thefool_cli.mcp_catalog import _build_server_config
    ...
    def test_http_api_key_requires_matching_env_declaration(self, catalog_dir):
        ...
        from thefool_cli.mcp_catalog import CatalogError, _parse_manifest
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from thefool_cli.mcp_catalog import install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from thefool_cli.mcp_catalog import install_entry
        from thefool_cli.config import get_env_value, load_config
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from thefool_cli.mcp_catalog import install_entry, uninstall_entry
        from thefool_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from thefool_cli.mcp_catalog import uninstall_entry
...
class TestPicker:
    def test_show_catalog_empty(self, catalog_dir, capsys):
        from thefool_cli.mcp_picker import show_catalog
    ...
    def test_install_by_name_success(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from thefool_cli.mcp_picker import install_by_name
        from thefool_cli.config import load_config
    ...
    def test_run_picker_non_tty_falls_back(self, catalog_dir, capsys, monkeypatch):
        ...
        from thefool_cli.mcp_picker import run_picker
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from thefool_cli import mcp_catalog
        from thefool_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "thefool_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no THEFOOL_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("THEFOOL_OPTIONAL_MCPS", raising=False)
        from thefool_cli.mcp_catalog import _catalog_root, _parse_manifest
