...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``fool desktop`` / ``fool gui``)
     Built inside the agent checkout under ``$FOOL_HOME/hermes-agent/``:
       ...
       - ``$FOOL_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$FOOL_HOME``:
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        ...
        # ``uninstall_gui``.
        from fool_cli.linux_desktop_entry import desktop_entry_path
...
def agent_is_installed(hermes_home: Path) -> bool:
    """Return True when a usable Python agent install exists under FOOL_HOME.
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``hermes-agent/fool_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$FOOL_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            from fool_cli.linux_desktop_entry import (
                desktop_entry_path,
