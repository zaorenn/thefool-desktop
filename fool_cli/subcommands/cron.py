"""``fool cron`` subcommand parser.

Extracted verbatim from ``fool_cli/main.py:main()`` — same arguments, same
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.fool/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        ...
        help=(
            ...
            "~/.fool/scripts/ that runs each tick BEFORE the agent. "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.fool/scripts/. Pass empty string to clear. "
