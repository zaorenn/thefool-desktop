...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects FOOL_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("FOOL_INTERACTIVE", "FOOL_GATEWAY_SESSION", "FOOL_EXEC_ASK", "FOOL_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["FOOL_INTERACTIVE"] = "1"
        session_key = os.getenv("FOOL_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No FOOL_INTERACTIVE or FOOL_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
