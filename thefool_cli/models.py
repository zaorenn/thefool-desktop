...
def _codex_curated_models() -> list[str]:
    ...
    """
    from thefool_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $THEFOOL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $THEFOOL_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see thefool_cli/model_setup_flows.py).
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$THEFOOL_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from thefool_cli.model_catalog import get_default_model_from_cache
...
# ``get_recommended_default_model`` in thefool_cli/web_server.py and
...
# thefool_constants.VALID_REASONING_EFFORTS ("none" included so an explicit
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from thefool_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from thefool_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from thefool_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from thefool_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``thefool_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from thefool_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from thefool_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``hermes model`` shows.
            from thefool_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $THEFOOL_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$THEFOOL_HOME/auth.json`` and external
