...
``fool_cli/dump.py`` and ``fool_cli/banner.py``).  That doesn't work inside
...
To make ``fool dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$FOOL_GIT_SHA``
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``FOOL_GIT_SHA`` build-arg fall through to live-git
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    written by the Dockerfile's ``FOOL_GIT_SHA`` build-arg and contains
