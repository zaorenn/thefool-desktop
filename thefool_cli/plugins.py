...
from thefool_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from thefool_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from thefool_cli.plugin_capabilities import (
...
def get_bundled_plugins_dir() -> Path:
    ...
    Honours ``THEFOOL_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("THEFOOL_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``THEFOOL_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
...
_PLUGINS_DEBUG = os.getenv("THEFOOL_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When THEFOOL_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "THEFOOL_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by thefool_cli.kanban_db when a task
    ...
    #   result: thefool_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
THEFOOL_EVENT_NAMESPACE = "hermes"
...
class PluginManifest:
    ...
    # ``thefool_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
...
class PluginContext:
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from thefool_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        ...
        Derived from ``THEFOOL_HOME`` via
        :func:`thefool_cli.profiles.get_active_profile_name`, so it works in
        ...
        ``THEFOOL_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`thefool_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        from thefool_cli.dashboard_auth.registry import restore_registration
        from thefool_cli.dashboard_auth.registry import snapshot_registration
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        # Lazy import to avoid circular: thefool_cli.main imports plugins indirectly
        from thefool_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, THEFOOL_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{THEFOOL_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("THEFOOL_SAFE_MODE"):
                logger.info("THEFOOL_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from thefool_cli.env_loader import load_hermes_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from thefool_cli.approval_transport import RegisteredApprovalTransport
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        if _env_enabled("THEFOOL_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set THEFOOL_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from thefool_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from thefool_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from thefool_cli.agent_plugins import load_agent_plugin
...
# multiple profiles via different THEFOOL_HOME directories, and a single
...
# thefool_constants.set_hermes_home_override). A process-wide single-slot
...
def get_plugin_manager() -> PluginManager:
    ...
    profile switch — via ``THEFOOL_HOME`` or the context-local
...
def _get_pre_tool_call_directive_details(
    ...
    from thefool_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
