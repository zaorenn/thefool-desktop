...
class Migrator:
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("THEFOOL_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("THEFOOL_HOME") or str(Path.home() / ".hermes"), help="Hermes home directory")
    parser.add_argument(
        "--workspace-target",
