...
shared by the CLI (``fool journey delete|edit``), the TUI ``/journey`` overlay
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (fool curator unpin {name})"}

    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()

    return {"ok": ok, "message": f"archived '{name}' — restore with: fool curator restore {name}" if ok else message}
