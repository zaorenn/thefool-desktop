"""Tests for thefool_cli.plugins_cmd — the ``hermes plugins`` CLI subcommand."""
...
class TestResolveGitExecutable:
    ...
    def test_prefers_shutil_which(self):
        import thefool_cli.plugins_cmd as pc
...
class TestReadManifest:
    ...
    def test_invalid_yaml_returns_empty_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="thefool_cli.plugins_cmd"):
            result = _read_manifest(tmp_path)
        assert result == {}
...
class TestCmdInstall:
    ...
    def test_install_requires_identifier(self):
        from thefool_cli.plugins_cmd import cmd_install

        with pytest.raises(SystemExit):
            cmd_install("")

    @patch("thefool_cli.plugins_cmd._resolve_git_url")
    ...
    @patch("thefool_cli.plugins_cmd._display_after_install")
    @patch("thefool_cli.plugins_cmd.shutil.move")
    @patch("thefool_cli.plugins_cmd.shutil.rmtree")
    @patch("thefool_cli.plugins_cmd._plugins_dir")
    @patch("thefool_cli.plugins_cmd._read_manifest")
    @patch("thefool_cli.plugins_cmd.subprocess.run")
...
class TestCmdUpdate:
    ...
    @patch("thefool_cli.plugins_cmd._sanitize_plugin_name")
    @patch("thefool_cli.plugins_cmd._plugins_dir")
    @patch("thefool_cli.plugins_cmd.subprocess.run")
    def test_update_git_pull_success(self, mock_run, mock_plugins_dir, mock_sanitize):
        from thefool_cli.plugins_cmd import cmd_update
...
class TestCmdRemove:
    ...
    def test_remove_deletes_plugin(self, mock_rmtree, mock_plugins_dir, mock_sanitize):
        from thefool_cli.plugins_cmd import cmd_remove
...
class TestCmdList:
    ...
    def test_list_empty_plugins_dir(self, mock_plugins_dir):
        from thefool_cli.plugins_cmd import cmd_list
...
class TestCopyExampleFiles:
    ...
    def test_handles_copy_error_gracefully(self, tmp_path):
        ...
        with patch(
            "thefool_cli.plugins_cmd.shutil.copy2",
...
class TestPromptPluginEnvVars:
    ...
    def test_prompts_for_missing_var_rich_format(self):
        from thefool_cli.plugins_cmd import _prompt_plugin_env_vars
        from unittest.mock import MagicMock, patch
        ...
        with patch("thefool_cli.config.get_env_value", return_value=None), \
             ...
             patch("thefool_cli.config.save_env_value") as mock_save:
    ...
    def test_secret_uses_masked_prompt(self):
        ...
        with patch("thefool_cli.config.get_env_value", return_value=None), \
             patch("thefool_cli.plugins_cmd.masked_secret_prompt", return_value="s3cret") as mock_prompt, \
             patch("thefool_cli.config.save_env_value"):
            _prompt_plugin_env_vars(manifest, console)
...
class TestProviderDiscovery:
    ...
    def test_save_context_engine(self, tmp_path, monkeypatch):
        ...
        from thefool_cli.plugins_cmd import _save_context_engine
    ...
    def test_discover_context_engines_empty(self):
        ...
        with patch("plugins.context_engine.discover_context_engines",
            ...
            from thefool_cli.plugins_cmd import _discover_context_engines
...
class TestSubdirInstallE2E:
    ...
    def test_installs_only_the_subdir_plugin(self, tmp_path, monkeypatch):
        ...
        from thefool_cli import plugins_cmd as pc
...
def test_portable_manifest_is_visible_to_plugin_cli(tmp_path):
    ...
    from thefool_cli.agent_plugins import PLUGIN_SCHEMA_V1
    from thefool_cli.plugins_cmd import _read_manifest_info
