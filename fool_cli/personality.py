...
This module deliberately has no module-level imports from ``fool_cli.config``
...
BUILTIN_PERSONALITIES: Dict[str, str] = {
    ...
    "pirate": "Arrr! Ye be talkin' to Captain The Fool, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
...
def resolve_ephemeral_system_prompt(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    prefer ``FOOL_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.
    """
...
def persist_personality(value: Any) -> bool:
    ...
    FOOL_HOME config.yaml atomically, preserving comments and ordering.
