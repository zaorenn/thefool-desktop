...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("FOOL_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "FOOL_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "FOOL_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `fool update` or `--replace` handoffs)
        ...
        message = (
            ...
            "with this token, then restart the gateway with 'fool gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                from fool_cli.commands import telegram_menu_commands, telegram_menu_max_commands
                if not self._bot:
                    return
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("FOOL_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("FOOL_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("FOOL_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("FOOL_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("FOOL_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            ...
            disable_fallback = (
                os.getenv("FOOL_TELEGRAM_DISABLE_FALLBACK_IPS", "")
                .strip()
                .lower()
                in {"1", "true", "yes", "on"}
            )
            ...
            if not fallback_ips and not disable_fallback:
                discovery_timeout = self._env_float_clamped(
                    "FOOL_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT",
                    5.0,
                    min_value=0.0,
                )
                logger.warning(
            ...
            _max_connect = 8
            _init_timeout = _env_float("FOOL_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or set FOOL_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                            f"FOOL_TELEGRAM_INIT_TIMEOUT to a lower value."
                        )
                    logger.warning(
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set FOOL_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``fool update --gateway``
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``fool model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from fool_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from fool_cli.models import PROVIDER_GROUPS
...
# fool_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("FOOL_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in fool_cli/gateway.py.
    """
    from fool_cli import setup as _setup_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `fool setup` to install Telegram support.",
