...
def test_codex_preflight_defangs_harmony_tokens_before_and_after_middleware(monkeypatch):
    ...
    monkeypatch.setattr(
        "thefool_cli.middleware.apply_llm_request_middleware",
        _request_middleware,
    )
    monkeypatch.setattr(
        "thefool_cli.middleware.run_llm_execution_middleware",
...
def test_try_refresh_copilot_client_credentials_rebuilds_client(monkeypatch):
    ...
    monkeypatch.setattr(
        "thefool_cli.copilot_auth.evict_cached_exchanged_token",
        lambda _raw: None,
    )
    monkeypatch.setattr(
...
def test_try_refresh_copilot_client_credentials_falls_back_when_exchange_unavailable(monkeypatch):
    ...
    monkeypatch.setattr("thefool_cli.copilot_auth.get_copilot_api_token", _boom)
    monkeypatch.setattr(run_agent, "OpenAI", _fake_openai)
