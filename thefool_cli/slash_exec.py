...
``CommandDef.execute`` (thefool_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/thefool_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``thefool_cli.commands`` does NOT import this module (the ``execute`` field is
