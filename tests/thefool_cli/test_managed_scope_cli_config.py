...
thefool_cli.config._load_config_impl, so the managed-scope merge has to be
...
def _load_cli_config(home):
    ...
    singleton), so monkeypatching THEFOOL_HOME after import doesn't move it.
    ...
    this only matters for tests that swap THEFOOL_HOME.)
    """
    import cli
