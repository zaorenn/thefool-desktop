...
`THEFOOL_REDACT_SECRETS` at module-import time. `thefool_cli/main.py` at
...
Fix: bridge `security.redact_secrets` from config.yaml → `THEFOOL_REDACT_SECRETS`
env var in `thefool_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports thefool_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("THEFOOL_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import thefool_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('THEFOOL_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["THEFOOL_HOME"] = str(hermes_home)
    env.pop("THEFOOL_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env THEFOOL_REDACT_SECRETS takes precedence over config.yaml."""
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (hermes_home / ".env").write_text("THEFOOL_REDACT_SECRETS=true\n")
