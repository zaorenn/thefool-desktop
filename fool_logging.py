...
Component separation:
    ...
    gui.log receives dashboard-side records from ``fool_cli.web_server``,
    ``fool_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from fool_constants import get_config_path, get_hermes_home
...
# Used by _ComponentFilter and exposed for ``fool logs --component``.
COMPONENT_PREFIXES = {
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from fool_cli.config import is_managed
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse fool_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when fool_cli.config isn't importable (bare
        # fool_logging consumers).
        try:
            from fool_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
