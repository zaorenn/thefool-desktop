...
from thefool_cli.auth import PROVIDER_REGISTRY, resolve_provider, resolve_api_key_provider_credentials
from thefool_cli.models import _PROVIDER_MODELS, _PROVIDER_LABELS, _PROVIDER_ALIASES, normalize_provider
...
class TestOllamaCloudModelCatalog:
    ...
    def test_provider_model_ids_returns_dynamic_models(self, tmp_path, monkeypatch):
        ...
        with patch("thefool_cli.models.fetch_api_models", return_value=["qwen3.5:397b"]), \
...
class TestOllamaCloudModelPicker:
    def test_ollama_cloud_shows_model_count(self, tmp_path, monkeypatch):
        ...
        from thefool_cli.model_switch import list_authenticated_providers
...
class TestOllamaCloudMergedDiscovery:
    def test_merges_live_and_models_dev(self, tmp_path, monkeypatch):
        ...
        with patch("thefool_cli.models.fetch_api_models", return_value=["qwen3.5:397b", "glm-5"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
...
class TestOllamaCloudProvidersNew:
    def test_overlay_exists(self):
        ...
        assert "ollama-cloud" in THEFOOL_OVERLAYS
        overlay = THEFOOL_OVERLAYS["ollama-cloud"]
        assert overlay.transport == "openai_chat"
        assert overlay.base_url_env_var == "OLLAMA_BASE_URL"

    def test_alias_resolves(self):
        from thefool_cli.providers import normalize_provider as np
...
class TestOllamaCloudSuffixStripping:
    ...
    def test_no_duplicate_when_live_clean_and_mdev_suffixed(self, tmp_path, monkeypatch):
        ...
        with patch("thefool_cli.models.fetch_api_models", return_value=["kimi-k2.6", "glm-5.1"]), \
             patch("agent.models_dev.fetch_models_dev", return_value=mock_mdev):
            result = fetch_ollama_cloud_models(force_refresh=True)
    ...
    def test_strip_suffix_helper(self):
        ...
        from thefool_cli.models import _strip_ollama_cloud_suffix
