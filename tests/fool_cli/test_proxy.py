...
from fool_cli.proxy.adapters import ADAPTERS, get_adapter
from fool_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from fool_cli.proxy.adapters.nous_portal import NousPortalAdapter
from fool_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(hermes_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic FOOL_HOME."""
...
def test_nous_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "fool_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic FOOL_HOME."""
    auth_path = hermes_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from fool_cli.proxy.server import create_app  # noqa: E402
