...
Runs a real subprocess with a temp FOOL_HOME whose config.yaml contains a
``${ENV_VAR}`` reference, plus a managed-scope overlay dir (FOOL_MANAGED_DIR).
...
production: ``FOOL_MANAGED_DIR`` is set explicitly, which bypasses the
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    out = _run_py(
        ...
        {
            "FOOL_HOME": str(home),
            "FOOL_MANAGED_DIR": str(managed_dir),
...
def test_writeback_roundtrip_byte_identical_when_unchanged(tmp_path):
    ...
    code = textwrap.dedent(
        ...
        p = Path(__import__('os').environ['FOOL_HOME']) / 'config.yaml'
    ...
    out = _run_py(code, {"FOOL_HOME": str(home)}, tmp_path)
