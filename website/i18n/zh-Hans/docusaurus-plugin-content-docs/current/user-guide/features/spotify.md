...
若未设置 `THEFOOL_SPOTIFY_CLIENT_ID`，Hermes 会内联引导你完成应用注册：
...
从 `~/.hermes/auth.json` 中移除 token。若还需清除应用配置，请从 `~/.hermes/.env` 中删除 `THEFOOL_SPOTIFY_CLIENT_ID`（以及 `THEFOOL_SPOTIFY_REDIRECT_URI`，如果你设置了的话），或重新运行向导。
...
**`INVALID_CLIENT: Invalid redirect URI`** — 你的 Spotify 应用设置中的 redirect URI 与 Hermes 使用的不匹配。默认值为 `http://127.0.0.1:43827/spotify/callback`。请将其添加到应用的允许 redirect URI 列表中，或在 `~/.hermes/.env` 中将 `THEFOOL_SPOTIFY_REDIRECT_URI` 设置为你注册的值。
...
| `~/.hermes/.env` | `THEFOOL_SPOTIFY_CLIENT_ID`，可选 `THEFOOL_SPOTIFY_REDIRECT_URI` |
