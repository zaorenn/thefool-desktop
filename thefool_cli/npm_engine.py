...
lives inside its **own** managed Node tree (``$THEFOOL_HOME/node``), installing
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    ...
    ``$THEFOOL_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$THEFOOL_HOME/node/etc/npmrc`` so that global
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    ...
    Installs the managed tree under ``$THEFOOL_HOME/node`` (reusing a healthy
