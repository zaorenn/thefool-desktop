...
# HERMES_*-named config that lacks a secret substring (e.g. FOOL_BASE_URL,
# FOOL_KANBAN_DB, HERMES_*_WEBHOOK).  The child only needs the few
# location/profile vars in _HERMES_CHILD_ALLOWED below; FOOL_RPC_SOCKET /
# FOOL_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# are non-secret runtime-location flags (the same set fool_cli treats as the
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    # config like FOOL_KANBAN_DB / FOOL_BASE_URL), but a sandbox script
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["FOOL_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("FOOL_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
_RPC_DIR = os.environ.get("FOOL_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"FOOL_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"FOOL_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("FOOL_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # selector from FOOL_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  FOOL_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["FOOL_RPC_SOCKET"] = rpc_endpoint
        child_env["FOOL_RPC_TOKEN"] = rpc_token
        ...
        # FOOL_TIMEZONE is an internal Hermes setting and must not leak
        # into child processes.
        _tz_name = os.getenv("FOOL_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("FOOL_TIMEZONE", None)
        ...
        if _uses_hermes_python_environment(_child_python):
            ...
            # per interpreter path) so "import fool_constants suddenly
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``fool tools`` (e.g. web is turned off),
