"""fool memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``fool memory setup`` — if the update-time refresh only reinstalled
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``fool update`` heals the active memory provider
    ...
    # data volume (FOOL_LAZY_INSTALL_TARGET). install_specs handles the
