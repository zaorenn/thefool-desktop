...
# argparse runs (it sets ``THEFOOL_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``thefool_cli.relaunch`` should carry over when
...
def build_top_level_parser():
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via THEFOOL_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to THEFOOL_ACCEPT_HOOKS=1 or "
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/thefool_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also THEFOOL_ACCEPT_HOOKS env var and "
