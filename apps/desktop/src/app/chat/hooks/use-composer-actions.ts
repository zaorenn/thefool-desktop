...
export const FOOL_PATHS_MIME = 'application/x-fool-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal The Fool sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  try {
    const internalRaw = transfer.getData(FOOL_PATHS_MIME)
