...
//
// When someone is in a hands-free "Hey The Fool" voice chat, the natural way to
...
// phrase (optionally addressed to The Fool), so a real turn that merely contains
...
// Optional address prefixes so "fool stop" / "ok stop" / "hey fool, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey fool', 'hey fool,', 'fool', 'fool,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("fool") is not a stop command on its own.
      continue
    }
 ...
 * to The Fool). Returns false for anything that merely contains "stop" as part of
