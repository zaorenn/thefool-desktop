...
Each profile is a fully independent FOOL_HOME directory with its own
...
Usage::
    ...
    fool profile create coder          # fresh profile + bundled skills
    fool profile create coder --clone  # also copy config, .env, SOUL.md, skills
    fool profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    hermes -p coder chat                 # or via flag
    fool profile use coder             # set as sticky default
    fool profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See fool_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `fool profile create --no-skills`.  When present in
...
# `fool skills install` or drop SKILL.md files into the profile's skills/.
...
# Allow-list for ``export_profile("default")``: when FOOL_HOME equals the
...
# artifacts* at the root of FOOL_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``fool_constants``.
...
def _get_profiles_root() -> Path:
    ...
    In Docker/custom deployments where FOOL_HOME points outside
    ``~/.hermes``, profiles live under ``FOOL_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) FOOL_HOME path.

    In standard deployments this is ``~/.hermes``.
    In Docker/custom deployments where FOOL_HOME is outside ``~/.hermes``
    (e.g. ``/opt/data``), returns FOOL_HOME directly.
    """
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its FOOL_HOME directory."""
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        # not be migrated. The next `fool doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    the profile name this is trivial, but a custom alias (``fool profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``fool profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``FOOL_HOME``.
    """
    try:
...
def profiles_to_serve(
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own FOOL_HOME. When
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``fool update`` skips re-seeding this profile's
    ...
    # Write the opt-out marker so seed_profile_skills() and `fool update`'s
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `fool profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches FOOL_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``fool profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "FOOL_HOME": str(profile_dir)},
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``FOOL_HOME``
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # ...or by FOOL_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("FOOL_HOME", "")
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``FOOL_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from fool_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set FOOL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("FOOL_HOME")
    try:
        os.environ["FOOL_HOME"] = str(profile_dir)
        from fool_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["FOOL_HOME"] = old_home
        elif "FOOL_HOME" in os.environ:
            del os.environ["FOOL_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: fool profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from FOOL_HOME.

    Returns ``"default"`` if FOOL_HOME is not set or points to ``~/.hermes``.
    Returns the profile name if FOOL_HOME points into ``~/.hermes/profiles/<name>``.
    Returns ``"custom"`` if FOOL_HOME is set to an unrecognized path.
    """
    from fool_constants import get_hermes_home
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      FOOL_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Hermes file the
      user may have lying alongside FOOL_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``fool sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``FOOL_REDACT_SECRETS`` — share archives
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: fool profile import <archive> --name <name>"
        )
    ...
    if canon == "default":
        raise ValueError(
            ...
            "Specify a different name: fool profile import <archive> --name <name>"
        )
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    validate_profile_name(canon)
    env_home = os.environ.get("FOOL_HOME", "").strip()
    if env_home:
