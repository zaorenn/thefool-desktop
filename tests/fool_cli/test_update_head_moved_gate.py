...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the fool_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns fool_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/fool_cli/test_cmd_update.py).
    """
    ...
    import fool_cli.gateway as hermes_gateway
