...
ORIGINAL_HOME = os.environ.get("FOOL_HOME")
ORIGINAL_AUTH = Path.home() / ".fool" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    real_env_file = Path.home() / ".fool" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".fool"))
...
def reset_module_state():
    """Drop cached modules so the new FOOL_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "fool_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original FOOL_HOME
    if ORIGINAL_HOME is not None:
        os.environ["FOOL_HOME"] = ORIGINAL_HOME
    else:
