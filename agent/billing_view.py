...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    ...
    Dev override: ``THEFOOL_DEV_BILLING_FIXTURE`` short-circuits to a fixture so the
    card-on-file / admin / scope states are testable offline (mirrors
    ``THEFOOL_DEV_CREDITS_FIXTURE`` for the usage model).
    """
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """Map ``THEFOOL_DEV_BILLING_FIXTURE`` to a :class:`BillingState` for offline UX.
    ...
    Mirrors ``THEFOOL_DEV_CREDITS_FIXTURE``; the usage *bar* still comes from
    ``THEFOOL_DEV_CREDITS_FIXTURE`` (set both to pair a bar with a billing state).
    """
    name = (os.getenv("THEFOOL_DEV_BILLING_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return BillingState(logged_in=False, error=f"unknown THEFOOL_DEV_BILLING_FIXTURE: {name}")
