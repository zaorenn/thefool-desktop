...
Helpers that stay in ``fool_cli.main`` (``get_hermes_home``,
...
``fool_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def _prune_never_active_keyed(db, args):
    ...
    """
    from fool_cli.session_filters import format_epoch, parse_duration_seconds
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "recover":
        ...
        from fool_cli.session_recovery import (
    ...
    if action == "import":
        from fool_cli.foreign_sessions import run_sessions_import
    ...
    if action == "list":
        from fool_state import workspace_key as _ws_key
        ...
        from fool_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from fool_cli.session_export_html import (
        ...
        # Markdown / QMD export
        from fool_cli.session_export_md import (
