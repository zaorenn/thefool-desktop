...
- ``fool_state.count_db_holders``: /proc-based best-effort probe for how
  many processes hold the DB file open (Linux only; None elsewhere/on error).
- ``fool_cli.doctor._render_state_db_stats``: formatting/threshold helper
...
from fool_state import SessionDB, collect_state_db_stats, count_db_holders
...
def test_render_healthy_stats_no_warnings():
    from fool_cli.doctor import _render_state_db_stats
...
def test_render_large_db_with_pending_rebuild_suggests_optimize():
    from fool_cli.doctor import STATE_DB_SIZE_WARN_BYTES, _render_state_db_stats
