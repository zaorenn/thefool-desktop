"""Tests for the Phase 4 s6 hooks in thefool_cli.profiles.
...
tests/thefool_cli/test_profiles.py; here we only exercise the new
...
def test_register_noop_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # reached on host.
    monkeypatch.setattr(
        "thefool_cli.service_manager.get_service_manager",
        lambda: _HostManager(),
    )
...
def test_register_passes_start_now_false(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mgr = _S6Manager()
    monkeypatch.setattr(
        "thefool_cli.service_manager.get_service_manager", lambda: mgr,
    )
...
def test_register_silent_when_detect_throws(
    ...
    monkeypatch.setattr(
        "thefool_cli.service_manager.detect_service_manager", _broken_detect,
    )
