...
child with the selected profile's `THEFOOL_HOME`, so the conversation runs
...
ExecStart=/path/to/venv/bin/python -m thefool_cli.main dashboard \
...
```bash
THEFOOL_DASHBOARD_BASIC_AUTH_USERNAME=admin
THEFOOL_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
THEFOOL_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
`THEFOOL_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **CLI — `hermes dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `hermes setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `THEFOOL_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hermes/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  hermes dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes THEFOOL_DASHBOARD_OAUTH_CLIENT_ID to ~/.hermes/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `THEFOOL_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `THEFOOL_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `hermes dashboard register` |
...
Bundled providers reported these issues:
  • nous: THEFOOL_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
**1. Log in and register the dashboard.** `hermes dashboard register` uses your existing Nous login to provision an OAuth client and writes `THEFOOL_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hermes/.env` for you:
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://hermes.example.com/auth/callback` and set `THEFOOL_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `THEFOOL_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `THEFOOL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `THEFOOL_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `THEFOOL_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `THEFOOL_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
cat >> ~/.hermes/.env <<EOF
THEFOOL_DASHBOARD_BASIC_AUTH_USERNAME=admin
THEFOOL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
THEFOOL_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `THEFOOL_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `THEFOOL_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `THEFOOL_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
```bash
export THEFOOL_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/hermes"
export THEFOOL_DASHBOARD_OIDC_CLIENT_ID="hermes-dashboard"
export THEFOOL_DASHBOARD_PUBLIC_URL="http://localhost:9119"
hermes dashboard --host 0.0.0.0 --port 9119 --no-open
```

`THEFOOL_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `THEFOOL_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `THEFOOL_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `THEFOOL_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$THEFOOL_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
from thefool_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `THEFOOL_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
THEFOOL_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  hermes dashboard --host 0.0.0.0
...
Prefer no plaintext at rest? Use `THEFOOL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.
...
The session refreshes automatically and survives restarts when `THEFOOL_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `THEFOOL_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `THEFOOL_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `THEFOOL_DASHBOARD_BASIC_AUTH_USERNAME` / `THEFOOL_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `THEFOOL_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `thefool_cli/web_dist/` which the FastAPI server serves as a static SPA.
