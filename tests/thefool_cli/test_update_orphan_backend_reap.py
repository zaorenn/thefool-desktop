...
(``python.exe -m thefool_cli.main serve``) survives the teardown race. The
...
def _holders(pid=200, cmdline="python.exe -m thefool_cli.main serve"):
...
def test_non_backend_holder_keeps_refusal():
    repl = _proc(300, ["python.exe", "-m", "thefool_cli.main", "chat"], ppid=999)
    fake = _fake_psutil({300: repl})
    with patch.dict(sys.modules, {"psutil": fake}):
        holders = _holders(pid=300, cmdline="python.exe -m thefool_cli.main chat")
