"""Tests for Qwen OAuth provider authentication (thefool_cli/auth.py).
...
def qwen_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "thefool_cli.auth._qwen_cli_auth_path", lambda: creds_path
    )
    return tmp_path
...
def test_get_qwen_auth_status_refreshes_expired_token(qwen_env):
    ...
    with patch(
        "thefool_cli.auth._refresh_qwen_cli_tokens", return_value=refreshed
    ) as mock_refresh:
        status = get_qwen_auth_status()
...
def test_model_flow_qwen_oauth_stale_token_shows_reauth_guidance(qwen_env, monkeypatch, capsys):
    from thefool_cli.main import _model_flow_qwen_oauth
    ...
    monkeypatch.setattr(
        "thefool_cli.auth._refresh_qwen_cli_tokens",
