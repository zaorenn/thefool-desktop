...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `fool_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("fool_cli.main")

    def fake_main():
        called.append("fool_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "fool_cli.main", fake_main_module)
