...
def _root() -> Path:
    ...
    # helper's token, including custom FOOL_HOME layouts.
...
def process_state(pid: int, creation_time_ns: int, hermes_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    launches_module = (
        argv[1:3] == ["-m", "fool_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "fool_cli.main" in argv[2])
    )
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves fool_cli's own location and prepends its parent, because the
    ...
    # the fool_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the fool_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not os.path.isfile(python_entry):
        raise ValueError("The Fool Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('fool_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_hermes(hermes_path: str) -> dict[str, Any]:
    ...
    if not os.path.isabs(hermes_path) or not os.path.isfile(path):
        raise ValueError("The Fool path is not an executable file")
