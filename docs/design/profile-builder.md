...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — FOOL_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with FOOL_HOME override |
...
### Seam #1 — hub-skill install cannot use the FOOL_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = FOOL_HOME / "skills"` at **module
...
runs `python -m fool_cli.main <subcommand>`, and `_apply_profile_override()`
...
A fresh subprocess re-imports `skills_hub` with the profile's FOOL_HOME bound
...
2. write model (synchronous, FOOL_HOME override)
3. write selected MCP servers (synchronous, FOOL_HOME override)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, FOOL_HOME override
...
page afterward. Mirror `_write_profile_model`'s FOOL_HOME-override helper for
...
- Backend E2E with isolated FOOL_HOME: POST a full create body
