...
# `export FOOL_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in fool_cli/main.py, gateway/run.py, and
# cli.py) or `FOOL_REDACT_SECRETS=false` in ~/.hermes/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("FOOL_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
def mask_secret(
    ...
    *,
    ...
    ``fool config``, ``fool status``, ``fool dump``, and anywhere
...
# (``security.redact_secrets: false`` / FOOL_REDACT_SECRETS) applies to
