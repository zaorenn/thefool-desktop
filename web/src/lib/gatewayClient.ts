...
import { FOOL_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
export class GatewayClient extends JsonRpcGatewayClient {
  ...
  async connect(token?: string): Promise<void> {
    ...
    if (!authParam[1]) {
      throw new Error(
        "Session token not available — page must be served by the Fool dashboard server",
      );
    }
