...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``fool config set`` (which rewrites
    ...
    ``config.yaml`` in place: fool never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``fool logs``
...
#   on every plugin install / ``fool update``.
...
# * ``FOOL_HOME`` / ``FOOL_PROFILE`` / ``FOOL_CONFIG`` /
#   ``FOOL_ENV`` — Hermes runtime location flags. Writing these into
...
# integration credentials follow that prefix (FOOL_LANGFUSE_PUBLIC_KEY,
# FOOL_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # NOT a HERMES_* blanket: integration credentials (FOOL_GEMINI_*,
    # FOOL_LANGFUSE_*, FOOL_SPOTIFY_*, ...) ARE allowed.
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # FOOL_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    "FOOL_LANGFUSE_SAMPLE_RATE",
    "FOOL_LANGFUSE_MAX_CHARS",
    "FOOL_LANGFUSE_CAPTURE",
    "FOOL_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    ...
    "FOOL_ACP_AUTH_METHOD",
    "FOOL_ACP_AUTO_APPROVE",
    "FOOL_COPILOT_ACP_COMMAND",
    "FOOL_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from fool_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set FOOL_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("FOOL_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    ...
    Two signals: the FOOL_MANAGED env var (set by the systemd service),
    or a .managed marker file in FOOL_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``fool_cli/`` — i.e. the git checkout for source
    installs, ``/opt/hermes`` inside the published image. It is a property of
    the running interpreter, NOT of ``$FOOL_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    ``$FOOL_HOME`` is a shared DATA directory — the Docker docs deliberately
    ...
    containerised gateway and a host install share one ``$FOOL_HOME``, a
    ...
    ``docker`` and ``fool update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $FOOL_HOME.
    try:
    ...
    #    host install whose shared $FOOL_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``fool update``.
    try:
        method = (
    ...
    # detect Nix installs that don't set FOOL_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``fool_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$FOOL_HOME``, so that two installs sharing one data directory
...
# Long-form text for ``fool update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# fool_cli/main.py; lives here so the wording stays consistent and we
...
✗ ``fool update`` doesn't apply inside the Docker container.
...
Notes:
  ...
  • Your config and session history live under ``$FOOL_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``fool update`` inside Docker.
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from FOOL_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or FOOL_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("FOOL_DEV") == "1":
        return None
...
# Re-export from fool_constants — canonical definition lives there.
from fool_constants import get_hermes_home, get_process_hermes_home  # noqa: F811,E402
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the FOOL_UID / FOOL_GID env vars set by Docker deployments.
    ...
    ownership. The entrypoint chowns the top-level FOOL_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("FOOL_UID", "").strip()
    gid_str = os.environ.get("FOOL_GID", "").strip()
    try:
...
def _chown_to_hermes_uid(path) -> None:
    """Chown ``path`` to ``FOOL_UID:FOOL_GID`` if those env vars are set.
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the fool group can
    share state with the gateway service.

    The mode can be overridden via the FOOL_HOME_MODE environment variable
    (e.g. FOOL_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse FOOL_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``FOOL_UID``/``FOOL_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("FOOL_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("FOOL_CONTAINER") or os.environ.get("FOOL_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set FOOL_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into FOOL_HOME, upgrading legacy empty templates.
...
def ensure_hermes_home():
    """Ensure ~/.fool directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``FOOL_HOME`` we only re-run
    ...
    if key in _HERMES_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``fool profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old FOOL_HOME would resurrect an empty skeleton
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"FOOL_HOME {home} does not exist. "
...
from fool_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `fool config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # should never break `fool update`.  Skill-config prompting is a
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``fool_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (fool_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (fool_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (fool_cli/setup.py)
    "known_plugin_toolsets", # written/read by fool_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # scalars are bridged into os.environ (gateway/run.py, fool send) so
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        # The per-version steps live in fool_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from fool_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from fool_cli.toolset_validation import validate_platform_toolsets
    ...
    # `fool update`, but only the version bump is persisted.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``fool config set``
    ...
    backends) — while display paths (``fool status``/``dump``) read ``name``
...
# Back-compat alias — canonical set lives in fool_cli.personality.
from fool_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`fool_cli.personality` — the single owner of
...
def render_personality_prompt(value: Any) -> str:
    ...
    from fool_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``FOOL_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`fool_cli.personality` (single owner).
    """
    from fool_cli.personality import resolve_ephemeral_system_prompt
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``FOOL_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`fool tools`, `fool setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `fool tools`
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``FOOL_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"FOOL_CUSTOM_{slug}_API_KEY" if slug else "FOOL_CUSTOM_API_KEY"
...
def show_config():
    ...
    # Warn on stale FOOL_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("FOOL_MAX_ITERATIONS")
    ...
    try:
        from fool_cli.personality import active_personality_name
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``FOOL_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("FOOL_MODEL", ""))
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``fool config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``fool config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``fool config set --force``.
    """
    if is_managed():
...
# ``_inject_profile_env_vars`` surfaces them in ``fool config`` UI so users
