import { forceRedraw, type MouseTrackingMode } from '@fool/ink'
...
export const coreCommands: SlashCommand[] = [
...

  {
    ...
    run: (_arg, ctx) => {
      ...
      // Exit code 42 signals the Python wrapper to exec `fool update`.
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('clipboard copy failed — try FOOL_TUI_FORCE_OSC52=1 to force the escape sequence')
        }
      }
