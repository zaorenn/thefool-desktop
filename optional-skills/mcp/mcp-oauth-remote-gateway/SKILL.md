...
empty or missing tokens directory (`$FOOL_HOME/mcp-tokens/`), and a reload that
...
```bash
env | grep -iE "FOOL_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`FOOL_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$FOOL_HOME/.env`. Those
...
### 2. Find FOOL_HOME and the config path

```bash
FOOL_HOME=$(python3 -c 'from fool_constants import get_hermes_home; print(get_hermes_home())')
echo "config: $FOOL_HOME/config.yaml"
echo "tokens: $FOOL_HOME/mcp-tokens/"
```
