...
def forced_files_client(monkeypatch, tmp_path):
    root = tmp_path / "data"
    monkeypatch.setenv("THEFOOL_DASHBOARD_FILES_ROOT", str(root))
...
def local_files_client(monkeypatch, tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.delenv("THEFOOL_DASHBOARD_FILES_ROOT", raising=False)
...
def test_other_credential_store_basenames_blocked(forced_files_client):
    ...
    point the managed root at THEFOOL_HOME itself (#57505), which contains
