...
from fool_cli import resource_limits
...
def test_fresh_process_import_without_posix_resource_is_a_safe_noop():
    code = textwrap.dedent(
        ...
        spec = importlib.util.spec_from_file_location(
            "fool_cli._resource_limits_without_posix_resource",
            module_path,
        )
...
def test_serve_startup_applies_limit_before_web_server(monkeypatch):
    ...
    import fool_cli.plugins
    import fool_cli.web_server

    # cmd_dashboard(headless_backend=True) exports FOOL_SERVE_HEADLESS=1 into
    ...
    # 404 path.
    monkeypatch.setenv("FOOL_SERVE_HEADLESS", "0")
    ...
    monkeypatch.setattr(fool_cli.plugins, "discover_plugins", lambda: None)
    monkeypatch.setattr(
        fool_cli.web_server,
        "start_server",
...
def test_named_profile_reroute_defers_limit_to_final_process(monkeypatch, tmp_path):
    ...
    assert calls == []
    assert exec_call["argv"][1:5] == ["-m", "fool_cli.main", "-p", "default"]
    assert exec_call["env"]["FOOL_HOME"] == str(tmp_path)
