...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    ...
    Dev override: ``FOOL_DEV_BILLING_FIXTURE`` short-circuits to a fixture so the
    card-on-file / admin / scope states are testable offline (mirrors
    ``FOOL_DEV_CREDITS_FIXTURE`` for the usage model).
    """
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """Map ``FOOL_DEV_BILLING_FIXTURE`` to a :class:`BillingState` for offline UX.
    ...
    Mirrors ``FOOL_DEV_CREDITS_FIXTURE``; the usage *bar* still comes from
    ``FOOL_DEV_CREDITS_FIXTURE`` (set both to pair a bar with a billing state).
    """
    name = (os.getenv("FOOL_DEV_BILLING_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return BillingState(logged_in=False, error=f"unknown FOOL_DEV_BILLING_FIXTURE: {name}")
