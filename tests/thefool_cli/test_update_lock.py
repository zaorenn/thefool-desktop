"""Cross-process update mutual exclusion (``thefool_cli.update_lock``).
...
def test_marker_path_follows_process_hermes_home(tmp_path, monkeypatch):
    ...
    All three resolve the *process* THEFOOL_HOME; a profile-scoped path would
...
class TestAncestryHandoff:
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("thefool_cli.update_lock._pid_alive", lambda pid: True)
