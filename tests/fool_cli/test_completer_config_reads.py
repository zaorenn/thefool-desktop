...
import fool_cli.commands as commands_mod
...
class TestToolsCompletionsReadonlyConfig:
    def test_uses_readonly_loader(self):
        ...
        with patch("fool_cli.config.load_config", counting_deepcopy), \
             patch("fool_cli.config.load_config_readonly", counting_readonly), \
             patch("fool_cli.plugins.get_portable_mcp_server_names_nowait", lambda: set()), \
             patch("fool_cli.tools_config._get_plugin_toolset_keys", lambda: set()), \
             patch("fool_cli.tools_config._homeassistant_credentials_present", lambda: False), \
             patch("fool_cli.tools_config._xai_credentials_present", lambda: False):
...
class TestPersonalityCompletionsMemo:
    ...
    def test_mtime_change_reparses(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(commands_mod, "_personalities_memo", None)
        with patch("cli.load_cli_config", counting_load_cli_config), \
             patch("fool_cli.config.get_config_path", fake_config_path):
