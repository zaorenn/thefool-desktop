...
THEFOOL_OVERLAYS: Dict[str, HermesOverlay] = {
    "moa": HermesOverlay(
    ...
    "qwen-oauth": HermesOverlay(
        ...
        base_url_env_var="THEFOOL_QWEN_BASE_URL",
    ),
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    overlay = THEFOOL_OVERLAYS.get(canonical)
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # Direct provider checks for providers not in THEFOOL_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    if canonical != raw:
        try:
            from thefool_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
