"""Tests for thefool_cli/loops.py — /loop recurring in-session wakeups."""
...
def hermes_home(tmp_path, monkeypatch):
    """Isolated THEFOOL_HOME so SessionDB.state_meta writes don't clobber the real one."""
...
class TestParseIntervalToken:
    def test_minutes(self):
        from thefool_cli.loops import parse_interval_token
...
class TestParseLoopArgs:
    def test_fixed_interval(self):
        from thefool_cli.loops import parse_loop_args
...
class TestFormatInterval:
    def test_render(self):
        from thefool_cli.loops import format_interval
...
class TestResponseSignalsComplete:
    def test_marker_on_own_line(self):
        from thefool_cli.loops import response_signals_complete
...
class TestLoopStateSerde:
    def test_round_trip(self):
        from thefool_cli.loops import LoopState
...
class TestPersistence:
    def test_save_load_clear(self, hermes_home):
        from thefool_cli.loops import LoopManager, load_loop
    ...
    def test_list_active_loops(self, hermes_home):
        from thefool_cli.loops import LoopManager, list_active_loops
    ...
    def test_migrate_to_session(self, hermes_home):
        from thefool_cli.loops import LoopManager, load_loop, migrate_loop_to_session
    ...
    def test_migrate_no_source(self, hermes_home):
        from thefool_cli.loops import migrate_loop_to_session
...
class TestTickLifecycle:
    ...
    def test_until_judge_done_stops(self, hermes_home):
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("thefool_cli.goals.judge_goal", return_value=("done", "suite green", False, None, False)):
    ...
    def test_until_judge_continue_loops(self, hermes_home):
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("thefool_cli.goals.judge_goal", return_value=("continue", "3 failures", False, None, False)):
    ...
    def test_until_judge_error_fails_open(self, hermes_home):
        ...
        state.next_due_at = time.time() - 1
        mgr.fire_tick()
        with patch("thefool_cli.goals.judge_goal", side_effect=RuntimeError("api down")):
...
class TestControls:
    def test_min_interval_enforced(self, hermes_home):
        from thefool_cli.loops import LoopManager, min_interval_seconds
...
class TestDispatchLoopCommand:
    ...
    def test_route_stored(self, hermes_home):
        from thefool_cli.loops import LoopManager, dispatch_loop_command, load_loop
