...
selectable via ``fool tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `fool tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `fool tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`fool setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `fool tools` "
        "→ Image Generation (run `fool plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `fool tools` → Image Generation."
                ),
...
# model/provider via `fool tools` or `/skills`.
