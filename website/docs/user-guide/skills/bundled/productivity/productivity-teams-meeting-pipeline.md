...
Before using the pipeline, verify these are set in `${FOOL_HOME:-~/.hermes}/.env`:
...
3. **Set up automated renewal immediately** via `fool cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
