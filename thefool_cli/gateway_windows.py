...
def _preserve_hermes_home_path(path: str | Path) -> str:
    """Render Hermes-owned paths under the configured THEFOOL_HOME spelling.
...
def _current_profile_cli_args() -> list[str]:
    ...
    from thefool_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "thefool_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("THEFOOL_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("THEFOOL_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("THEFOOL_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["THEFOOL_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["THEFOOL_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["THEFOOL_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("THEFOOL_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("THEFOOL_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("THEFOOL_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during thefool_cli boot.
    from thefool_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    (or ``<THEFOOL_HOME>/gateway-service/<task_name>.cmd`` so per-profile
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``THEFOOL_HOME`` whenever it
    ...
    source checkout only if ``THEFOOL_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports THEFOOL_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m thefool_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "THEFOOL_HOME={hermes_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "THEFOOL_GATEWAY_DETACHED=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports thefool_constants-based logic during startup.
    ...
    prog_args = [python_exe_path, "-m", "thefool_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('THEFOOL_HOME')}) = {_quote_vbs_string(hermes_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('THEFOOL_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "thefool_cli.main"]
    if profile_arg:
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (THEFOOL_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m thefool_cli.main
    ...
    from thefool_cli.config import get_hermes_home
    from thefool_cli.gateway import PROJECT_ROOT
    ...
    if hermes_home:
        env_overlay["THEFOOL_HOME"] = hermes_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m thefool_cli.main gateway run`` directly — NOT
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("THEFOOL_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("THEFOOL_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from thefool_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from thefool_cli.gateway import find_gateway_pids, _profile_arg
