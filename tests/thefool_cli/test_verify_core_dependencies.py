...
def temp_pyproject(tmp_path, monkeypatch):
    """Point thefool_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("thefool_cli.main._resolve_install_target_python", return_value=py), \
             patch("thefool_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("thefool_cli.main._run_install_with_heartbeat"):

            from thefool_cli.main import _verify_core_dependencies_installed
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("thefool_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("thefool_cli.main._run_install_with_heartbeat") as mock_install:
            from thefool_cli.main import _verify_core_dependencies_installed
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        with patch("thefool_cli.main._is_windows", return_value=True):
            from thefool_cli.main import _resolve_install_target_python
