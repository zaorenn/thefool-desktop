...
Setting `addToSystemPackages = true` does two things: puts the `hermes` CLI on your system PATH **and** sets `FOOL_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `hermes` in your shell creates a separate `~/.hermes/` directory.
:::
...
This bypasses `settings` entirely — no merge, no generation. The file is copied as-is to `$FOOL_HOME/config.yaml` on each activation.
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$FOOL_HOME/.env` at activation time (`nixos-rebuild switch`). Hermes reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
The agent identity file is separate: Hermes loads its primary `SOUL.md` from `$FOOL_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.hermes/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
:::tip
Environment variables in `env` values are resolved from `$FOOL_HOME/.env` at runtime. Use `environmentFiles` to inject secrets — never put tokens directly in Nix config.
:::
...
Tokens are stored in `$FOOL_HOME/mcp-tokens/<server-name>.json` and persist across restarts and rebuilds.
...
Plugins are symlinked into `$FOOL_HOME/plugins/` at activation time. Hermes discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
nix build .#checks.x86_64-linux.managed-guard       # FOOL_MANAGED blocks mutation
...
| `stateDir` | `str` | `"/var/lib/hermes"` | State directory (`FOOL_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `hermes` CLI to system PATH and set `FOOL_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `environmentFiles` | `listOf str` | `[]` | Paths to env files with secrets. Merged into `$FOOL_HOME/.env` at activation time |
