...
Hermes owns its own uv binary at ``$THEFOOL_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$THEFOOL_HOME/bin`` so the installer writes directly there — no PATH
...
from thefool_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime
...
def managed_uv_path() -> Path:
    ...
    ``$THEFOOL_HOME/bin/uv`` on POSIX, ``$THEFOOL_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    the call site from the *old*, already-imported ``thefool_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    and, offline, an unbounded hang risk. A stamp file under THEFOOL_HOME
...
def _reload_hermes_constants():
    """Re-execute ``thefool_constants`` from disk and return the fresh module.

    ``hermes update`` imports ``thefool_constants`` from the OLD checkout,
    ...
    return importlib.reload(thefool_constants)
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("thefool_cli.main")
...
def _install_uv(target: Path) -> None:
    ...
    ``$THEFOOL_HOME/bin/`` instead of ``~/.local/bin/``.
    """
