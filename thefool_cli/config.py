...
# * ``THEFOOL_HOME`` / ``THEFOOL_PROFILE`` / ``THEFOOL_CONFIG`` /
#   ``THEFOOL_ENV`` — Hermes runtime location flags. Writing these into
...
# integration credentials follow that prefix (THEFOOL_LANGFUSE_PUBLIC_KEY,
# THEFOOL_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # NOT a HERMES_* blanket: integration credentials (THEFOOL_GEMINI_*,
    # THEFOOL_LANGFUSE_*, THEFOOL_SPOTIFY_*, ...) ARE allowed.
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(THEFOOL_HOME, THEFOOL_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # THEFOOL_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # THEFOOL_TOOL_PROGRESS variant is fully unsupported since the v12 config
    ...
    "THEFOOL_LANGFUSE_ENV",
    "THEFOOL_LANGFUSE_RELEASE",
    "THEFOOL_LANGFUSE_SAMPLE_RATE",
    "THEFOOL_LANGFUSE_MAX_CHARS",
    "THEFOOL_LANGFUSE_CAPTURE",
    "THEFOOL_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "THEFOOL_ACP_AUTH_METHOD",
    "THEFOOL_ACP_AUTO_APPROVE",
    "THEFOOL_COPILOT_ACP_COMMAND",
    "THEFOOL_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from thefool_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set THEFOOL_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("THEFOOL_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    ...
    Two signals: the THEFOOL_MANAGED env var (set by the systemd service),
    or a .managed marker file in THEFOOL_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``thefool_cli/`` — i.e. the git checkout for source
    installs, ``/opt/hermes`` inside the published image. It is a property of
    the running interpreter, NOT of ``$THEFOOL_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    2. Legacy home-scoped stamp ``$THEFOOL_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. THEFOOL_MANAGED env / .managed marker (NixOS managed mode)
    ...
    ``$THEFOOL_HOME`` is a shared DATA directory — the Docker docs deliberately
    ...
    containerised gateway and a host install share one ``$THEFOOL_HOME``, a
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $THEFOOL_HOME.
    try:
    ...
    #    host install whose shared $THEFOOL_HOME was stamped by a co-located
    ...
    # detect Nix installs that don't set THEFOOL_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``thefool_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$THEFOOL_HOME``, so that two installs sharing one data directory
...
# thefool_cli/main.py; lives here so the wording stays consistent and we
...
Notes:
  ...
  • Your config and session history live under ``$THEFOOL_HOME`` (``/opt/data``
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from THEFOOL_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or THEFOOL_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("THEFOOL_DEV") == "1":
        return None
...
# Re-export from thefool_constants — canonical definition lives there.
from thefool_constants import get_hermes_home, get_process_hermes_home  # noqa: F811,E402
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the THEFOOL_UID / THEFOOL_GID env vars set by Docker deployments.
    ...
    ownership. The entrypoint chowns the top-level THEFOOL_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("THEFOOL_UID", "").strip()
    gid_str = os.environ.get("THEFOOL_GID", "").strip()
    try:
...
def _chown_to_hermes_uid(path) -> None:
    """Chown ``path`` to ``THEFOOL_UID:THEFOOL_GID`` if those env vars are set.
...
def _secure_dir(path):
    ...
    The mode can be overridden via the THEFOOL_HOME_MODE environment variable
    (e.g. THEFOOL_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse THEFOOL_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``THEFOOL_UID``/``THEFOOL_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("THEFOOL_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("THEFOOL_CONTAINER") or os.environ.get("THEFOOL_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set THEFOOL_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into THEFOOL_HOME, upgrading legacy empty templates.
...
def ensure_hermes_home():
    ...
    After the first successful pass for a given ``THEFOOL_HOME`` we only re-run
    ...
    # silently mkdir-ing the old THEFOOL_HOME would resurrect an empty skeleton
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"THEFOOL_HOME {home} does not exist. "
...
from thefool_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``thefool_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (thefool_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (thefool_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (thefool_cli/setup.py)
    "known_plugin_toolsets", # written/read by thefool_cli/tools_config.py toolset-save flow
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if floor_refused:
        ...
        # The per-version steps live in thefool_cli.config_migrations as a
    ...
    try:
        ...
        from thefool_cli.toolset_validation import validate_platform_toolsets
...
# Back-compat alias — canonical set lives in thefool_cli.personality.
from thefool_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`thefool_cli.personality` — the single owner of
...
def render_personality_prompt(value: Any) -> str:
    ...
    from thefool_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``THEFOOL_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`thefool_cli.personality` (single owner).
    """
    from thefool_cli.personality import resolve_ephemeral_system_prompt
...
def load_config() -> Dict[str, Any]:
    ...
    (which change ``THEFOOL_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``THEFOOL_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"THEFOOL_CUSTOM_{slug}_API_KEY" if slug else "THEFOOL_CUSTOM_API_KEY"
...
def show_config():
    ...
    # Warn on stale THEFOOL_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("THEFOOL_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale THEFOOL_MAX_ITERATIONS={_env_ghost} "
    ...
    try:
        from thefool_cli.personality import active_personality_name
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``THEFOOL_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("THEFOOL_MODEL", ""))
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (thefool_cli/plugins_cmd.py, thefool_cli/plugin_index.py). Absent from
