"""Unit tests for fool_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/fool_cli/test_pty_bridge.py.  Drives
...
``fool_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from fool_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from fool_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from fool_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp
    ...
    def test_non_numeric_falls_back_to_min(self):
        from fool_cli.win_pty_bridge import _MAX_COLS, _clamp
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('FOOL_PTY_TEST=' + os.environ.get('FOOL_PTY_TEST',''))",
            ],
            env={**os.environ, "FOOL_PTY_TEST": "pty-env-works"},
        )
        try:
