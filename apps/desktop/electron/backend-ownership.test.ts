...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'fool serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/fool serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m fool_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/fool --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\The Fool Runtime\\fool.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('fool chat --query serve'), false)
