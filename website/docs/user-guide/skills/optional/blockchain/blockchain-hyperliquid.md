...
| | |
|---|---|
| Source | Optional — install with `fool skills install official/blockchain/hyperliquid` |
...
The script reads `${FOOL_HOME:-~/.hermes}/.env` for two optional defaults:
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${FOOL_HOME:-~/.hermes}/.env`.
