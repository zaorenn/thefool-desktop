...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `fool_cli/models.py` may change between releases) | `hermes model` → MiniMax (OAuth) |
