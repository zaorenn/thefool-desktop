...
from fool_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
FOOL_HOME = get_hermes_home()
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.fool or ~/.hermes/profiles/coder)
...
from fool_cli.models import _HERMES_USER_AGENT
from fool_cli.vercel_auth import describe_vercel_auth
...
from fool_constants import is_termux as _is_termux
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    ...
    from fool_cli.backup import _QUICK_STATE_FILES
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``fool_cli.sqlite_safe_read``).
    ...
    """
    from fool_cli.sqlite_safe_read import (
        has_live_connection,
...
def _format_db_size(db_path: Path) -> str:
    ...
    from fool_cli.backup import _format_size
...
def _report_database_journal_modes(
    ...
    from fool_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = hermes_home if hermes_home is not None else FOOL_HOME
    try:
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("FOOL_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from fool_cli.auth import get_minimax_oauth_auth_status
            return bool((get_minimax_oauth_auth_status() or {}).get("logged_in"))
        except Exception:
            return False
    if normalized == "xai":
        try:
            from fool_cli.auth import get_xai_oauth_auth_status
...
from fool_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or legacy_trigram:
            detail += (
                "; run 'fool sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    # FOOL_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor removed its only consumer (the v3→4 migration) — it is silently
    # ignored. FOOL_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("FOOL_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("FOOL_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches fool_cli.__version__.
    ...
    """
    try:
        from fool_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != fool_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'fool update', or set "
            "fool_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-fool and dashboard static services are up
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `fool profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `fool doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from fool_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the FOOL_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("FOOL_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via FOOL_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    os.environ.setdefault("FOOL_INTERACTIVE", "1")

    # Handle `fool doctor --ack <id>` as a fast path. Persist the ack and
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`fool doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from fool_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
    ...
    # Detect drift between pyproject.toml and fool_cli/__init__.py versions
    ...
    if env_path.exists():
        ...
        # matches fool_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'fool setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'fool setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'fool setup' to create one")
                issues.append("Run 'fool setup' to create .env")
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from fool_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from fool_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'fool config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from fool_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hermes/.env or run 'fool setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'fool setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'fool config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from fool_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from fool_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'fool setup' to migrate config")
                else:
                    issues.append("Run 'fool doctor --fix' or 'fool setup' to migrate config")
            else:
                ...
                if should_fix:
                    ...
                    for k in stale_root_keys:
                        if not model_section.get(k):
                            ...
                            raw_config.pop(k)
                    from fool_cli.config import atomic_config_write
                    atomic_config_write(config_path, raw_config)
                    check_ok("Migrated stale root-level keys into model section")
                    fixed_count += 1
                else:
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'fool doctor --fix'")
        except Exception:
            pass

        # Detect stale FOOL_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives FOOL_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from fool_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("FOOL_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"FOOL_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `fool setup` run)",
                )
                if should_fix:
                    if remove_env_value("FOOL_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale FOOL_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove FOOL_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the FOOL_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale FOOL_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'fool doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from fool_cli.config import load_env as _load_env_depr
            from fool_cli.config import read_user_config_raw as _read_raw_depr
    ...
    if state_db_path.exists():
        try:
            ...
            from fool_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'fool doctor --fix' "
                        "(or 'fool sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from fool_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'fool doctor --fix' "
                        "(or 'fool sessions repair') to recover hidden sessions"
                    )
            else:
        ...
        try:
            from fool_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'fool sessions optimize-storage' "
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'fool doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        if _venv_bin is None:
            check_warn(
                ...
                "(fool not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                check_ok(f"{_cmd_link_display}/fool exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/fool not found",
                    "(fool command may not work outside the venv)"
                )
                if should_fix:
                    ...
                    issues.append(f"Missing {_cmd_link_display}/fool symlink — run 'fool doctor --fix'")
    ...
    try:
        from fool_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `fool update` does, so a session's first browser call
            ...
            # `fool update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `fool doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable FOOL_HOME mirror
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'fool config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``fool doctor``.
    ...
    # never the legitimate source for `fool doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'fool setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = FOOL_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: fool skills list)")
    ...
    try:
        from fool_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = FOOL_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: fool memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: fool memory setup",
                    "No Honcho API key — run 'fool memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run fool memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: fool memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: fool memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from fool_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
    ...
    # `fool doctor --live` (real network calls; bounded + read-only).
    try:
        from fool_cli.doctor_live import maybe_run_live_checks
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'fool doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
