...
def _run_switch(monkeypatch, result, cmd="/model MiniMax-M3 --global"):
    ...
    monkeypatch.setattr("thefool_cli.model_switch.switch_model", lambda **kw: result)
    monkeypatch.setattr(
...
def test_session_only_switch_does_not_touch_config(monkeypatch):
    ...
    monkeypatch.setattr("thefool_cli.model_switch.switch_model", lambda **kw: _make_result())
    monkeypatch.setattr(
        "thefool_cli.inventory.load_picker_context",
        lambda: (_ for _ in ()).throw(RuntimeError("no picker context in test")),
    )
