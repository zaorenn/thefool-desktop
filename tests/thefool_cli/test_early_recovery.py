"""Tests for thefool_cli._early_recovery — the dependency-light bootstrap
repair that runs BEFORE thefool_cli.main's third-party imports (#57828 / #58004).

Covers:
- entry-point lifecycle: a broken core import (dotenv) crashes the import of
  thefool_cli.main WITHOUT early recovery, and imports fine when recovery runs
...
from thefool_cli import _early_recovery as er
...
def _run_lifecycle_subprocess(tmp_path: Path, *, repair: bool) -> subprocess.CompletedProcess:
    ...
    script.write_text(
        textwrap.dedent(
            ...
            import thefool_cli._early_recovery as er
...
def test_broken_dotenv_crashes_main_import_without_repair(tmp_path):
    """Negative control: the shadow really breaks importing thefool_cli.main,
...
def test_early_recovery_module_is_stdlib_only(tmp_path):
    ...
    script.write_text(
        textwrap.dedent(
            ...
            STDLIB = set(sys.stdlib_module_names) | {"thefool_cli"}
            ...
            import thefool_cli._early_recovery  # noqa: F401
...
def test_core_marker_triggers_install_before_any_native_import(
    ...
    import thefool_cli._install_repair  # noqa: F401  (pre-import for patch)
...
def test_core_marker_marks_attempts_and_keeps_marker_on_install_failure(
    ...
    import thefool_cli._install_repair  # noqa: F401
