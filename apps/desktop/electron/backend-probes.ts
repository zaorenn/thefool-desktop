 ...
 * path) but no fool_cli in its site-packages: the resolver hands back
 * a backend the spawn step can't actually run, and the user gets a
 * dead-on-arrival "ModuleNotFoundError: No module named 'fool_cli'"
 ...
 * Honours FOOL_PROBE_TIMEOUT_MS when it parses as a positive integer.
 */
function resolveProbeTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.FOOL_PROBE_TIMEOUT_MS
 ...
 * dies on spawn.
 *
 * The probe intentionally imports fool_cli.config, not just the top-level
