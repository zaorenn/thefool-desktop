 ...
 *   2. PortableGit under %LOCALAPPDATA%\fool\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'fool', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'fool', 'git', 'usr', 'bin', 'bash.exe'))
  }
