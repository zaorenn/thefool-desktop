...
``fool_cli.mcp_startup``), so the scheduler bailed immediately and a slow MCP
...
import fool_cli.mcp_startup as startup
...
def test_entry_in_flight_sees_startup_thread(clean_discovery_globals):
    """Desktop/dashboard surface: discovery thread lives on fool_cli.mcp_startup.
...
def test_startup_module_exposes_in_flight_helpers(clean_discovery_globals):
    """fool_cli.mcp_startup gains the in-flight/join helpers entry delegates to."""
