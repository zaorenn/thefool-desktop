...
The fool gateway (and anything else you want to keep running) is easier to manage with systemd. On modern WSL, enable it once inside your distro:
...
Clone inside WSL. Always, unless you have a specific reason not to. A typical Hermes workflow (`fool chat`, tool calls that `rg`/`ripgrep` the repo, file watchers, background gateway) will be dramatically faster and more reliable against `~/code/myrepo` than `/mnt/c/Users/you/myrepo`.
...
- Using the **OpenAI-compatible API server** (exposed by `fool gateway` when `API_SERVER_ENABLED=true`) from a Windows-side tool. See the [API Server feature page](/user-guide/features/api-server).
...
If you enabled systemd per the setup section above, `fool gateway` and the API server work the way they do on any Linux machine. Use the gateway setup wizard:
...
**Massive slowness on `git status` / `fool chat` in a repo.**
