 ...
 * unwrapWindowsVenvFoolCommand(). Each of the three functions here pins one
 ...
 *   3. resolveVenvFoolCommand() — unwrapWindowsVenvFoolCommand() returned
...
export interface ResolveVenvFoolCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportFoolCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
 ...
 * ONLY after smoke-testing that interpreter with canImportFoolCli(). A
 ...
 */
export function resolveVenvFoolCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvFoolCommandDeps
): {
  label: string
  command: string
  args: string[]
