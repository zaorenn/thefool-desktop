...
def test_dispatch_all_handles_partial_failure(
    ...
    from fool_cli import gateway as gw
    ...
    monkeypatch.setattr(
        "fool_cli.service_manager.detect_service_manager", lambda: "s6",
    )
    monkeypatch.setattr(
        "fool_cli.service_manager.get_service_manager", lambda: rec,
    )
...
def _stub_s6(monkeypatch: pytest.MonkeyPatch, *, on_s6: bool) -> _CallRecorder:
    ...
    rec = _CallRecorder()
    monkeypatch.setattr(
        "fool_cli.service_manager.detect_service_manager",
...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("fool_cli.gateway._profile_suffix", lambda: "")

    def missing_sleep(file: str, args: list[str]) -> None:
        raise FileNotFoundError(2, "No such file or directory", file)

    monkeypatch.setattr("fool_cli.gateway.os.execvp", missing_sleep)
    block_calls: list[bool] = []
    monkeypatch.setattr(
        "fool_cli.gateway._block_until_terminated",
        lambda: block_calls.append(True),
    )
    monkeypatch.delenv("FOOL_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.delenv("FOOL_GATEWAY_NO_SUPERVISE", raising=False)
