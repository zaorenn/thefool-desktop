...
from fool_cli.auth import AuthError, resolve_provider
from fool_cli.colors import Colors, color
from fool_cli.config import get_env_path, get_env_value, get_hermes_home, load_config
...
from fool_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    ...
    print(color("│                 ⚕ Fool Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: fool portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: fool model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: fool auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: fool auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: fool model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from fool_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from fool_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    try:
        from fool_cli.active_sessions import (
