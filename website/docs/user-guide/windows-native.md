...
| `-SkipSetup` | off | Skip the post-install `fool setup` wizard |
| `-FoolHome` | `%LOCALAPPDATA%\hermes` | Override data directory |
...
On first launch (and on demand when a missing tool is detected), Hermes runs a small Python bootstrapper — `fool_cli/dep_ensure.py` — that checks for and lazily installs the non-Python dependencies it needs. On Windows, the relevant ones are:
...
|---|---|---|
| CLI (`fool chat`, `fool setup`, `fool gateway`, …) | ✓ | ✓ |
...
1. `FOOL_GIT_BASH_PATH` environment variable if set.
...
The installer sets `FOOL_GIT_BASH_PATH` explicitly so fresh PowerShell sessions don't have to re-discover. Override it if you want Hermes to use a specific bash — for example, your system Git Bash or a WSL-hosted bash via a symlink.
...
The fix is in `fool_cli/stdio.py::configure_windows_stdio()`, called early in every entry point (`cli.py::main`, `fool_cli/main.py::main`, `gateway/run.py::main`). It:
...
**Opt out:** `FOOL_DISABLE_WINDOWS_UTF8=1` in the environment falls back to the legacy cp1252 stdio path. Useful for bisecting an encoding bug; unlikely to be the right setting in normal operation.
...
`fool gateway install` on Windows uses **Scheduled Tasks** with a Startup-folder fallback — no admin required.
...
1. `schtasks /Create /SC ONLOGON /RL LIMITED /TN FoolGateway` — registers a task that runs at your login with standard (non-elevated) permissions. No UAC prompt.
...
```powershell
fool gateway status      # Merged view: schtasks + Startup folder + running PID
fool gateway start       # Starts the scheduled task now
fool gateway stop        # Graceful SIGTERM equivalent (TerminateProcess via psutil)
fool gateway restart
fool gateway uninstall   # Removes schtasks entry, Startup shortcut, pid file
```

`fool gateway status` is idempotent — call it a thousand times in a row and it will never accidentally kill the gateway. (Pre-PR #21561 it silently did, via `os.kill(pid, 0)` colliding with `CTRL_C_EVENT` at the C level — see "process management internals" below if you care about the story.)
...
- Playwright Chromium is auto-installed on first run (`npx playwright install chromium`). If installation fails, `fool doctor` surfaces it with a fix-it hint.
...
Hermes honors both `$env:X` (process-scope) and User environment variables (permanent, set in System Properties → Environment Variables). Setting API keys in `%LOCALAPPDATA%\hermes\.env` (your `FOOL_HOME`) is the normal path — same as Linux:
...
| Variable | Effect |
|---|---|
| `FOOL_GIT_BASH_PATH` | Override bash.exe discovery. Point at any bash — full Git-for-Windows, WSL bash via symlink, MSYS2, Cygwin. The installer sets this automatically. |
| `FOOL_DISABLE_WINDOWS_UTF8` | Set to `1` to disable the UTF-8 stdio shim and fall back to the locale code page. Useful for bisecting an encoding bug. |
...
The `fool uninstall` CLI subcommand also handles the case where the schtasks entry was registered under a different task name (older installs) — it searches by install path rather than by hardcoded task name.
...
Check `fool gateway status` — it merges the schtasks entry, the Startup-folder shortcut (if used), and the live PID. If schtasks is registered but not running, group policy may be blocking `ONLOGON` triggers. Run `schtasks /Query /TN FoolGateway /V /FO LIST` to see the task's failure reason, or fall back to the Startup-folder path by uninstalling and reinstalling with `FOOL_GATEWAY_FORCE_STARTUP=1`.
...
Chromium is auto-installed on first run. If the install failed (rate-limited GitHub, Playwright CDN hiccup), run `fool doctor` — it will surface the missing Chromium and print the exact `npx playwright install chromium` command to fix it.
...
The UTF-8 stdio shim didn't activate. Check that `FOOL_DISABLE_WINDOWS_UTF8` is NOT set (`Get-ChildItem env:FOOL_DISABLE_WINDOWS_UTF8`). If it's empty and you still see `?`, the console host (very old `cmd.exe`) may not support UTF-8 at all — switch to Windows Terminal.
