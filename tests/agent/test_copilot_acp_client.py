...
class CopilotACPClientSafetyTests(unittest.TestCase):
    ...
    def test_read_text_file_redacts_sensitive_content(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # agent.redact snapshots FOOL_REDACT_SECRETS at import time into
    ...
    def test_write_text_file_respects_safe_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(os.environ, {"FOOL_WRITE_SAFE_ROOT": str(safe_root)}, clear=False):
                response = self._dispatch(
                    {
                        "jsonrpc": "2.0",
                        "id": 5,
        ...
        self.assertIn("FOOL_WRITE_SAFE_ROOT", str(response["error"]))
...
def test_run_prompt_preserves_real_home_when_profile_home_available(monkeypatch, tmp_path):
    ...
    # Hermeticity: an ambient FOOL_REAL_HOME (exported by Hermes' own
    ...
    assert captured["kwargs"]["env"]["FOOL_REAL_HOME"] == str(real_home)
