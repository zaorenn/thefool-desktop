...
``acp_adapter/entry.py`` (via the shared ``thefool_bootstrap.harden_import_path``
...
def test_slash_worker_imports_from_cwd_with_colliding_utils(tmp_path):
    ...
    env = {k: v for k, v in os.environ.items() if k != "THEFOOL_PYTHON_SRC_ROOT"}
...
def test_sys_path_guard_runs_before_cli_import():
    ...
    ``thefool_bootstrap.harden_import_path()`` call precedes ``import cli``."""
    ...
    assert harden_call_line is not None, (
        "slash_worker.py must call thefool_bootstrap.harden_import_path()"
    )
...
def test_guard_delegates_to_shared_helper_not_inline():
    ...
    assert '{"", "."}' not in src and "{'', '.'}" not in src, (
        "slash_worker.py should delegate to thefool_bootstrap.harden_import_path, "
        "not re-implement the guard inline"
    )
    assert "thefool_bootstrap.harden_import_path()" in src, (
        "slash_worker.py must call the shared thefool_bootstrap.harden_import_path guard"
    )
