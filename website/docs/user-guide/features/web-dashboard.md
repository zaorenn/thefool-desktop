...
:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `fool setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
child with the selected profile's `FOOL_HOME`, so the conversation runs
...
When you run `fool dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `fool dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
ExecStart=/path/to/venv/bin/python -m fool_cli.main dashboard \
...
```bash
FOOL_DASHBOARD_BASIC_AUTH_USERNAME=admin
FOOL_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
FOOL_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `fool config set` and the gateway read from.
:::
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `fool profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `fool skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `fool mcp` reads from.
...
`fool mcp catalog` / `fool mcp install` use.
...
`fool setup gateway`. The page lists every supported channel (Telegram,
...
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `fool curator`.
...
`FOOL_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
|-------|-----------|----------|
| `fool dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `fool dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `fool dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `fool dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Hermes doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `fool dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
- **CLI — `fool dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `fool setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `FOOL_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hermes/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  fool dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes FOOL_DASHBOARD_OAUTH_CLIENT_ID to ~/.hermes/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `FOOL_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `FOOL_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `fool dashboard register` |
...
Bundled providers reported these issues:
  • nous: FOOL_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `fool dashboard register` (Nous Portal) or install a
...
**1. Log in and register the dashboard.** `fool dashboard register` uses your existing Nous login to provision an OAuth client and writes `FOOL_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hermes/.env` for you:

```bash
fool setup            # if you're not already logged into Nous Portal
fool dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes FOOL_DASHBOARD_OAUTH_CLIENT_ID to ~/.hermes/.env
```
...
```bash
fool dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://hermes.example.com/auth/callback` and set `FOOL_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `FOOL_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `FOOL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `FOOL_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `FOOL_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `FOOL_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
cat >> ~/.hermes/.env <<EOF
FOOL_DASHBOARD_BASIC_AUTH_USERNAME=admin
FOOL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
FOOL_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `FOOL_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `FOOL_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `FOOL_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
```bash
export FOOL_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/hermes"
export FOOL_DASHBOARD_OIDC_CLIENT_ID="hermes-dashboard"
export FOOL_DASHBOARD_PUBLIC_URL="http://localhost:9119"
fool dashboard --host 0.0.0.0 --port 9119 --no-open
```

`FOOL_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `FOOL_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `FOOL_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `FOOL_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$FOOL_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
from fool_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The bundled first consumer is the **drain** provider (`plugins/dashboard_auth/drain`): `nous-account-service` provisions a per-agent secret via `FOOL_DASHBOARD_DRAIN_SECRET`, and the provider verifies inbound bearer tokens against it with a constant-time compare, registering `/api/gateway/drain` as token-authable. It **fails closed** — a weak/short secret (< 256 bits) is rejected at registration and the endpoint stays disabled; it's a no-op when the env var is unset. Behavioural knobs (`scope`, `min_secret_chars`) live under `dashboard.drain_auth` in `config.yaml`.
...
```bash
# Quick env-var path.
FOOL_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  fool dashboard --host 0.0.0.0
...
You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`fool dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
fool dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `FOOL_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.
...
The session refreshes automatically and survives restarts when `FOOL_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `FOOL_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `FOOL_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `FOOL_DASHBOARD_BASIC_AUTH_USERNAME` / `FOOL_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `FOOL_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `fool_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `fool update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `fool dashboard` will build it on first launch.
