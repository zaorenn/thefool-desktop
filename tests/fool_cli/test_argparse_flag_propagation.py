...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("fool_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "fool_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets FOOL_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `fool gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: fool_cli.main is a very heavy
    # import (previously 11 separate `python -m fool_cli.main` spawns with a
...
class TestChatSubparserInheritedValueFlags:
    ...
    Uses the real `fool_cli._parser.build_top_level_parser()` rather than
