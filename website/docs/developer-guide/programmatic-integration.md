...
`fool acp` starts a stdio JSON-RPC server speaking ACP. Used in production by VS Code (Zed Industries' ACP extension), Zed, and any JetBrains IDE with an ACP plugin.
...
```bash
fool acp                  # serve ACP on stdio
fool acp --check          # verify ACP dependencies and adapter imports
fool acp --setup          # interactive provider/model setup for ACP terminal auth
```
...
Provider-aware resolution (the same model name picks the right format for whatever provider you're on) is built in. See `fool_cli/model_switch.py`.
