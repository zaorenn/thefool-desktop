...
``fool_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running The Fool web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running The Fool web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Fool Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # backend as `fool dashboard --no-open --tui --host ... --port ...`. The
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Fool backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Fool backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports FOOL_SERVE_HEADLESS=1). `dashboard` leaves it
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "portal. Also settable via FOOL_DASHBOARD_PORTAL_URL. Mainly for "
