...
cd ../
python -m thefool_cli.main web --no-open
...
`hermes dashboard` on port 9119 serves the **built** bundle from `thefool_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../thefool_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
