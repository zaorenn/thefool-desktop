...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    ...
    Dev override: ``THEFOOL_DEV_CREDITS_FIXTURE`` short-circuits to a fixture so
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """Map ``THEFOOL_DEV_CREDITS_FIXTURE`` to a usage model for offline UX work.

    Recognized names: ``free | healthy | low | topup | depleted``. Returns
    ``None`` when the env var is unset (real portal path runs).
    """
    name = (os.getenv("THEFOOL_DEV_CREDITS_FIXTURE") or "").strip().lower()
    if not name:
        return None
