import { Box, Text, useInput, useStdout } from '@fool/ink'
...
export function ModelPicker({
  ...
  useInput((ch, key) => {
                ...
                prev.map(p =>
                  p.slug === provider.slug
                    ? {
                        ...
                        models: [],
                        total_models: 0,
                        warning: p.key_env ? `paste ${p.key_env} to activate` : 'run `fool model` to configure'
                      }
                    : p
                )
              )
    ...
    if (key.return) {
      if (stage === 'provider') {
        ...
        if (provider.authenticated === false) {
          ...
          // Other auth types: no-op (warning shown tells them to run fool model)
          return
        }
    ...
    // filter. With Ctrl held, @fool/ink reports `ch` as the key name ('g'),
