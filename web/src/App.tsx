...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'fool update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts The Fool gateway process. Connected channels and active sessions will reconnect afterward."
      }
