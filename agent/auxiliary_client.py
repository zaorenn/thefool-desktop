...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``THEFOOL_CA_BUNDLE`` /
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    try:
        from thefool_cli.auth import resolve_api_key_provider_credentials
        from thefool_cli.models import fetch_models_with_pricing
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``THEFOOL_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``THEFOOL_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("THEFOOL_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("THEFOOL_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
# in lockstep with thefool_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``thefool_cli.__version__`` is
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("THEFOOL_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``thefool_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _nous_api_key(provider: dict) -> str:
    ...
    from thefool_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from thefool_cli.auth import _agent_key_is_usable
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``thefool_cli.auth``'s singleton runtime resolver for older
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from thefool_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from thefool_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from thefool_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if not _aux_probe_active():
        ...
        try:
            from thefool_cli.models import get_nous_recommended_aux_model
...
def _try_azure_foundry(
    *,
    ...
    :func:`thefool_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from thefool_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``thefool_cli.fallback_config.resolve_entry_api_key``).
    """
...
def resolve_provider_client(
    ...
    try:
        ...
        logger.debug("thefool_cli.auth not available for provider %s", provider)
        return None, None
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`thefool_cli.plugins.PluginContext.register_auxiliary_task`) the
