...
def test_failing_gate_short_circuits_judge():
    ...
    mgr.add_gate("exit 5")
    with patch("thefool_cli.goals.judge_goal") as mock_judge, \
         patch("thefool_cli.goals.workspace_fingerprint", return_value=""):
...
def test_unchanged_workspace_skips_rerun():
    ...
    mgr.add_gate("exit 1")
    with patch("thefool_cli.goals.workspace_fingerprint", return_value="fp-1"), \
         patch("thefool_cli.goals.judge_goal"):
        mgr.evaluate_after_turn("turn 1")
        # Second turn, same fingerprint — run_gate must NOT run again.
        with patch("thefool_cli.goals.run_gate") as mock_run:
...
def test_changed_workspace_reruns_gate():
    ...
    with patch("thefool_cli.goals.judge_goal"):
        with patch("thefool_cli.goals.workspace_fingerprint", return_value="fp-1"):
            mgr.evaluate_after_turn("turn 1")
        with patch("thefool_cli.goals.workspace_fingerprint", return_value="fp-2"), \
             patch("thefool_cli.goals.run_gate", return_value=(False, 1, "still red")) as mock_run:
