...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal The Fool path and cannot be attached")
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal The Fool path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
