...
* The requested key is passed to the child ONLY via the ``THEFOOL_SECRET_KEY``
...
* The startup/apply path runs the helper exactly ONCE (with an empty
  ``THEFOOL_SECRET_KEY``) — it is never called per-key in a loop, so a
...
# thefool_cli.env_loader can consume both providers identically.
...
def _run_helper(
    ...
    The key is passed as DATA via ``THEFOOL_SECRET_KEY`` — never interpolated
    ...
    if source_env is os.environ:
        ...
        env = dict(source_env)
    env["THEFOOL_SECRET_KEY"] = secret_key
...
def get_command_secret(
    *,
    ...
    ``THEFOOL_SECRET_KEY``.  Returns None on any failure — never raises."""
...
def apply_command_secrets(
    *,
    ...
    # THEFOOL_SECRET_KEY and parse its stdout as a dotenv blob.
    stdout = _run_helper(command, "", timeout_seconds, max_output_bytes)
    if stdout is None:
