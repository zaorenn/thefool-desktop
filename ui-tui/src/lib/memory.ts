...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because FOOL_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.FOOL_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.hermes', 'heapdumps')
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.FOOL_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.FOOL_HEAPDUMP_MAX_BYTES?.trim()
