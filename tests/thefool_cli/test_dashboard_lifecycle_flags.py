...
from thefool_cli.main import cmd_dashboard
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("thefool_cli.main._scan_dashboard_processes", return_value=[]), \
    ...
    def test_status_with_processes(self, capsys):
        processes = [
            ...
            (12346, "python -m thefool_cli.main dashboard --host 0.0.0.0 --port 9120"),
        ]
        with patch("thefool_cli.main._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("thefool_cli.main._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
...
class TestDashboardStop:
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("thefool_cli.main._find_stale_dashboard_pids",
                   side_effect=lambda: next(scans)), \
             patch("thefool_cli.main._kill_stale_dashboard_processes"), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
...
class TestLifecycleFlagsTakePrecedence:
    ...
    def test_stop_does_not_fall_through_to_server_start(self):
        ...
        with patch("thefool_cli.main._find_stale_dashboard_pids",
                   return_value=[]), \
             patch.dict(sys.modules, {"thefool_cli.web_server": fake_ws}), \
...
class TestArgparseWiring:
    ...
    def test_flags_are_registered(self):
        from thefool_cli.main import main as _cli_main  # noqa: F401
        ...
        import importlib
        mod = importlib.import_module("thefool_cli.main")
