...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    FOOL_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    assert pid == 4242
    assert captured["env"]["FOOL_HOME"] == str(profile)
    assert captured["env"]["FOOL_KANBAN_TASK"] == "t_spawn_tools"
