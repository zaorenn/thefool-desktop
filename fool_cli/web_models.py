...
Extracted verbatim from ``fool_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from fool_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ProfileCreate(BaseModel):
    ...
    # because skills_hub.SKILLS_DIR is import-time-bound and the FOOL_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under FOOL_HOME.
    output: str = ""
