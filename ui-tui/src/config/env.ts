import type { MouseTrackingMode } from '@thefool/ink'
...
export const STARTUP_RESUME_ID = (process.env.THEFOOL_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.THEFOOL_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.THEFOOL_TUI_IMAGE ?? '').trim()
...
//
// - THEFOOL_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - THEFOOL_TUI_DISABLE_MOUSE=1 forces mouse off — the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.THEFOOL_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.THEFOOL_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.THEFOOL_TUI_NO_CONFIRM)
...
export const DASHBOARD_TUI_MODE = truthy(process.env.THEFOOL_TUI_DASHBOARD)

// THEFOOL_DEV_CREDITS — dev-only live-spend readout (Δ status segment + "(dev credits)"
// banner). Throwaway dev scaffolding; the whole readout gates on this one flag.
export const DEV_CREDITS_MODE = truthy(process.env.THEFOOL_DEV_CREDITS)

const inlineOverride = parseToggle(process.env.THEFOOL_TUI_INLINE)
...
// less fragile. Override explicitly with THEFOOL_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.THEFOOL_TUI_FPS)
