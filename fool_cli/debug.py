...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``fool debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``fool debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `fool debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``fool debug share`` invocation added ~20 MB of resident Python
    ...
    ``fool debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----FoolDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from fool_cli.logs import LOG_FILES
...
# desktop app talks to a remote/docker/SSH backend, `fool debug share` runs
...
def _capture_dump() -> str:
    """Run ``fool dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``fool dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``fool debug share`` (CLI) and the
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `fool debug share --local` to print the report instead.\n")
        sys.exit(1)
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``fool debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from fool_cli.diagnostics_upload import share_to_nous
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `fool debug share --local` to print the report instead, "
            "or `fool debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``fool debug`` call.
    ...
    # best-effort — any failure is swallowed so ``fool debug`` stays
