...
2. User plugins: ``$FOOL_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$FOOL_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$FOOL_HOME/plugins/model-providers/`` if it exists."""
    try:
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple FOOL_HOME profiles don't alias each other.
...
def _discover_entry_point_providers() -> None:
    ...
    * **Provider targets only.** The ``hermes_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$FOOL_HOME``) keep their
    ...
    try:
        from fool_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$FOOL_HOME/plugins/model-providers/<name>/``
    ...
    #    filesystem steps means a bundled or ``$FOOL_HOME`` profile of the
    ...
    # 2. User plugins — under $FOOL_HOME/plugins/model-providers/<name>/.
