"""``fool_cli/_scan_venv_blockers.py`` — Standalone venv-process scan for JSON consumption.

Invoked by the Desktop Electron app::

    venv\\Scripts\\python.exe -m fool_cli._scan_venv_blockers
...
def _is_pausable_gateway(cmdline: str) -> bool:
    ...
    fool_cli.main gateway run`` and the uv-side interpreter re-running the
...
def main() -> None:
    ...
    try:
        from fool_cli.main import _detect_venv_python_processes  # noqa: PLC0415
