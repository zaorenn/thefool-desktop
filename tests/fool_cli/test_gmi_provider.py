...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from fool_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # fool model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in FOOL_OVERLAYS
        overlay = FOOL_OVERLAYS["gmi"]
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a FoolAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("FoolAgent/")

    def test_gmi_profile_declares_hermes_user_agent(self):
        """The GMI plugin sets a FoolAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("FoolAgent/"), (
            f"expected GMI profile User-Agent to start with 'FoolAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("fool_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "fool_cli.main.cmd_chat",
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("fool_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("fool_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("fool_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            "fool_cli.auth.deactivate_provider",
        ), patch(
            "builtins.input",
            return_value="",
        ):
            from fool_cli.main import _model_flow_api_key_provider
