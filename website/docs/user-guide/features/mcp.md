...
```bash
fool mcp                # interactive picker (default)
fool mcp catalog        # plain-text list, scriptable
fool mcp install n8n    # install a catalog entry by name
```
...
- **OAuth** (third-party provider like Google/GitHub) — Hermes points you at
  `fool auth <provider>` if you haven't authenticated already.
...
written (if not). Re-run `fool mcp configure <name>` once the server is
reachable to refine.
...
Hermes`) for that entry instead of silently hiding it. Run `fool update`
...
MCPs are never auto-updated. Re-run `fool mcp install <name>` to refresh
...
Or: `fool mcp install figma`, then `fool mcp login figma`.
:::
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `fool mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `fool mcp login` can list the tools and look like it worked, but every real tool call later times out. `fool mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `fool mcp login googledrive` — with the pre-registered client, Hermes skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.hermes/config.yaml` from inside a running Hermes session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `fool mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.
...
For well-known MCP servers, `fool mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
fool mcp add codex --preset codex
```
...
You can pick any local name (`fool mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.
...
```bash
fool mcp serve              # Normal mode
fool mcp serve --verbose    # Debug logging on stderr
```
...
- The embedded `fool mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Hermes, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
