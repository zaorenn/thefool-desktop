...
:::tip
If your OpenClaw setup was multi-provider, `fool setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `fool update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
...
| Gateway auth token | `gateway.auth.token` | `.env` → `FOOL_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `fool cron create` |
...
| Skills registry | `archive/skills-registry-config.json` | Use `fool skills config` |
...
4. **Verify API keys** — run `fool status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart hermes-gateway`

6. **Check session policies** — run `fool config show` and verify the `session_reset` value matches your expectations.
...
OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `fool config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
