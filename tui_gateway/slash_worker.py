"""Persistent slash-command worker — one FoolCLI per TUI session.
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before FoolCLI snapshots tools.
...
def _run(cli: FoolCLI, command: str) -> str:
...
def main():
    ...
    # Start before the (hundreds-of-ms) FoolCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = FoolCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
