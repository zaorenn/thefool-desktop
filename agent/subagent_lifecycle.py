...
_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="fool-lifecycle")
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError(
                "No active The Fool parent session is available."
            )
        ...
        if not subagent_id:
            raise SubagentLifecycleError("The Fool failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
    ...
    def _validate_request(request: SubagentLaunchRequest, parent: Any) -> None:
        ...
        if request.working_directory is not None:
            raise SubagentLifecycleError(
                "working_directory is not supported because The Fool delegates use isolated task environments."
            )
        if request.blocked_tools:
            raise SubagentLifecycleError(
                "Per-tool blocking is not supported; use allowed_toolsets. The Fool always blocks unsafe child tools."
            )
        try:
            metadata_bytes = len(
