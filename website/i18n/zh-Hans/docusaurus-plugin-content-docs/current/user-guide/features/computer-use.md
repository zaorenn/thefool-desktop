...
- **私有 SPI 风险。** Apple 可能在任何 OS 更新中更改 SkyLight 的符号接口。Hermes 始终安装最新版 cua-driver，并在已安装的二进制文件低于其测试基线版本（按操作系统分别设定）时发出警告。没有版本固定开关——如需可复现的版本，请将 `THEFOOL_CUA_DRIVER_CMD` 指向特定的二进制文件。
...
```
THEFOOL_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
```
