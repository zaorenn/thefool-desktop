"""Tests for thefool_cli/_scan_venv_blockers.py.
...
from thefool_cli._scan_venv_blockers import (
...
def test_redact_long_flag_value_space_separated() -> None:
    ...
    raw = "python.exe -m thefool_cli.main serve --token ghp_abc123 --host 10.0.0.1"
    result = _redact_sensitive_cmdline(raw)
    assert result == "python.exe -m thefool_cli.main serve --token <redacted>"
...
def test_redact_normal_host_port_profile_remain() -> None:
    raw = "python.exe -m thefool_cli.main serve --host 10.0.0.1 --port 9119 --profile work"
    result = _redact_sensitive_cmdline(raw)
...
def test_redact_no_sensitive_flags_is_noop() -> None:
    raw = "python.exe -m thefool_cli.main serve --host 127.0.0.1"
...
def test_redact_short_flags_not_redacted() -> None:
    ...
    raw = "python.exe -m thefool_cli.main serve -t web -p default -k somearg"
    result = _redact_sensitive_cmdline(raw)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        " -m thefool_cli.main gateway run --replace",
        ...
        "python.exe -m thefool_cli.main --profile work gateway run",
        ...
        "python.exe -m thefool_cli.main --profile gateway gateway run",
        "python.exe -m thefool_cli.main -p gateway gateway run",
        # bare `gateway` defaults to `run` (mirrors the canonical matcher)
        "python.exe -m thefool_cli.main gateway",
        # case variations survive
        "PYTHON.EXE -m thefool_cli.main GATEWAY RUN",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "python.exe -m thefool_cli.main serve --host 127.0.0.1 --port 8756",
        # other gateway subcommands are not running gateways
        "python.exe -m thefool_cli.main gateway stop",
        "python.exe -m thefool_cli.main gateway status",
        "python.exe -m thefool_cli.main gateway install",
        # operator REPL / stray script
        "python.exe",
        "python.exe myscript.py gateway run",  # not a thefool_cli.main invocation
        "",
    ],
)
...
def test_main_exempts_gateway_chain_but_keeps_other_holders(monkeypatch, capsys):
    ...
    gateway_launcher = (
        12,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m thefool_cli.main gateway run --replace",
    )
    gateway_worker = (
        34,
        "python.exe",
        r'"C:\u\uv\python\python.exe"  -m thefool_cli.main gateway run --replace',
    )
...
def test_main_desktop_serve_backend_still_blocks(monkeypatch, capsys):
    ...
    serve = (
        78,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m thefool_cli.main serve --host 127.0.0.1",
    )
...
def test_main_gateway_with_long_managed_runtime_path_is_exempt(monkeypatch, capsys):
    ...
    before `-m thefool_cli.main gateway run`, so the exemption never matched
    ...
    gateway = (91, "python.exe", long_exe + "  -m thefool_cli.main gateway run --replace")
