...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (THEFOOL_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via THEFOOL_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in THEFOOL_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from thefool_cli.providers import THEFOOL_OVERLAYS
        assert "bedrock" in THEFOOL_OVERLAYS
