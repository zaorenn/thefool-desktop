...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the thefool_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns thefool_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/thefool_cli/test_cmd_update.py).
    """
    ...
    import thefool_cli.gateway as hermes_gateway
