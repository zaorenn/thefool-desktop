"""Cross-session FOOL_SESSION_ID leak via the shared bash snapshot.
...
``FOOL_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $FOOL_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (FOOL_SESSION_* / UI /
...
def test_export_snippet_shape():
    ...
    assert "${!FOOL_SESSION_*}" in snippet
    assert "${!FOOL_CRON_AUTO_DELIVER_*}" in snippet
    assert "FOOL_UI_SESSION_ID" in snippet
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$FOOL_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "FOOL_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
