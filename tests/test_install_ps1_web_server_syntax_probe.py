...
because ``thefool_cli/web_server.py`` inside the installed checkout still
...
``thefool_cli/web_server.py`` and fails the stage when that syntax probe fails.
"""
...
def test_install_ps1_compiles_web_server_source_after_web_deps_probe() -> None:
    ...
    probe = re.search(
        r'import fastapi, uvicorn[\s\S]{0,1200}?-m py_compile "\$InstallDir\\thefool_cli\\web_server\.py"',
        text,
    )
    assert probe is not None, (
        "install.ps1 must syntax-check thefool_cli/web_server.py after the "
...
def test_install_ps1_fails_stage_when_web_server_syntax_probe_fails() -> None:
    ...
    assert (
        ...
        "install.ps1 must fail the install stage when thefool_cli/web_server.py "
