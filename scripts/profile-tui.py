#!/usr/bin/env python3
"""Drive the Hermes TUI under THEFOOL_DEV_PERF and summarize the pipeline.
...
launches `node dist/entry.js` directly with THEFOOL_TUI_RESUME set so it
bypasses the thefool_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  THEFOOL_PERF_LOG     (default ~/.hermes/perf.log)
  THEFOOL_PERF_NODE    (default node from $PATH)
  THEFOOL_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests THEFOOL_DEV_PERF wiring is broken).
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("THEFOOL_HOME") or "").strip()
        return Path(val) if val else Path.home() / ".hermes"

DEFAULT_TUI_DIR = Path(
    os.environ.get("THEFOOL_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("THEFOOL_PERF_LOG", str(get_hermes_home() / "perf.log")))
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — THEFOOL_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["THEFOOL_DEV_PERF"] = "1"
    env["THEFOOL_DEV_PERF_MS"] = str(args.threshold_ms)
    env["THEFOOL_DEV_PERF_LOG"] = str(log)
    env["THEFOOL_TUI_RESUME"] = sid
    ...
    node = os.environ.get("THEFOOL_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="THEFOOL_DEV_PERF_MS (0 = capture all)")
