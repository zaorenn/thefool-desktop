...
- ``thefool_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``thefool_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``thefool_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``thefool_cli.main`` imports
``managed_uv``, ``thefool_constants``, and friends only in its late path; the
...
:func:`thefool_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from thefool_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (thefool_cli.main's version lives behind imports)."""
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    # thefool_constants is stdlib-only, so the canonical layout helper is safe
