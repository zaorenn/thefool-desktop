...
|---|---|---|
| Ship it by | `fool profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `fool profile update` pulls new versions | Re-send the file |
...
| Made with | `fool profile install` / `update` | `/export` and `/import`, or `fool profile export` / `import` |
...
…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `fool profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
:::caution
**Hermes does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `fool profile install` or `fool profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
Recipients who run `fool profile update research-bot` will pull the latest.
...
`fool profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `fool profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
When the lead ships v1.1 (better SOUL, new skill), engineers run `fool profile update pr-reviewer` and everyone's on the new version within minutes.
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `fool profile info <name>` and hold off on updates until you're ready.
:::
...
```bash
fool profile export research-bot
fool profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
fool profile import ./research-bot.tar.gz
fool profile import ./research-bot.tar.gz --name research-bot-2
```
...
- `install`, `update`, `info` live inside `fool profile` — not a parallel command tree.
...
- [`fool profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
