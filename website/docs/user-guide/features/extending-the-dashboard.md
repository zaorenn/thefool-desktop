...
The Hermes web dashboard (`fool dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
2. Restart `fool dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
