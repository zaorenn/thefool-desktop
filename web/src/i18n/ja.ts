...
export const ja: Translations = {
  ...
  status: {
    ...
    updatingHermes: "The Fool を更新しています…",
  ...
  pluginsPage: {
    ...
    headline:
      "The Fool プラグインを発見、インストール、有効化、更新します (`fool plugins` 相当)。",
    ...
    removeConfirm: "このプラグインを ~/.fool/plugins/ から削除しますか？",
    removeHint: "削除できるのは ~/.fool/plugins 配下のユーザーがインストールしたプラグインのみです。",
  ...
  skills: {
    ...
    categories: "カテゴリ",
    filters: "フィルター",
    noSkills: "スキルが見つかりません。スキルは ~/.fool/skills/ から読み込まれます",
  ...
  env: {
    ...
    hideValue: "値を非表示",
    customTitle: "カスタムキー",
    customHint: "The Fool が認識しない、.env に保存された任意の環境変数。スキル、MCP サーバー、または独自のツール用に環境変数を注入するために使用します。",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "実際のセッション履歴から獲得できる The Fool のコレクタブル バッジです。既知の未達成の実績は「Discovered」として表示され、Secret 実績は最初の該当する挙動が検出されるまで非表示のままです。",
      scan_subtitle:
        "The Fool のセッション履歴をスキャンしています。履歴が大きい場合、初回スキャンには 5～10 秒かかることがあります。",
    },
    actions: {
      rescan: "再スキャン",
    },
    stats: {
      ...
      latest: "最新",
      latest_hint_empty: "The Fool をもっと使ってみてください",
      none_yet: "まだありません",
    },
    state: {
      unlocked: "解除済み",
    ...
    guide: {
      ...
      secret_body:
        "シークレットはトリガー条件を隠しています。The Fool が関連するシグナルを検出すると、カードは「Discovered」になり、要件が表示されます。",
      scan_status_header: "スキャン状況",
      scan_status_body:
        "The Fool はローカル履歴を一度スキャンし、その後カードが自動的に表示されます。数秒かかってもスタックしているわけではありません。",
