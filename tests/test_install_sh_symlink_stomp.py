...
symlink to the pip-generated entry point at ``$FOOL_BIN`` (i.e.
...
shim's ``exec "$FOOL_BIN" "$@"`` then self-recursed and ``hermes`` hung on
every invocation.
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    ``FOOL_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
