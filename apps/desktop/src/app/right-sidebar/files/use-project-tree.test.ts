...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { foolDesktop: { readDir: typeof readDir } }).foolDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): FoolReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { foolDesktop: unknown }).foolDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: FoolReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<FoolReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { foolDesktop: unknown }).foolDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.foolDesktop is missing', async () => {
    delete (window as unknown as { foolDesktop?: unknown }).foolDesktop
