...
|---------|-------------|
| `fool auth` | 交互式池管理向导 |
| `fool auth list` | 显示所有池和凭证 |
| `fool auth list <provider>` | 显示指定提供商的池 |
| `fool auth add <provider>` | 添加凭证（提示选择类型和密钥） |
| `fool auth add <provider> --type api-key --api-key <key>` | 非交互式添加 API 密钥 |
| `fool auth add <provider> --type oauth` | 通过浏览器登录添加 OAuth 凭证 |
| `fool auth remove <provider> <index>` | 按从 1 开始的索引删除凭证 |
| `fool auth reset <provider>` | 清除所有冷却时间/耗尽状态 |

## 轮换策略

通过 `fool auth` → "Set rotation strategy" 配置，或在 `config.yaml` 中设置：
...
通过 `fool model` 设置自定义端点时，会自动生成类似 "Together.ai" 或 "Local (localhost:8080)" 的名称，该名称即成为池的键。
...
| 手动条目 | 通过 `fool auth add` 添加 | 持久化至 auth.json |

自动初始化的条目在每次池加载时更新——如果你删除了某个环境变量，其池条目会自动清除。通过 `fool auth add` 添加的手动条目永远不会被自动清除。
...
2. **`fool_cli/auth_commands.py`** — CLI 命令和交互式向导
3. **`fool_cli/runtime_provider.py`** — 感知池的凭证解析
