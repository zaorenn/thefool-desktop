...
# when thefool_constants is cached from a pre-update version during `hermes update`.
...
def _scan_gateway_pids(
    ...
    # Forward slashes on both sides of the THEFOOL_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # the command doesn't advertise *some other* profile. THEFOOL_HOME
    ...
    try:
        if is_windows():
            ...
            # watchdog probe.
            from thefool_cli._subprocess_compat import bounded_probe_run
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "thefool_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m thefool_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose THEFOOL_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # interpreter and captures the stable cwd + env overlay (THEFOOL_HOME,
    ...
    if sys.platform == "win32":
        try:
            from thefool_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / THEFOOL_HOME) the windowless
        # base interpreter needs to import thefool_cli.  Empty on POSIX, where
...
def _hermes_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``THEFOOL_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("THEFOOL_HOME="):
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``THEFOOL_HOME``.

    Under ``sudo``, ``THEFOOL_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hermes_home` falls back to ``/root/.hermes`` — the wrong
    profile. The unit file pins ``THEFOOL_HOME`` for the actual gateway
    ...
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("THEFOOL_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("THEFOOL_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["THEFOOL_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from thefool_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from thefool_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (thefool_cli/gateway.py restart
    ...
    if is_windows():
        try:
            ...
            from thefool_cli.gateway_windows import get_task_name
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (THEFOOL_HOME-scoped).
...
from thefool_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    ``THEFOOL_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("THEFOOL_GATEWAY_DETACHED", "").strip().lower()
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from thefool_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
def _profile_suffix() -> str:
    """Derive a service-name suffix from the current THEFOOL_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (THEFOOL_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this THEFOOL_HOME.

    Default ``~/.hermes`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.hermes/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other THEFOOL_HOME appends a short hash for uniqueness.
    """
    ...
    try:
        ...
        if not _is_service_installed():
            ...
            if supports_systemd:
                ...
                from thefool_cli import gateway_windows
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # thefool_constants cached from before this symbol existed. See
            # _reload_hermes_constants() in thefool_cli/managed_uv.py.
            from thefool_cli.managed_uv import _reload_hermes_constants
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current THEFOOL_HOME to the equivalent under a target user's home.
    ...
    """
    current_hermes_raw = os.environ.get("THEFOOL_HOME", "").strip()
    current_hermes = (
    ...
    # different THEFOOL_HOME into the generated service unit.
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m thefool_cli.main``, so module
    ...
    ``THEFOOL_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if THEFOOL_HOME
...
def _append_node_dir_for_service(
    ...
    The Hermes-managed Node under ``$THEFOOL_HOME/node`` goes first when it
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's THEFOOL_HOME (stable, always exists)
...
Group={group_name}
ExecStart={python_path} -m thefool_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="THEFOOL_HOME={hermes_home}"
Restart=always
RestartSec=5
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m thefool_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="THEFOOL_HOME={hermes_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned THEFOOL_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo hermes gateway … --system``, THEFOOL_HOME is often stripped
    # and falls back to ``/root/.hermes``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / THEFOOL_HOME comparisons
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir THEFOOL_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved THEFOOL_HOME
    ...
    exported a throwaway ``THEFOOL_HOME=/tmp/...`` — writing it to the real
    ...
    with ``THEFOOL_HOME=/tmp/hermes-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="THEFOOL_HOME=..."`` lines and launchd
    ``<key>THEFOOL_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'THEFOOL_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>THEFOOL_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp THEFOOL_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: THEFOOL_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported THEFOOL_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single THEFOOL_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only THEFOOL_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp THEFOOL_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="THEFOOL_HOME=..."``. Writing that to the developer's
    ...
    # THEFOOL_HOME into the unit (manual E2E homes like /tmp/hermes-e2e-NNN
...
def systemd_install(
    ...
    # Existing system units already pin THEFOOL_HOME; adopt it before any
...
def systemd_start(system: bool = False):
    ...
    # THEFOOL_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m thefool_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active THEFOOL_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "thefool_cli.main"]
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    """
    from thefool_constants import display_hermes_home as _dhh
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from thefool_cli.resource_limits import configured_nofile_soft_limit
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``THEFOOL_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    try:
        ...
        from thefool_cli.profiles import normalize_profile_name
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    unrelated gateway processes from other THEFOOL_HOME roots.
    """
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("THEFOOL_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        ...
        "$THEFOOL_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set THEFOOL_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Service-style launchers set THEFOOL_GATEWAY_DETACHED=1; older wrappers
    ...
    _gateway_detached = (
        os.getenv("THEFOOL_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # THEFOOL_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("THEFOOL_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``THEFOOL_GATEWAY_MAX_STARTS`` /
    # ``THEFOOL_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("THEFOOL_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("THEFOOL_GATEWAY_START_WINDOW_S")
...
def _setup_standard_platform(platform: dict):
    ...
    from thefool_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from thefool_cli.telegram_managed_bot import (
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``thefool_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from thefool_cli import setup as _s
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`thefool_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from thefool_cli.service_manager import (
    ...
    if profile is None:
        ...
        # THEFOOL_HOME=<root>/profiles/<name>, "" for the default root,
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      2. ``THEFOOL_S6_SUPERVISED_CHILD`` is exported by
         ...
         → ...).
      3. ``--no-supervise`` (or ``THEFOOL_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("THEFOOL_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("THEFOOL_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${THEFOOL_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if THEFOOL_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or THEFOOL_GATEWAY_NO_SUPERVISE=1) to opt out\n"
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            from thefool_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
