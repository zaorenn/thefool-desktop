...
Runs a real subprocess with a temp THEFOOL_HOME whose config.yaml contains a
``${ENV_VAR}`` reference, plus a managed-scope overlay dir (THEFOOL_MANAGED_DIR).
...
production: ``THEFOOL_MANAGED_DIR`` is set explicitly, which bypasses the
...
def test_behavioral_read_gets_expansion_and_overlay_while_writeback_stays_raw(
    ...
    out = _run_py(
        code,
        {
            ...
            "THEFOOL_MANAGED_DIR": str(managed_dir),
...
def test_writeback_roundtrip_byte_identical_when_unchanged(tmp_path):
    ...
    code = textwrap.dedent(
        ...
        p = Path(__import__('os').environ['THEFOOL_HOME']) / 'config.yaml'
    ...
    out = _run_py(code, {"THEFOOL_HOME": str(home)}, tmp_path)
