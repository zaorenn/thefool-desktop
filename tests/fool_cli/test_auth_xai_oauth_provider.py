...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("fool_cli.auth.httpx.Client", _factory)
    return holder
...
def test_refresh_xai_oauth_tokens_preserves_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.auth.refresh_xai_oauth_pure", _fake_pure)
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
# ``XAI_BASE_URL`` / ``FOOL_XAI_BASE_URL`` are a credential-leak vector
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    monkeypatch.setattr("fool_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``fool model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``fool model``
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from fool_cli.auth import _xai_oauth_discovery
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `fool model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `fool auth list` reflects truth and
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``fool auth remove xai-oauth 1`` for a
    ...
    from agent.credential_pool import load_pool
    from fool_cli.auth_commands import auth_remove_command
    ...
    # Act: the user runs `fool auth remove xai-oauth 1`.
...
def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``fool model`` re-login (``_login_xai_oauth``) must clear the
    ...
    new_access = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    monkeypatch.setattr(
        "fool_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
                "access_token": new_access,
    ...
    monkeypatch.setattr(
        "fool_cli.auth._update_config_for_provider",
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`fool auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``fool auth status``, ``fool setup``, and the
