...
Usage:
    fool pairing list              # Show all pending + approved users
    fool pairing approve <platform> <request-id|code>  # Approve a pairing request
    fool pairing revoke <platform> <user_id> # Revoke user access
    fool pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle fool pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: fool pairing {list|approve|revoke|clear-pending}")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: fool pairing approve <platform> <request-id>")
