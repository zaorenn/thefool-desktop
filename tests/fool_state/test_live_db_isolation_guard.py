...
REAL_ROOT = fool_state._real_platform_state_root()
...
class TestProductionPathRefused:
    ...
    def test_default_resolution_to_production_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(
            fool_state, "DEFAULT_DB_PATH", fool_state._IMPORT_DEFAULT_DB_PATH
        )
        with pytest.raises(RuntimeError, match="live-system guard"):
            SessionDB()
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_hermes_home_default_resolution_works(self, tmp_path, monkeypatch):
        """Argless SessionDB() under a hermetic FOOL_HOME must succeed."""
        monkeypatch.setenv("FOOL_HOME", str(tmp_path / "hermetic-home"))
        monkeypatch.setattr(
            fool_state, "DEFAULT_DB_PATH", fool_state._IMPORT_DEFAULT_DB_PATH
        )
        db = SessionDB()
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        fool_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSessionStoreLoudFailure:
    def test_guard_error_is_not_swallowed_into_jsonl_fallback(
        ...
        monkeypatch.setattr(fool_state, "SessionDB", _boom)
        with pytest.raises(RuntimeError, match="live-system guard"):
...
class TestSubprocessChildCovered:
    def test_child_without_hermes_home_is_refused(self, tmp_path):
        """A subprocess child of a test (no FOOL_HOME) must be blocked.
        ...
        env = {
            ...
            if k not in ("FOOL_HOME", "PYTEST_PLUGINS", "PYTHONPATH")
        }
    ...
    def test_child_with_tmp_hermes_home_succeeds(self, tmp_path):
        """Same child, hermetic FOOL_HOME: must work — no false positive."""
        env = {
            k: v
            for k, v in os.environ.items()
        ...
        env["PYTEST_CURRENT_TEST"] = "tests/fake.py::test_child (call)"
        env["FOOL_HOME"] = str(tmp_path / "child-home")
        env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
        code = (
            "from fool_state import SessionDB\n"
            "db = SessionDB()\n"
