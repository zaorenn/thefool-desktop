...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    fool logs                    Show last 50 lines of agent.log
    fool logs -f                 Follow agent.log in real time
    fool logs errors             Show last 50 lines of errors.log
    fool logs gateway -n 100     Show last 100 lines of gateway.log
    fool logs gui -f             Follow gui.log in real time
    fool logs desktop -f         Follow desktop.log (Electron app boot/backend)
    fool logs --level WARNING    Only show WARNING and above
    fool logs --session abc123   Filter by session ID
    fool logs --component tools  Only show tool-related lines
    fool logs --since 1h         Lines from the last hour
    fool logs --since 30m -f     Follow, starting from 30 min ago
    fool logs list               List available log files with sizes
""",
    )
