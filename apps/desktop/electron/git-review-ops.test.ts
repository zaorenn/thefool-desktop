...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fool-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'fool-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'The Fool Test'], { cwd: dir })
