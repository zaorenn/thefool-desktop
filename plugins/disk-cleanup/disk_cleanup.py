...
Rules:
  ...
  - empty dirs    → always delete (under FOOL_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly FOOL_HOME and /tmp/hermes-*
...
def get_state_dir() -> Path:
    """State dir — separate from ``$FOOL_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$FOOL_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under FOOL_HOME or ``/tmp/hermes-*``.
...
# Paths under $FOOL_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so FOOL_HOME is resolved
    # exactly once.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside FOOL_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under FOOL_HOME, but never recurse into known
    # durable state trees.  Some installs place the Hermes checkout, venv,
    # and desktop build under FOOL_HOME; a full rglob over that tree can
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under FOOL_HOME (e.g. /tmp/hermes-*) — fall through.
        pass
