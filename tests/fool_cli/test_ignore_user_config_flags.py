"""Tests for --ignore-user-config and --ignore-rules flags on `fool chat`.
...
argparse → cmd_chat → cli.main() → FoolCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("FOOL_IGNORE_USER_CONFIG", "FOOL_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``FOOL_IGNORE_USER_CONFIG=1``.
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With FOOL_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("FOOL_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``FoolCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting FOOL_IGNORE_RULES=1 flips FoolCLI.ignore_rules True."""
        monkeypatch.setenv("FOOL_IGNORE_RULES", "1")

        # Import FoolCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of FoolCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.FoolCLI)
        # Replicate the exact logic from cli.py FoolCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("FOOL_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``fool_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("FOOL_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("FOOL_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("FOOL_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("FOOL_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "FOOL_IGNORE_USER_CONFIG" not in os.environ
        assert "FOOL_IGNORE_RULES" not in os.environ
