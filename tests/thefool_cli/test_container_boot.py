"""Tests for thefool_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $THEFOOL_HOME under tmp_path; no real
...
from thefool_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr(
        "thefool_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _seed_default_root(
    ...
    *,
    ...
    THEFOOL_HOME root (the implicit default profile)."""
    if state is not None:
...
def test_main_skips_reconcile_in_dashboard_container_s6v3(
    ...
    """
    from thefool_cli import container_boot
