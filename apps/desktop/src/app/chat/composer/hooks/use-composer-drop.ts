...
import { extractDroppedFiles, FOOL_PATHS_MIME, partitionDroppedFiles } from '../../hooks/use-composer-actions'
...
export function useComposerDrop({
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, FOOL_PATHS_MIME)) {
      return
    }
