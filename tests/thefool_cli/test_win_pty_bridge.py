"""Unit tests for thefool_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/thefool_cli/test_pty_bridge.py.  Drives
...
``thefool_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from thefool_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from thefool_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from thefool_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp
    ...
    def test_non_numeric_falls_back_to_min(self):
        from thefool_cli.win_pty_bridge import _MAX_COLS, _clamp
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('THEFOOL_PTY_TEST=' + os.environ.get('THEFOOL_PTY_TEST',''))",
            ],
            env={**os.environ, "THEFOOL_PTY_TEST": "pty-env-works"},
        )
        try:
