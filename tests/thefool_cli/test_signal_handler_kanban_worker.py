...
The fix: when the process is a dispatcher-spawned worker (``THEFOOL_KANBAN_TASK``
...
def _synthetic_worker_script() -> str:
    ...
    shape without needing the full thefool_cli boot path (config, providers,
...
def _is_alive_like_dispatcher(pid: int) -> bool:
    """Mirrors thefool_cli/kanban_db.py:_pid_alive on Linux.
...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    """With THEFOOL_KANBAN_TASK set, SIGTERM should kill the process in <2s
    even when a non-daemon thread is still alive."""
    proc = _spawn_synthetic({"THEFOOL_KANBAN_TASK": "t_test_28181"})
    try:
        t0 = time.time()
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.02)
        pytest.fail(
            "process still alive 2s after SIGTERM with THEFOOL_KANBAN_TASK set "
...
def test_sigterm_without_kanban_task_env_uses_keyboard_interrupt_path():
    """Without THEFOOL_KANBAN_TASK, the original KeyboardInterrupt path runs.
