...
class TestExitSummaryResumeHint:
    ...
    def test_resume_hint_no_profile_flag_on_custom(self, capsys):
        cli_obj = _make_cli()
        with patch("fool_cli.profiles.get_active_profile_name", return_value="custom"):
            cli_obj._print_exit_summary()
        out = capsys.readouterr().out
        # "custom" is the standard FOOL_HOME indicator — no -p needed.
        assert "hermes --resume 20260524_000001_abc123" in out
        assert " -p " not in out

    def test_resume_hint_includes_profile_flag_for_named_profile(self, capsys):
        cli_obj = _make_cli()
        with patch("fool_cli.profiles.get_active_profile_name", return_value="dev"):
            cli_obj._print_exit_summary()
        out = capsys.readouterr().out
    ...
    def test_resume_hint_falls_back_when_profile_lookup_fails(self, capsys):
        ...
        cli_obj = _make_cli()
        with patch(
            "fool_cli.profiles.get_active_profile_name",
