...
function ensurePackagedApp() {
  if (process.env.FOOL_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function ensureDmg() {
  ...
  if (process.env.FOOL_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.FOOL_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of fool_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'hermes-agent', 'fool_cli', 'main.py')
...
function help() {
  ...
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + FOOL_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  FOOL_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
