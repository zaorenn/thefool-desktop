...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "fool_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # FOOL_HOME.  For a standard install (FOOL_HOME unset) that root is
        ...
        assert env.get("FOOL_HOME") == str(get_default_hermes_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (FOOL_DESKTOP=1) must NOT
