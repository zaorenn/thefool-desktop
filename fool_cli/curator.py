...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.fool/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    print(
        "\nThis will replace the current ~/.fool/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.fool/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.fool/skills/ from a curator snapshot, or a single "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by fool_cli.main fallthrough)."""
