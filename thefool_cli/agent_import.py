...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in THEFOOL_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → THEFOOL_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in THEFOOL_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in THEFOOL_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → THEFOOL_HOME/skills/codex-imports/<name>/
...
class AgentImporter:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → THEFOOL_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``hermes import-agent`` (invoked from thefool_cli.main)."""
    from thefool_cli.config import get_config_path, load_config, save_config
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    from thefool_cli.setup import Colors, color, print_header, print_info
