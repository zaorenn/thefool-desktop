...
# THEFOOL_HOME paths we've already pulled external secrets for during this
# process.  ``load_hermes_dotenv()`` is called at module-import time from
# several hot modules (cli.py, thefool_cli/main.py, run_agent.py,
...
def _known_hermes_env_keys() -> set[str]:
    ...
    """
    from thefool_cli.config import _EXTRA_ENV_KEYS
    from thefool_cli.config_defaults import OPTIONAL_ENV_VARS
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    ...
    ``tests/thefool_cli/test_dump_env_visibility.py``), and a startup scrub
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        local_env["THEFOOL_HOME"] = str(home)
...
def reset_secret_source_cache() -> None:
    """Forget which THEFOOL_HOME paths have already had external secrets applied.
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``thefool_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from thefool_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    home_path = Path(hermes_home or os.getenv("THEFOOL_HOME", Path.home() / ".hermes"))
    ...
    from thefool_cli import _early_recovery
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``thefool_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process THEFOOL_HOME: the shared bridge reads the
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of THEFOOL_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    owns the once-per-THEFOOL_HOME guard, the post-apply ASCII
    ...
    time from several hot modules (cli.py, thefool_cli/main.py,
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # shared cache here lets main.py's early bridge and thefool_logging reuse
...
def _process_hermes_home() -> Path:
    """The THEFOOL_HOME the shared config cache is keyed to."""
    try:
