...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew fool_cli.backup's datetime forward instead of sleeping.
    ...
    import fool_cli.backup as _backup
...
def _make_hermes_tree(root: Path) -> None:
    ...
    for db_name in ("memory_store.db", "fool_state.db"):
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        from fool_cli.backup import _should_exclude
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import fool_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside FOOL_HOME."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from fool_cli.backup import _validate_backup_zip
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FOOL_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from fool_cli.backup import run_backup, run_import
        ...
        monkeypatch.setenv("FOOL_HOME", str(dst_home))
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from fool_cli.backup import _detect_prefix
...
class TestImportAtomicWrites:
    ...
    tests pin the invariant for both restore branches: the FOOL_HOME branch
    ...
    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside FOOL_HOME."""
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        FOOL_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "fool_cli.backup._preserve_file_owner",
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from fool_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from fool_cli import backup as backup_mod
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake FOOL_HOME with critical state files."""
    ...
    def test_state_db_safely_copied(self, hermes_home):
        from fool_cli.backup import create_quick_snapshot
...
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, hermes_home, capsys):
        ...
        """
        import json
        from fool_cli.backup import create_quick_snapshot, list_quick_snapshots
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, hermes_home):
        ...
        from fool_cli.backup import create_quick_snapshot, restore_quick_snapshot
...
class TestPreUpdateBackup:
    ...
    def test_skips_symlinked_files(self, hermes_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside FOOL_HOME."""
        from fool_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_hermes_tree(root)
        # Point FOOL_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("FOOL_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve FOOL_HOME dynamically and their caches are
    ...
    def test_config_off_disables_everything_silently(self, hermes_home, capsys):
        ...
        from fool_cli.main import _run_pre_update_backup
...
class TestPreMigrationBackup:
    ...
    def test_does_not_touch_pre_update_backups(self, hermes_home):
        ...
        from fool_cli.backup import create_pre_update_backup, create_pre_migration_backup
...
# location, NOT under FOOL_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------
...
class TestMemoryProviderExternalPaths:
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under FOOL_HOME,
        ...
        # External state did NOT leak into FOOL_HOME.
