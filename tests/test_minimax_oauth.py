"""Tests for MiniMax OAuth provider (fool_cli/auth.py).
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure():
    ...
    with patch("fool_cli.auth.get_provider_auth_state", return_value=stale_state), \
         patch("fool_cli.auth._refresh_minimax_oauth_state", side_effect=_terminal_refresh), \
         patch("fool_cli.auth._minimax_save_auth_state", side_effect=_capture_save):
        with pytest.raises(AuthError) as exc_info:
            resolve_minimax_oauth_runtime_credentials()
...
def test_get_minimax_oauth_auth_status_not_logged_in():
    with patch("fool_cli.auth.get_provider_auth_state", return_value=None):
...
def test_generic_auth_status_dispatches_minimax_oauth():
    ...
    with patch("fool_cli.auth.get_provider_auth_state", return_value=state):
...
def test_token_provider_refreshes_when_near_expiry():
    ...
    with patch("fool_cli.auth.get_provider_auth_state", return_value=state), \
         patch("httpx.Client") as mock_client_class, \
         patch("fool_cli.auth._minimax_save_auth_state"):
...
def test_token_provider_quarantines_state_on_terminal_refresh():
    ...
    with patch("fool_cli.auth.get_provider_auth_state", return_value=state), \
         patch("httpx.Client") as mock_client_class, \
         patch(
             "fool_cli.auth._minimax_save_auth_state",
...
def test_refresh_error_body_bounded_and_readable_with_real_client():
    ...
    from fool_cli.auth import _refresh_minimax_oauth_state
