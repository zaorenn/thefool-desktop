...
def isolated_session(monkeypatch, tmp_path):
    ...
    FOOL_HOME so the real user's command_allowlist doesn't leak in."""
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: FOOL_INTERACTIVE is set, the
    ...
    def test_pre_and_post_fire_with_expected_kwargs(
        ...
        with patch("fool_cli.plugins.invoke_hook", side_effect=fake_invoke_hook):
    ...
    def test_plugin_hook_crash_does_not_break_approval(
        ...
        with patch("fool_cli.plugins.invoke_hook", side_effect=boom):
            result = check_all_command_guards(
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: FOOL_GATEWAY_SESSION is set and a
