...
def _(rid, params: dict) -> dict:
    ...
    ``fool_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m fool_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "fool_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m fool_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from fool_cli.moa_config import moa_usage, normalize_moa_config
    ...
    if name == "loop":
        ...
        if not session:
            return _err(rid, 4001, "no active session")
        try:
            from fool_cli.loops import LoopManager, dispatch_loop_command
        except Exception as exc:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind FOOL_HOME to the session's profile so get_skill_commands()
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("FOOL_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("FOOL_BASE_URL", "") or cfg.get("base_url", "")
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off FOOL_HOME, so scoping the
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``fool journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "browse":
            from fool_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from fool_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `fool mcp` offers, so
    ...
    try:
        ...
        from fool_cli import mcp_catalog
...
# (fool_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes FOOL_HOME via set_hermes_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses fool_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from fool_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from fool_cli.config import load_config, save_config, save_env_value
        from fool_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from fool_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    the SAME interactive MCP OAuth machinery ``fool mcp login`` uses
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``fool plugins`` / the dashboard, so the three surfaces
