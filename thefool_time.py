...
Resolution order:
  1. ``THEFOOL_TIMEZONE`` environment variable
...
from thefool_constants import get_config_path
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("THEFOOL_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
...
def reset_cache() -> None:
    ...
    config edit or ``THEFOOL_TIMEZONE`` update) to force ``get_timezone()`` /
