...
import type { FoolReadDirEntry, FoolReadDirResult } from '@/global'
...
export type ProjectTreeEntry = FoolReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: FoolReadDirEntry) {
...
async function filterIgnored(entries: FoolReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<FoolReadDirResult> {
