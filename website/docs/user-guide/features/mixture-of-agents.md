...
- **`fool model`** and the **Dashboard model picker** — a `Mixture of Agents` provider row appears with your preset names as its models.
...
- MoA is no longer listed under `fool tools`; there is no `moa` toolset to enable.
