...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using FOOL_HOME."""
    try:
        from fool_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("FOOL_HOME", Path.home() / ".hermes")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("FOOL_HOME", Path.home() / ".hermes")
...
class EventBridge:
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            ...
            db_file = Path(os.environ.get("FOOL_HOME", Path.home() / ".hermes")) / "state.db"
        try:
