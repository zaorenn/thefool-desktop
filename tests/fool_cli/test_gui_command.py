"""Tests for ``fool gui`` desktop launcher wiring."""
...
from fool_cli import main as cli_main
...
def _stable_keychain_detection(monkeypatch):
    ...
    monkeypatch.delenv("FOOL_DESKTOP_PASSWORD_STORE", raising=False)
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("fool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("fool_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("fool_cli.main._desktop_build_needed", return_value=True), \
         patch("fool_cli.main._write_desktop_build_stamp"), \
         patch("fool_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("fool_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("fool_cli.main._register_linux_desktop_entry"), \
         patch("fool_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    from fool_constants import iter_hermes_node_dirs
    ...
    with patch("fool_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("fool_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("fool_cli.main._desktop_build_needed", return_value=True), \
         patch("fool_cli.main._write_desktop_build_stamp"), \
         patch("fool_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("fool_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("fool_cli.main.subprocess.run", return_value=launch_ok), \
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("fool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("fool_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("fool_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("fool_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("fool_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("fool_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_registers_linux_desktop_entry_before_launch(tmp_path, monkeypatch):
    """`fool desktop` gives the app a launcher presence on Linux."""
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    packaged_exe = _make_packaged_executable(root, monkeypatch)

    registered: list[Path] = []
    monkeypatch.setattr("fool_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr(
        "fool_cli.linux_desktop_entry.install_desktop_entry",
        lambda project_root: registered.append(project_root) or (tmp_path / "hermes.desktop"),
    )

    launch_ok = subprocess.CompletedProcess([str(packaged_exe)], 0)

    with patch("fool_cli.main._desktop_build_needed", return_value=False), \
         patch("fool_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("fool_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("fool_cli.main.subprocess.run", return_value=launch_ok), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_launches_even_when_desktop_entry_install_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.linux_desktop_entry.is_supported", lambda: True)
    monkeypatch.setattr("fool_cli.linux_desktop_entry.install_desktop_entry", boom)
    ...
    with patch("fool_cli.main._desktop_build_needed", return_value=False), \
         patch("fool_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("fool_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("fool_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skips_desktop_entry_off_linux(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.linux_desktop_entry.is_supported", lambda: False)

    def fail(_project_root):
        raise AssertionError("must not install a desktop entry off Linux")

    monkeypatch.setattr("fool_cli.linux_desktop_entry.install_desktop_entry", fail)
...
def test_detect_linux_password_store_via_dbus_secret_service(monkeypatch):
    ...
    with patch("fool_cli.main.subprocess.run", return_value=ping_ok) as mock_run:
        assert cli_main._detect_linux_password_store() == "gnome-libsecret"
...
def test_detect_linux_password_store_none_when_no_keychain(monkeypatch):
    ...
    with patch("fool_cli.main.subprocess.run", return_value=ping_fail):
        assert cli_main._detect_linux_password_store() is None
    with patch("fool_cli.main.subprocess.run", side_effect=FileNotFoundError):
        assert cli_main._detect_linux_password_store() is None
...
def test_gui_linux_packaged_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("fool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("fool_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("fool_cli.main._desktop_build_needed", return_value=True), \
         patch("fool_cli.main._write_desktop_build_stamp"), \
         patch("fool_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("fool_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("fool_cli.config.load_config", return_value={}), \
         patch("fool_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("fool_cli.main._detect_linux_password_store", return_value="gnome-libsecret"), \
         patch("fool_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["FOOL_DESKTOP_PASSWORD_STORE"] == "gnome-libsecret"
...
def test_gui_linux_source_launch_bridges_detected_password_store(tmp_path, monkeypatch):
    ...
    with patch("fool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("fool_cli.config.load_config", return_value={}), \
         patch("fool_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("fool_cli.main._detect_linux_password_store", return_value="kwallet6"), \
         patch("fool_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
    ...
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["FOOL_DESKTOP_PASSWORD_STORE"] == "kwallet6"
...
def test_gui_config_password_store_skips_detection(tmp_path, monkeypatch):
    ...
    with patch("fool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("fool_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("fool_cli.main._detect_linux_password_store") as mock_detect, \
         patch("fool_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
...
def test_gui_explicit_password_store_env_wins_over_config_and_detection(tmp_path, monkeypatch):
    _clear_keychain_env(monkeypatch)
    monkeypatch.setenv("FOOL_DESKTOP_PASSWORD_STORE", "basic")
    root = _make_desktop_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    _make_packaged_executable(root, monkeypatch)
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["FOOL_DESKTOP_PASSWORD_STORE"] == "basic"
...
def test_gui_password_store_bridge_is_linux_only(tmp_path, monkeypatch):
    ...
    mock_detect.assert_not_called()
    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert "FOOL_DESKTOP_PASSWORD_STORE" not in launch_env
