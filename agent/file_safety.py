...
def _hermes_home_path() -> Path:
    """Resolve the active FOOL_HOME (profile-aware) without circular imports."""
    try:
...
def get_safe_write_roots() -> set[str]:
    """Return resolved FOOL_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("FOOL_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside FOOL_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Three categories are blocked:
      ...
      * Internal Hermes cache files under ``FOOL_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under FOOL_HOME and the global Hermes
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # Resolve BOTH the active FOOL_HOME (profile-aware) AND the global
    # Hermes root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (FOOL_HOME points at
...
#
# Hermes profiles are separate FOOL_HOME dirs under
...
# Profile-scoped directories under FOOL_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from FOOL_HOME.
...
#
#   <FOOL_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.hermes/...
#
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    ...
    FOOL_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host FOOL_HOME)
    """
