...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Fool Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'The Fool docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Fool Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'The Fool docs', url: 'https://example.com/docs' }
    ])
  })
})
