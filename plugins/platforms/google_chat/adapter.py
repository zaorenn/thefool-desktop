...
# ``model_tools`` import time, so ``fool status``, ``fool chat``, etc.
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``fool setup``.
    ...
    print_info("Restart the gateway: fool gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``fool cron`` running as a
    separate process from ``fool gateway``).  Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `fool setup` to install Google Chat support.",
