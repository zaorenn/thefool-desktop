...
Profile-aware location: ``$FOOL_HOME/logs/dashboard-auth.log``.
...
from ``fool_constants`` or other fool_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$FOOL_HOME/logs/dashboard-auth.log``.

    Uses ``fool_constants.get_hermes_home()`` (a leaf module — no import
