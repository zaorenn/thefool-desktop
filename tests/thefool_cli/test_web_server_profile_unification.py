...
profile switcher can target any profile's THEFOOL_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via THEFOOL_HOME.
"""
import json
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import thefool_cli.mcp_config as mcp_config
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same THEFOOL_HOME the rest
        ...
        monkeypatch.setattr(
            "thefool_cli.tools_config.valid_post_setup_keys",
