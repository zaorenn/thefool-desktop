...
import pytest
from fool_cli.model_switch import list_authenticated_providers, switch_model
...
def test_list_authenticated_providers_no_duplicate_labels_across_schemas(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.models.fetch_api_models", lambda *a, **k: None)
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("fool_cli.model_switch.resolve_alias", return_value=None), \
         patch("fool_cli.model_switch.list_provider_models", return_value=[]), \
         patch("fool_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("fool_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         patch("fool_cli.models.detect_provider_for_model", return_value=None), \
         patch("fool_cli.model_switch.get_model_info", return_value=None), \
         patch("fool_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("fool_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.models.fetch_api_models", _fake_fetch)
...
def test_current_custom_model_is_surfaced_in_builtin_provider_row(monkeypatch):
    ...
    monkeypatch.setattr(
        "fool_cli.models.cached_provider_model_ids",
