...
def test_set_session_env_sets_contextvars(monkeypatch):
    ...
    monkeypatch.delenv("FOOL_SESSION_SOURCE", raising=False)
    monkeypatch.delenv("FOOL_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("FOOL_SESSION_CHAT_NAME", raising=False)
    monkeypatch.delenv("FOOL_SESSION_CHAT_TYPE", raising=False)
    monkeypatch.delenv("FOOL_SESSION_USER_ID", raising=False)
    monkeypatch.delenv("FOOL_SESSION_USER_NAME", raising=False)
    ...
    assert get_session_env("FOOL_SESSION_PLATFORM") == "telegram"
    assert get_session_env("FOOL_SESSION_SOURCE") == ""
    assert get_session_env("FOOL_SESSION_CHAT_ID") == "-1001"
    assert get_session_env("FOOL_SESSION_CHAT_NAME") == "Group"
    assert get_session_env("FOOL_SESSION_CHAT_TYPE") == "group"
    assert get_session_env("FOOL_SESSION_USER_ID") == "123456"
    assert get_session_env("FOOL_SESSION_USER_NAME") == "alice"
    assert get_session_env("FOOL_SESSION_THREAD_ID") == "17585"

    # os.environ should NOT be touched
    assert os.getenv("FOOL_SESSION_PLATFORM") is None
    assert os.getenv("FOOL_SESSION_SOURCE") is None
    assert os.getenv("FOOL_SESSION_CHAT_TYPE") is None
    assert os.getenv("FOOL_SESSION_THREAD_ID") is None
...
def test_clear_session_env_restores_previous_state(monkeypatch):
    ...
    assert get_session_env("FOOL_SESSION_PLATFORM") == ""
    assert get_session_env("FOOL_SESSION_CHAT_ID") == ""
    assert get_session_env("FOOL_SESSION_CHAT_NAME") == ""
    assert get_session_env("FOOL_SESSION_CHAT_TYPE") == ""
    assert get_session_env("FOOL_SESSION_USER_ID") == ""
    assert get_session_env("FOOL_SESSION_USER_NAME") == ""
    assert get_session_env("FOOL_SESSION_THREAD_ID") == ""
...
def test_get_session_env_falls_back_to_os_environ(monkeypatch):
    ...
    assert get_session_env("FOOL_SESSION_PLATFORM") == "discord"
...
def test_session_key_falls_back_to_os_environ(monkeypatch):
    ...
    monkeypatch.setenv("FOOL_SESSION_KEY", "env-session-123")

    # No contextvar set — should read from os.environ
    assert get_session_env("FOOL_SESSION_KEY") == "env-session-123"

    # Set contextvar — should prefer it
    tokens = set_session_vars(session_key="ctx-session-456")
    assert get_session_env("FOOL_SESSION_KEY") == "ctx-session-456"

    # After clear — should return "" (explicitly cleared), not os.environ (#10304)
    clear_session_vars(tokens)
    assert get_session_env("FOOL_SESSION_KEY") == ""
...
def test_session_key_no_race_condition_with_contextvars(monkeypatch):
    ...
    async def handler(key: str, delay: float):
        ...
        try:
            ...
            read_back = get_session_env("FOOL_SESSION_KEY")
    ...
    try:
        result = await runner._run_in_executor_with_context(
            lambda: {
                ...
                "user_id": get_session_env("FOOL_SESSION_USER_ID"),
...
def test_cron_session_contextvar_preserves_legacy_env_fallback(monkeypatch):
    ...
    monkeypatch.setenv("FOOL_CRON_SESSION", "1")

    assert get_session_env("FOOL_CRON_SESSION") == "1"
...
def test_cron_session_explicit_blank_masks_leaked_env(monkeypatch):
    ...
    try:
        assert get_session_env("FOOL_CRON_SESSION") == ""
    finally:
        clear_session_vars(tokens)
