...
Build the real image and verify at runtime:
  ...
  2. PYTHONDONTWRITEBYTECODE and THEFOOL_DISABLE_LAZY_INSTALLS are set
  3. /opt/hermes/.install_method contains "docker" (code-scoped stamp)
  4. $THEFOOL_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $THEFOOL_HOME is healed (removed) on boot
"""
...
def test_hermes_disable_lazy_installs_and_dont_write_bytecode(
    ...
    THEFOOL_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    assert "ENV_OK" in r.stdout, (
        ...
        f"THEFOOL_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $THEFOOL_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $THEFOOL_HOME was not healed on boot: "
        f"{r.stdout}"
    )
