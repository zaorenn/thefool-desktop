 ...
 *  - no FOOL_DESKTOP_DEV_SERVER → closed (an unpackaged `electron .` against
 *    dist/ is how the packaged app gets smoke tested; it should behave like
 *    the packaged app).
 *  - otherwise                    → open on 9222, or FOOL_DESKTOP_CDP_PORT.
 *
 * `FOOL_DESKTOP_CDP_PORT=off` (or `0` / `false`) opts out for anyone who
...
function resolveDevCdpPort({ env, isPackaged, devServer }: DevCdpInput): DevCdpDecision {
  ...
  const requested = (env.FOOL_DESKTOP_CDP_PORT ?? '').trim()
...
function describeDevCdpDecision(decision: DevCdpDecision): string | null {
  switch (decision.reason) {
    ...
    case 'invalid-port':
      return `FOOL_DESKTOP_CDP_PORT is not a valid port (expected an integer ${MIN_PORT}-${MAX_PORT}, or "off"); renderer debugging is disabled.`

    case 'opted-out':
      return 'renderer debugging disabled by FOOL_DESKTOP_CDP_PORT.'
