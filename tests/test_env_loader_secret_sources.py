"""Tests for the secret-source tracking in ``fool_cli.env_loader``.

These cover the small public surface that lets `fool model` / `fool setup`
...
from fool_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    ...
    hot modules (cli.py, fool_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per FOOL_HOME per process.
    """
    ...
    # cli.py, fool_cli/main.py, run_agent.py, trajectory_compressor.py,
