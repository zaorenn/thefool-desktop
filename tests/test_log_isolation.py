...
`thefool_cli/main.py` calls `setup_logging()` at module scope, which resolves
...
the whole pytest session's logging to `<THEFOOL_HOME>/logs/agent.log`.

If THEFOOL_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets THEFOOL_HOME at module scope for that reason - this
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(thefool_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
...
class TestLogIsolation:
    def test_hermes_home_is_sandboxed_before_imports(self):
        ...
        # fixture has sandboxed THEFOOL_HOME, so reading it here would pass even
        # with the conftest block deleted. Assert the value captured at conftest
        # import, which is the moment that actually matters.
        from tests.conftest import THEFOOL_HOME_AT_CONFTEST_IMPORT as home

        assert home, "conftest must set THEFOOL_HOME before test modules import"
        assert Path(home).resolve() != _real_hermes_home().resolve(), (
            f"THEFOOL_HOME pointed at the operator's real home ({home}) when "
    ...
    def test_importing_the_cli_does_not_target_the_real_logs(self):
        pytest.importorskip("thefool_cli.main")
