"""Tests for fool_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `fool_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import fool_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("fool_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "fool_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from fool_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``fool gateway run``, an agent action, the Open WebUI helper) holds
    the per-FOOL_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root FOOL_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `fool gateway run --replace` (no -p flag).
    assert "fool gateway run --replace" in default_text
...
#
# `fool gateway stop` inside a container dispatches through
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    # Mark the drop so fake rm can refuse unlink outside FOOL_HOME the way
