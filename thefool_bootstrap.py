...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``thefool_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``thefool_bootstrap``, never ``thefool_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    """
    root = src_root or os.environ.get("THEFOOL_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``THEFOOL_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("THEFOOL_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import thefool_bootstrap``
# (or ``from thefool_bootstrap import apply_windows_utf8_bootstrap``) at
