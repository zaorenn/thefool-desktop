...
from fool_cli.goals import judge_goal
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    a worker — i.e. whenever FOOL_KANBAN_* is present but not ours."""
    try:
...
def _reject_delegated_child_mutation(tool_name: str) -> Optional[str]:
    ...
    inherited FOOL_KANBAN_* env vars are not proof of dispatcher ownership.
...
def _check_kanban_mode() -> bool:
    ...
    1. ``FOOL_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    ...
    if _is_delegated_child_context():
        return False
    if os.environ.get("FOOL_KANBAN_TASK") and _is_dispatcher_owned_worker():
        return True
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if not _is_dispatcher_owned_worker():
        ...
        return None
    env_tid = os.environ.get("FOOL_KANBAN_TASK")
    return env_tid or None


def _worker_run_id(task_id: str) -> Optional[int]:
    """Return this worker's dispatcher run id when it is scoped to task_id."""
    if os.environ.get("FOOL_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("FOOL_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``FOOL_KANBAN_TASK`` set
    ...
    ``FOOL_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``FOOL_KANBAN_DB`` → ``FOOL_KANBAN_BOARD`` env → current symlink
...
#   - No-op outside dispatcher-spawned worker context (no ``FOOL_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``FOOL_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``FOOL_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``FOOL_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("FOOL_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("FOOL_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("FOOL_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    (``FOOL_KANBAN_TASK`` set) and ``agent`` exposes ``steer``. Returns True
    ...
    by ``FOOL_PROFILE``) are skipped to avoid echoing itself.
    """
    tid = os.environ.get("FOOL_KANBAN_TASK")
    ...
    own = (os.environ.get("FOOL_PROFILE") or "").strip()
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set FOOL_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # FOOL_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = os.environ.get("FOOL_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        with httpx.stream(
            "GET",
            current_url,
            headers={"User-Agent": "fool-kanban/attach"},
            timeout=30,
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("FOOL_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets FOOL_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    # Prefer the request-scoped api_server origin binding: FOOL_SESSION_ID
    ...
    session_id = (
        ...
        or os.environ.get("FOOL_SESSION_ID")
    )
    ...
    try:
        ...
        try:
            ...
            if _inherit_project and project_id is None:
                _self_tid = os.environ.get("FOOL_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("FOOL_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``FOOL_SESSION_PLATFORM``,
      ``FOOL_SESSION_CHAT_ID``, and ``FOOL_SESSION_CHAT_TYPE`` are set in
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``FOOL_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # FOOL_SESSION_KEY into the subprocess env. Treat that
            # as a "tui" subscription so the TUI notification poller
            # (tui_gateway/server.py) can pick it up.
            #
            # FOOL_SESSION_ID is intentionally NOT a fallback here:
            ...
            # poller keys on FOOL_SESSION_KEY.
            session_key = (
                get_session_env("FOOL_SESSION_KEY", "")
                or os.environ.get("FOOL_SESSION_KEY", "")
            )
            if not session_key:
        ...
        chat_type = get_session_env("FOOL_SESSION_CHAT_TYPE", "") or None
        delivery_mode = "notify+wake" if is_gateway_session else None
        thread_id = get_session_env("FOOL_SESSION_THREAD_ID", "") or None
        user_id = get_session_env("FOOL_SESSION_USER_ID", "") or None
        user_id_alt = get_session_env("FOOL_SESSION_USER_ID_ALT", "") or None
        message_id = get_session_env("FOOL_SESSION_MESSAGE_ID", "") or ""
        notifier_profile = (
            get_session_env("FOOL_SESSION_PROFILE", "")
            or os.environ.get("FOOL_PROFILE")
        )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to FOOL_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: FOOL_KANBAN_DB env → "
    "FOOL_KANBAN_BOARD env → the 'current' symlink under the kanban "
...
KANBAN_ATTACH_URL_SCHEMA = {
    ...
    "description": (
        "Attach a file to a task by URL — The Fool downloads it server-side "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to FOOL_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
