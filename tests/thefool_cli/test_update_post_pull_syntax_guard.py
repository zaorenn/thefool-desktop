...
(e.g. orphan merge-conflict markers in ``thefool_cli/config.py``), the CLI
...
markers in ``thefool_cli/config.py``; users who ran ``hermes update`` in
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except thefool_constants.py
    for relpath in hermes_main._UPDATE_CRITICAL_FILES:
        if relpath == "thefool_constants.py":
            continue
