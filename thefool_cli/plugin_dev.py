...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary THEFOOL_HOME
    ...
    stack.enter_context(
        patch.dict(
            os.environ,
            {
                ...
                "THEFOOL_BUNDLED_PLUGINS": str(bundled),
                "THEFOOL_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from thefool_cli.plugins import SUPPORTED_MANIFEST_VERSION
    ...
    if schema:
        from thefool_cli.plugins import _CONFIG_SCHEMA_TYPES
