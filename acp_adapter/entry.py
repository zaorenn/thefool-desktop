...
def _load_env() -> None:
    """Load .env from THEFOOL_HOME (default ``~/.hermes``)."""
...
def _print_version() -> None:
    from thefool_cli import __version__ as hermes_version
...
def _run_setup() -> None:
    from thefool_cli.main import main as hermes_main
...
def main(argv: list[str] | None = None) -> None:
    ...
    if os.environ.get("THEFOOL_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
