...
# Python-driven tool session (e.g. The Fool terminal tool). A pre-set PYTHONPATH
...
if [ -n "${FOOL_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$FOOL_INSTALL_DIR"
...
#   data still at /root/.hermes (FOOL_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$FOOL_HOME/.no-bundled-skills so future"
            echo "                   'fool update' runs never inject bundled skills either"
...
# intentional, but they leave the checkout dirty — which forces `fool update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (The Fool-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download The Fool","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install fool command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `fool` command
...
# Point a The Fool-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $FOOL_HOME/node/bin — which is NOT on
...
# The Fool's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no The Fool-managed npm, so calling it on
...
configure_managed_node_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$FOOL_HOME/node/etc/npmrc"
}

get_hermes_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/fool" ]; then
        echo "$link_dir/fool"
    else
        echo "fool"
    fi
...
install_uv() {
    ...
    # The Fool owns its own uv at $FOOL_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (fool_cli/managed_uv.py) looks in the same
    # place, so install.sh and `fool update` stay in sync.
    local _managed_uv="$FOOL_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $FOOL_HOME/bin ..."
    mkdir -p "$FOOL_HOME/bin"
    ...
    # directly into $FOOL_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$FOOL_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the The Fool-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing The Fool-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing The Fool-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a The Fool-managed Node from a previous run over a too-old system one.
    if [ -x "$FOOL_HOME/node/bin/node" ] && [ -x "$FOOL_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$FOOL_HOME/node/bin/node" --version)"; then
        export PATH="$FOOL_HOME/node/bin:$PATH"
        log_success "Node.js $("$FOOL_HOME/node/bin/node" --version) found (The Fool-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing The Fool-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (The Fool requires Node >=26) — installing The Fool-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    # the fool command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$FOOL_HOME/node"
    mkdir -p "$FOOL_HOME"
    mv "$extracted_dir" "$FOOL_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$FOOL_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$FOOL_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$FOOL_HOME/node/bin/npx"  "$node_link_dir/npx"
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. The Fool install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "The Fool itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `fool update` path
                # (#4735).
            ...
            # cannot succeed — mirror ``fool update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if The Fool behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up fool command..."
    ...
    if [ ! -x "$FOOL_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$FOOL_ENTRYPOINT" ]; }; then
        log_warn "The Fool launcher prerequisites not found"
    ...
    # Create a user-facing shim for the fool command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $FOOL_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$FOOL_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/fool"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/fool" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$FOOL_BIN" "$FOOL_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/fool" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    rm -f "$command_link_dir/fool-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/fool-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$FOOL_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/fool-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$FOOL_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/fool-agent"
    log_success "Installed hermes-agent launcher → $command_link_display_dir/fool-agent"
    ...
    # this launcher those hosts report The Fool as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/fool-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/fool-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$FOOL_BIN" "$FOOL_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/fool-acp" <<EOF
#!/usr/bin/env bash
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# The Fool — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# The Fool — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
...
copy_config_templates() {
    ...
    # This MUST match DEFAULT_SOUL_MD in fool_cli/default_soul.py — the
...
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        # skills_sync.py and `fool update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            > "$FOOL_HOME/.no-bundled-skills" 2>/dev/null || true
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# The Fool browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "The Fool will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# The Fool browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `fool update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `fool tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$FOOL_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside The Fool' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$FOOL_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'fool tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `fool tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `fool computer-use install` on their own).
    ...
    # Same upstream installer `fool computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the The Fool install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching The Fool'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'fool tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: fool computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'fool setup' after install."
        return 0
    fi
    ...
    # Run fool setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m fool_cli.main setup < /dev/tty
    else
        python -m fool_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for The Fool to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$FOOL_HOME/whatsapp/session/creds.json"
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'fool gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $FOOL_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $FOOL_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: fool gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: fool gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
        ...
        log_info "Skipped. Start the gateway later with: fool gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.hermes-bootstrap-complete, which tells the The Fool
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $FOOL_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $FOOL_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $FOOL_HOME/cron/, sessions/, logs/"
    ...
    echo -e "   ${GREEN}fool gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}fool update${NC}       Update to latest version"
    echo ""
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$FOOL_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$FOOL_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$FOOL_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    # which every consumer (tools/browser_tool.py, `fool update`'s npx
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$FOOL_HOME/node" --silent --ignore-scripts \
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the The Fool-managed Node to PATH, so
    ...
    if [ "$OS" = "linux" ]; then
        ...
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/The Fool.app" \
            "$desktop_dir/release/mac/The Fool.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # importing Python cannot reverse it by loading FOOL_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if FOOL_HOME="$FOOL_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from fool_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `fool update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the The Fool-managed Node
            # provisioned during prerequisites/node-deps (at $FOOL_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $FOOL_HOME.
    # $FOOL_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'fool update' on this host install.
