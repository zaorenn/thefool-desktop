...
class TestNamedProfileHintIntegration:
    ...
    ``FOOL_HOME`` and no resolver mocks.
    """
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """FOOL_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".hermes"
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "FOOL_AGENT_HELP_GUIDANCE", "HELP")
    ...
    with (
        ...
        patch("fool_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("fool_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under FOOL_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from fool_cli import config as _cfgmod
