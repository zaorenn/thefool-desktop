...
> Run `fool gateway setup` and pick **Teams Meetings** for a guided walk-through.
...
Operator actions stay in the CLI (the `teams-pipeline` subcommand is registered by the `teams_pipeline` plugin — enable it via `fool plugins enable teams_pipeline` or set `plugins.enabled: [teams_pipeline]` in `config.yaml`):
