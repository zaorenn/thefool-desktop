"""Guards for fool_cli._startup_fast — the pre-import version fast path.
...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from fool_cli.config import ...`` silently makes `hermes --version`
...
def test_startup_fast_import_weight():
    ...
    assert not offenders, (
        f"fool_cli._startup_fast imported heavy modules: {offenders} — "
...
def _run_version(env_overrides: dict) -> subprocess.CompletedProcess:
    ...
    env.pop("FOOL_DEV", None)
    return subprocess.run(
        [sys.executable, "-m", "fool_cli.main", "--version"],
        capture_output=True,
        text=True,
        timeout=60,
        cwd=REPO_ROOT,
...
def test_fast_version_parity_off_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version({"FOOL_HOME": str(home), "TERMUX_VERSION": ""})
    assert result.returncode == 0, result.stderr
    out = result.stdout
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version(
        {"FOOL_HOME": str(home), "TERMUX_VERSION": "0.118"}
    )
    assert result.returncode == 0, result.stderr
