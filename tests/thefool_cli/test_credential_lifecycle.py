...
THEFOOL_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
def hermes_home(monkeypatch, tmp_path):
    """Fresh THEFOOL_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("THEFOOL_HOME", str(home))
    from thefool_cli.config import invalidate_env_cache
