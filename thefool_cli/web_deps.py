...
Why this exists
---------------
``thefool_cli/web_server.py`` owns all dashboard runtime state: the ephemeral
...
``APIRouter`` modules under ``thefool_cli/web_routers/`` need those helpers, but
...
thin proxy that resolves ``thefool_cli.web_server.<name>`` *at call time*.  This
...
def _server():
    """Return the live ``thefool_cli.web_server`` module (imported on demand)."""
    mod = sys.modules.get("thefool_cli.web_server")
    if mod is None:  # pragma: no cover - routers are only mounted by web_server
        import thefool_cli.web_server as mod  # type: ignore[no-redef]
    return mod
