...
def _run_switch(monkeypatch, result, cmd="/model MiniMax-M3 --global"):
    ...
    monkeypatch.setattr("fool_cli.model_switch.switch_model", lambda **kw: result)
    monkeypatch.setattr(
        "fool_cli.inventory.load_picker_context",
        lambda: (_ for _ in ()).throw(RuntimeError("no picker context in test")),
    )
    cli_mod.FoolCLI._handle_model_switch(_StubCLI(), cmd)
    return saved
...
def test_session_only_switch_does_not_touch_config(monkeypatch):
    ...
    monkeypatch.setattr("fool_cli.model_switch.switch_model", lambda **kw: _make_result())
    monkeypatch.setattr(
        "fool_cli.inventory.load_picker_context",
        lambda: (_ for _ in ()).throw(RuntimeError("no picker context in test")),
    )

    cli_mod.FoolCLI._handle_model_switch(_StubCLI(), "/model MiniMax-M3 --session")
...
def _run_apply(monkeypatch, result, persist_global=True):
    ...
    monkeypatch.setattr(cli_mod, "save_config_value", _fake_save)
    cli_mod.FoolCLI._apply_model_switch_result(_StubCLI(), result, persist_global)
    return saved
