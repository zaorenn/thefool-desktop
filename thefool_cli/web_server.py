...
Usage:
    python -m thefool_cli.main web          # Start on http://127.0.0.1:9119
    python -m thefool_cli.main web --port 8080
"""
...
from thefool_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
WEB_DIST = Path(os.environ["THEFOOL_WEB_DIST"]) if "THEFOOL_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    real gateway on the same THEFOOL_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    ...
    ``thefool_cli.gateway``. But the first WS connection and its initial
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from thefool_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    """
    try:
        from thefool_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import thefool_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # Desktop-spawned backends (THEFOOL_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hermes
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("THEFOOL_DESKTOP") == "1":
        ...
        try:
            from thefool_cli.gateway import _reap_unsupervised_gateway_orphans
...
from thefool_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# THEFOOL_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("THEFOOL_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
...
#
# This list is defined in ``thefool_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from thefool_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from thefool_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from thefool_cli.dashboard_auth.token_auth import token_auth_middleware
...
from thefool_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from thefool_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from thefool_cli.model_normalize import normalize_model_for_provider
    from thefool_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from thefool_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    """
    from thefool_state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "THEFOOL_DASHBOARD_FILES_ROOT"
...
# THEFOOL_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to THEFOOL_HOME.
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no THEFOOL_HOME resolution, so it blocks these trees wherever
...
def _default_hermes_root_is_opt_data() -> bool:
    raw = os.environ.get("THEFOOL_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # (the container/hosted layout) or when THEFOOL_DASHBOARD_FILES_ROOT is set.
    if _default_hermes_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    ``THEFOOL_HOME/images/`` — the same directory ``clipboard.paste`` /
...
# gate + path hardening as /api/fs). Logic lives in ``thefool_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from thefool_cli import web_git as _web_git  # noqa: E402
...
from thefool_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from thefool_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from thefool_cli.profiles import _check_gateway_running, profiles_to_serve
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors thefool_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        # THEFOOL_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of thefool_cli.gateway blocks the
        ...
        try:
            from thefool_cli.auth import get_nous_session_validity
        ...
        # disk-exhaustion incidents). One statvfs call on THEFOOL_HOME's
        ...
        try:
            from thefool_state import SessionDB as _SDB
            from thefool_constants import get_hermes_home as _ghh
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from thefool_cli.auth import get_nous_auth_status_local
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from thefool_cli.debug import build_debug_share
...
def _spawn_hermes_action(
    ...
    *,
    ...
    Uses the running interpreter's ``thefool_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "thefool_cli.main", *subcommand]
    ...
    action_env = {**os.environ, "THEFOOL_NONINTERACTIVE": "1"}
    ...
    action_id = (env_overrides or {}).get("THEFOOL_ACTION_ID")
    if action_id:
...
async def gateway_drain(request: Request):
    ...
    active (no ``THEFOOL_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_hermes():
    ...
    try:
        proc = _spawn_hermes_action(
            ["update"],
            "hermes-update",
            env_overrides={"THEFOOL_ACTION_ID": action_id},
        )
    except Exception as exc:
...
async def check_hermes_update(force: bool = False):
    ...
    try:
        from thefool_cli.banner import check_for_updates
...
from thefool_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from thefool_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from thefool_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from thefool_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # (THEFOOL_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
# in thefool_cli/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    """
    try:
        from thefool_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    try:
        ...
        from thefool_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from thefool_cli.moa_config import normalize_moa_config, validate_moa_payload
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (thefool_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from thefool_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from thefool_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from thefool_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from thefool_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from thefool_cli.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level THEFOOL_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from thefool_cli.credential_lifecycle import remove_provider_env_credential
...
# thefool_cli/setup_hidden_env.py. Shared with the `hermes setup gateway`
# wizard so the surfaces ask for the same things.
from thefool_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # THEFOOL_HOME contextvar override (#56986 / #69143), so the profile's
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the THEFOOL_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized THEFOOL_HOME)
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from thefool_cli import auth as hauth
        if provider_id == "nous":
    ...
    def _run():
        with _profile_scope(profile):
            ...
            if provider_id == "anthropic":
                ...
                try:
                    from thefool_cli.auth import clear_provider_auth
            ...
            try:
                from thefool_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("THEFOOL_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from thefool_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from thefool_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from thefool_cli.auth import _save_codex_tokens
...
def _session_db_read_probe_statements() -> tuple:
    ...
    """
    from thefool_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from thefool_state import SessionDB, is_malformed_db_error
...
async def get_logs(
    ...
    from thefool_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from thefool_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (THEFOOL_HOME already
    ...
    THEFOOL_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, THEFOOL_HOME)."""
    from thefool_cli import profiles as profiles_mod
...
from thefool_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from thefool_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/thefool_cli/
        # test_config_read_guard.py). The THEFOOL_HOME override scopes
...
#
# Wraps the same config data layer the CLI uses (thefool_cli.mcp_config), so
...
def _normalize_mcp_server_create(
    ...
    from thefool_cli.mcp_security import validate_mcp_server_entry
...
from thefool_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from thefool_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from thefool_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
#
# Wraps the same JSON store the CLI uses (thefool_cli.webhook); the webhook
...
async def list_webhooks():
    import thefool_cli.webhook as wh
...
async def list_credential_pool():
    ...
    from thefool_cli.auth import read_credential_pool
...
from thefool_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from thefool_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local THEFOOL_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local THEFOOL_HOME override (same mechanism as
    ...
    """
    from thefool_constants import set_hermes_home_override, reset_hermes_home_override
    from thefool_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the THEFOOL_HOME override. Returns the
    number of skills newly disabled.
    """
    from thefool_constants import set_hermes_home_override, reset_hermes_home_override
    from thefool_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    imported the modules before a THEFOOL_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
from thefool_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from thefool_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
if sys.platform.startswith("win"):
    try:
        from thefool_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from thefool_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from thefool_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from thefool_cli.dashboard_auth.audit import AuditEvent, audit_log
        from thefool_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``THEFOOL_TUI_RESUME`` env var —
    matching what ``thefool_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``THEFOOL_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``THEFOOL_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``THEFOOL_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``THEFOOL_HOME`` at the profile dir in the child env. Every spawned
    ...
    ``THEFOOL_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from thefool_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # An explicit profile scope below still overrides THEFOOL_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    env.setdefault("THEFOOL_TUI_DISABLE_MOUSE", "1")
    env.setdefault("THEFOOL_TUI_INLINE", "1")
    ...
    env["THEFOOL_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["THEFOOL_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["THEFOOL_TUI_RESUME"] = resume

    if sidecar_url:
        env["THEFOOL_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile THEFOOL_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["THEFOOL_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``THEFOOL_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("THEFOOL_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from thefool_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from thefool_cli.console_engine import HermesConsoleEngine
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("THEFOOL_TUI_RESUME") or raw_resume
...
# THEFOOL_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`thefool_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from thefool_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    _headless = os.environ.get("THEFOOL_SERVE_HEADLESS") == "1"
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``THEFOOL_HOME/dashboard-themes/<name>.yaml``), inject a minimal
...
def _discover_user_themes() -> list:
    ...
    live under the server's own ``THEFOOL_HOME``.
    """
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as thefool_cli.plugins):
    1. User plugins:    ~/.hermes/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.hermes/plugins/  (only if THEFOOL_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from thefool_cli.plugins import get_bundled_plugins_dir
    ...
    # THEFOOL_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``THEFOOL_HOME=<root>/profiles/<name>``), the launch home is the
    ...
    # ``THEFOOL_HOME`` unchanged when it *is* the root), mirroring how
    # ``thefool_cli.plugins`` resolves plugin install locations. The
    ...
    # ``thefool_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("THEFOOL_ENABLE_PROJECT_PLUGINS"):
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from thefool_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from thefool_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from thefool_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from thefool_cli.plugins_cmd import dashboard_remove_user_plugin
...
from thefool_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``THEFOOL_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("THEFOOL_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``THEFOOL_PARENT_PID`` is the Electron Desktop PID, not necessarily this
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``THEFOOL_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("THEFOOL_PARENT_PID")
    start_marker = os.environ.get("THEFOOL_PARENT_START_MARKER")
    nonce = os.environ.get("THEFOOL_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("THEFOOL_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``THEFOOL_SERVE_HEADLESS``), so
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing THEFOOL_DASHBOARD_OAUTH_CLIENT_ID).
            ...
            try:
                ...
                from thefool_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
    ...
    # THEFOOL_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("THEFOOL_DESKTOP") == "1":
                try:
                    from thefool_cli.dashboard_procs import (
            ...
            # for standalone `hermes serve` (no THEFOOL_PARENT_PID env).
            ...
            ready_token = "THEFOOL_BACKEND_READY" if headless else "THEFOOL_DASHBOARD_READY"
