...
Verifies:
  - Tools are gated on THEFOOL_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `hermes chat` sessions (no THEFOOL_KANBAN_TASK) must have
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: THEFOOL_HOME isolated, THEFOOL_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("THEFOOL_HOME", str(home))
    monkeypatch.setenv("THEFOOL_PROFILE", "test-worker")
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("THEFOOL_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated THEFOOL_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to THEFOOL_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``THEFOOL_PROFILE`` so a worker can't forge a comment
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from THEFOOL_PROFILE in the fixture, not the
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("THEFOOL_HOME", str(home))
    monkeypatch.setenv("THEFOOL_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# ---------------------------------------------------------------------------
#
# A worker process has THEFOOL_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no THEFOOL_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # THEFOOL_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no THEFOOL_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via THEFOOL_KANBAN_BOARD env var,
...
def multi_board_env(monkeypatch, tmp_path):
    ...
    THEFOOL_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("THEFOOL_HOME", str(home))
    # Make sure neither THEFOOL_KANBAN_DB nor THEFOOL_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("THEFOOL_KANBAN_DB", raising=False)
    monkeypatch.delenv("THEFOOL_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("THEFOOL_KANBAN_TASK", raising=False)
    monkeypatch.setenv("THEFOOL_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: THEFOOL_SESSION_PLATFORM + THEFOOL_SESSION_CHAT_ID set.
# - TUI sessions: THEFOOL_SESSION_KEY (or THEFOOL_SESSION_ID) set, with
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("THEFOOL_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("THEFOOL_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("THEFOOL_SESSION_USER_ID", "user-9")
    monkeypatch.setenv("THEFOOL_SESSION_USER_ID_ALT", "alt-user-9")
    monkeypatch.setenv("THEFOOL_SESSION_CHAT_TYPE", "forum")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports THEFOOL_SESSION_KEY.
    We should still auto-subscribe, with platform='tui' and
    chat_id=<key>."""
    from tools import kanban_tools as kt
    monkeypatch.delenv("THEFOOL_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("THEFOOL_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("THEFOOL_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("THEFOOL_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("THEFOOL_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("THEFOOL_SESSION_ID", raising=False)
...
def test_create_does_not_subscribe_in_cli_session(monkeypatch, worker_env):
    ...
    monkeypatch.delenv("THEFOOL_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("THEFOOL_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("THEFOOL_SESSION_KEY", raising=False)
    monkeypatch.delenv("THEFOOL_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    monkeypatch.setenv("THEFOOL_HOME", str(home))
    monkeypatch.setenv("THEFOOL_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("THEFOOL_SESSION_CHAT_ID", "channel-1")
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting THEFOOL_ALLOW_PRIVATE_URLS on a private network.
...
def default_url_guard(monkeypatch):
    ...
    Clears THEFOOL_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
