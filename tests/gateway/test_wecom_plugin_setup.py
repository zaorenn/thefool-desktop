...
``fool_cli.config`` (get_env_value / save_env_value / remove_env_value),
``fool_cli.cli_output`` (prompt / prompt_yes_no / print_*), and
``fool_cli.setup`` (prompt_choice); we patch each at its source module so
