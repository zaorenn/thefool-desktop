...
_HERMES_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (FOOL_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `fool tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (FOOL_KANBAN_TASK env "
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in fool_cli/tools_config.py).
        "posture": True,
    },
    ...
    # the `fool send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "hermes-cron": {
        ...
        # core tools users see interactively — then `fool tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as hermes-cli; gated by `fool tools`",
        "tools": _HERMES_CORE_TOOLS,
        "includes": []
    },
