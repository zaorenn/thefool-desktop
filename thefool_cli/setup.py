...
from thefool_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from thefool_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def is_noninteractive() -> bool:
    ...
    ``THEFOOL_NONINTERACTIVE=1`` (see ``thefool_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("THEFOOL_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``THEFOOL_NONINTERACTIVE=1`` or a closed/redirected
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        from thefool_cli.auth import resolve_provider
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from thefool_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from thefool_cli.main import select_provider_and_model
    try:
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into THEFOOL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("THEFOOL_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # THEFOOL_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from thefool_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _profile_name_from_hermes_home(hermes_home) -> str | None:
    """Return the active profile name when THEFOOL_HOME is a profile dir."""
...
def _setup_qqbot():
    ...
    from thefool_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from thefool_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``thefool_cli.auth`` — lists every supported
    ...
    """
    try:
        from thefool_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from thefool_cli.config
    ...
    if section_key == "model":
        ...
        from thefool_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from thefool_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from thefool_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if not (migration_ran and _skip_configured_section(config, "gateway", "Messaging Platforms")):
        ...
        from thefool_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from thefool_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
