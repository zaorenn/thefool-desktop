...
- **Anthropic** — Claude models (direct API, OAuth via `fool auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `fool model` or by editing `~/.hermes/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
Yes. Run `fool model`, select **Custom endpoint**, and enter your server's URL:
...
**Solution:** Exit your session and use `fool model` from your terminal to add new providers:
...
After adding a new provider via `fool model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `fool model` (from terminal) |
| Enter/change API keys | `fool model` (from terminal) |
...
# Or set directly
fool config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```
...
# Set a valid model
fool config set FOOL_MODEL anthropic/claude-opus-4.7

# Or specify per-session
fool chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `fool chat --provider <alternative>` to route to a different backend
...
fool chat --model openrouter/google/gemini-3-flash-preview
```
...
- Or switch to the terminal interface for administrative tasks: `fool chat`
...
**Solution:**
- Verify your bot token is valid with `fool gateway setup`
...
#### WSL: Gateway keeps disconnecting or `fool gateway start` fails
...
tmux new -s hermes 'fool gateway run'
# Reattach later: tmux attach -t hermes

# Option 3: Background via nohup
nohup fool gateway run > ~/.hermes/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'fool gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `fool gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
fool gateway install    # Re-snapshots your current PATH
fool gateway start      # Detects the updated plist and reloads
```
...
**Solution:**
- Try a faster/smaller model: `fool chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `fool chat -t "terminal"`
...
fool config show | grep -A 12 mcp_servers
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `fool profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.
...
### What happens when I run `fool update`?

`fool update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `fool update` once — it covers every profile on the machine.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `fool skills config` settings don't seem to take effect.

**Solution:** Use `fool skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`fool gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
4. On the new machine, run `fool setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
fool profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
fool profile import ./work-backup.tar.gz work
```
...
### `fool backup` vs `fool profile export`

| Feature | `fool backup` | `fool profile export` |
| :--- | :--- | :--- |
...
:::tip
`fool backup` produces a consistent snapshot even while Hermes is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
fool config show | head -20

# Re-run model selection
fool model

# Or test with a known-good model
fool chat -q "hello" --model anthropic/claude-opus-4.7
```
