...
from fool_cli.setup import setup_agent_settings
...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    .env `FOOL_MAX_ITERATIONS` fallback was removed because it was
    ...
    monkeypatch.setattr("fool_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("fool_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("fool_cli.setup.save_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("fool_cli.setup.remove_env_value", lambda *args, **kwargs: None)
    monkeypatch.setattr("fool_cli.setup.save_config", lambda *args, **kwargs: None)
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "fool_cli.setup.get_env_value",
        lambda key: "60" if key == "FOOL_MAX_ITERATIONS" else "",
    )
    monkeypatch.setattr("fool_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("fool_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("fool_cli.setup.save_env_value", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr(
        "fool_cli.setup.remove_env_value",
    ...
    assert "FOOL_MAX_ITERATIONS" in removed_keys
