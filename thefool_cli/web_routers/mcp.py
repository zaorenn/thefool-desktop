...
web_server - reached via the late-binding seam in :mod:`thefool_cli.web_deps`
...
from thefool_cli.web_deps import late, LateState
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from thefool_cli.mcp_config import _replace_mcp_servers
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the THEFOOL_HOME
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds THEFOOL_HOME-derived paths in the child.
