...
* Stateless request/response adapters (the API server,
  ...
  ``X-Fool-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Fool-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Fool-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Fool-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "X-Fool-Session-Id is rejected (403) on an unauthenticated API "
