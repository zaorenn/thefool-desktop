...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains thefool_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'thefool_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    try:
        from thefool_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from thefool_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``THEFOOL_HOME`` and ``THEFOOL_GIT_BASH_PATH``
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    ...
    # Also match if THEFOOL_HOME was customised to somewhere else — find-and-nuke
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete THEFOOL_HOME and THEFOOL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("THEFOOL_HOME", "THEFOOL_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its THEFOOL_HOME directory.
    ...
    current THEFOOL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m thefool_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "thefool_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's THEFOOL_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    userData dir — nothing under ``$THEFOOL_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from thefool_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
...
def run_uninstall(args):
    ...
    # offer to clean them up too instead of leaving zombie THEFOOL_HOMEs
    ...
    # their alias wrappers, and wiping their THEFOOL_HOME dirs. Otherwise
...
def _perform_uninstall(
    *,
    ...
    wipe ``$THEFOOL_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        log_info("Removing THEFOOL_HOME / THEFOOL_GIT_BASH_PATH User env vars...")
    ...
    #     but the packaged app + Electron userData live OUTSIDE THEFOOL_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from thefool_cli.gui_uninstall import uninstall_gui
    ...
    #     under THEFOOL_HOME but they're install tooling, not config — safe to
    ...
    if full_uninstall:
        ...
        #     alias wrapper. The profile THEFOOL_HOME dirs live under
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m thefool_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import thefool_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``thefool_constants`` + ``thefool_cli.colors``
    (and lazily ``thefool_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m thefool_cli.uninstall")
    parser.add_argument(
        "--mode",
