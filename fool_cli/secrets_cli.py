"""CLI handlers for ``fool secrets bitwarden ...``.
...
# that process is ``fool update``, the self-lock preflight detects the
# loaded native module and defers (#86781).  Keep the backend import lazy:
# this module is registered parse-time from ``fool_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``fool_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import fool_cli.secrets_cli``
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every The Fool process."
    )
    console.print(
        "  Status:  [cyan]fool secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]fool secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]fool secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]fool secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — The Fool will skip BSM "
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `fool secrets bitwarden setup` to pick a "
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "run `fool secrets bitwarden setup` (or set "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`fool secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]fool[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]fool secrets bitwarden setup[/cyan] and pick EU or "
