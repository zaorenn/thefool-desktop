...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("thefool_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("thefool_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch("thefool_cli.profiles.get_active_profile_name", return_value=names[0] if names else "default"),
    ]
...
def test_decompose_fanout_false_invalid_llm_assignee_uses_default(kanban_home):
    ...
    try:
        with _patch_aux_client(llm_payload), _patch_extra_body(), patch(
            "thefool_cli.kanban_decompose._load_config",
