...
from fool_cli import config as hermes_config
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "fool_cli.memory_setup._install_dependencies",
...
def test_reload_updated_runtime_modules_restores_new_hermes_constants_symbol(monkeypatch):
    ...
    monkeypatch.delattr(fool_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(fool_constants, "apply_subprocess_home_env")

    hermes_main._reload_updated_runtime_modules()

    assert callable(fool_constants.apply_subprocess_home_env)
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``fool update``'s ``git stash push --include-untracked`` does not sweep it
