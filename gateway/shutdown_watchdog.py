...
2. An event-loop heartbeat file at ``<FOOL_HOME>/state/gateway.heartbeat`` so
...
def _process_hermes_home() -> Path:
    """FOOL_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<FOOL_HOME>/state/gateway.heartbeat``."""
