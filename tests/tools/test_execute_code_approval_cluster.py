...
def gw_session(monkeypatch):
    """A clean gateway session: THEFOOL_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "THEFOOL_HOME": "/h", "THEFOOL_PROFILE": "p",
        "THEFOOL_CONFIG": "/c.yaml", "THEFOOL_ENV": "/e",
        "THEFOOL_DELEGATED_CHILD_CONTEXT": "1",
        # other HERMES_* → dropped (broad prefix removed)
        "THEFOOL_BASE_URL": "https://x", "THEFOOL_INTERACTIVE": "1",
        "THEFOOL_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "THEFOOL_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "THEFOOL_BASE_URL", "THEFOOL_INTERACTIVE", "THEFOOL_KANBAN_DB",
