...
# httpx is imported lazily: it costs ~30ms at import time and fool_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("fool_cli.auth.httpx.Client", ...) keeps working in tests.
...
from fool_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
def get_anthropic_key() -> str:
    ...
    """
    from fool_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from fool_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``fool auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `fool model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("FOOL_OAUTH_TRACE", "").strip().lower()
...
def _auth_file_path() -> Path:
    ...
    # Seat belt: if pytest is running and FOOL_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch FOOL_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set FOOL_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom FOOL_HOME that is not a profile).
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``fool auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``fool auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``fool auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    if pconfig is None:
        from fool_cli.providers import get_provider
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `fool logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from fool_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'fool doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'fool model' for available providers, or run 'fool doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `fool auth add openrouter`
    ...
    # `fool auth list` showing the credential while requests go out with no
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``FOOL_PORTAL_BASE_URL`` /
    ...
    ``FOOL_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("FOOL_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: fool auth add nous",
        provider="nous",
        code=reason,
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("FOOL_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `fool model` walks users into a broken Qwen setup flow.
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("FOOL_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set FOOL_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("FOOL_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("FOOL_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("FOOL_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `fool auth spotify` again.",
            provider="spotify",
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `fool auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `fool auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    # Persist so subsequent `fool auth spotify` runs skip the wizard.
    save_env_value("FOOL_SPOTIFY_CLIENT_ID", raw)
...
def login_spotify_command(args) -> None:
    ...
    # with "FOOL_SPOTIFY_CLIENT_ID is required".
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `fool auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `fool auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `fool auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `fool auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``fool setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``fool auth add openai-codex``
      ...
      ``fool auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    ...
    with _auth_store_lock():
        ...
        # ``fool auth add openai-codex`` created (which must not be
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    if response.status_code == 429:
        ...
        # "run fool auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `fool auth` to re-authenticate."
            )
            relogin_required = True
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("FOOL_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("FOOL_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `fool model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `fool model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `fool model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `fool model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.hermes/auth.json even when FOOL_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``fool auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `fool model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``FOOL_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `fool model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # with a clear error so the user can re-run `fool model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``fool model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("FOOL_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("FOOL_XAI_BASE_URL", "").strip().rstrip("/")
...
#
# File lives at ${FOOL_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
...
# It is OUTSIDE any named profile's FOOL_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``FOOL_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<hermes-root>/shared/``, where ``<hermes-root>`` is what
    :func:`fool_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.hermes/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``FOOL_HOME`` deployments at
    ``<FOOL_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("FOOL_SHARED_AUTH_DIR", "").strip()
    if override:
...
def _nous_shared_store_path() -> Path:
    ...
    # state. Tests must set FOOL_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set FOOL_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No FOOL_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        # FOOL_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``fool auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``fool auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `fool auth add nous --type oauth`. Best-
    ...
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("FOOL_PORTAL_BASE_URL")
...
# ~350ms even on the failure path, and read-only UI surfaces (`fool tools`, status panels,
# subscription-feature checks) call it many times per render — `fool tools` → "All Platforms"
...
# `fool auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `fool auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `fool auth` and
    # `fool model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("FOOL_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("FOOL_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``fool doctor`` runs the live
    ...
    try:
        from fool_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                ...
                info["hint"] = (
                    ...
                    "is skipped here. Run `fool doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set FOOL_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from fool_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `fool logout` historically keyed off auth.json.active_provider only.
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`fool_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (fool_cli.model_selection_guards): the cost
    ...
    try:
        ...
        from fool_cli.model_search import model_search_text
    ...
    from fool_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from fool_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'fool model' or 'fool setup' instead."""
    print("The 'fool login' command has been removed.")
    print("Use 'fool auth' to manage credentials,")
    print("'fool model' to select a provider, or 'fool setup' for full setup.")
    raise SystemExit(0)
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("FOOL_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _login_xai_oauth(
    ...
    *,
    ...
    # xAI credential re-enabled. ``fool auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``fool auth list`` would show
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `fool model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``fool status`` that just want to know
    ...
    *,
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `fool model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``FOOL_PORTAL_BASE_URL`` set
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if not selected_model:
            ...
            print("  Run `fool model` again to switch to Nous Portal.")
            return
