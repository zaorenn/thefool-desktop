...
Configuration
-------------
Secret (lives in $THEFOOL_HOME/.env or the environment):
...
Behavioral settings (live in $THEFOOL_HOME/mem0.json, set via `hermes memory
setup`):
...
def _load_config() -> dict:
    """Load config from env vars, with $THEFOOL_HOME/mem0.json overrides.
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $THEFOOL_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $THEFOOL_HOME/mem0.json) —
