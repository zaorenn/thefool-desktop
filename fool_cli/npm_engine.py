...
lives inside its **own** managed Node tree (``$FOOL_HOME/node``), installing
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    ...
    ``$FOOL_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$FOOL_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading The Fool-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Fool-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="fool-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  The Fool could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    ...
    Installs the managed tree under ``$FOOL_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning a Fool-managed Node.js runtime "
