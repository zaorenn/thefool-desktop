...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    monkeypatch.setattr("fool_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("fool_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("fool_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("fool_cli.auth.get_provider_auth_state", lambda provider: {})
