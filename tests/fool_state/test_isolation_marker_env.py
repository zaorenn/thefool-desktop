...
child at the same moment the child lost the ``FOOL_HOME`` redirect. That
pairing is exactly how fixture rows (dm:123 / chat-1 / wx-chat) landed in a
developer's production state.db.

``FOOL_TEST_ISOLATION`` is Hermes's own marker: the hermetic conftest
exports it (value = the isolation root) before any test module imports, it
inherits into children by default, and ``fool_state`` honors it as a
...
def test_conftest_exports_the_marker():
    ...
    assert os.environ.get("FOOL_TEST_ISOLATION"), (
        "FOOL_TEST_ISOLATION must be exported by tests/conftest.py so "
...
def test_marker_alone_reports_test_context(monkeypatch):
    ...
    monkeypatch.setenv("FOOL_TEST_ISOLATION", "/tmp/some-isolation-root")
    assert fool_state._running_under_pytest() is True
...
def test_no_signals_reports_production(monkeypatch):
    ...
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    monkeypatch.delenv("PYTEST_VERSION", raising=False)
    monkeypatch.delenv("FOOL_TEST_ISOLATION", raising=False)
    assert fool_state._running_under_pytest() is False
...
def test_child_with_rebuilt_env_keeping_marker_refuses_production_db():
    ...
    stripped, FOOL_HOME lost) but which keeps the marker must still refuse
    to open the production state.db."""
    env = _minimal_env(FOOL_TEST_ISOLATION="/tmp/pytest-isolation-root")
...
def test_child_bypass_env_disarms_guard_even_with_marker():
    ...
    DB: FOOL_STATE_DB_GUARD_BYPASS=1, not marker-stripping."""
    env = _minimal_env(
        FOOL_TEST_ISOLATION="/tmp/pytest-isolation-root",
        FOOL_STATE_DB_GUARD_BYPASS="1",
    )
    result = _spawn_probe(env)
