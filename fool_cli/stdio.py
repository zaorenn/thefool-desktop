...
Entry points (``cli.py`` ``main``, ``fool_cli/main.py`` CLI dispatch,
...
def configure_windows_stdio() -> bool:
    ...
    Set ``FOOL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("FOOL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
...
def _augment_path_with_known_tools() -> None:
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "fool", "git", "cmd"),
        os.path.join(local_appdata, "fool", "git", "bin"),
        os.path.join(local_appdata, "fool", "git", "usr", "bin"),
        ...
        os.path.join(local_appdata, "fool", "hermes-agent", "venv", "Scripts"),
