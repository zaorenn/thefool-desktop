...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from fool_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from fool_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch fool_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("fool_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
