...
running under the dispatcher (env var ``THEFOOL_KANBAN_TASK`` set) or when
...
from thefool_cli.goals import judge_goal
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    a worker — i.e. whenever THEFOOL_KANBAN_* is present but not ours."""
    try:
...
def _reject_delegated_child_mutation(tool_name: str) -> Optional[str]:
    ...
    inherited THEFOOL_KANBAN_* env vars are not proof of dispatcher ownership.
...
def _check_kanban_mode() -> bool:
    ...
    1. ``THEFOOL_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    ...
    if _is_delegated_child_context():
        return False
    if os.environ.get("THEFOOL_KANBAN_TASK") and _is_dispatcher_owned_worker():
        return True
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if not _is_dispatcher_owned_worker():
        ...
        return None
    env_tid = os.environ.get("THEFOOL_KANBAN_TASK")
    return env_tid or None


def _worker_run_id(task_id: str) -> Optional[int]:
    """Return this worker's dispatcher run id when it is scoped to task_id."""
    if os.environ.get("THEFOOL_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("THEFOOL_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("THEFOOL_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("THEFOOL_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``THEFOOL_KANBAN_TASK`` set
    ...
    ``THEFOOL_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``THEFOOL_KANBAN_DB`` → ``THEFOOL_KANBAN_BOARD`` env → current symlink
...
#   - No-op outside dispatcher-spawned worker context (no ``THEFOOL_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``THEFOOL_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``THEFOOL_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``THEFOOL_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("THEFOOL_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("THEFOOL_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("THEFOOL_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    (``THEFOOL_KANBAN_TASK`` set) and ``agent`` exposes ``steer``. Returns True
    ...
    by ``THEFOOL_PROFILE``) are skipped to avoid echoing itself.
    """
    tid = os.environ.get("THEFOOL_KANBAN_TASK")
    ...
    own = (os.environ.get("THEFOOL_PROFILE") or "").strip()
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set THEFOOL_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # THEFOOL_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = os.environ.get("THEFOOL_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("THEFOOL_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets THEFOOL_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    # Prefer the request-scoped api_server origin binding: THEFOOL_SESSION_ID
    ...
    session_id = (
        ...
        or os.environ.get("THEFOOL_SESSION_ID")
    )
    ...
    try:
        ...
        try:
            ...
            if _inherit_project and project_id is None:
                _self_tid = os.environ.get("THEFOOL_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("THEFOOL_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    - **Gateway** (telegram/discord/slack/etc): ``THEFOOL_SESSION_PLATFORM``,
      ``THEFOOL_SESSION_CHAT_ID``, and ``THEFOOL_SESSION_CHAT_TYPE`` are set in
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``THEFOOL_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # THEFOOL_SESSION_KEY into the subprocess env. Treat that
            # as a "tui" subscription so the TUI notification poller
            # (tui_gateway/server.py) can pick it up.
            #
            # THEFOOL_SESSION_ID is intentionally NOT a fallback here:
            ...
            # poller keys on THEFOOL_SESSION_KEY.
            session_key = (
                get_session_env("THEFOOL_SESSION_KEY", "")
                or os.environ.get("THEFOOL_SESSION_KEY", "")
            )
            if not session_key:
        ...
        chat_type = get_session_env("THEFOOL_SESSION_CHAT_TYPE", "") or None
        ...
        user_id_alt = get_session_env("THEFOOL_SESSION_USER_ID_ALT", "") or None
        message_id = get_session_env("THEFOOL_SESSION_MESSAGE_ID", "") or ""
        notifier_profile = (
            get_session_env("THEFOOL_SESSION_PROFILE", "")
            or os.environ.get("THEFOOL_PROFILE")
        )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to THEFOOL_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: THEFOOL_KANBAN_DB env → "
    "THEFOOL_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to THEFOOL_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
