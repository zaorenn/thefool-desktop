...
:::info 首次运行行为
在全新安装时（或 pre-curator 版本在 `fool update` 后首次 tick 时），curator **不会立即运行**。首次观测会将 `last_run_at` 设为"当前时间"，并将第一次真正的运行推迟整整一个 `interval_hours`。这给了你一个完整的间隔时间来审查技能库、固定重要内容，或在 curator 真正触碰它之前完全退出。

如果你想在 curator 真正运行之前查看它*会*做什么，请运行 `fool curator run --dry-run`——它会生成相同的审查报告，但不会修改技能库。
:::
...
:::note 旧版配置
早期版本使用独立的 `curator.auxiliary.{provider,model}` 块。该路径仍然有效，但会输出一条弃用日志——请迁移到上方的 `auxiliary.curator`，使 curator 与其他所有辅助任务共享相同的管道（`fool model`、控制台 Models 标签页、`base_url`、`api_key`、`timeout`、`extra_body`）。
:::
...
fool curator run            # trigger a review now (blocks until the LLM pass finishes)
fool curator run --background  # fire-and-forget: start the LLM pass in a background thread
...
你也可以随时通过 `fool curator backup --reason "before-refactor"` 手动创建快照。`--reason` 字符串会写入快照的 `manifest.json`，并在 `--list` 中显示。
...
设置 `curator.backup.enabled: false` 可禁用自动快照。手动 `fool curator backup` 命令仅在 `enabled: true` 时才能工作——该标志对两条路径对称生效，因此不会在变更性运行中意外跳过 pre-run 快照。

`fool curator status` 还会列出五个最近最少使用的技能——快速查看哪些技能可能即将变为 stale。
...
- `~/.hermes/skills/.hub/lock.json`（通过 `fool skills install` 安装的技能）。
...
1. 运行 `fool curator run --dry-run` 查看 curator 具体会提出什么建议。
2. 使用 `fool curator pin <name>` 保护任何你不希望被触碰的内容。
3. 或者在 `config.yaml` 中设置 `curator.enabled: false`，如果你更愿意自己管理技能库。

归档始终可通过 `fool curator restore <name>` 恢复，但事先 pin 比事后追查合并结果要容易得多。
:::

如果你想保护某个特定技能不被触碰——例如你依赖的手写技能——请使用 `fool curator pin <name>`。详见下一节。
...
- **Agent 的 `skill_manage` 工具**拒绝对其执行 `delete`，并提示用户使用 `fool curator unpin <name>`。修补和编辑仍然可以进行，因此 agent 可以在遇到问题时改进已固定技能的内容，无需反复 pin/unpin/re-pin。
...
只有 **agent 创建**的技能才能被固定——捆绑和 hub 安装的技能本就不受 curator 变更，若你尝试固定它们，`fool curator pin` 会拒绝并给出说明。
...
如果某次运行将多个技能合并到一个总括技能下（或合并了近似重复项），运行结束时打印的用户可见摘要会包含一个明确的重命名映射，显示 curator 应用的每个 `旧名称 → 新名称` 对。这是对逐技能状态转换行的补充，因此当一批重命名落地时，你可以一眼发现，无需对比 JSON 报告。该提示也会在 `fool curator pin` 下显示，以便你在需要时立即固定新标签。
...
- **仅针对单次运行：** `fool curator pause`——暂停跨会话持久有效；使用 `resume` 重新启用。
