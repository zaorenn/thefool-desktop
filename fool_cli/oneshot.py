...
configured for "cli" in `fool tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (FOOL_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `fool chat`:
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to FOOL_INFERENCE_MODEL
...
def _run_agent(
    ...
    env_model = os.getenv("FOOL_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from fool_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("FOOL_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                FOOL_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via FOOL_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via FOOL_YOLO_MODE=1
