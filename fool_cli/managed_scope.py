...
This is DISTINCT from ``fool_cli.config.is_managed()`` / ``FOOL_MANAGED``,
...
def _under_pytest() -> bool:
    ...
    that exercise managed scope set ``FOOL_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$FOOL_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
    ...
    """
    override = os.environ.get("FOOL_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through fool_cli.config.load_config) to honor managed
    scope. Mirrors fool_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from fool_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
