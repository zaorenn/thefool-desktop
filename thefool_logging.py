...
Component separation:
    ...
    gui.log receives dashboard-side records from ``thefool_cli.web_server``,
    ``thefool_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from thefool_constants import get_config_path, get_hermes_home
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from thefool_cli.config import is_managed
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse thefool_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when thefool_cli.config isn't importable (bare
        # thefool_logging consumers).
        try:
            from thefool_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
