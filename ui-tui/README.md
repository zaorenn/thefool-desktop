...
Interpreter resolution order is: `THEFOOL_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.hermes/.hermes_history` or under `THEFOOL_HOME`.
...
ui-tui/
  ...
  src/
    ...
    types/
      hermes-ink.d.ts            type declarations for @thefool/ink
