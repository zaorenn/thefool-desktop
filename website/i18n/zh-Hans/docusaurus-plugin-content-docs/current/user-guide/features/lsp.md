...
Hermes 会自动检测 PATH 上或 `<THEFOOL_HOME>/lsp/bin/` 中的二进制文件。
...
lsp:
  ...
  # 处理缺失服务器二进制文件的策略。
  #   auto    — 通过 npm/pip/go install 安装到 <THEFOOL_HOME>/lsp/bin
...
当 `install_strategy: auto` 时，Hermes 将二进制文件安装到 `<THEFOOL_HOME>/lsp/bin/`。
NPM 包安装到 `<THEFOOL_HOME>/lsp/node_modules/`，bin 符号链接位于上一级目录。
...
该二进制文件不在 PATH 上，也不在 `<THEFOOL_HOME>/lsp/bin/` 中。运行
