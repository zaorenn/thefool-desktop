...
import thefool_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``THEFOOL_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` THEFOOL_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir THEFOOL_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="THEFOOL_HOME=/tmp/pytest-of-alice/pytest-42/'
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="THEFOOL_HOME=/tmp/hermes-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="THEFOOL_HOME={tmp_path}/hermes-home"\n'
...
class TestGeneratedSystemdUnits:
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        import thefool_cli.resource_limits as resource_limits
...
class TestSystemUnitHermesHome:
    """THEFOOL_HOME in system units must reference the target user, not root."""
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        monkeypatch.setenv("THEFOOL_HOME", str(root_hermes))
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "thefool_constants.iter_hermes_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "thefool_constants.hermes_managed_node_tree_present",
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'THEFOOL_HOME=/home/alice/.hermes' in unit
        assert '/root/.hermes' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's THEFOOL_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        hermes_home = str(gateway_cli.get_hermes_home().resolve())
        assert f'THEFOOL_HOME={hermes_home}' in unit
...
class TestSystemUnitRefreshSyncsHermesHome:
    ...
    def test_refresh_adopts_unit_hermes_home_before_rewriting(self, tmp_path, monkeypatch):
        ...
        # Correct installed unit (operator's THEFOOL_HOME + drain timeout).
        monkeypatch.setenv("THEFOOL_HOME", str(alice_hermes))
        ...
        # Simulate sudo without inherited THEFOOL_HOME (falls back to root).
        monkeypatch.setenv("THEFOOL_HOME", str(root_hermes))
        ...
        assert os.environ["THEFOOL_HOME"] == str(alice_hermes)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned THEFOOL_HOME *before* it reads/compares the unit.
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("THEFOOL_HOME", str(root_profile))
        ...
        assert f'THEFOOL_HOME={target_home / ".hermes" / "profiles" / "mybot"}' in unit

    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".hermes" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("THEFOOL_HOME", str(profile_dir))
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("THEFOOL_HOME", str(root_home / ".hermes"))
        ...
        # WorkingDirectory is anchored at the target user's THEFOOL_HOME (stable,
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m thefool_cli.main gateway run --replace\n"
    )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m thefool_cli.main gateway run
          - python path/to/thefool_cli/main.py gateway run
          - hermes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m thefool_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/thefool_cli/main.py gateway run",
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        if project_root is None:
            import thefool_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "thefool_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "thefool_cli.main", "gateway", "status", "-l", "--help"],
...
class TestServiceWorkingDirIsStable:
    ...
    (THEFOOL_HOME), never the source checkout / worktree, so a relocated or
