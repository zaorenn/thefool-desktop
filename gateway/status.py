...
def _gateway_command_subcommand(command: str | None) -> str | None:
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `fool gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `fool status` show per-profile
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``fool gateway stop`` on Windows would be misclassified as an
