...
from fool_cli.config import get_hermes_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    ...
    exports. ``fool debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``FOOL_GIT_SHA`` build-arg (see ``fool_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``fool dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from fool_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `fool auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `fool auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["hermes-cli"])  # FOOL-SEAM: toolset-rename
