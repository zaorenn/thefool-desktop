...
* Be supported-platform-only — no-op silently elsewhere so ``fool update``
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_missing_explicit_override_does_not_install_standard_driver(
        ...
        monkeypatch.setenv("FOOL_CUA_DRIVER_CMD", "/missing/custom/cua-driver")
        with patch.object(
                 tools_config,
                 "_resolved_cua_driver_cmd",
                 return_value=None,
...
class TestRequireConfirmedUpdate:
    """`fool update` passes require_confirmed_update=True: the full
    ...
    Explicit `fool computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `fool computer-use install --upgrade` (default flag): the old
    ...
    def test_incompatible_driver_repairs_despite_indeterminate_check(self):
        ...
        driver fails the runtime contract, the `fool update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `fool update`.
    """
...
class TestCuaVersionSummary:
    """`fool computer-use status` prints one line, whatever the binary says.

    A binary chosen by FOOL_CUA_DRIVER_CMD is under no obligation to answer
