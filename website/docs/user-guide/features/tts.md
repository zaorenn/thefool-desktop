...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, OpenAI TTS is available through the **[Tool Gateway](tool-gateway.md)** without a separate OpenAI API key. New installs can run `fool setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Nous Subscription** for just TTS via `fool model` or `fool tools`.
:::
...
**Install via `fool tools`** → Voice & TTS → Piper — Hermes runs `pip install piper-tts` for you. Or install manually: `pip install piper-tts`.
...
| A voice-listing API used by `fool setup` | **Plugin** (override `list_voices()`) |
...
Enable it (`fool plugins enable my-tts`), point `tts.provider` at it (`tts.provider: my-tts` in `config.yaml`), and the `text_to_speech` tool will route through your plugin.

#### Optional hooks

Override these on your provider class for richer integration:

- `list_voices()` → list of `{id, display, language, gender, preview_url}` dicts shown in `fool tools`.
- `list_models()` → list of `{id, display, languages, max_text_length}` dicts.
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power the picker row in `fool tools` / `fool setup`. Without this, the plugin still works but its row in the picker is minimal.
...
Enable it (`fool plugins enable my-stt`), set `stt.provider: my-stt` in `config.yaml`, and voice-message transcription will route through your plugin.
...
- `get_setup_schema()` → return `{name, badge, tag, env_vars: [{key, prompt, url}]}` to power picker rows in `fool tools` / `fool setup` (the picker category for STT is not yet shipped — this metadata is available to plugins for forward compatibility).
