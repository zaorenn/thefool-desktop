...
Add, remove, or reorder entries here — both `fool setup` and
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `fool model`
    flow without maintaining a separate static list.
    """
    from fool_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $FOOL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $FOOL_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see fool_cli/model_setup_flows.py).
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$FOOL_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
#   tui_desc    — longer description for the `fool model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `fool model` TUI
...
# slug as a top-level row in the interactive `fool model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``fool model``, the
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from fool_cli.model_catalog import get_default_model_from_cache
...
# ``fool model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in fool_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``fool auth add openai-codex`` without
    ``fool model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``fool model`` picker offers first. For metered aggregators
...
# fool_constants.VALID_REASONING_EFFORTS ("none" included so an explicit
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from fool_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from fool_cli.model_catalog import get_curated_nous_models
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``fool model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from fool_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from fool_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``fool_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``fool auth add copilot`` and by ``_seed_from_env``
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from fool_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from fool_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``fool model`` shows.
            from fool_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $FOOL_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$FOOL_HOME/auth.json`` and external
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `fool moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
