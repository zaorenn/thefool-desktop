...
3. **Project** — `./.hermes/plugins/<name>/` (requires `THEFOOL_ENABLE_PROJECT_PLUGINS=1`)
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `THEFOOL_HOME` or `/tmp/hermes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under THEFOOL_HOME | always | Never |
...
**State** — everything lives at `$THEFOOL_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `THEFOOL_HOME` or `/tmp/hermes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `THEFOOL_LANGFUSE_MAX_CHARS`. |
...
| `THEFOOL_LANGFUSE_RELEASE` | — | Release/version tag |
| `THEFOOL_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `THEFOOL_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `THEFOOL_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |
...
- Unlock state is persisted to `$THEFOOL_HOME/plugins/hermes-achievements/state.json`
...
**State files** — live under `$THEFOOL_HOME/plugins/hermes-achievements/`:
...
**Opting out:** Delete or rename `plugins/hermes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.hermes/plugins/hermes-achievements/` that ships no dashboard. The plugin's state files under `$THEFOOL_HOME/plugins/hermes-achievements/` survive — reinstalling preserves your unlock history.
