...
def hermes_home(tmp_path, monkeypatch):
    """Set up an isolated THEFOOL_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO thefool_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from thefool_cli.debug import _upload_paste_rs
        ...
        with patch("thefool_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "thefool_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from thefool_cli.debug import upload_to_pastebin

        with patch("thefool_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("thefool_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, hermes_home):
        ...
        from thefool_cli.debug import _capture_log_snapshot
...
class TestMissingLogNote:
    ...
    def test_report_carries_the_note_for_a_remote_backend(self, hermes_home):
        ...
        from thefool_cli.debug import collect_debug_report
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of THEFOOL_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    def hermes_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated THEFOOL_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".hermes"
        home.mkdir()
        monkeypatch.setenv("THEFOOL_HOME", str(home))
    ...
    def test_force_true_works_when_redaction_disabled(
        ...
        input unchanged when THEFOOL_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("THEFOOL_REDACT_SECRETS", "false")

        from thefool_cli.debug import _capture_log_snapshot

        assert os.environ.get("THEFOOL_REDACT_SECRETS", "") == "false"
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from thefool_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, hermes_home):
        ...
        with patch("thefool_cli.dump.run_dump") as mock_dump:
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, hermes_home, capsys):
        ...
        from thefool_cli.debug import run_debug_share
        ...
        with patch("thefool_cli.dump.run_dump"), \
             patch("thefool_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("thefool_cli.debug.upload_to_pastebin",
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        ...
        with patch("thefool_cli.dump.run_dump"), \
             patch("thefool_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("thefool_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from thefool_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from thefool_cli.debug import delete_paste
        ...
        with patch("thefool_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    def test_records_pending_to_json(self, hermes_home):
        ...
        from thefool_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, hermes_home):
        ...
        from thefool_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, hermes_home):
        ...
        with patch("thefool_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, hermes_home):
        from thefool_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("thefool_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, hermes_home):
        ...
        with patch(
            "thefool_cli.debug.delete_paste",
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, hermes_home):
        ...
        with patch("thefool_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from thefool_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("thefool_cli.debug.delete_paste",
...
class TestBuildDebugShare:
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, hermes_home):
        ...
        with patch("thefool_cli.dump.run_dump"), patch(
            "thefool_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("thefool_cli.debug._schedule_auto_delete"):
...
class TestCollectShareBundle:
    ...
    def test_no_redact_omits_banner(self, hermes_home):
        from thefool_cli.debug import collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, hermes_home):
        ...
        from thefool_cli.debug import build_nous_bundle
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, hermes_home, capsys):
        ...
        with patch("thefool_cli.dump.run_dump"), patch(
            "thefool_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, hermes_home, capsys):
        ...
        with patch("thefool_cli.dump.run_dump"), patch(
            "thefool_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, hermes_home):
        ...
        with patch("thefool_cli.dump.run_dump"), patch(
            "thefool_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("thefool_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    def _captured(self, cmd_original):
        ...
        with patch("thefool_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    ...
    def test_non_interactive_requires_yes(self, hermes_home, capsys, monkeypatch):
        ...
        with patch("thefool_cli.dump.run_dump"), \
             patch("thefool_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
