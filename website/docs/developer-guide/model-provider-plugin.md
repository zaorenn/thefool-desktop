...
The only required file is `__init__.py`. `plugin.yaml` is used by `fool plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
| `fool model` picker | `fool_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `fool doctor` | `fool_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `fool setup` | `fool_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| `display_name` | str | Human label shown in `fool model` picker |
