"""CLI for the Hermes Kanban board — ``fool kanban …`` subcommand.
...
from fool_cli import kanban_swarm as ks
...
def _check_dispatcher_presence(
    ...
    - ``running=True``: a gateway is alive for this FOOL_HOME and its
    ...
    Used by ``fool kanban create`` (and callers) to warn when a task
    ...
    be running under a different FOOL_HOME than the profile the request
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`fool kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    # FOOL_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `fool kanban boards switch <slug>` or the "
            "FOOL_KANBAN_BOARD env var). Use `fool kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `fool project list`.")
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $FOOL_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $FOOL_PROFILE or 'user')")
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $FOOL_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `fool gateway start`.",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $FOOL_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $FOOL_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``fool kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: fool kanban <action> [options]\n"
                "Run 'fool kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0

    # Fast-fail for clearer CLI UX only. The durable trust boundary is lower in
    # fool_cli.kanban_db, because children can import DB mutators directly.
    ...
    # env-var pin for the duration of this call. Using FOOL_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `fool kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # FOOL_HOME. Previously only `init` and `daemon` triggered
    ...
    with board_scope:
        ...
        # every `fool kanban repair` into "could not initialize database"
...
def _profile_author() -> str:
    ...
    for env in ("FOOL_PROFILE_NAME", "FOOL_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("FOOL_DELEGATED_CHILD_CONTEXT"))


# ---------------------------------------------------------------------------
# Boards management (fool kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``fool kanban boards <action>``.
    ...
    DB, so a fresh FOOL_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `fool kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `fool kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `fool kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `fool kanban boards delete <slug>` (alias), the
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from fool_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``fool kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "fool kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    fool gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`fool kanban list --status ready` / "
                    f"`fool kanban list --status blocked` for recent "
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from fool_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from fool_cli import kanban_decompose as decomp
