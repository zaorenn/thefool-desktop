...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `thefool_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("thefool_cli.main")

    def fake_main():
        called.append("thefool_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "thefool_cli.main", fake_main_module)
    ...
    assert called == ["thefool_cli.main"]
