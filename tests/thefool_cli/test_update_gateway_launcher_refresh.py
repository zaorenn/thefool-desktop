...
2. ``thefool_cli.main._refresh_windows_gateway_launchers`` — ``hermes
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "thefool_cli.main", "gateway", "run"]
    with mock.patch.object(
        gateway_windows, "_stable_gateway_working_dir", return_value=str(tmp_path)
    ), mock.patch("thefool_cli.config.get_hermes_home", return_value=str(tmp_path)):
