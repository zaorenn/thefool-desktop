...
// Profile-aware: the Python launcher exports THEFOOL_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.hermes matches get_hermes_home()'s default.
const bootFilePath = () => join(process.env.THEFOOL_HOME ?? join(homedir(), '.hermes'), 'tui-theme-boot.json')

// Never touch the user's real ~/.hermes from test runs (the TS suite has no
// THEFOOL_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into THEFOOL_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.THEFOOL_TUI_THEME || env.THEFOOL_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.THEFOOL_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.THEFOOL_TUI_BACKGROUND
  ) {
    env.THEFOOL_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.THEFOOL_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.THEFOOL_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into THEFOOL_TUI_THEME.
