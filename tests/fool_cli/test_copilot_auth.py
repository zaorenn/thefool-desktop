"""Tests for fool_cli.copilot_auth — Copilot token validation and resolution."""
...
class TestResolveToken:
    ...
    def test_gh_token_second_priority(self, monkeypatch):
        from fool_cli.copilot_auth import resolve_copilot_token
    ...
    def test_gh_cli_classic_pat_raises(self, monkeypatch):
        ...
        with patch("fool_cli.copilot_auth._try_gh_cli_token", return_value="ghp_classic"):
    ...
    def test_invalid_env_var_skips_gh_cli_fallback(self, monkeypatch):
        ...
        with patch("fool_cli.copilot_auth._try_gh_cli_token") as mock_cli:
            token, source = resolve_copilot_token()
        assert token == ""
        assert source == ""
...
class TestRequestHeaders:
    ...
    def test_default_headers_include_openai_intent(self):
        ...
        assert headers["User-Agent"] == "FoolAgent/1.0"
