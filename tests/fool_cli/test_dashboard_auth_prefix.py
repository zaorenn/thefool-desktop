...
from fool_cli.dashboard_auth import clear_providers, register_provider
from fool_cli.dashboard_auth import prefix as prefix_mod
from tests.fool_cli.conftest_dashboard_auth import StubAuthProvider
...
# ---------------------------------------------------------------------------
# FOOL_DASHBOARD_PUBLIC_URL / dashboard.public_url override
# ---------------------------------------------------------------------------


class TestPublicUrlOverride:
    """``dashboard.public_url`` (env override:
    ``FOOL_DASHBOARD_PUBLIC_URL``) lets an operator force the absolute
    ...
    def patch_config(self, monkeypatch):
        """Replace ``fool_cli.config.load_config`` with a stub
    ...
    def test_env_overrides_config_public_url(
        ...
        monkeypatch.setenv(
            "FOOL_DASHBOARD_PUBLIC_URL", "https://from-env.example",
        )
    ...
    def test_malformed_public_url_falls_through_to_reconstruction(
        ...
        for bad in [
            ...
            monkeypatch.setenv("FOOL_DASHBOARD_PUBLIC_URL", bad)
            redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        of "I set FOOL_DASHBOARD_PUBLIC_URL but the callback is still
        ...
        patch_config(None)
        monkeypatch.setenv("FOOL_DASHBOARD_PUBLIC_URL", "hermes.domain.com")
        ...
        assert any(
            "FOOL_DASHBOARD_PUBLIC_URL" in m
...
class TestCookiePathRespectsPrefix:
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from fool_cli.dashboard_auth.cookies import set_pkce_cookie
