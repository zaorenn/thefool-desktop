...
This module is also the home of the ``THEFOOL_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    "I set THEFOOL_DASHBOARD_PUBLIC_URL but the OAuth callback is still
...
# ---------------------------------------------------------------------------
# THEFOOL_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``THEFOOL_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("THEFOOL_DASHBOARD_PUBLIC_URL", "")
    env_clean = _normalise_public_url(env_raw)
    if env_clean:
        return env_clean
    _warn_if_malformed("THEFOOL_DASHBOARD_PUBLIC_URL env var", env_raw)
