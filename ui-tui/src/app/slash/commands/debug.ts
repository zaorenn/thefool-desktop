...
import { terminalBackgroundHex } from '@fool/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $FOOL_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see FOOL_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['FOOL_TUI_BACKGROUND', process.env.FOOL_TUI_BACKGROUND ?? '(unset)'],
            ['FOOL_TUI_THEME', process.env.FOOL_TUI_THEME ?? '(unset)'],
