...
|---|---|---|
| I just want Hermes working on my machine | `fool setup` | Run a real chat and verify it responds |
| I already know my provider | `fool model` | Save the config, then start chatting |
| I want a bot or always-on setup | `fool gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `fool model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `fool model` first | Add routing and fallback only after the base chat works |
...
The single most important setup step. Use `fool model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `fool setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `fool update`. Re-enable anything later with `fool tools`, seed skills with `fool skills opt-in --sync`, or tune settings with `fool setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `fool model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `fool model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `fool model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `fool_cli/models.py` may change between releases) | `fool model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `fool model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `fool model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `fool model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `fool model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `fool model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::
...
```bash
fool gateway setup    # Interactive platform configuration
```
...
- `fool tools` — tune tool access per platform
- `fool skills` — browse and install reusable workflows
...
```bash
fool skills browse                      # list everything available
fool skills search kubernetes           # find skills by keyword
fool skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `fool skills browse` shows the exact slugs to use.
...
|---|---|---|
| Hermes opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `fool model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `fool gateway setup` and check `fool gateway status` |
| `hermes --continue` can't find old session | Switched profiles or session never saved | Check `fool sessions list` and confirm you're in the right profile |
...
| `fool doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
| `fool model` | Choose your LLM provider and model |
| `fool tools` | Configure which tools are enabled per platform |
| `fool setup` | Full setup wizard (configures everything at once) |
| `fool doctor` | Diagnose issues |
| `fool update` | Update to latest version |
| `fool gateway` | Start the messaging gateway |
...
- **[Moving to another machine](/reference/faq#exporting-hermes-to-another-machine)** — `fool backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
