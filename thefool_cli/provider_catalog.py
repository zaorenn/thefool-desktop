...
they were not: the CLI picker read :data:`thefool_cli.models.CANONICAL_PROVIDERS`
...
* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`thefool_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
# thefool_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
def provider_catalog() -> list[ProviderDescriptor]:
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = THEFOOL_OVERLAYS.get(slug)
