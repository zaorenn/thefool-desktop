"""Dashboard process-hygiene helpers — extracted from ``fool_cli/main.py``.
...
helpers that STAY in ``fool_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``fool_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``fool_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``fool_cli.main`` reference (call-time; keeps patches working)."""
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `fool_cli.gateway._scan_gateway_pids` and avoids the
...
def _hermes_home_for_pid(pid: int) -> str | None:
    """Best-effort ``FOOL_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("FOOL_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"FOOL_HOME="):
...
def _profile_key_for_respawn(
    ...
    """Stable owner key: ``FOOL_HOME`` when known, else ``--profile`` / ``-p``.

    ``FOOL_HOME`` ending in ``profiles/<name>`` is normalized to
...
def _filter_dashboard_respawn_candidates(
    ...
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``FOOL_DESKTOP_CHILD_PID``) owns their
...
def _kill_stale_dashboard_processes(
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    ...
    # backend child, it sets FOOL_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("FOOL_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot FOOL_HOME before the kill so per-profile caps
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (FOOL_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("FOOL_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
def _hermes_home_dir() -> Path:
    """Resolved Hermes home (FOOL_HOME override or ~/.hermes)."""
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    # only check the suffix so a relocated FOOL_HOME (different leading path)
...
def _reap_orphaned_desktop_local_serves(
    *,
    ...
    running under FOOL_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never FOOL_DESKTOP_CHILD_PID entries
