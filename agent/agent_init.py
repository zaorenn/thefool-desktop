...
from thefool_cli.timeouts import get_provider_request_timeout
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from thefool_cli.providers import THEFOOL_OVERLAYS, get_provider

        overlay = THEFOOL_OVERLAYS.get(provider)
    ...
    try:
        ...
        from thefool_cli.models import normalize_provider as normalize_model_provider
        from thefool_cli.providers import normalize_provider as normalize_registry_provider
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$THEFOOL_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$THEFOOL_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .hermes.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / THEFOOL_HOME
    ...
    agent._cache_ttl = "5m"
    try:
        from thefool_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind THEFOOL_DEV_CREDITS.
    ...
    from thefool_logging import setup_logging, setup_verbose_logging
    ...
    if agent.verbose_logging:
        ...
        # on the fact that thefool_logging.setup_logging() does not
        ...
        # handler level inside thefool_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from thefool_cli.config import load_config_readonly as _load_br_cfg
    ...
    # A multiplexed gateway may enter a different THEFOOL_HOME after
    ...
    # worker (kanban_show tool is present iff THEFOOL_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["THEFOOL_SESSION_ID"] = agent.session_id
    ...
    try:
        from thefool_cli.config import load_config_readonly as _load_sess_cfg
    ...
    try:
        from thefool_cli.config import load_config_readonly as _load_agent_config
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from thefool_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from thefool_cli.plugins import get_plugin_context_engine
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from thefool_cli.model_switch import _check_hermes_model_warning
