...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("thefool_cli.setup.ensure_hermes_home", {}),
        ("thefool_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("thefool_cli.config.is_managed", {"return_value": False}),
        ("thefool_cli.setup.load_config", {"return_value": {}}),
        ("thefool_cli.setup.save_config", {}),
        ("thefool_cli.setup.get_env_value", {"return_value": None}),
        ("thefool_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("thefool_cli.setup._print_setup_summary", {}),
        ("thefool_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("thefool_cli.setup.load_config", {"return_value": {}}),
        ("thefool_cli.setup.save_config", {}),
        ("thefool_cli.auth.get_active_provider", {"return_value": None}),
        ("thefool_cli.setup.get_env_value", {"return_value": None}),
        ("thefool_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="thefool_cli.setup.prompt_choice",
                quick="thefool_cli.setup._run_quick_setup",
                model="thefool_cli.setup.setup_model_provider",
                terminal="thefool_cli.setup.setup_terminal_backend",
                agent="thefool_cli.setup.setup_agent_settings",
                gateway="thefool_cli.setup.setup_gateway",
                tools="thefool_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("thefool_cli.setup.prompt_choice", {"return_value": 0}),
                first="thefool_cli.setup._run_first_time_quick_setup",
            )
            from thefool_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "thefool_cli.setup.run_setup_wizard",
