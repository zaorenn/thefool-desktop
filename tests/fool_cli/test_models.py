"""Tests for the fool_cli models module."""
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("fool_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("fool_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("fool_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("fool_cli.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("fool_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from fool_cli.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from fool_cli.models import _find_openrouter_slug
        with patch("fool_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "fool_cli.models.fetch_nous_recommended_models",
...
class TestCheckNousFreeTierCache:
    ...
    @patch("fool_cli.nous_account.get_nous_portal_account_info")
...
class TestNousRecommendedModels:
    ...
    def test_fetch_caches_per_portal_url(self):
        from fool_cli.models import fetch_nous_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("fool_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("fool_cli.models.fetch_nous_recommended_models", return_value=payload):
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("fool_cli.models.fetch_nous_recommended_models", return_value=payload),
            patch("fool_cli.models.check_nous_free_tier", side_effect=RuntimeError("boom")),
        ):
...
class TestFormatPricePerMtok:
    ...
    def test_standard_prices_keep_two_decimals(self):
        from fool_cli.models import _format_price_per_mtok
