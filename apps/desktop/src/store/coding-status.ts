...
import type { FoolGitWorktree, FoolRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: FoolGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, FoolRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, FoolGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<FoolRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<FoolGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<FoolRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<FoolGitWorktree[]>>()
const $noRepoStatus = atom<FoolRepoStatus | null>(null)
const $noWorktrees = atom<FoolGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<FoolRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<FoolGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: FoolRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: FoolGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<FoolRepoStatus | null>
  seq: number
  target: string
}
