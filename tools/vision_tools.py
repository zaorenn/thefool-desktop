...
# ``agent.auxiliary_client`` pulls credential_pool → thefool_cli.auth → httpx
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("THEFOOL_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: THEFOOL_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("THEFOOL_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $THEFOOL_HOME/cache/vision/ and cleaned up
