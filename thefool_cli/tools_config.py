...
from thefool_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from thefool_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    :func:`thefool_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from thefool_cli.cli_output import (  # noqa: E402 — late import block
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from thefool_cli.auth import _read_xai_oauth_tokens
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from thefool_cli.plugins import get_plugin_toolset_keys_nowait
...
from thefool_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set THEFOOL_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "THEFOOL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "THEFOOL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "THEFOOL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "THEFOOL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "THEFOOL_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("THEFOOL_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``THEFOOL_CUA_DRIVER_CMD`` is not obliged to answer
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $THEFOOL_HOME/bin is never on PATH, so a bare which()
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    override = os.environ.get("THEFOOL_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    THEFOOL_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    if repair_existing:
        ...
        if os.environ.get("THEFOOL_CUA_DRIVER_CMD", "").strip():
            _print_info(
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Hermes-managed ``$THEFOOL_HOME/bin`` copy does.
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        try:
            from thefool_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from thefool_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _prompt_toolset_checklist(
    ...
    *,
    ...
    from thefool_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
def _agent_browser_installed() -> bool:
    ...
    from thefool_cli.nous_subscription import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from thefool_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (thefool_cli/web_server.py
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    """
    try:
        from thefool_cli.inventory import (
