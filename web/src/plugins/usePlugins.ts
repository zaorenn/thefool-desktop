...
import { api, THEFOOL_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${THEFOOL_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${THEFOOL_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
