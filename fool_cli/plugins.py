...
from fool_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from fool_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from fool_cli.plugin_capabilities import (
...
# ---------------------------------------------------------------------------
#
# Set ``FOOL_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
...
_PLUGINS_DEBUG = os.getenv("FOOL_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When FOOL_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "FOOL_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by fool_cli.kanban_db when a task
    ...
    #   result: fool_cli.kanban_db.DispatchResult (spawned, reclaimed,
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `fool plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    # ``fool_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
...
class PluginContext:
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from fool_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        ...
        Derived from ``FOOL_HOME`` via
        :func:`fool_cli.profiles.get_active_profile_name`, so it works in
        ...
        ``FOOL_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`fool_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        from fool_cli.dashboard_auth.registry import restore_registration
        from fool_cli.dashboard_auth.registry import snapshot_registration
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``fool model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: fool_cli.main imports plugins indirectly
        from fool_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("FOOL_SAFE_MODE"):
                logger.info("FOOL_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from fool_cli.env_loader import load_hermes_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `fool plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from fool_cli.approval_transport import RegisteredApprovalTransport
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        if _env_enabled("FOOL_ENABLE_PROJECT_PLUGINS"):
            ...
            logger.debug(
                "Project plugins disabled (set FOOL_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("FOOL_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from fool_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from fool_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        # Record an enabled placeholder for introspection (`fool plugins
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from fool_cli.agent_plugins import load_agent_plugin
...
# multiple profiles via different FOOL_HOME directories, and a single
...
# fool_constants.set_hermes_home_override). A process-wide single-slot
...
def get_plugin_manager() -> PluginManager:
    ...
    profile switch — via ``FOOL_HOME`` or the context-local
...
def _get_pre_tool_call_directive_details(
    ...
    from fool_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
