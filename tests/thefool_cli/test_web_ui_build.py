...
(mirroring the desktop build), recorded in a stamp file under $THEFOOL_HOME,
NOT mtime comparison — so ``git pull`` / ``hermes update`` that rewrite
source mtimes without changing content no longer fool it.

Critical invariant: the dashboard Vite build outputs to thefool_cli/web_dist/
(vite.config.ts: outDir: "../../thefool_cli/web_dist"), NOT web/dist/.
...
def _isolated_hermes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("THEFOOL_HOME", str(tmp_path / "_hermes_home"))
...
def _make_web_dir(tmp_path: Path) -> tuple[Path, Path]:
    ...
    dist_dir = tmp_path / "thefool_cli" / "web_dist"
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_web_install_omits_workspace_and_scrubs_esbuild_override(
        ...
        with patch("thefool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("thefool_cli.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("thefool_cli.main._run_with_idle_timeout", return_value=build_cp) as mock_build:
    ...
    def test_workspace_root_install_names_update_closure(self, tmp_path, monkeypatch):
        ...
        with patch("thefool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("thefool_cli.main.subprocess.run", return_value=install_cp) as mock_run, \
             patch("thefool_cli.main._run_with_idle_timeout", return_value=build_cp):
            result = _build_web_ui(web_dir)
    ...
    def test_web_build_uses_idle_timeout_helper(self, tmp_path):
        ...
        build_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("thefool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("thefool_cli.main.subprocess.run", return_value=install_cp), \
             patch("thefool_cli.main._run_with_idle_timeout", return_value=build_cp) as mock_idle:
            result = _build_web_ui(web_dir)
...
class TestBuildWebUIRetryAndStaleFallback:
    ...
    def test_retries_build_once_on_failure(self, tmp_path):
        ...
        with patch("thefool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("thefool_cli.main._time.sleep") as mock_sleep, \
             patch("thefool_cli.main.subprocess.run", return_value=install_ok), \
             patch("thefool_cli.main._run_with_idle_timeout",
    ...
    def test_falls_back_to_stale_dist_when_retry_also_fails(self, tmp_path, capsys):
        ...
        with patch("thefool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("thefool_cli.main._time.sleep"), \
             patch("thefool_cli.main.subprocess.run", return_value=install_ok), \
             patch("thefool_cli.main._run_with_idle_timeout",
...
class TestBuildWebUIFlock:
    ...
    def test_contended_lock_without_dist_waits_then_skips_fresh_build(self, tmp_path):
        ...
        import fcntl
        import threading
        from thefool_cli.main import _build_web_ui as build
        ...
        try:
            with patch("thefool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
                 patch("thefool_cli.main.subprocess.run") as mock_run:
...
class TestBuildRecoversFromMissingToolchain:
    def test_reinstalls_and_retries_when_the_build_cannot_resolve_tsc(self, tmp_path):
        ...
        with patch("thefool_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             ...
             patch("thefool_cli.main._run_with_idle_timeout", side_effect=[build_fail, build_ok]) as mock_build, \
             patch("thefool_cli.main._web_ui_build_needed", return_value=True), \
             patch("thefool_cli.main._write_web_ui_build_stamp"), \
             patch("thefool_cli.main._time.sleep"):
            result = _build_web_ui(web_dir)
    ...
    def test_healthy_tree_builds_without_an_extra_install(self, tmp_path):
        ...
        with patch("thefool_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
             patch("thefool_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
             patch("thefool_cli.main._run_with_idle_timeout", return_value=build_ok) as mock_build, \
             patch("thefool_cli.main._web_ui_build_needed", return_value=True), \
             patch("thefool_cli.main._write_web_ui_build_stamp"):
            result = _build_web_ui(web_dir)
