...
# $FOOL_HOME — auth.json, .env, config.yaml — ends up root-owned and
...
# $FOOL_HOME is uid-aligned with the supervised processes. When invoked
...
#
# Opt-out: set FOOL_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
case "${FOOL_DOCKER_EXEC_AS_ROOT:-}" in
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "hermes-shim: re-run with --user hermes or set FOOL_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi
