...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors thefool_cli.main.
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    with patch("thefool_cli.main._windows_native_machine", return_value="AMD64"):
...
def test_gate_fails_clearly_without_backup(tmp_path, capsys):
    ...
    with patch("thefool_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("thefool_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    ...
    with patch("thefool_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("thefool_cli.main._resolve_node_runtime_npm", return_value="npm.cmd"), \
         ...
         patch("thefool_cli.main._stop_desktop_processes_locking_build", return_value=[]), \
         patch("thefool_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("thefool_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("thefool_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("thefool_cli.main._windows_native_machine", return_value="AMD64"), \
         patch("thefool_cli.main.subprocess.run", return_value=pack_ok), \
