...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``fool_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``fool_bootstrap``, never ``fool_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    """
    root = src_root or os.environ.get("FOOL_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``FOOL_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("FOOL_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import fool_bootstrap``
# (or ``from fool_bootstrap import apply_windows_utf8_bootstrap``) at
