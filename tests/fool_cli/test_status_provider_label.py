...
from fool_cli.status import _effective_provider_label


def _label_with(config, env_base_url=""):
    with patch("fool_cli.status.resolve_requested_provider", return_value="auto"), \
         patch("fool_cli.status.resolve_provider", return_value="openrouter"), \
         patch("fool_cli.status.load_config", return_value=config), \
         patch("fool_cli.status.get_env_value",
...
def test_non_openrouter_provider_untouched():
    with patch("fool_cli.status.resolve_requested_provider", return_value="anthropic"), \
         patch("fool_cli.status.resolve_provider", return_value="anthropic"):
