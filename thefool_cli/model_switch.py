...
- ``thefool_cli.providers``        -- canonical provider identity + overlays
- ``thefool_cli.model_normalize``  -- per-provider name formatting
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from thefool_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``thefool_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from thefool_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from thefool_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from thefool_cli.config import load_config
            from thefool_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from thefool_cli.providers import resolve_user_provider as _ruser
    ...
    # thefool_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from thefool_cli.models import normalize_opencode_base_url
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from thefool_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    """
    from thefool_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from thefool_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from thefool_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from thefool_cli.providers import THEFOOL_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from thefool_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from thefool_cli.auth import is_runtime_provider_routable
    ...
    # --- Section 2: Hermes-only providers (THEFOOL_OVERLAYS) ---
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in THEFOOL_OVERLAYS.items():
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from thefool_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from thefool_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (THEFOOL_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        ...
        if not has_creds:
            try:
                from thefool_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(hermes_id)
                )
    ...
    from thefool_cli.providers import THEFOOL_OVERLAYS
    from thefool_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Hermes provider ID.
    # THEFOOL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    ...
    for pid, overlay in THEFOOL_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from thefool_cli.auth import get_provider_auth_state as _nous_state
    ...
    # in PROVIDER_TO_MODELS_DEV or THEFOOL_OVERLAYS (keeps /model in sync
    # with `hermes model`).
    try:
        from thefool_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, thefool_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from thefool_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`thefool_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from thefool_cli.models import fetch_openrouter_models
