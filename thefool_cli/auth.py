...
# httpx is imported lazily: it costs ~30ms at import time and thefool_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("thefool_cli.auth.httpx.Client", ...) keeps working in tests.
...
from thefool_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from thefool_cli import __version__ as _HERMES_CLI_VERSION
...
def get_anthropic_key() -> str:
    ...
    """
    from thefool_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from thefool_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("THEFOOL_OAUTH_TRACE", "").strip().lower()
...
def _auth_file_path() -> Path:
    ...
    # Seat belt: if pytest is running and THEFOOL_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch THEFOOL_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set THEFOOL_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom THEFOOL_HOME that is not a profile).
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    if pconfig is None:
        from thefool_cli.providers import get_provider
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``THEFOOL_PORTAL_BASE_URL`` /
    ...
    ``THEFOOL_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("THEFOOL_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("THEFOOL_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("THEFOOL_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set THEFOOL_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("THEFOOL_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("THEFOOL_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("THEFOOL_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    save_env_value("THEFOOL_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("THEFOOL_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved THEFOOL_SPOTIFY_CLIENT_ID to ~/.hermes/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "THEFOOL_SPOTIFY_CLIENT_ID is required".
...
def resolve_codex_runtime_credentials(
    *,
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("THEFOOL_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("THEFOOL_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.hermes/auth.json even when THEFOOL_HOME points at a profile path
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``THEFOOL_XAI_BASE_URL``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("THEFOOL_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("THEFOOL_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("THEFOOL_CA_BUNDLE")
...
#
# File lives at ${THEFOOL_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
...
# It is OUTSIDE any named profile's THEFOOL_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``THEFOOL_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<hermes-root>/shared/``, where ``<hermes-root>`` is what
    :func:`thefool_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.hermes/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``THEFOOL_HOME`` deployments at
    ``<THEFOOL_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("THEFOOL_SHARED_AUTH_DIR", "").strip()
    if override:
...
def _nous_shared_store_path() -> Path:
    ...
    # state. Tests must set THEFOOL_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set THEFOOL_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No THEFOOL_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        # THEFOOL_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
    ...
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("THEFOOL_PORTAL_BASE_URL")
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    raw_args = os.getenv("THEFOOL_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from thefool_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from thefool_cli.config import clear_model_endpoint_credentials
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`thefool_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (thefool_cli.model_selection_guards): the cost
    ...
    try:
        ...
        from thefool_cli.model_search import model_search_text
    ...
    from thefool_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from thefool_cli.config import save_config, load_config
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("THEFOOL_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``THEFOOL_PORTAL_BASE_URL`` set
