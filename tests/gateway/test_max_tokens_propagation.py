...
Precedence verified here:
    FOOL_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    """Isolated FOOL_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``fool_cli`` / ``gateway`` so each
    ...
    monkeypatch.delenv("FOOL_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("fool_cli", "gateway"))
    }
    ...
    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("fool_cli", "gateway")):
                del sys.modules[mod]
