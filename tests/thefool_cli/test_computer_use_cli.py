...
def _run(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "thefool_cli.main", "computer-use", *args],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def _invoke(monkeypatch: pytest.MonkeyPatch, *args: str) -> int:
    ...
    cli_main = import_module("thefool_cli.main")
...
def test_computer_use_status_returns_nonzero_for_incompatible_custom_driver(
    ...
    monkeypatch.setenv("THEFOOL_CUA_DRIVER_CMD", driver)
    monkeypatch.setattr(cua_backend, "resolve_cua_driver_cmd", lambda: driver)
    monkeypatch.setattr(
        tools_config,
        "_cua_driver_contract_status",
    ...
    assert _invoke(monkeypatch, "status") == 1
    output = capsys.readouterr().out
    assert "custom binary from THEFOOL_CUA_DRIVER_CMD" in output
