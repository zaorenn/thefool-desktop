...
class TestHandleSkillsSlashInstallFlags:
    ...
    def test_yes_flag_sets_skip_confirm(self):
        ...
        with patch("thefool_cli.skills_hub.do_install") as mock_install:
...
class TestHandleSkillsSlashUninstallFlags:
    ...
    def test_no_flags_still_skips_confirm(self):
        """Slash commands always skip confirmation — input() hangs in TUI."""
        from thefool_cli.skills_hub import handle_skills_slash
        with patch("thefool_cli.skills_hub.do_uninstall") as mock_uninstall:
...
class TestDoInstallSkipConfirm:
    ...
    @patch("thefool_cli.skills_hub.input", return_value="n")
    def test_without_skip_confirm_prompts_user(self, mock_input):
        ...
        with patch("thefool_cli.skills_hub._console"), \
             patch("tools.skills_hub.ensure_hub_dirs"), \
             patch("tools.skills_hub.GitHubAuth"), \
             patch("tools.skills_hub.create_source_router") as mock_router, \
             patch("thefool_cli.skills_hub._resolve_short_name", return_value="test/skill"), \
             patch("thefool_cli.skills_hub._resolve_source_meta_and_bundle") as mock_resolve:
...
class TestDoUninstallSkipConfirm:
    ...
    def test_skip_confirm_bypasses_input(self):
        ...
        from thefool_cli.skills_hub import do_uninstall
        with patch("thefool_cli.skills_hub._console") as mock_console, \
