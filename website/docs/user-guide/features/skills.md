...
By default every profile is seeded with the bundled skill catalog, and each `fool update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:
...
```bash
fool profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
fool skills opt-out            # stop future seeding — nothing on disk is touched
fool skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
fool skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `fool update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `fool skills opt-in`) to re-enable.

:::note Safe by default
`fool skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Hermes installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
```bash
fool chat --toolsets skills -q "What skills do you have?"
fool chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, Hermes asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `fool setup` or `~/.hermes/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `fool config migrate` prompts for unconfigured settings, and `fool config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
- `description` (optional) — short text shown in `/bundles` and `fool bundles list`.
...
fool bundles create research

# Overwrite an existing bundle
fool bundles create backend-dev --skill ... --force
...
```bash
fool skills browse                              # Browse all hub skills (official first)
fool skills browse --source official            # Browse only official optional skills
fool skills search kubernetes                   # Search all sources
fool skills search react --source skills-sh     # Search the skills.sh directory
fool skills search https://mintlify.com/docs --source well-known
fool skills inspect openai/skills/k8s           # Preview before installing
fool skills install openai/skills/k8s           # Install with security scan
fool skills install official/security/1password
fool skills install skills-sh/vercel-labs/json-render/json-render-react --force
fool skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
fool skills list --source hub                   # List hub-installed skills
fool skills check                               # Check installed hub skills for upstream updates
fool skills update                              # Reinstall hub skills with upstream changes when needed
fool skills audit                               # Re-scan all hub skills for security
fool skills uninstall k8s                       # Remove a hub skill
fool skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
fool skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
fool skills publish skills/my-skill --to github --repo owner/repo
fool skills snapshot export setup.json          # Export skill config
fool skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `fool skills search <query> --source skills-sh`. Hermes resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
```bash
fool skills search https://mintlify.com/docs --source well-known
fool skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
fool skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```
...
```bash
fool skills search airbnb --source browse-sh
fool skills inspect browse-sh/airbnb.com/search-listings-ddgioa
fool skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
fool skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
fool skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `fool skills update` re-fetches from the same URL automatically when you want to refresh.
...
`fool skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
```bash
fool skills check          # Report which installed hub skills changed upstream
fool skills update         # Reinstall only the skills with updates available
fool skills update react   # Update one specific installed hub skill
fool skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `fool skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Hermes users add with `fool skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
```bash
fool skills tap add my-org/hermes-skills
fool skills search deploy
fool skills install my-org/hermes-skills/deploy-runbook
```
...
The `fool skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `fool skills tap list` shows the effective path per tap.
...
```bash
fool skills install owner/repo/skills/my-workflow
```
...
```bash
fool skills tap list                                # show all configured taps
fool skills tap add myorg/skills-repo               # add (default path: skills/)
fool skills tap remove myorg/skills-repo            # remove
```
...
## Bundled skill updates (`fool skills reset`)

Hermes ships with a set of bundled skills in `skills/` inside the repo. On install and on every `fool update`, a sync pass copies those into `~/.hermes/skills/` and records a manifest at `~/.hermes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).
...
`fool skills reset` is the escape hatch:
...
fool skills reset google-workspace --restore
