...
try:
    from fool_cli.banner import prefetch_update_check
...
try:
    _slash_timeout = float(os.environ.get("FOOL_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("FOOL_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("FOOL_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # gateway's launch FOOL_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["FOOL_HOME"] assignment.
        ...
        env = build_subprocess_env(
            ...
            extra={"FOOL_HOME": str(profile_home)} if profile_home else None,
        )
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from fool_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from fool_cli.lifecycle import invoke_hook
...
try:
    _SESSION_TTL_S = float(os.environ.get("FOOL_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from fool_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from fool_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its FOOL_HOME
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s FOOL_HOME around a pet RPC handler.
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``fool dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("FOOL_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # FOOL_HOME so config/skills/model resolve to it, and hand the
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``hermes`` / ``hermes --tui`` / CLI) is started from a
    ...
    try:
        ...
        from fool_state import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from fool_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``fool_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg() -> dict:
    ...
    ``fool_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _apply_managed(cfg: dict) -> dict:
    ...
    fool_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read FOOL_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # FOOL_SESSION_ID even though agent_init set it via
...
def resolve_skin() -> dict:
    try:
        from fool_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``fool config set display.skin``) OR edited the active skin's colors in
...
def _resolve_model() -> str:
    env = (
        os.environ.get("FOOL_MODEL", "")
        or os.environ.get("FOOL_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from fool_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``FOOL_DESKTOP=1`` and ``FOOL_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``FOOL_DESKTOP_TERMINAL=1`` → "tui"
    ...
    """
    if is_truthy_value(os.environ.get("FOOL_DESKTOP")) and not is_truthy_value(
        os.environ.get("FOOL_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads FOOL_MODEL /
    FOOL_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    ...
    # No fallback to _resolve_model() here: that reads FOOL_MODEL /
    # FOOL_INFERENCE_MODEL, which `hermes --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("FOOL_TUI_PROVIDER", "").strip()
    ...
    try:
        from fool_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("FOOL_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
from fool_state import _BARE_BILLING_PROVIDERS
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind FOOL_HOME to the session's profile so load_soul_md() and
...
def _load_approval_mode() -> str:
    ...
    canonical ``fool_cli.config.load_config`` path applies managed-scope
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("FOOL_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    paths run with ``FOOL_DESKTOP=1``. Keying GUI capability off that env var
...
def _resolve_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] FOOL_TUI_TOOLSETS=all enables every toolset; "
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown FOOL_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in FOOL_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid FOOL_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid FOOL_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    cfg = None
    try:
        from fool_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (FOOL_MODEL /
    # FOOL_INFERENCE_MODEL / FOOL_TUI_PROVIDER / FOOL_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # FOOL_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("FOOL_DEV_CREDITS")):
        try:
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            try:
                from fool_cli.personality import available_personalities
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from fool_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from fool_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from fool_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    """Built-ins + user overrides, via fool_cli.personality (single owner)."""
    from fool_cli.personality import available_personalities
...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    Same contract as fool_cli.personality.resolve_personality — (name,
    ...
    """
    from fool_cli.personality import normalize_personality_name
...
def _prompt_text(value) -> str:
    ...
    Delegates to fool_cli.personality (single owner).
    """
    from fool_cli.personality import prompt_text
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("FOOL_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("FOOL_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses fool_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("FOOL_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("FOOL_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("FOOL_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("FOOL_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped FOOL_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / FOOL_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The FOOL_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("FOOL_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
# (agent/billing_view.py + fool_cli/nous_billing.py) — same as /topup.
...
#
# Layout:  $FOOL_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _maybe_fire_tui_loop_tick(sid: str, session: dict) -> None:
    ...
    """
    try:
        from fool_cli.loops import LoopManager, goal_blocks_loop_tick
    except Exception:
        return
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=FOOL_SESSION_KEY`` (see
    ...
    # DB when FOOL_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``FOOL_UI_SESSION_ID`` as
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from fool_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from fool_cli.voice import is_continuous_active
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: fool_cli.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific FOOL_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile FOOL_HOME
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile FOOL_HOME override, while the docker/ssh
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from fool_cli.model_switch import parse_model_switch_args
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("FOOL_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
    ...
    if key in {"prompt", "personality", "skin"}:
        try:
            ...
            if key == "prompt":
                ...
                # through fool_cli.personality (single owner) and never
                # touches the user-owned global system prompt.
                from fool_cli.personality import persist_personality
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under FOOL_HOME (~/.hermes by
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    FOOL_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: FOOL_HOME itself and the dirs in
...
def _discover_repos_payload(
    ...
    repos even with zero fool sessions. Session-derived roots cover repos
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if a0 == "gateway":
        return "`fool gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`fool sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`fool config edit` needs $EDITOR in a real terminal"
    return None
...
def _model_picker_context(agent):
    ...
    from fool_cli.inventory import load_picker_context
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("FOOL_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("FOOL_VOICE_TTS", "").strip() == "1"


def _any_session_running() -> bool:
    """True while any session's agent turn is in flight.

    Registered as the voice busy-probe (``fool_cli.voice.set_voice_busy_probe``)
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["FOOL_VOICE"] = "0"
                    os.environ["FOOL_VOICE_TTS"] = "0"
                    try:
                        from fool_cli.voice import stop_continuous
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via fool_cli.voice.speak_text with spoken barge-in.
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["FOOL_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["FOOL_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from fool_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from fool_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import fool_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from fool_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from fool_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
