...
- `SOUL.md` — a durable persona file that lives in `THEFOOL_HOME` and serves as the agent's identity (slot #1 in the system prompt)
...
More precisely, it uses the current instance's `THEFOOL_HOME`, so if you run Hermes with a custom home directory, it will use:

```text
$THEFOOL_HOME/SOUL.md
```
...
If Hermes loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `THEFOOL_HOME`, the personality belongs to the Hermes instance itself.
