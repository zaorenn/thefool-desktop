...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `fool update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``fool curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``fool update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``fool curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `fool curator run --dry-run` to preview now"
            )
            save_state(state)
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: fool curator status
        keep an umbrella stable: fool curator pin document-tools
    ...
    lines.append("full report: fool curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: fool curator pin {example}"
            )
    return "\n".join(lines)
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``fool curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `fool curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `fool update` only if "
        ...
        # recorded in state so `fool curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``fool model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `fool model` → auxiliary
