...
This is DISTINCT from ``thefool_cli.config.is_managed()`` / ``THEFOOL_MANAGED``,
...
def _under_pytest() -> bool:
    ...
    that exercise managed scope set ``THEFOOL_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$THEFOOL_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
    ...
    """
    override = os.environ.get("THEFOOL_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through thefool_cli.config.load_config) to honor managed
    scope. Mirrors thefool_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from thefool_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
