...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  ...
  assert.match(out, /FOOL_DESKTOP=1/)
})
...
test('open() abort kills an in-flight SSH child instead of waiting for timeout', async () => {
  ...
  const conn = new SshConnection(
    { host: 'box', user: 'me' },
    { spawnFn: () => child, mux: true, controlDir: '/tmp/d', connectTimeoutMs: 1000 }
  )
...
test('open() rejects a control-dir that is a symlink', async () => {
  ...
  try {
    ...
    // Windows refuses symlink creation (EPERM) unless the process is elevated
    // or Developer Mode is on. The BEHAVIOUR under test is real everywhere;
    // only the ability to build the fixture is privileged. Skipping is honest;
    // leaving the test permanently red is not -- a suite nobody can get green
    // is a suite nobody reads, which is how three stale-token bugs shipped.
