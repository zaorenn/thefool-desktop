...
`FOOL_WRITE_SAFE_ROOT` restricts `write_file` and `patch` to the directory prefix(es) you list — anything outside is hard-blocked. Multiple roots are separated by `:` on Unix:
...
```bash
FOOL_WRITE_SAFE_ROOT=/path/to/project:/home/you/.hermes
```
