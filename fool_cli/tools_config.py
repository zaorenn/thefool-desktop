...
from fool_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``fool tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`fool_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from fool_cli.cli_output import (  # noqa: E402 — late import block
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from fool_cli.auth import _read_xai_oauth_tokens
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from fool_cli.plugins import get_plugin_toolset_keys_nowait
...
from fool_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set FOOL_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "FOOL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "FOOL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "FOOL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "FOOL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "FOOL_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("FOOL_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``FOOL_CUA_DRIVER_CMD`` is not obliged to answer
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $FOOL_HOME/bin is never on PATH, so a bare which()
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    a further ~600s wait on Windows). ``fool computer-use install
    ...
    override = os.environ.get("FOOL_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    FOOL_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: fool computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: fool computer-use doctor")
            return False
    if ok and before:
        try:
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    check — only the Hermes-managed ``$FOOL_HOME/bin`` copy does.
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        try:
            from fool_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: fool auth spotify")
            return
        ...
        try:
            ...
            _print_info("    Run later: fool auth spotify")
        except Exception as exc:
        ...
        try:
            from fool_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
        ...
        try:
            ...
            _print_info("    Run later: fool auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: fool auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: fool auth add xai-oauth"
                )
        else:
...
def run_post_setup_command(args) -> int:
    """``fool tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: fool tools post-setup <key>")
        return 2
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by fool tools — default enabled
                enabled_toolsets.add(pts)
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from fool_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (fool_cli/web_server.py
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    """
    try:
        from fool_cli.inventory import (
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, fool mcp
