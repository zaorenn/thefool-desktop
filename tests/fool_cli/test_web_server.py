"""Tests for fool_cli.web_server and related config utilities."""
...
def _install_example_plugin(_isolate_hermes_home):
    """Drop the example-dashboard fixture into the per-test FOOL_HOME
    ...
    ``FOOL_BUNDLED_PLUGINS`` override because the bundled dir is
    ...
    #      and start serving requests against a torn-down FOOL_HOME.
    app = web_server.app
...
class TestSessionTokenInjection:
    """The desktop shell mints FOOL_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import fool_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("FOOL_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("FOOL_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        """Create a TestClient and isolate the state DB under the test FOOL_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import fool_state
        from fool_constants import get_hermes_home
        from fool_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(fool_state, "DEFAULT_DB_PATH", get_hermes_home() / "state.db")
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(fool_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = fool_state.SessionDB.__init__
        ...
        # the class on fool_state covers every open the helper makes.
        monkeypatch.setattr(fool_state.SessionDB, "__init__", counting_init)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        FOOL_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from fool_constants import get_hermes_home
        from fool_cli.config import load_env
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from fool_constants import get_hermes_home
        from fool_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide FOOL_HOME honcho.json; snapshot and
    ...
    def test_get_media_requires_auth(self):
        from fool_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_hermes_spawns_with_action_id(self, monkeypatch):
        ...
        assert calls == [
            (["update"], "hermes-update", {"FOOL_ACTION_ID": "a" * 32})
        ]
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/fool_cli/test_setup_hidden_env.py.
        """
        from fool_cli.web_server import (
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from fool_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from fool_cli.config import save_env_value
        from fool_cli.web_server import _SESSION_TOKEN
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from fool_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from fool_cli.web_server import _parse_model_ids
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from fool_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from fool_cli import profiles as profiles_mod
        from fool_cli.config import custom_endpoint_key_env
        from fool_constants import get_hermes_home
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from fool_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from fool_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from fool_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from fool_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from fool_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from fool_constants import get_hermes_home
        import fool_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        ...
        from fool_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "fool_cli.nous_subscription.get_nous_portal_account_info",
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from fool_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from fool_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from fool_cli.web_server import _normalise_theme_definition
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        ...
        monkeypatch.setattr(
            fool_state, "DEFAULT_DB_PATH", get_hermes_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``fool_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in fool_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home):
        ...
        # Pin the SessionDB to the isolated FOOL_HOME so each test
...
class TestPluginAPIAuth:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home, _install_example_plugin):
        ...
        installs it into the per-test ``FOOL_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into FOOL_HOME by the class-level
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        FOOL_HOME override scopes a request to another profile."""
        from fool_constants import (
            reset_hermes_home_override,
            set_hermes_home_override,
        )
        ...
        monkeypatch.setenv("FOOL_HOME", str(launch_home))
        from fool_cli import web_server
        token = set_hermes_home_override(str(other))
        try:
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets FOOL_HOME=<root>/profiles/<name>) must still discover user
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        import fool_cli.main as main_mod
        ...
        assert env["FOOL_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from fool_cli.pty_bridge import PtyUnavailableError
        ...
        import fool_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from fool_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import fool_cli.main as cli_main
    import fool_cli.web_server as ws
    ...
    gateway_url = env.get("FOOL_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hermes_home, _install_example_plugin):
        ...
        the per-test ``FOOL_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_hermes_home):
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under FOOL_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("FOOL_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
