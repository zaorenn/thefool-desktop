...
def forced_files_client(monkeypatch, tmp_path):
    root = tmp_path / "data"
    monkeypatch.setenv("FOOL_DASHBOARD_FILES_ROOT", str(root))
...
def local_files_client(monkeypatch, tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.delenv("FOOL_DASHBOARD_FILES_ROOT", raising=False)
...
def test_other_credential_store_basenames_blocked(forced_files_client):
    ...
    point the managed root at FOOL_HOME itself (#57505), which contains
