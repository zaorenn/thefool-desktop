...
class TestCronApprovalModeParsing:
    ...
    def test_explicit_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("fool_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "deny"}}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("fool_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "approve"}}):
    ...
    def test_off_maps_to_approve(self):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("fool_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "off"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_allow_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("fool_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "allow"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_yes_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("fool_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "yes"}}):
            assert _get_cron_approval_mode() == "approve"
    ...
    def test_unknown_value_defaults_to_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("fool_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": "maybe"}}):
            assert _get_cron_approval_mode() == "deny"
    ...
    def test_yaml_boolean_false_maps_to_deny(self):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("fool_cli.config.load_config_readonly", return_value={"approvals": {"cron_mode": False}}):
...
class TestCronDenyMode:
    """When FOOL_CRON_SESSION is set and cron_mode=deny, dangerous commands are blocked."""
...
class TestCronApproveMode:
    """When FOOL_CRON_SESSION is set and cron_mode=approve, dangerous commands pass through."""
...
class TestCronDenyModeAllGuards:
    ...
    def test_tirith_import_error_fail_closed_blocks_in_cron_deny(self, monkeypatch):
        ...
        with (
            ...
            mock_patch("tools.approval.detect_dangerous_command",
                       return_value=(False, None, None)),
            mock_patch("fool_cli.config.load_config_readonly",
...
class TestCronModeInteractions:
    ...
    def test_yolo_overrides_cron_deny(self, monkeypatch):
        ...
        # prompt injection from runtime-setting FOOL_YOLO_MODE). When the
...
class TestCronWithGatewayOrigin:
    ...
    cron/scheduler.py binds FOOL_SESSION_PLATFORM via contextvars for
