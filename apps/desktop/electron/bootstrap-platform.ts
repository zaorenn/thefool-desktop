 ...
 * null to keep it enabled. `FOOL_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  const override = String(env.FOOL_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()

  if (GPU_OVERRIDE_ON.has(override)) {
    return 'override (FOOL_DESKTOP_DISABLE_GPU)'
  }
...
function resolveLinuxPasswordStore(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  const requested = String(env.FOOL_DESKTOP_PASSWORD_STORE || '').trim()
  ...
  if (!LINUX_PASSWORD_STORES.has(requested)) {
    return { store: null, warning: `ignoring unknown FOOL_DESKTOP_PASSWORD_STORE value: ${requested}` }
  }
