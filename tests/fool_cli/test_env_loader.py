...
def test_latin1_fallback_stream_honors_override(tmp_path, monkeypatch):
    ...
    from fool_cli.env_loader import _load_dotenv_with_fallback
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    from fool_cli.env_loader import _sanitize_env_file_if_needed

    env_file = tmp_path / ".env"
    content = "FOOL_TEST_KEY=hello_utf32\nSECOND_KEY=world\n"
    raw = codecs.BOM_UTF32_LE + content.encode("utf-32-le")
    env_file.write_bytes(raw)

    with caplog.at_level(logging.WARNING, logger="fool_cli.env_loader"):
...
def test_known_keys_absent_from_user_env_are_cleared(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FOOL_ACP_AUTH_METHOD", "cursor_login")
    ...
    # FOOL_ACP_AUTH_METHOD and COPILOT_CLI_PATH are NOT in the profile .env → cleared
    assert "FOOL_ACP_AUTH_METHOD" not in os.environ
...
def test_empty_assignment_in_user_env_is_preserved(tmp_path, monkeypatch):
    ...
    Empty ``FOOL_ACP_AUTH_METHOD=`` tells the ACP adapter to skip
    ...
    home = tmp_path / "hermes"
    home.mkdir()
    (home / ".env").write_text("FOOL_ACP_AUTH_METHOD=\n", encoding="utf-8")
    ...
    assert "FOOL_ACP_AUTH_METHOD" in os.environ
    assert os.environ["FOOL_ACP_AUTH_METHOD"] == ""
...
def test_no_user_env_does_not_clear_anything(tmp_path, monkeypatch):
    ...
    assert os.getenv("FOOL_ACP_AUTH_METHOD") == "cursor_login"
...
def test_known_key_explicitly_set_in_user_env_is_kept(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "FOOL_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    ...
    assert os.getenv("FOOL_ACP_AUTH_METHOD") == "claude_code_cli"
...
def test_export_prefixed_known_key_in_user_env_is_kept(tmp_path, monkeypatch):
    ...
    home.mkdir()
    (home / ".env").write_text(
        "export FOOL_ACP_AUTH_METHOD=claude_code_cli\n", encoding="utf-8"
    )
    monkeypatch.setenv("FOOL_ACP_AUTH_METHOD", "cursor_login")
    load_hermes_dotenv(hermes_home=home)
    assert os.getenv("FOOL_ACP_AUTH_METHOD") == "claude_code_cli"
...
def test_cleanup_scope_is_the_profile_managed_set():
    ...
    """
    from fool_cli.env_loader import _PROFILE_MANAGED_ENV_KEYS
...
# the shared fool_cli.config.apply_terminal_config_to_env helper.
# ---------------------------------------------------------------------------
...
def _seed_terminal_home(tmp_path, monkeypatch, *, config_yaml=None, env_text=None):
    ...
    # The bridge is scoped to the process FOOL_HOME (a different profile's
