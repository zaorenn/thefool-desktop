...
from thefool_cli import uninstall
...
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Point both ``Path.home()`` and ``THEFOOL_HOME`` at a throwaway dir."""
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))
    monkeypatch.setenv("THEFOOL_HOME", str(home / ".hermes"))
    return home
...
class TestCrashDurability:
    def test_shell_config_survives_an_interrupted_rewrite(self, fake_home: Path):
        ...
        # Path.home()/THEFOOL_HOME patches the fake_home fixture installed.
