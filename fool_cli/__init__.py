...
- fool chat          - Interactive chat (same as ./hermes)
- fool gateway       - Run gateway in foreground
- fool gateway start - Start gateway service
...
def _ensure_utf8():
    ...
    can even start — e.g. `fool setup` on a fresh Pi.
    ...
    fool_cli/stdio.py::configure_windows_stdio() runs later from the entry
