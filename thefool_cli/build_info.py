...
``thefool_cli/dump.py`` and ``thefool_cli/banner.py``).  That doesn't work inside
...
image was built from, the Docker build writes the build-time ``$THEFOOL_GIT_SHA``
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``THEFOOL_GIT_SHA`` build-arg fall through to live-git
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    written by the Dockerfile's ``THEFOOL_GIT_SHA`` build-arg and contains
