...
2. ``fool update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``fool_cli/main.py`` (the ``fool update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``fool_constants`` after the
