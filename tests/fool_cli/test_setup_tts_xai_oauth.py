...
def test_run_xai_oauth_login_from_setup_does_not_hijack_active_provider(
    ...
    `active_provider`), so `fool setup tts` OAuth login hijacked the main
    chat provider.
    """
    ...
    monkeypatch.setattr("fool_cli.auth._is_remote_session", lambda: True)

    from fool_cli.auth import is_source_suppressed, suppress_credential_source
    from fool_cli.setup import _run_xai_oauth_login_from_setup
