...
the existing module-level functions in thefool_cli.gateway and
thefool_cli.gateway_windows directly. This protocol is a thin facade
...
# in ``thefool_cli.gateway`` (systemd/launchd) and ``thefool_cli.gateway_windows``
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in thefool_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from thefool_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from thefool_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from thefool_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from thefool_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in thefool_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from thefool_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from thefool_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from thefool_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from thefool_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``thefool_cli.gateway_windows`` (Scheduled Task /
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's THEFOOL_HOME blindly;
    derive the shared profile root from the current THEFOOL_HOME and map the
    ...
    hermes_home = Path(os.environ.get("THEFOOL_HOME", "/opt/data"))
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources THEFOOL_HOME (and any extra env) via with-contenv —
             so e.g. ``-e THEFOOL_HOME=/data/hermes`` is honored at run
        ...
        root THEFOOL_HOME profile" (the implicit profile that exists at
        the top of $THEFOOL_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``thefool_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$THEFOOL_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``THEFOOL_HOME``,
        ...
        allocator in ``thefool_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export THEFOOL_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's THEFOOL_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``hermes gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-THEFOOL_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The THEFOOL_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own THEFOOL_HOME and s6 guarantees a
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${THEFOOL_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the THEFOOL_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e THEFOOL_HOME=/data/hermes``
        ...
        return (
            ...
            f': "${{THEFOOL_HOME:=/opt/data}}"\n'
            f'log_dir="$THEFOOL_HOME/logs/gateways/{prof}"\n'
