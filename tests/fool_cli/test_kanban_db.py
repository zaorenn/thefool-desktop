"""Tests for the Kanban DB layer (fool_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated FOOL_HOME with an empty kanban DB."""
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("FOOL_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("FOOL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
class TestSharedBoardPaths:
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # FOOL_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile FOOL_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # FOOL_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is FOOL_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["FOOL_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["FOOL_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["FOOL_KANBAN_TASK"] == "t_dispatch_env"
        assert env["FOOL_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "FOOL_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see fool_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import fool_state as _hermes_state
    monkeypatch.setattr(
    ...
    with _patch("fool_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        fool_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "fool_cli.kanban_db.sqlite3.connect",
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'fool kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m fool_cli.main`.
    ...
    import shutil
    import sys
    import fool_cli.kanban_db as kb

    monkeypatch.delenv("FOOL_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_hermes_argv()
    assert argv == [sys.executable, "-m", "fool_cli.main"]
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `fool_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m fool_cli.main --version`
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="fool_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from fool_cli.kanban_db import connect
