...
def server():
    ...
    # e.g. fool_cli.active_sessions would bind the mocked get_hermes_home
    ...
    with patch.dict("sys.modules", {
        "fool_constants": MagicMock(get_hermes_home=MagicMock(return_value="/tmp/hermes_test")),
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `FOOL_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("FOOL_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("FOOL_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient FOOL_HOME
    ...
    # The gateway's own FOOL_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `fool config set` does)
    ...
    import fool_cli.skin_engine as skin_engine
    ...
    # Activate midnight, as `fool config set display.skin midnight` would.
