...
| `THEFOOL_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `THEFOOL_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
Set `THEFOOL_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
