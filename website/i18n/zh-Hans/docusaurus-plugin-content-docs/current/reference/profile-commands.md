...
管理 profile 的顶级命令。不带子命令运行 `fool profile` 将显示帮助信息。
...
| `--description "<text>"` | 一到两句话描述该 profile 的用途。供 kanban 编排器根据角色而非仅凭 profile 名称来路由任务。可跳过，稍后通过 `fool profile describe` 添加。持久化保存在 `<profile_dir>/profile.yaml` 中。 |
| `--no-skills` | 创建一个**空** profile，不启用任何内置 skill。会在 profile 目录中写入 `.no-bundled-skills` 标记，使后续 `fool update` 不再重新植入内置 skill 集，且拒绝与 `--clone`、`--clone-from` 或 `--clone-all` 组合使用（因为这些选项会复制 skill）。适用于不应继承完整 skill 目录的窄化编排器 profile 或沙箱 profile。 |
...
:::info
`fool profile export` / `import` 仍是在**本机进行 profile 本地备份和恢复**的正确命令。发行版（`install` / `update` / `info`）是独立概念：通过 git 分发 profile，供他人安装。
:::
...
`fool profile list` 也会在 `Distribution` 列中显示发行版名称和版本，`fool profile show <name>` / `delete <name>` 会显示来源 URL，让你一眼看出哪些 profile 来自 git 仓库，哪些是本地创建的。
...
3. 告知接收方运行 `fool profile install <your-repo-url>`。
...
安装后，Tab 补全适用于：
- `fool profile <TAB>` — 子命令（list、use、create 等）
- `fool profile use <TAB>` — profile 名称
