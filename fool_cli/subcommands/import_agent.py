...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into The Fool",
        description=(
            "One-command import of another coding agent's setup into The Fool. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Fool equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'fool setup' for those."
        ),
    )
    parser.add_argument(
        "agent",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing The Fool items on name conflicts (default: skip)",
    )
    parser.add_argument(
