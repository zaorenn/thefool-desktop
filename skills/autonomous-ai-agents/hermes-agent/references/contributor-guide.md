...
Config: `~/.hermes/config.yaml` (settings), `~/.hermes/.env` (API keys) — both under `$FOOL_HOME` when it is set.
...
1. Add `CommandDef` to `COMMAND_REGISTRY` in `fool_cli/commands.py`
...
- Tests auto-redirect `FOOL_HOME` to temp dirs — never touch real `~/.hermes/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/fool_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_hermes_home()` from `fool_constants` for all paths (profile-safe)
