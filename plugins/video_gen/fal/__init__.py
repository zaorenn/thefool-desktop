    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                ...
                f"  • Pick a different model via `fool tools` → Video Generation."
            ) from exc
        raise
        ...
        if not _check_fal_video_available():
            return error_response(
                error=(
                    ...
                    "(run `fool tools` → Video Generation → FAL to configure) "
                    "or sign in to Nous (`fool setup`) for managed gateway access."
                ),
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `fool tools` → Video Generation."
                    ),
