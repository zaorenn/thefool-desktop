...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``FOOL_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``FOOL_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``fool kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.hermes``. In Docker / custom
deployments where ``FOOL_HOME`` points outside ``~/.hermes`` (e.g.
``/opt/hermes``), ``<root>`` is ``FOOL_HOME``. Legacy env-var
overrides still work:

* ``FOOL_KANBAN_DB`` — pin the database file path directly.
* ``FOOL_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``FOOL_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``FOOL_KANBAN_DB``,
``FOOL_KANBAN_WORKSPACES_ROOT``, and ``FOOL_KANBAN_BOARD`` into
...
from fool_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``fool_constants.VALID_REASONING_EFFORTS`` plus
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("FOOL_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active FOOL_HOME so dispatcher- and
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    """
    try:
        from fool_cli.lifecycle import has_hook
...
# ``FOOL_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``FOOL_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("FOOL_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``FOOL_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("FOOL_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``FOOL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "FOOL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
def kanban_home() -> Path:
    ...
    1. ``FOOL_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``FOOL_HOME`` is ``<root>/profiles/<name>``, and returns
       ``FOOL_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``FOOL_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("FOOL_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``fool kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``FOOL_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``fool kanban boards
    ...
    env = os.environ.get("FOOL_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``fool kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``FOOL_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("FOOL_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``FOOL_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("FOOL_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``FOOL_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("FOOL_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``fool kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``fool_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``FOOL_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (fool_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``FOOL_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("FOOL_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``fool_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from fool_cli.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``fool gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from fool_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see fool_cli.sqlite_safe_read), so it must only run once the board has
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``fool kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``FOOL_KANBAN_DB`` env → ``FOOL_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `fool kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from fool_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        from fool_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See fool_state._WAL_INCOMPAT_MARKERS for detection logic.
                from fool_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``fool kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``FOOL_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from fool_cli.sqlite_safe_read import file_length_matches_header
...
def create_task(
    ...
    *,
    ...
    if project_id:
        from fool_cli import projects_db as _pdb
...
# Canonical sort-order mappings for ``fool kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `fool kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``fool kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``fool kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``fool kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``fool kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``fool kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``FOOL_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
...
#      via ``fool kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``FOOL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``fool kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``fool kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from fool_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def resolve_max_in_progress(configured: Optional[int]) -> Optional[int]:
    ...
    Callers that parse config (gateway dispatcher, ``fool kanban dispatch``)
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``FOOL_KANBAN_DB``
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from fool_cli.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from fool_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `fool kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from fool_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_hermes_argv() -> list[str]:
    ...
    # ``fool_cli.main`` is the console-script target declared in
...
def _resolve_hermes_argv() -> list[str]:
    ...
    1. ``$FOOL_BIN`` — explicit operator override. Path-like values are
    ...
    3. ``sys.executable -m fool_cli.main`` — fallback for setups where
    ...
    local (not imported from gateway) because ``fool_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("FOOL_BIN", "").strip()
    if env_bin:
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``FOOL_KANBAN_TASK`` is set.
    """
    if not hermes_home:
        return None
    try:
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for FOOL_HOME=%r (%s)",
            hermes_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    ``FOOL_KANBAN_DB`` / ``FOOL_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject FOOL_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `hermes -p <name>` the
    # _apply_profile_override() runs *before* fool_constants is imported.
    # If FOOL_HOME is absent from the child's env, get_hermes_home() falls
    # back to Path.home() / ".hermes" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from fool_cli.profiles import resolve_profile_env
    try:
        env["FOOL_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via FOOL_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # FOOL_HOME never had profiles created.
        pass
    if task.tenant:
        env["FOOL_TENANT"] = task.tenant
    env["FOOL_KANBAN_TASK"] = task.id
    env["FOOL_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `fool kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["FOOL_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["FOOL_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["FOOL_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["FOOL_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["FOOL_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["FOOL_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # rewrites FOOL_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hermes_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["FOOL_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["FOOL_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["FOOL_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["FOOL_KANBAN_BOARD"] = resolved_board
    # FOOL_PROFILE is the author the kanban_comment tool defaults to.
    # `hermes -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["FOOL_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited FOOL_TUI=1
    ...
    # older fool builds on PATH that predate the flag's precedence.
    env.pop("FOOL_TUI", None)

    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped FOOL_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("FOOL_HOME"))
    if worker_toolsets:
    ...
    # `fool kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``fool kanban daemon`` is systemd-friendly.
    ...
    dispatcher and ``fool kanban dispatch`` — the standalone daemon must
    ...
    while not stop_event.is_set():
        try:
            ...
            # dispatcher and `fool kanban dispatch` do (OOF-30): explicit
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled FOOL_PROFILE values like "hermes-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``fool_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from fool_constants import get_default_hermes_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``fool kanban assignees`` for the terminal.
