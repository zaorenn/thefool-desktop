...
Pure display functions with no FoolCLI state dependency.
"""
import json
import logging
import os
...
from fool_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

# Çizim TEK KAYNAKTAN geliyor (``fool/brand_art.py``).
#
# Burada bir kopya duruyordu ve ``cli.py``deki kopya elle düzeltildiğinde
# bu atlandı -- CANLI olan buydu, yani ``fool`` yazan herkes dev harflerle
# HERMES-AGENT ve altında Hermes'in asasını görüyordu. Harfler ``█``dan
# çizildiği için hiçbir metin taraması bunu yakalayamıyor.
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `fool update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Fool update is available.

    Two paths: if ``FOOL_REVISION`` is set (nix builds embed it), compare
    ...
    embedded_rev = os.environ.get("FOOL_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # FOOL_REVISION (that's nix-only). Returning None makes both the Rich
    ...
    try:
        from fool_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $FOOL_HOME/hermes-agent/ may be a stale copy from --clone-all;
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Fool git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$FOOL_HOME/hermes-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``fool_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from fool_cli.build_info import get_build_sha
...
def format_banner_version_label() -> str:
    ...
    base = f"{_FOOL_NAME} v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from fool_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else FOOL_CADUCEUS
    except Exception:
        _bskin = None
        _hero = FOOL_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Fool Labs[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Fool Labs[/]")

    if os.getenv("FOOL_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from fool_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from fool_cli.codex_runtime_switch import get_current_runtime
        from fool_cli.config import load_config as _load_cfg
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else FOOL_AGENT_LOGO
        console.print(_logo)
        console.print()
