...
ancestors by design — a CLI ``fool update`` IS the venv python.  Before
...
Windows ``fool update`` exited 2 before even fetching and the update
    ...
    def test_import_main_does_not_load_self_locking_modules(self):
        result = self._run(
            textwrap.dedent(
                ...
                from fool_cli.update_cmd import _SELF_LOCKING_NATIVE_MODULES
                loaded = [
    ...
    def test_update_dispatch_does_not_load_cryptography(self):
        result = self._run(
            textwrap.dedent(
                ...
                with patch("fool_cli.main._cmd_update_check", lambda *a, **k: 0):
                    try:
                        m.main()
                    except SystemExit:
                        pass
