...
key is present in `${THEFOOL_HOME:-~/.hermes}/.env` (or macOS Keychain) before firing the kanban.
...
preference: macOS Keychain → `${THEFOOL_HOME:-~/.hermes}/.env` → environment variable.
