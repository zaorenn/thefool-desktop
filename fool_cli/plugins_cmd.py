...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from fool_cli.config import get_env_value, save_env_value  # noqa: F811
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update The Fool.",
                ) from None
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from fool_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (target / "plugin.json").exists() and not (
        ...
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Fool plugin.",
        )
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from fool_cli.plugin_capabilities import parse_declared_capabilities
    ...
    for entry in _discover_all_plugins():
        ...
        if entry[5] == key or entry[0] == key:
            if entry[3] == "entrypoint":
                from fool_cli.plugins import discover_entrypoint_manifests
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from fool_cli.plugin_capabilities import CAPABILITY_REGISTRY
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        from fool_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``fool_cli.main`` never covers them. After a
    ...
    try:
        ...
        if dirty:
            ...
            push = _run_plugin_git(
                ...
                "-m", "fool-plugin-update-autostash",
            )
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from fool_cli.plugin_dev import doctor_plugin
...
def cmd_search(
    ...
    *,
    ...
    from fool_cli.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        search_index,
    )
...
def plugins_command(args) -> None:
    ...
    if action == "install":
        ...
        from fool_cli.plugin_packs import pack_command
