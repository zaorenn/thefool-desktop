...
from fool_cli.dashboard_auth.base import (
    InvalidCodeError,
    InvalidCredentialsError,
    ProviderError,
)
from fool_cli.dashboard_auth.cookies import (
    clear_pkce_cookie,
...
from fool_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``FOOL_DASHBOARD_PUBLIC_URL`` env var or
    ...
    from fool_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``fool_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from fool_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_native_authorize(
    ...
    from fool_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from fool_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from fool_cli.dashboard_auth import list_session_providers
    from fool_cli.dashboard_auth.base import RefreshExpiredError
