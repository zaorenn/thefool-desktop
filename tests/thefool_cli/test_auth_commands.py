...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from thefool_cli.auth_commands import auth_add_command
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "thefool_cli.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("thefool_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from thefool_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from thefool_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "thefool_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from thefool_cli.auth_commands import auth_remove_command
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from thefool_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from thefool_cli.auth import is_source_suppressed
    from thefool_cli.auth_commands import auth_remove_command

    with patch(
        "thefool_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "thefool_cli.copilot_auth.get_copilot_api_token",
