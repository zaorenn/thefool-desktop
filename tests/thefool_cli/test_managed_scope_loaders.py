...
thefool_cli.config.load_config, so the managed overlay has to be wired into each.
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("THEFOOL_MANAGED_DIR", str(managed))
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # thefool_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("THEFOOL_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import thefool_time

    assert thefool_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (THEFOOL_TIMEZONE, THEFOOL_REDACT_SECRETS, THEFOOL_MAX_ITERATIONS,
    ...
    # writing THEFOOL_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
