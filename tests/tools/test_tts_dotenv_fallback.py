...
``fool_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``fool_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    ...
    ``fool_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``fool_cli.config.load_env`` to simulate ``~/.hermes/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while fool_cli.config.get_env_value is patched must
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            # os.environ is empty.
            from fool_cli.config import get_env_value as live_get
