...
3. Point Hermes at the extracted bundle — the directory that contains
   ...
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<FOOL_HOME>/lsp/PowerShellEditorServices`, or
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <FOOL_HOME>/lsp/bin
...
`<FOOL_HOME>/lsp/bin/`. NPM packages land in
`<FOOL_HOME>/lsp/node_modules/` with bin symlinks one level up.
...
The binary isn't on PATH and isn't in `<FOOL_HOME>/lsp/bin/`. Run
