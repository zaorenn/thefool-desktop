...
REAL_ROOT = thefool_state._real_platform_state_root()
...
class TestProductionPathRefused:
    ...
    def test_default_resolution_to_production_raises(self, monkeypatch):
        ...
        Simulates the escape vector: THEFOOL_HOME leaked/reset to the real
        home (subprocess child, stale worktree, gateway-launched shell) so
        ``_default_db_path()`` resolves the production DB.
        """
        monkeypatch.setenv("THEFOOL_HOME", str(REAL_ROOT))
        ...
        monkeypatch.setattr(
            thefool_state, "DEFAULT_DB_PATH", thefool_state._IMPORT_DEFAULT_DB_PATH
        )
        with pytest.raises(RuntimeError, match="live-system guard"):
            SessionDB()
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_hermes_home_default_resolution_works(self, tmp_path, monkeypatch):
        """Argless SessionDB() under a hermetic THEFOOL_HOME must succeed."""
        monkeypatch.setenv("THEFOOL_HOME", str(tmp_path / "hermetic-home"))
        monkeypatch.setattr(
            thefool_state, "DEFAULT_DB_PATH", thefool_state._IMPORT_DEFAULT_DB_PATH
        )
        db = SessionDB()
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        thefool_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSessionStoreLoudFailure:
    def test_guard_error_is_not_swallowed_into_jsonl_fallback(
        ...
        monkeypatch.setattr(thefool_state, "SessionDB", _boom)
        with pytest.raises(RuntimeError, match="live-system guard"):
...
class TestSubprocessChildCovered:
    def test_child_without_hermes_home_is_refused(self, tmp_path):
        """A subprocess child of a test (no THEFOOL_HOME) must be blocked.
        ...
        env = {
            ...
            if k not in ("THEFOOL_HOME", "PYTEST_PLUGINS", "PYTHONPATH")
        }
    ...
    def test_child_with_tmp_hermes_home_succeeds(self, tmp_path):
        """Same child, hermetic THEFOOL_HOME: must work — no false positive."""
        env = {
            k: v
            for k, v in os.environ.items()
        ...
        env["PYTEST_CURRENT_TEST"] = "tests/fake.py::test_child (call)"
        env["THEFOOL_HOME"] = str(tmp_path / "child-home")
        env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
        code = (
            "from thefool_state import SessionDB\n"
            "db = SessionDB()\n"
