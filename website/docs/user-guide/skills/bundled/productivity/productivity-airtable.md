...
4. Store the token in `${FOOL_HOME:-~/.hermes}/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${FOOL_HOME:-~/.hermes}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
