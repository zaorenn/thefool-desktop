...
def test_refresh_secret_sources_noop_without_plugin_sources(monkeypatch):
    ...
    monkeypatch.setattr(
        "fool_cli.env_loader.reset_secret_source_cache",
        lambda: called.__setitem__("reset", called["reset"] + 1),
    )
    monkeypatch.setattr(
        "fool_cli.env_loader.load_hermes_dotenv",
...
def test_real_plugin_source_discovery_applies_dotenv(monkeypatch, tmp_path):
    ...
    (plugin_dir / "__init__.py").write_text(
        ...
        "    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:\n"
        "        return FetchResult(secrets={\n"
        "            'FOOL_TEST_PLUGIN_BOOTSTRAP': 'from-plugin',\n"
        "        })\n\n"
        "def register(ctx):\n"
    ...
    monkeypatch.delenv("FOOL_TEST_PLUGIN_BOOTSTRAP", raising=False)

    try:
        PluginManager().discover_and_load()

        assert os.environ["FOOL_TEST_PLUGIN_BOOTSTRAP"] == "from-plugin"
        assert [source.name for source in reg.list_plugin_sources()] == [
            "fixturevault"
        ]
    finally:
        os.environ.pop("FOOL_TEST_PLUGIN_BOOTSTRAP", None)
        reg._reset_registry_for_tests()
        env_loader.reset_secret_source_cache()
