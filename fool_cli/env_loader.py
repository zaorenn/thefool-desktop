...
# FOOL_HOME paths we've already pulled external secrets for during this
# process.  ``load_hermes_dotenv()`` is called at module-import time from
# several hot modules (cli.py, fool_cli/main.py, run_agent.py,
...
def _known_hermes_env_keys() -> set[str]:
    ...
    """
    from fool_cli.config import _EXTRA_ENV_KEYS
    from fool_cli.config_defaults import OPTIONAL_ENV_VARS
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    ...
    ``tests/fool_cli/test_dump_env_visibility.py``), and a startup scrub
...
def reset_secret_source_cache() -> None:
    """Forget which FOOL_HOME paths have already had external secrets applied.
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``fool_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from fool_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    from fool_cli import _early_recovery
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``fool_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process FOOL_HOME: the shared bridge reads the
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of FOOL_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    owns the once-per-FOOL_HOME guard, the post-apply ASCII
    ...
    time from several hot modules (cli.py, fool_cli/main.py,
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # shared cache here lets main.py's early bridge and fool_logging reuse
...
def _process_hermes_home() -> Path:
    """The FOOL_HOME the shared config cache is keyed to."""
    try:
