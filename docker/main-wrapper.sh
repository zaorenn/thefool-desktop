...
# with-contenv before touching FOOL_HOME / PATH. On the non-PID-1
...
if [ -z "${FOOL_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${FOOL_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export FOOL_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset FOOL_MAIN_WRAPPER_ENV_READY
...
_hermes_orig_cwd="${FOOL_ORIG_CWD:-$PWD}"
