...
1. ``FoolTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``FoolMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# FoolTokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("FOOL_HOME", str(tmp_path))
        from tools.mcp_oauth import FoolTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# FoolMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = FoolTokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Hermes Agent",
    )
