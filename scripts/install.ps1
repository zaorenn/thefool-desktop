# ============================================================================
# The Fool Installer for Windows
# ============================================================================
...
param(
    ...
    # builds apps/desktop into a launchable The Fool.exe.
    #
    # Why opt-in:
    #   * The Fool-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched The Fool.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live The Fool.exe
...
# FOOL_HOME and InstallDir -- in that short form:
...
# Put the The Fool-managed Node dir at the FRONT of the persisted User PATH.
#
...
# Upgrade the The Fool-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "The Fool-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }
...
function Resolve-UvCmd {
    ...
    # sibling process and FOOL_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under The Fool-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun The Fool setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which The Fool uses to run shell commands.
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    The Fool needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the The Fool copy of Git ourselves is predictable and
    ...
    ``FOOL_GIT_BASH_PATH`` (User scope) so The Fool can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent The Fool features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  The Fool needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``FOOL_GIT_BASH_PATH`` (User env scope) so The Fool can find it even before
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("FOOL_GIT_BASH_PATH", $candidate, "User")
            $env:FOOL_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set FOOL_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- The Fool may not find Git Bash."
    Write-Info "If needed, set FOOL_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (The Fool requires Node >=26)"
    }
    ...
    Write-Info "Installing The Fool-managed Node.js $NodeVersion LTS..."
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed -- mirror ``fool update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if The Fool behaves unexpectedly."
                        } else {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($extractedDir) {
                    ...
                    # `fool update` (see the notes at the shared clone-path
    ...
    # next `fool update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under The Fool-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        if ($env:OS -eq "Windows_NT") {
            ...
            # `pythonw.exe -m fool_cli.main gateway run` straight out of
            ...
            # The Fool children re-exec through .hermes-runtime, so killing only
            ...
            # user (see fool_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-The Fool " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `fool update` / the
...
function Restore-VenvBackup {
    ...
    # venv so The Fool (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
"@ 2>$null
    ...
    # here, not 30 seconds later when the user runs `fool`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close The Fool processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close The Fool processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves The Fool and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        ...
        # `fool` command while hermes-agent.exe / hermes-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m fool_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `fool dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\fool_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `fool dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `fool dashboard` should now work."
            } else {
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        ...
        # (#83797): unrelated projects start resolving python to The Fool'
...
function Write-BootstrapMarker {
    # Writes $InstallDir\.hermes-bootstrap-complete which tells the The Fool
    ...
    # The Fool-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `fool update` which moves HEAD legitimately).
...
function Copy-ConfigTemplates {
    ...
    # (the default PS5 behaviour), and The Fool's prompt-injection scanner
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (The Fool-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'fool setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
...
# failure is non-fatal (browser_exec can still run via uvx, and `fool tools`
...
function Install-BrowserUseCli {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside The Fool' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'fool tools')"
        }
    } catch {
...
# paths still lazy-install via install_cua_driver() (fool_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `fool computer-use install` runs. Bounded
        ...
        # The Fool' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                ...
                Write-Info "Install later with: fool computer-use install"
            }
        } else {
...
# the final `electron` -> `The Fool` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable The Fool.exe. Only called from
    ...
    # resolves apps/desktop/release/win-unpacked/The Fool.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the The Fool-managed
    ...
    # The --dir mode produces an unpacked The Fool.exe in
    ...
    # are NOT trying to work around). The The Fool icon + product name are
    # stamped onto The Fool.exe by our own rcedit step (Set-DesktopExeIdentity)
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'The Fool.lnk'),
        ...
        # with the The Fool icon, but without this the shortcut can keep drawing
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'fool setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m fool_cli.main setup
    } else {
        python -m fool_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: fool gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: fool gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: fool gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "   fool setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   fool config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   fool config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   fool gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   fool update       " -NoNewline -ForegroundColor Green
...
if ($IncludeDesktop) {
    ...
    # (The Fool-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding The Fool to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
