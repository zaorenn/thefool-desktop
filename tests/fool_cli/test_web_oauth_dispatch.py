"""Regression tests for the OAuth dispatcher in fool_cli.web_server.
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "fool_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "fool_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "fool_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from fool_cli import web_server as ws
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to fool_cli.auth.get_auth_status (the
    ...
    with patch("fool_cli.auth.get_auth_status", return_value=fake_status):
