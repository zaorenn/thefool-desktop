...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.foolDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.foolDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.foolDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }
  ...
  const signInRemote = async () => {
    ...
    try {
      await window.foolDesktop?.oauthLogoutConnectionConfig?.()
      const result = await window.foolDesktop?.oauthLoginConnectionConfig(remoteReauth.url)
  ...
  const openLogs = () => void window.foolDesktop?.revealLogs().catch(() => undefined)
