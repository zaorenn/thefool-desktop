...
2. Spawn: _default_spawn sets the THEFOOL_KANBAN_GOAL_MODE env vars only
...
class TestCLIJudgeGate:
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from thefool_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("thefool_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("thefool_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("thefool_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("thefool_cli.kanban._worker_run_id_for", lambda _: None)
