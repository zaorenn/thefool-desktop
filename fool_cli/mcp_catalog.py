...
from fool_constants import get_hermes_home, get_optional_mcps_dir
...
from fool_cli.cli_output import prompt as _prompt_input
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this The Fool understands version {_MANIFEST_VERSION})"
        )
...
def _build_server_config(
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from fool_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from fool_cli.mcp_config import _probe_single_server
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    from fool_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new Fool session to load its tools.",
        Colors.GREEN,
    ))
