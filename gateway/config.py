...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from fool_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (fool_cli/web_server.py) so
...
class GatewayConfig:
    ...
    # gateway behaves exactly as before — single FOOL_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through fool_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `fool config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `fool config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``fool config set gateway.streaming.*``
