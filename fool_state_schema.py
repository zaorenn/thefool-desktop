...
``fool_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import fool_state (cycle) — shared
module-level constants live in fool_state_common.
"""
...
# Moved methods logged under the "fool_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("fool_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``fool_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `fool update` until the user's
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `fool sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `fool db optimize`) must keep its EXISTING
