...
from thefool_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from thefool_cli import codex_runtime_plugin_migration as crpm
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "thefool_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: THEFOOL_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``THEFOOL_HOME``
    ...
    def test_real_hermes_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate THEFOOL_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("THEFOOL_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("THEFOOL_HOME") == real_path

    def test_unset_hermes_home_omits_env_key(self, monkeypatch):
        """When THEFOOL_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever THEFOOL_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert "THEFOOL_HOME" not in env, (
            f"THEFOOL_HOME should not be set when env var is unset, got: "
            f"{env.get('THEFOOL_HOME')!r}"
        )
