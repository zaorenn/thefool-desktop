  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('The Fool is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /The Fool is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\The Fool's\\python.exe" }, 'inspect', [
    'C:\\x y\\fool.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'fool_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /The Fool''s/)
  assert.match(script, /C:\\x y\\fool\.exe/)
})
