...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hermes desktop`` / ``hermes gui``)
     Built inside the agent checkout under ``$THEFOOL_HOME/hermes-agent/``:
       ...
       - ``$THEFOOL_HOME/desktop-build-stamp.json`` (the build freshness stamp)
...
the app name ("Hermes"), separate from ``$THEFOOL_HOME``:
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    if sys.platform == "darwin":
        ...
        # ``uninstall_gui``.
        from thefool_cli.linux_desktop_entry import desktop_entry_path
...
def agent_is_installed(hermes_home: Path) -> bool:
    """Return True when a usable Python agent install exists under THEFOOL_HOME.
    ...
    if (agent_root / "thefool_cli").is_dir():
        return True
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``hermes-agent/thefool_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$THEFOOL_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            from thefool_cli.linux_desktop_entry import (
                desktop_entry_path,
