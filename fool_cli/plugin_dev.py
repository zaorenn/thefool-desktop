"""Runtime-backed validation behind ``fool plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    stack.enter_context(
        patch.dict(
            ...
            {
                "FOOL_HOME": str(home),
                "FOOL_BUNDLED_PLUGINS": str(bundled),
                "FOOL_ENABLE_PROJECT_PLUGINS": "0",
            },
            clear=False,
        )
    )
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from fool_cli.plugins import SUPPORTED_MANIFEST_VERSION
    ...
    if schema:
        from fool_cli.plugins import _CONFIG_SCHEMA_TYPES
