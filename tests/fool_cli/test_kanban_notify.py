...
def kanban_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FOOL_MEDIA_ALLOW_DIRS", str(tmp_path))
    kb.init_db()
    return home
...
async def test_notifier_artifact_delivery_skips_missing_files(kanban_home, tmp_path, monkeypatch):
    ...
    import os
    os.environ["FOOL_KANBAN_TASK"] = tid
    try:
        kt._handle_complete({
        ...
        os.environ.pop("FOOL_KANBAN_TASK", None)
...
def test_migration_backfills_legacy_gateway_subs_to_notify_wake(kanban_home):
    from fool_cli.kanban_db import _migrate_add_optional_columns
