...
class TestBuildOrHeaders:
    ...
    def test_none_config_load_config_fails_gracefully(self):
        ...
        with patch("fool_cli.config.load_config", side_effect=RuntimeError("boom")), patch("fool_cli.config.load_config_readonly", side_effect=RuntimeError("boom")):
...
class TestEnvVarOverrides:
    ...
    def test_invalid_env_ttl_dropped(self, monkeypatch, ttl):
        ...
        monkeypatch.setenv("FOOL_OPENROUTER_CACHE", "1")
        monkeypatch.setenv("FOOL_OPENROUTER_CACHE_TTL", ttl)
    ...
    def test_valid_env_ttl_boundaries(self, monkeypatch, ttl):
        ...
        monkeypatch.setenv("FOOL_OPENROUTER_CACHE", "yes")
        monkeypatch.setenv("FOOL_OPENROUTER_CACHE_TTL", ttl)
    ...
    def test_no_env_vars_falls_through_to_config(self, monkeypatch):
        ...
        monkeypatch.delenv("FOOL_OPENROUTER_CACHE", raising=False)
        monkeypatch.delenv("FOOL_OPENROUTER_CACHE_TTL", raising=False)
