...
1. `fool_cli/auth.py` decides how credentials are found.
2. `fool_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `fool_cli/models.py` and `fool_cli/main.py` make the provider show up in the CLI. (`fool_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `fool_cli/auth.py`
2. `fool_cli/models.py`
3. `fool_cli/runtime_provider.py`
4. `fool_cli/main.py`
...
:::tip
`fool_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `fool setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$FOOL_HOME/plugins/model-providers/` both get picked up.
...
7. `fool model` menu includes the provider
8. `fool setup` wizard delegates to `main.py` automatically
...
User plugins at `$FOOL_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `fool model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `fool_cli/auth.py`
- `_PROVIDER_LABELS` in `fool_cli/models.py`
- `_PROVIDER_ALIASES` in both `fool_cli/auth.py` and `fool_cli/models.py`
- CLI `--provider` choices in `fool_cli/main.py`
...
## Step 2: Add auth metadata in `fool_cli/auth.py`
...
## Step 3: Add model catalog and aliases in `fool_cli/models.py`
...
## Step 4: Resolve runtime data in `fool_cli/runtime_provider.py`
...
## Step 5: Wire the CLI in `fool_cli/main.py`

A provider is not discoverable until it shows up in the interactive `fool model` flow.

Update these in `fool_cli/main.py`:
...
:::tip
`fool_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `fool model` and `fool setup` automatically.
:::
...
- `tests/fool_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/fool_cli/test_model_switch_custom_providers.py` (and adjacent `tests/fool_cli/test_model_switch_*.py`)
- `tests/fool_cli/test_setup_model_provider.py`
...
python -m pytest tests/fool_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/fool_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m fool_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `fool_cli/auth.py`
- [ ] aliases added in `fool_cli/auth.py` and `fool_cli/models.py`
- [ ] model catalog added in `fool_cli/models.py`
- [ ] runtime branch added in `fool_cli/runtime_provider.py`
- [ ] CLI wiring added in `fool_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `fool model` but not `fool setup`
