...
| | |
|---|---|
| Source | Optional — install with `fool skills install official/payments/stripe-link-cli` |
...
```
fool mcp add stripe-link --command "npx" --args "@stripe/link-cli --mcp"
```

Then `fool mcp list` should show `stripe-link`. The same approval rules apply — MCP doesn't bypass the Link app approval step.
