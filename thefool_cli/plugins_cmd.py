...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from thefool_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from thefool_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from thefool_cli.plugin_capabilities import parse_declared_capabilities
    ...
    for entry in _discover_all_plugins():
        ...
        if entry[5] == key or entry[0] == key:
            if entry[3] == "entrypoint":
                from thefool_cli.plugins import discover_entrypoint_manifests
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from thefool_cli.plugin_capabilities import CAPABILITY_REGISTRY
...
def cmd_capabilities(name: Optional[str] = None) -> None:
    ...
    from thefool_cli.plugin_capabilities import granted_capabilities
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    ...
    """
    from thefool_cli.plugins import ENTRY_POINTS_GROUP
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        from thefool_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``thefool_cli.main`` never covers them. After a
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from thefool_cli.plugin_dev import doctor_plugin
...
def cmd_search(
    ...
    *,
    ...
    from thefool_cli.plugin_index import (
        SECURITY_FOOTER,
        load_index,
        search_index,
    )
...
def plugins_command(args) -> None:
    ...
    if action == "install":
        ...
        from thefool_cli.plugin_packs import pack_command
