...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'fool -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 The Fool (@fool): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 fool -p mr-tester chat --in "~" -Q -q "Message from 🤖 The Fool: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('fool -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('fool sessions list')).toBeNull()
  })
})

describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    const output = 'session_id: 20260813_220347_f69ac6\nHi The Fool! Good to hear from you.'

    expect(replyTextFromResult({ output })).toBe('Hi The Fool! Good to hear from you.')
  })
