...
Extracted verbatim from ``thefool_cli/main.py:main()`` (god-file Phase 2).
...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser.add_argument(
        "--hermes-root",
        help="Override the Hermes source root used by Desktop (sets THEFOOL_DESKTOP_HERMES_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets THEFOOL_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
