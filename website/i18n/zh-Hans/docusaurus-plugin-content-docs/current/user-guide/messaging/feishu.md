...
4. 为应用开启 **机器人** 能力。
5. 运行 `fool gateway setup`，选择 **飞书 / Lark**，并在提示时输入凭据。
...
Gateway 驱动的更新提示使用原生飞书 `Yes` / `No` 卡片，而非回退到纯文本回复。当 `fool update --gateway` 需要确认时，适配器将所选答案记录到 Hermes 的 `.update_response` 文件中，并将卡片内联替换为已解决状态。
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | 设置两个环境变量，或通过 `fool gateway setup` 配置 |
