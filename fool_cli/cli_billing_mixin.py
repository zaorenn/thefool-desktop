...
Import discipline mirrors ``fool_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded; honors FOOL_DEV_CREDITS_FIXTURE for offline testing.
        """
    ...
    def _show_subscription(self):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                print("  Run `fool portal` to log in, then /subscription.")
            return
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``fool_cli.auth``): ``webbrowser.open()`` returns ``True`` even when
        ...
        if not url:
            return False
        try:
            from fool_cli.auth import _can_open_graphical_browser, _is_remote_session
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from fool_cli.nous_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `fool portal` and allow Remote Spending, then re-run /subscription.")
            return
        confirm_choices = [
        ...
        try:
            from fool_cli import nous_billing as _nb
    ...
    def _show_billing(self, command: str = "/topup"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `fool portal` to log in, then /topup.")
            return
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `fool portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `fool portal` to log in again.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `fool portal` and allow Remote Spending, then retry.")
            return
        confirm_choices = [
            ("yes", "Allow Remote Spending", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
