...
def container_env(tmp_path, monkeypatch):
    """Set up a fake FOOL_HOME with .container-mode file."""
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    monkeypatch.setenv("FOOL_HOME", str(hermes_home))
    monkeypatch.delenv("FOOL_DEV", raising=False)
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("fool_constants.is_container", return_value=False):
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from fool_cli.main import _exec_in_container
