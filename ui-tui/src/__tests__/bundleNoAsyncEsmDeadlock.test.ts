 ...
 *
 * Root cause: re-exporting `ink-text-input` from `@thefool/ink`'s
 * entry-exports drags the upstream `ink` package into the bundle. That
 * `ink` graph and our in-tree `@thefool/ink` graph reference each other
...
describe('TUI bundle (issue #31227)', () => {
  ...
  it('does not bundle the upstream ink package or ink-text-input', () => {
    ...
    // that #31227 was about. The in-tree fork at @thefool/ink replaces
    // all of `ink`; nothing in ui-tui imports `TextInput` from
    // `@thefool/ink` so the re-export is unused dead weight.
  ...
  it('has the @thefool/ink entry-exports module compiled to sync init', () => {
