...
What it does
------------
1. Spawns a SCRATCH dashboard (``fool dashboard``) bound to loopback on a
   free port, with an ISOLATED ``FOOL_HOME`` (temp dir, minimal seeded state).
...
2. Arms the synthetic GIL-heavy turn seam (``FOOL_ISO_CERTIFY_SYNTH_TURN=1``,
...
the scratch FOOL_HOME; ``--isolation on|off`` sets it. Run BOTH:
...
class ScratchDashboard:
    ...
    def __enter__(self) -> "ScratchDashboard":
        ...
        env["FOOL_HOME"] = str(self.home)
        env["HOME"] = str(self.home.parent) if str(self.home.parent) else env.get("HOME", "")
        env["FOOL_HOME"] = str(self.home)
        env["PYTHONPATH"] = str(REPO_ROOT) + os.pathsep + env.get("PYTHONPATH", "")
        env["FOOL_ISO_CERTIFY_SYNTH_TURN"] = "1"
        cmd = [
            python, "-m", "fool_cli.main", "dashboard",
...
def run_certify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        with ScratchDashboard(
            ...
            env_extra={"FOOL_DASHBOARD_SESSION_TOKEN": token},
        ) as dash:
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--isolation", choices=["on", "off"], default="on",
                   help="set dashboard.turn_isolation in the scratch FOOL_HOME")
    ...
    p.add_argument("--keep-home", action="store_true", help="do not delete the scratch FOOL_HOME on exit")
