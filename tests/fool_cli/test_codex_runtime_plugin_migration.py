...
from fool_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from fool_cli import codex_runtime_plugin_migration as crpm
...
class TestStripUnmanagedPluginTables:
    ...
    ``fool codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "fool_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
class TestFoolHomeLeakGuard:
    ...
    def test_real_hermes_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate FOOL_HOME (not a tempdir path) DOES propagate so the
        ...
        monkeypatch.setenv("FOOL_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("FOOL_HOME") == real_path

    def test_unset_hermes_home_omits_env_key(self, monkeypatch):
        """When FOOL_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever FOOL_HOME its launcher (systemd, gateway, shell)
        ...
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert "FOOL_HOME" not in env, (
            f"FOOL_HOME should not be set when env var is unset, got: "
            f"{env.get('FOOL_HOME')!r}"
        )
