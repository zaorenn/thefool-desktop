...
import thefool_cli.auth as auth
...
def hermes_home(tmp_path, monkeypatch):
    """Point THEFOOL_HOME at a tmp dir so we never touch the real auth store.
...
#
# _load_auth_store lives in thefool_cli/auth.py, but several other modules read
...
class TestAuthJsonSiblingReaders:
    ...
    def test_auxiliary_nous_provider_reads_non_ascii_store(self, hermes_home, windows_default_encoding, monkeypatch):
        ...
        # THEFOOL_HOME env from the fixture doesn't reach it — point it at
    ...
    def test_read_shared_nous_state_reads_non_ascii_store(
        ...
        """thefool_cli.auth._read_shared_nous_state must read a non-ASCII store.
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        ...
        """thefool_cli.main._has_any_provider_configured reads auth.json.
        ...
        # thefool_cli.auth; patch it on the source module so the local import
        # sees the fake.
