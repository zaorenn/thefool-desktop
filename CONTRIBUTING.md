...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `fool skills browse` (labeled "official") and install it with `fool skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `fool skills install`.
...
- Integrate with `fool memory setup` via `post_setup()` — no need to touch core code
...
├── cli.py                    # FoolCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (hermes-cli, hermes-telegram, etc.)
├── fool_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── fool_cli/               # CLI command implementations
...
- **Session persistence**: All conversations are stored in SQLite (`fool_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.hermes/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `fool setup` or update `~/.hermes/.env` locally.
...
Add to `_BUILTIN_SKINS` dict in `fool_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `fool_cli/skin_engine.py` for the full schema and existing skins as examples.
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `fool_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `fool_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `fool_cli/gateway_windows.py::
