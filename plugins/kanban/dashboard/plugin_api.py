...
``fool_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``fool_cli/web_server.py``.

This means ``fool dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from fool_cli import kanban_db
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    WS auth gate (``fool_cli.web_server._ws_auth_ok``).
    ...
    """
    try:
        from fool_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``fool_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from fool_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from fool_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``FOOL_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from fool_cli.kanban import _check_dispatcher_presence
                from fool_constants import get_hermes_home

                # Scope the probe to the request's active home. The dashboard
                # backend can run under a different FOOL_HOME than the
...
from fool_cli.kanban_db import (  # noqa: E402
...
# spawn failures, stuck-blocked). See fool_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``fool kanban diagnostics`` which shells to this
...
def specify_task_endpoint(
    ...
    # FOOL_KANBAN_BOARD env var — this endpoint runs in FastAPI's
    ...
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        ...
        from fool_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def model_options():
    ...
    Thin wrapper around ``fool_cli.inventory.build_models_payload`` — the
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from fool_constants import get_hermes_home  # type: ignore
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``fool profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from fool_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    # FOOL_KANBAN_BOARD env var would let concurrent requests for
    # different boards race and cross-write (issue #38323).
    with kanban_db.scoped_current_board(board or kanban_db.DEFAULT_BOARD):
        from fool_cli import kanban_decompose  # noqa: WPS433 (intentional)
