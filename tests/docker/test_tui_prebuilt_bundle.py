...
package-lock.json. Without THEFOOL_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV THEFOOL_TUI_DIR=/opt/hermes/ui-tui`` in the Dockerfile, which
...
def test_hermes_tui_dir_env_is_set(built_image: str) -> None:
    """THEFOOL_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$THEFOOL_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/hermes/ui-tui", (
        f"THEFOOL_TUI_DIR={r.stdout.strip()!r} (expected /opt/hermes/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    py = (
        ...
        "from thefool_cli.main import _tui_need_npm_install, _find_bundled_tui, _make_tui_argv\n"
    ...
    # With THEFOOL_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
