...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects THEFOOL_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("THEFOOL_INTERACTIVE", "THEFOOL_GATEWAY_SESSION", "THEFOOL_EXEC_ASK", "THEFOOL_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["THEFOOL_INTERACTIVE"] = "1"
        session_key = os.getenv("THEFOOL_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No THEFOOL_INTERACTIVE or THEFOOL_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
