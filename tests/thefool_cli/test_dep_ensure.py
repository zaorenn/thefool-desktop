...
def test_find_install_script_from_checkout(tmp_path):
    ...
    """
    from thefool_cli.dep_ensure import _find_install_script
    scripts_dir = tmp_path / "scripts"
    scripts_dir.mkdir()
    (scripts_dir / "install.sh").write_text("#!/bin/bash", encoding="utf-8")
    path, shell = _find_install_script(package_dir=tmp_path / "thefool_cli", repo_root=tmp_path)
...
def test_has_npx_agent_browser_true_when_npx_resolves():
    ...
    from thefool_cli.dep_ensure import _has_npx_agent_browser
...
def test_find_agent_browser_lazy_install_cycle_terminates(monkeypatch):
    ...
    import shutil
    import tools.browser_tool as bt
    from thefool_cli import dep_ensure
...
def test_ensure_dependency_uses_powershell_on_windows(tmp_path):
    ...
    with patch("thefool_cli.dep_ensure._DEP_CHECKS", {"node": lambda: False}), \
         patch("thefool_cli.dep_ensure._find_install_script", return_value=(scripts_dir / "install.ps1", "powershell")), \
         patch("thefool_cli.dep_ensure.shutil") as mock_shutil, \
         patch("thefool_constants.get_hermes_home", return_value=tmp_path / "fakehome"), \
