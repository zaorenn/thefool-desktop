...
3. **项目（Project）** — `./.hermes/plugins/<name>/`（需要 `FOOL_ENABLE_PROJECT_PLUGINS=1`）
...
| Hook | 行为 |
|---|---|
| `post_tool_call` | 当 `write_file` / `terminal` / `patch` 在 `FOOL_HOME` 或 `/tmp/hermes-*` 内创建匹配 `test_*`、`tmp_*` 或 `*.test.*` 的文件时，静默追踪为 `test` / `temp` / `cron-output`。 |
...
**状态** — 所有内容存储在 `$FOOL_HOME/disk-cleanup/`：
...
FOOL_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # 或你的自托管 URL
```
...
| `FOOL_LANGFUSE_RELEASE` | — | 发布/版本标签 |
| `FOOL_LANGFUSE_SAMPLE_RATE` | `1.0` | 传递给 SDK 的采样率（0.0–1.0） |
| `FOOL_LANGFUSE_MAX_CHARS` | `12000` | 消息内容 / 工具参数 / 工具结果的单字段截断长度 |
| `FOOL_LANGFUSE_DEBUG` | `false` | 向 `agent.log` 输出详细插件日志 |
...
- 解锁状态持久化到 `$FOOL_HOME/plugins/hermes-achievements/state.json`
...
**状态文件** — 位于 `$FOOL_HOME/plugins/hermes-achievements/`：
