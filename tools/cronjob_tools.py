...
# uses 30s) — comfortably below the 1800s default FOOL_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (FOOL_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with FOOL_CRON_TIMEOUT=0
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("FOOL_SESSION_PLATFORM")
    origin_chat_id = get_session_env("FOOL_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        if thread_id and origin_platform == "slack":
            message_id = get_session_env("FOOL_SESSION_MESSAGE_ID") or None
        ...
        return {
            ...
            "chat_name": get_session_env("FOOL_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes FOOL_SESSION_USER_ID to
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``FOOL_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    delivery target — the ``FOOL_CRON_AUTO_DELIVER_*`` contextvars that
    ...
    * Every other element (``local``, ``all``, explicit ``platform:...``)
    ...
    if not is_truthy_value(get_session_env("FOOL_CRON_SESSION", "")):
        return deliver

    def _creator_target() -> str:
        platform = get_session_env("FOOL_CRON_AUTO_DELIVER_PLATFORM", "").strip()
        chat_id = get_session_env("FOOL_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
        if not platform or not chat_id:
            return "local"
        thread_id = get_session_env("FOOL_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
        if thread_id:
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within FOOL_HOME/scripts/.
...
def _try_dispatch_background_run(
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("FOOL_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `fool cron create/edit --model`, or hand-edited jobs). The agent
