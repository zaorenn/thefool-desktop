...
# container recreation. Environment variables are written to $THEFOOL_HOME/.env
...
{ inputs, ... }: {
  ...
  let
    ...
    # THEFOOL_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "hermes-documents" { } (
      ...
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'THEFOOL_DOC_EOF'\n${value}\nHERMES_DOC_EOF"
        ) cfg.documents
      )
    );
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      THEFOOL_UID="''${THEFOOL_UID:?THEFOOL_UID must be set}"
      THEFOOL_GID="''${THEFOOL_GID:?THEFOOL_GID must be set}"

      # ── Group: ensure a group with GID=$THEFOOL_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$THEFOOL_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="hermes"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$THEFOOL_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$THEFOOL_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$THEFOOL_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$THEFOOL_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$THEFOOL_UID" -g "$THEFOOL_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$THEFOOL_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$THEFOOL_UID:$THEFOOL_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure THEFOOL_HOME is owned by the target user.
      ...
      if [ -n "''${THEFOOL_HOME:-}" ] && [ -d "$THEFOOL_HOME" ]; then
        find "$THEFOOL_HOME" \! -user "$THEFOOL_UID" -exec chown "$THEFOOL_UID:$THEFOOL_GID" {} +
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$THEFOOL_UID" --regid="$THEFOOL_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $THEFOOL_HOME/.env.
  ...
  in {
    options.services.hermes-agent = with lib; {
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .hermes/ subdir (THEFOOL_HOME).";
      };
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $THEFOOL_HOME/.env at activation time.
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $THEFOOL_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $THEFOOL_HOME/mcp-tokens/.
              '';
            };
      ...
      addToSystemPackages = mkOption {
        ...
        description = ''
          ...
          THEFOOL_HOME system-wide (via environment.variables) so interactive
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the hermes CLI to system PATH and export THEFOOL_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.hermes/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.THEFOOL_HOME = "${cfg.stateDir}/.hermes";
      })
      ...
      # ── Activation: link config + auth + documents ────────────────────
      {
        system.activationScripts."hermes-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.hermes/.container-mode <<'THEFOOL_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hermes_bin=${containerDataDir}/current-package/bin/hermes
    THEFOOL_CONTAINER_MODE_EOF
          ...
          # Hermes reads $THEFOOL_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.hermes/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'THEFOOL_NIX_ENV_EOF'
    ${envFileContent}
    THEFOOL_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.hermes-agent = {
          ...
          environment = {
            HOME = cfg.stateDir;
            THEFOOL_HOME = "${cfg.stateDir}/.hermes";
            THEFOOL_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $THEFOOL_HOME/.env by the activation script. load_hermes_dotenv()
        ...
        systemd.services.hermes-agent = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              THEFOOL_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              THEFOOL_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env THEFOOL_UID="$THEFOOL_UID" \
                --env THEFOOL_GID="$THEFOOL_GID" \
                --env THEFOOL_HOME=${containerDataDir}/.hermes \
                --env THEFOOL_MANAGED=true \
